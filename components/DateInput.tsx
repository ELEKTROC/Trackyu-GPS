...
export const DateInput: React.FC<DateInputProps> = ({
  ...
  const baseClasses = variant === 'compact' ? 'px-2 py-1.5 text-sm' : 'px-3 py-2.5 text-sm';
  ...
  return (
    <div className="relative">
      {label && (
        <label
          htmlFor={inputId}
          className="block text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wide mb-1"
        >
          {label}
        </label>
      )}
