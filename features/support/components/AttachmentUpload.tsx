...
export const AttachmentUpload: React.FC<AttachmentUploadProps> = ({ ticketId }) => {
    ...
    const handleDelete = async (attachmentId: string) => {
        if (!await confirm({ message: 'Supprimer cette pièce jointe ?', variant: 'danger', title: 'Confirmer la suppression', confirmLabel: 'Supprimer' })) return;
            ...
            {attachments.length > 0 && (
                <div className="space-y-2">
                    {attachments.map(attachment => (
                        <div key={attachment.id} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
