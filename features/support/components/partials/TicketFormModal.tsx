...
import type { Client, Vehicle, SystemUser, Invoice, Tier, TicketCategory } from '../../../../types';
...
export const TicketFormModal: React.FC<TicketFormModalProps> = ({
  ...
  const clientPaymentStatus = useMemo(() => {
    ...
    const clientInvoices = invoices.filter(
      (inv) => inv.clientId === ticketForm.clientId || inv.tier_id === ticketForm.clientId
    );
  ...
  const selectedVehicle = useMemo(
    () => vehicles.find((v) => v.id === ticketForm.vehicleId),
    [vehicles, ticketForm.vehicleId]
  );

  const selectedClient = useMemo(
    () => clients.find((c) => c.id === ticketForm.clientId),
    [clients, ticketForm.clientId]
  );
      ...
      footer={
        <>
          ...
          <button
            ...
            disabled={isSaving}
            className="px-4 py-2 bg-[var(--primary)] text-white rounded-lg hover:bg-[var(--primary-light)] transition-colors shadow-sm disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-2"
          >
            {isSaving && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--text-primary)] mb-1">
              ...
              {selectedClient && clientVehicles.length > 0 && (
                <span className="ml-2 text-xs font-normal text-[var(--primary)]">
                  ({clientVehicles.length} véhicule{clientVehicles.length > 1 ? 's' : ''})
                </span>
              )}
            </label>
            <select
            ...
            >
              <option value="">
                {clientVehicles.length === 0 && ticketForm.clientId
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setTicketForm({
                    ...
                    interventionType: '',
                    priority:
                      (category as TicketCategory & { defaultPriority?: string; default_priority?: string })
                        ?.defaultPriority ||
                      (category as TicketCategory & { defaultPriority?: string; default_priority?: string })
                        ?.default_priority ||
                      ticketForm.priority,
              ...
              <div>
                ...
                >
                  ...
                  {allNatures
                    ...
                    .map((nature) => (
                      <option key={nature.id} value={nature.label}>
                        {nature.label}
                      </option>
                    ))}
                </select>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setTicketForm({
                    ...
                    description,
                    priority: selected
                      ? (selected as TicketSubcategory & { defaultPriority?: string; default_priority?: string })
                          .defaultPriority ||
                        (selected as TicketSubcategory & { defaultPriority?: string; default_priority?: string })
                          .default_priority ||
                        ticketForm.priority
              ...
              <input
                ...
                value={
                  ticketForm.receivedAt
                    ? new Date(
                        new Date(ticketForm.receivedAt).getTime() -
                          new Date(ticketForm.receivedAt).getTimezoneOffset() * 60000
                      )
                        .toISOString()
                        .slice(0, 16)
                ...
                onChange={(e) =>
                  setTicketForm({ ...ticketForm, receivedAt: e.target.value ? new Date(e.target.value) : new Date() })
                }
              />
            </div>
            <div>
                ...
                onClick={() => {
                  ...
                  input.onchange = (e) => {
                    ...
                    if (rejected.length > 0) {
                      showToast(
                        `${rejected.length} fichier(s) rejeté(s) car > 10 Mo : ${rejected.map((f) => f.name).join(', ')}`,
                        'warning'
                      );
                    }
