...
import { 
    Settings, Clock, Tag, FolderTree, Plus, Edit2, Trash2, Save, X, 
    Check, AlertCircle, Eye, EyeOff, ChevronDown, ChevronRight,
    Wrench, MessageSquare, Bug, HelpCircle, Zap, Shield, FileText, AlertTriangle,
    Lock, Globe, RotateCcw
} from 'lucide-react';
...
export const SupportSettingsPanel: React.FC = () => {
    ...
    const fetchInterventionSettings = async () => {
        try {
            const [types, natures] = await Promise.all([
    ...
    const deleteCategory = async (category: Category) => {
        if (!await confirm({ message: `Voulez-vous vraiment désactiver la catégorie "${category.name}" ?`, variant: 'warning', title: 'Confirmer la désactivation', confirmLabel: 'Désactiver' })) return;
    ...
    const deleteSubcategory = async (subcategory: Subcategory) => {
        if (!await confirm({ message: `Voulez-vous vraiment désactiver "${subcategory.name}" ?`, variant: 'warning', title: 'Confirmer la désactivation', confirmLabel: 'Désactiver' })) return;
    ...
    const saveIntType = async (type: Partial<InterventionTypeConfig>) => {
        ...
        try {
            ...
            showToast(isNew ? TOAST.CRUD.CREATED('Type d\'intervention') : TOAST.CRUD.UPDATED('Type d\'intervention'), 'success');
    ...
    const deleteIntType = async (typeId: string) => {
        if (!await confirm({ message: 'Voulez-vous vraiment supprimer ce type d\'intervention ?', variant: 'danger', title: 'Confirmer la suppression' })) return;
        setSaving(true);
        try {
    ...
    const saveIntNature = async (nature: Partial<InterventionNatureConfig>) => {
        ...
        try {
            ...
            showToast(isNew ? TOAST.CRUD.CREATED('Nature d\'intervention') : TOAST.CRUD.UPDATED('Nature d\'intervention'), 'success');
            await fetchInterventionSettings();
    ...
    const deleteIntNature = async (natureId: string) => {
        if (!await confirm({ message: 'Voulez-vous vraiment supprimer cette nature ?', variant: 'danger', title: 'Confirmer la suppression' })) return;
        setSaving(true);
        try {
        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400">
                        Gérez les catégories principales des tickets
                    </p>
                </div>
                <button
                ...
                <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                        ...
                        <select
                            ...
                            onChange={(e) => setNewSubcategory({ ...newSubcategory, default_priority: e.target.value as Subcategory['default_priority'] })}
                            className="px-3 py-2 border rounded-lg dark:bg-slate-800 dark:border-slate-600"
                                                ...
                                                {isEditing ? (
                                                    ...
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <span className={`text-xs px-2 py-0.5 rounded-full ${getPriorityColor(subcat.default_priority)}`}>
                                                                {subcat.default_priority}
                                                            </span>
                                                            <span className="text-xs text-slate-500">
                                                                SLA: {subcat.sla_hours}h
                                                            </span>
                                                            {!subcat.is_active && (
                ...
                {interventionTypes.map(type => {
                    ...
                    return (
                        <div key={type.id} className="border rounded-xl dark:border-slate-700 overflow-hidden">
                            <div className={`p-4 flex items-center justify-between ${isExpanded ? 'bg-slate-50 dark:bg-slate-800' : 'bg-white dark:bg-slate-900'}`}>
                                <div className="flex items-center gap-3">
                                    <button
                                        onClick={() => {
                                    ...
                                    {isEditing ? (
                                        <div className="flex items-center gap-2">
                                            ...
                                            />
                                            <button onClick={() => saveIntType(editingIntType)} className="p-1 text-green-600"><Check className="w-4 h-4" /></button>
                                            <button onClick={() => setEditingIntType(null)} className="p-1 text-red-600"><X className="w-4 h-4" /></button>
                                        </div>
                                    ) : (
                                        <div className="flex items-center gap-2">
                                            <span className="font-bold text-slate-800 dark:text-white">{type.label}</span>
                                            <span className="text-[10px] font-mono bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded text-slate-500 uppercase">{type.code}</span>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button onClick={() => setEditingIntType(type)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-400"><Edit2 className="w-4 h-4" /></button>
                                    {!type.isSystem && (
                                        <button onClick={() => deleteIntType(type.id)} className="p-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg text-red-400"><Trash2 className="w-4 h-4" /></button>
                                    )}
                                </div>
                            </div>
                                    ...
                                    {typeNatures.length === 0 ? (
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            {typeNatures.map(nature => (
                                                <div key={nature.id} className="flex items-center justify-between p-3 rounded-lg border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 transition-colors">
                                                    {editingIntNature?.id === nature.id ? (
                                                        <div className="flex-1 flex gap-2">
                                                            ...
                                                            />
                                                            <button onClick={() => saveIntNature(editingIntNature)} className="text-green-600"><Check className="w-4 h-4" /></button>
                                                            <button onClick={() => setEditingIntNature(null)} className="text-red-600"><X className="w-4 h-4" /></button>
                                                        </div>
                                                    ) : (
                                                        <>
                                                            <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{nature.label}</span>
                                                            <div className="flex items-center gap-1">
                                                                <button onClick={() => setEditingIntNature(nature)} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded text-slate-400"><Edit2 className="w-3.5 h-3.5" /></button>
                                                                {!nature.isSystem && (
                                                                    <button onClick={() => deleteIntNature(nature.id)} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/20 rounded text-red-400 group"><Trash2 className="w-3.5 h-3.5" /></button>
                                                                )}
                                                            </div>
                                                        </>
                                                    )}
    ...
    const handleResetOverrides = async () => {
        if (!await confirm({ message: 'Êtes-vous sûr de vouloir réinitialiser toutes les catégories aux valeurs système ? Vos personnalisations seront perdues.', variant: 'warning', title: 'Confirmer la réinitialisation', confirmLabel: 'Réinitialiser' })) {
            return;
        }
        try {
