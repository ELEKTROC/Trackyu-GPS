...
import {
    MessageSquare, LayoutDashboard, Columns3, AlertTriangle, Settings,
    Plus, Search, Filter, Clock, User, Trash2, Edit2, Shield, Play, Check, Lock,
    Paperclip, Zap, X, Send, BookOpen, TrendingUp, CheckCircle, Timer,
    ChevronDown, MoreVertical, ArrowUpRight, Calendar, Target, Users,
    Download, Upload, FileSpreadsheet, FileText, RotateCcw, Headset,
    CheckSquare, Square, MinusSquare, ChevronLeft, ChevronRight, SearchX
} from 'lucide-react';
...
import {
    FILTERS_CONFIG, PRIORITY_ORDER, getStatusInfo, getSlaStatus,
    INTERVENTION_TYPES_CONFIG, MACROS
} from '../utils';
...
export const SupportViewV2: React.FC = () => {
    const { tickets, clients, interventions, vehicles, users, updateTicket, addTicket, addIntervention, tiers, slaConfig, ticketCategories, ticketSubcategories, invoices, deleteTicket } = useDataContext();
    ...
    const visibleSupportTabs = useMemo(() => {
        const baseTabs = isMobile
            ? SUPPORT_TABS.filter(t => !SUPPORT_MOBILE_HIDDEN.has(t.id))
            : SUPPORT_TABS;
    ...
    useEffect(() => { setCurrentPage(1); }, [statusFilter, priorityFilter, categoryFilter]);

    // Server-side paginated ticket list
    const { data: ticketPageData, isLoading: loadingTicketList } = useQuery({
        queryKey: ['tickets-paged', user?.tenantId, currentPage, PAGE_SIZE, statusFilter, priorityFilter, categoryFilter, debouncedSearch],
    ...
    const [ticketForm, setTicketForm] = useState<any>({
        id: '', clientId: '', subject: '', category: "Commercial",
        subCategory: '', interventionType: '', priority: 'MEDIUM', vehicleId: '', description: '', assignedTo: ''
    });
    ...
    useEffect(() => {
        const loadMacros = async () => {
            ...
            try {
                ...
                setMacros(data.map((m: MacroApiResponse) => ({ 
                    id: m.id, 
                    label: m.label, 
                    text: m.text, 
                    category: m.category,
    ...
    const ticketStaff = useMemo(() => users.filter(u =>
    ...
    const clientMetrics = useMemo(() => {
        ...
        const clientVehicles = vehicles.filter(v => v.client === clientDetails.name || v.client === clientDetails.id || v.clientId === clientDetails.id);
        ...
        const clientInvoices = invoices.filter(inv => inv.clientId === clientDetails.id || inv.tier_id === clientDetails.id);
        const revenueYTD = clientInvoices.filter(inv => inv.status === 'PAID').reduce((sum, inv) => sum + (inv.amountTTC || inv.amount || 0), 0);
        // Real payment status from invoices
        const hasOverdue = clientInvoices.some(inv => inv.status === 'OVERDUE' || (inv.status === 'SENT' && inv.dueDate && new Date(inv.dueDate) < new Date()));
    ...
    const ticketCounts = useMemo(() => {
        ...
        FILTERS_CONFIG.forEach(f => { if (f.id !== 'ALL') counts[f.id] = tickets.filter(t => t.status === f.id).length; });
        return counts;
    }, [tickets]);
    ...
    const filteredTickets = useMemo(() => {
        return tickets.filter(t => {
            ...
            const vehicleName = t.vehicleId ? (vehicles.find(v => v.id === t.vehicleId)?.name || vehicles.find(v => v.id === t.vehicleId)?.plate || '') : '';
    ...
    const slaStats = useMemo(() => {
        ...
        return { critical: criticalSla, warning: warningSla, ok: okSla, respectRate: slaRespect, total: activeTickets.length };
    }, [tickets, slaConfig]);
    ...
    const openCreateModal = () => {
        setIsEditMode(false); setFormErrors({});
        setTicketForm({ id: '', clientId: '', subject: '', category: "Support Technique", subCategory: '', interventionType: '', priority: 'MEDIUM', vehicleId: '', description: '', assignedTo: '', source: 'TrackYu', receivedAt: new Date() });
        setIsCreateModalOpen(true);
    };

    const openEditModal = () => {
        if (!selectedTicket) return;
        setIsEditMode(true); setFormErrors({});
    ...
    const handleSaveTicket = async () => {
        ...
        if (!isEditMode) {
            const validationResult = CreateTicketSchema.safeParse({ ...ticketForm, vehicleId: ticketForm.vehicleId || undefined, subCategory: ticketForm.subCategory || undefined });
            if (!validationResult.success) {
                const errors: Record<string, string> = {};
                validationResult.error.issues.forEach(issue => { if (issue.path[0]) errors[issue.path[0].toString()] = issue.message; });
                setFormErrors(errors);
                const fieldNames: Record<string, string> = { clientId: 'Client', subject: 'Sujet', description: 'Description', category: 'Catégorie', priority: 'Priorité' };
                const errorFields = Object.keys(errors).map(k => fieldNames[k] || k).join(', ');
        ...
        try {
            if (isEditMode && selectedTicket) {
                ...
                const createdTicket = await addTicket({ ...ticketForm, id: newId, tenantId: user?.tenantId || '', status: 'OPEN', createdAt: new Date(), updatedAt: new Date(), messages: [] } as Ticket);
    ...
    const handleStatusClick = (status: string) => { setTargetStatus(status); setStatusReason(''); setIsStatusModalOpen(true); };
    ...
    const handleBulkTakeCharge = async () => {
        ...
        if (isCurrentUserTech) { showToast(TOAST.SUPPORT.TICKET_TECH_ONLY, 'warning'); return; }
        ...
        try {
            ...
            setSelectedTicketIds(new Set());
            showToast(TOAST.SUPPORT.BATCH_TAKEN(result.updated, ids.length - result.updated), result.updated > 0 ? 'success' : 'error');
        } catch { showToast(TOAST.SUPPORT.BATCH_TAKEN(0, selectedTicketIds.size), 'error'); }
    ...
    const handleBulkResolve = async () => {
        ...
        try {
            ...
            queryClient.invalidateQueries({ queryKey: ['tickets-paged'] });
            setSelectedTicketIds(new Set());
            showToast(TOAST.SUPPORT.BATCH_RESOLVED(result.updated, ids.length - result.updated), result.updated > 0 ? 'success' : 'error');
        } catch { showToast(TOAST.SUPPORT.BATCH_RESOLVED(0, selectedTicketIds.size), 'error'); }
        finally { setBulkProcessing(false); }
    };
    ...
    const confirmStatusChange = async () => {
        if (!selectedTicketId || !targetStatus || !statusReason.trim()) { showToast(TOAST.SUPPORT.REASON_REQUIRED, "error"); return; }
        const ticket = tickets.find(t => t.id === selectedTicketId);
        if (ticket) {
            try {
    ...
    const saveMacro = async () => {
        if (!newMacro.label || !newMacro.text) { showToast(TOAST.MACRO.LABEL_AND_TEXT_REQUIRED, "error"); return; }
        try {
            if (isEditingMacro) {
                ...
                setMacros(prev => prev.map(m => m.id === isEditingMacro.id ? { 
                    ...m, 
                    label: updated.label, 
                    text: updated.text, 
        ...
        setNewMacro({ label: '', text: '', category: '' }); setIsEditingMacro(null);
    };
    ...
    const handleDeleteTicket = async (ticketId: string, ticketSubject: string) => {
        if (!await confirm({ message: `Supprimer le ticket "${ticketSubject}" ?\n\nCette action est irréversible.`, variant: 'danger', title: 'Confirmer la suppression', confirmLabel: 'Supprimer' })) return;
        try {
    ...
    const handleExportCSV = () => {
        const headers = ['ID', 'Sujet', 'Client', 'Catégorie', 'Priorité', 'Statut', 'Canal', 'Date réception', 'Assigné à', 'Créé le', 'Description'];
...
CLIENT-002;Installation demandée;Demande d'intervention;MEDIUM;Nouvelle installation;VEH-002`;
    ...
    const getSlaBadge = (ticket: Ticket) => {
        ...
        if (sla === 'CRITICAL') return <span className="px-1.5 py-0.5 bg-red-100 text-red-700 rounded text-[10px] font-bold">SLA !</span>;
        if (sla === 'WARNING') return <span className="px-1.5 py-0.5 bg-orange-100 text-orange-700 rounded text-[10px] font-bold">SLA</span>;
        return null;
    };
    ...
    const getRemainingTime = (ticket: Ticket) => {
        ...
        const limit = Number(config[priority] || config[(ticket.priority || '').toUpperCase() as keyof typeof config]) || 48;

        const elapsed = (Date.now() - new Date(ticket.createdAt).getTime()) / (1000 * 60 * 60);
        const remaining = limit - elapsed;
        if (remaining < 0) return <span className="text-red-600 font-bold">Dépassé de {Math.abs(Math.round(remaining))}h</span>;
        ...
        <div className="h-[calc(100vh-204px)] lg:h-[calc(100vh-140px)] flex flex-col space-y-4 animate-in fade-in duration-500">
            ...
            <div className="flex items-center justify-between gap-2">
                    ...
                    {visibleSupportTabs.map(tab => (
                        ...
                        >
                            ...
                            {tab.id === 'SLA' && slaStats.critical.length > 0 && (
                                <span className="px-1.5 py-0.5 bg-red-500 text-white rounded-full text-[10px] font-bold">{slaStats.critical.length}</span>
                            )}
                        </button>
                    ))}
                </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="hidden sm:flex items-center gap-1 border-r border-slate-200 dark:border-slate-700 pr-2">
                        ...
                        <button onClick={handleExportPDF} className="flex items-center gap-1 px-2 py-1.5 text-xs font-bold rounded border border-red-200 text-red-600 bg-red-50 dark:bg-red-900/20 dark:border-red-800 dark:text-red-400 hover:bg-red-100 transition-colors" title="Export PDF">
                            <Download className="w-3.5 h-3.5" /> PDF
                        </button>
                    </div>
                    <button onClick={() => setIsImportModalOpen(true)} className="hidden sm:flex items-center gap-1 px-3 py-1.5 bg-amber-600 text-white text-xs font-bold rounded-lg hover:bg-amber-700">
                ...
                <div className="flex-1 overflow-y-auto space-y-6 pb-16 lg:pb-0">
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-[10px] text-slate-400">
                                        {(() => { const now = new Date(); return tickets.filter(t => t.status === 'RESOLVED' && t.resolvedAt && new Date(t.resolvedAt).getMonth() === now.getMonth() && new Date(t.resolvedAt).getFullYear() === now.getFullYear()).length; })()} ce mois
                                    </p>
                                </div>
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-2xl font-bold text-indigo-600">
                                        {resolutionStats.avgResponseTime !== null ? formatTicketDuration(resolutionStats.avgResponseTime) : '-'}
                                    </p>
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-2xl font-bold text-violet-600">
                                        {resolutionStats.avgHandlingTime !== null ? formatTicketDuration(resolutionStats.avgHandlingTime) : '-'}
                                    </p>
                        ...
                        <Card className="p-4 border-l-4 border-l-teal-500">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-[10px] text-slate-400">{resolutionStats.withinSLA}/{resolutionStats.count} dans les temps</p>
                                </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div className="h-64">
                                <ResponsiveContainer width="100%" height="100%" minHeight={200} minWidth={200} initialDimension={{ width: 200, height: 200 }}>
                                    <PieChart>
                                        ...
                                        >
                                            {[{ color: '#3b82f6' }, { color: '#f97316' }, { color: '#a855f7' }, { color: '#22c55e' }, { color: '#64748b' }].map((entry, index) => (
                        ...
                        <Card className="p-6">
                            ...
                            <div className="h-64">
                                <ResponsiveContainer width="100%" height="100%" minHeight={200} minWidth={200} initialDimension={{ width: 200, height: 200 }}>
                                    <BarChart data={ticketCategories.map((cat: TicketCategory) => ({ name: cat.name.substring(0, 15), count: tickets.filter(t => t.category === cat.name).length }))}>
                        ...
                        <Card className="p-6 border-l-4 border-l-red-500">
                            ...
                            <div className="space-y-2">
                                {slaStats.critical.slice(0, 5).map(t => (
                                    <div key={t.id} onClick={() => { setSelectedTicketId(t.id); setActiveTab('TICKETS'); }} className="flex items-center justify-between p-3 bg-red-50 dark:bg-red-900/20 rounded-lg cursor-pointer hover:bg-red-100 transition-colors">
                                        <div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className={`px-2 py-0.5 rounded text-xs font-bold border ${getPriorityBadge(t.priority)}`}>{t.priority}</span>
                                            {getRemainingTime(t)}
                                        </div>
                                    </div>
                                ))}
                            ...
                            <table className="w-full text-sm">
                                <thead className="bg-slate-50 dark:bg-slate-800 sticky top-0 z-10">
                                    <tr className="border-b border-slate-200 dark:border-slate-700">
                                        <th className="text-left px-3 py-3 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-300">Agent</th>
                                        <th className="text-center px-3 py-3 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-300">Créés</th>
                                        <th className="text-center px-3 py-3 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-300">Assignés</th>
                                        <th className="text-center px-3 py-3 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-300">Résolus</th>
                                        <th className="text-center px-3 py-3 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-300">En cours</th>
                                        <th className="text-center px-3 py-3 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-300">Temps moy.</th>
                                        <th className="text-center px-3 py-3 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-300">Taux SLA</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {(() => {
                                        const agentMap = new Map<string, { name: string; created: number; assigned: number; resolved: number; inProgress: number; totalResTime: number; resCount: number; slaOk: number; slaTotal: number }>();
                                        ticketStaff.forEach(tech => {
                                            agentMap.set(tech.id, { name: tech.name || tech.email, created: 0, assigned: 0, resolved: 0, inProgress: 0, totalResTime: 0, resCount: 0, slaOk: 0, slaTotal: 0 });
                                        });
                                        tickets.forEach(t => {
                                            // Créés par cet agent
                                        ...
                                        const agents = Array.from(agentMap.values()).filter(a => a.assigned > 0 || a.created > 0).sort((a, b) => b.assigned - a.assigned);
                                        if (agents.length === 0) return <tr><td colSpan={7} className="text-center py-4 text-slate-400">Aucune donnée agent</td></tr>;
                                        let totalCreated = 0, totalAssigned = 0, totalResolved = 0, totalInProgress = 0, sumTotalResTime = 0, sumResCount = 0, sumSlaOk = 0, sumSlaTotal = 0;
                                        ...
                                        const rows = agents.map((a, i) => {
                                            ...
                                            return (
                                                <tr key={i} className="density-row border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                                    ...
                                                    <td className="text-center py-2 px-3 text-xs text-slate-600 dark:text-slate-300">{a.resCount > 0 ? `${hours}h${mins.toString().padStart(2, '0')}` : '—'}</td>
                                                    <td className="text-center py-2 px-3">
                                                        <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${slaRate >= 80 ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' : slaRate >= 50 ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300'}`}>
                                                            {slaRate}%
                                                        </span>
                                                    </td>
                                                </tr>
                                            ...
                                            <tr key="total" className="border-t-2 border-slate-300 dark:border-slate-600 bg-slate-100 dark:bg-slate-800 font-bold">
                                                ...
                                                <td className="text-center py-3 px-3 text-slate-800 dark:text-white text-base">
                                                    <div className="flex flex-col">
                                                        ...
                                                        <span className="text-[10px] text-green-500 font-normal">{totalAssigned > 0 ? Math.round((totalResolved/totalAssigned)*100) : 0}%</span>
                                                    </div>
                                                </td>
                                                <td className="text-center py-3 px-3 text-slate-800 dark:text-white text-base">
                                                    <div className="flex flex-col">
                                                        <span>{totalInProgress}</span>
                                                        <span className="text-[10px] text-orange-500 font-normal">{totalAssigned > 0 ? Math.round((totalInProgress/totalAssigned)*100) : 0}%</span>
                                                    </div>
                                                </td>
                                                <td className="text-center py-3 px-3 text-slate-800 dark:text-white text-base font-mono">{sumResCount > 0 ? `${gHours}h${gMins.toString().padStart(2, '0')}` : '—'}</td>
                                                <td className="text-center py-3 px-3">
                                                    <span className={`px-2 py-1 rounded-full text-sm font-bold shadow-sm ${globalSlaRate >= 80 ? 'bg-green-500 text-white' : globalSlaRate >= 50 ? 'bg-yellow-500 text-white' : 'bg-red-500 text-white'}`}>
                                                        {globalSlaRate}%
                                                    </span>
                                                </td>
                                            </tr>
            ...
            {activeTab === 'TICKETS' && (
                <div className="flex flex-col lg:flex-row h-full gap-4 overflow-hidden">
                    ...
                    <div className={`w-full lg:w-[460px] shrink-0 flex-col bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden lg:max-h-none ${isMobile && selectedTicketId ? 'hidden' : 'flex h-full'}`}>
                        {/* Search & Filters */}
                        <div className="p-4 border-b border-slate-200 dark:border-slate-700 space-y-3">
                            <div className="relative">
                                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                                <input
                                    type="text" placeholder="Rechercher par ID, sujet, client, agent, véhicule, message..."
                        ...
                        {selectedTicketIds.size > 0 && (
                            <div className="px-4 py-2.5 bg-[var(--primary-dim)] border-b border-[var(--border)] flex items-center justify-between gap-2">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button onClick={handleBulkResolve} disabled={bulkProcessing}
                                    ...
                                    <button onClick={clearSelection} className="px-2 py-1.5 text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 text-xs transition-colors">
                        ...
                        <div className="flex-1 overflow-y-auto pb-4 lg:pb-0">
                            {loadingTicketList ? (
                                ...
                                (statusFilter !== 'ALL' || priorityFilter !== 'ALL' || categoryFilter !== 'ALL' || !!debouncedSearch)
                                    ? <EmptyState icon={SearchX} title="Aucun résultat" description="Aucun ticket ne correspond aux filtres actifs. Modifiez ou réinitialisez vos filtres." />
                                    : <EmptyState icon={MessageSquare} title="Aucun ticket" description="Aucun ticket d'assistance n'a encore été créé." />
                            ) : (
                                        ...
                                        <div key={ticket.id} onClick={() => setSelectedTicketId(ticket.id)}
                                            ...
                                            <div className="flex items-start justify-between mb-2">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold border ${getPriorityBadge(ticket.priority)}`}>{ticket.priority}</span>
                                                    {getSlaBadge(ticket)}
                                                    {(ticket.escalationCount ?? 0) > 0 && (
                                                        <span className="px-1 py-0.5 rounded text-[10px] font-bold border border-orange-300 bg-orange-50 text-orange-600 dark:bg-orange-900/30 dark:text-orange-300" title={`Escaladé ${ticket.escalationCount}×`}>
                                                            ↑{ticket.escalationCount}
                                                        </span>
                                                    )}
                                                </div>
                                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${getStatusInfo(ticket.status).style}`}>
                                                    {getStatusInfo(ticket.status).label}
                                                </span>
                                            </div>
                                            <p className="font-medium text-slate-800 dark:text-white text-sm mb-1 line-clamp-1">{ticket.subject}</p>
                                            <p className="text-xs text-slate-500 flex items-center gap-2">
                                                <User className="w-3 h-3" /> {getClientName(ticket)}
                                                <span className="text-slate-300">•</span>
                                                {ticket.source && ticket.source !== 'TrackYu' && (
                                                    <><span className="text-[10px]">{ticket.source === 'Appel' ? '📞' : ticket.source === 'WhatsApp' ? '💬' : ticket.source === 'Visite' ? '🏢' : ticket.source === 'SMS' ? '📱' : '🌐'}</span><span className="text-slate-300">•</span></>
                                                )}
                                                <Clock className="w-3 h-3" /> {new Date(ticket.createdAt).toLocaleDateString('fr-FR')} {new Date(ticket.createdAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
                                            </p>
                    ...
                    <div className={`flex-1 flex flex-col bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden ${isMobile && !selectedTicketId ? 'hidden' : ''}`}>
                        {selectedTicket ? (
                            <>
                                {/* Header */}
                                    ...
                                    <div className="flex items-start justify-between">
                                        <div>
                                            <div className="flex items-center gap-3 mb-1">
                                                ...
                                                <span className={`px-2 py-0.5 rounded text-xs font-bold border ${getStatusInfo(selectedTicket.status).style}`}>
                                                    {getStatusInfo(selectedTicket.status).label}
                                                </span>
                                                {getSlaBadge(selectedTicket)}
                                                {(selectedTicket.escalationCount ?? 0) > 0 && (
                                                    <span className="px-2 py-0.5 rounded text-xs font-bold border border-orange-300 bg-orange-50 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300 dark:border-orange-600 flex items-center gap-1" title={selectedTicket.escalatedAt ? `Dernière escalade: ${new Date(selectedTicket.escalatedAt).toLocaleString()}` : ''}>
                                            ...
                                            <p className="text-sm text-slate-500 flex items-center gap-2 flex-wrap">
                                                ...
                                                {selectedTicket.source && (
                                                    ...
                                                    }`}>{selectedTicket.source === 'Appel' ? '📞' : selectedTicket.source === 'WhatsApp' ? '💬' : selectedTicket.source === 'Visite' ? '🏢' : selectedTicket.source === 'SMS' ? '📱' : '🌐'} {selectedTicket.source}</span>
                                                )}
                                                <span className="text-slate-300">|</span>
                                                <Clock className="w-4 h-4" /> {selectedTicket.receivedAt ? `Reçu ${new Date(selectedTicket.receivedAt).toLocaleString('fr-FR')}` : new Date(selectedTicket.createdAt).toLocaleString()}
                                                <span className="text-slate-300">|</span>
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            {!['RESOLVED', 'CLOSED'].includes(selectedTicket.status) && (
                                                <>
                                                    ...
                                                    {hasPermission('DELETE_TICKETS') && (
                                                        <button onClick={() => handleDeleteTicket(selectedTicket.id, selectedTicket.subject)} className="p-2 bg-white dark:bg-slate-700 border border-slate-200 rounded-lg hover:border-red-300 hover:bg-red-50" title="Supprimer">
                                                    ...
                                                    <button
                                                        onClick={() => {
                                                            ...
                                                            const vehicle = selectedTicket.vehicleId ? vehicles.find(v => v.id === selectedTicket.vehicleId) : null;
                                                            ...
                                                            const mapSubCategoryToType = (subCat?: string): InterventionType => {
                                                                ...
                                                                if (lower.includes('installation') || lower.includes('transfert') || lower.includes('balise') || lower.includes('jauge')) return 'INSTALLATION';
                                                            ...
                                                            setInterventionInitialData({
                                                                ...
                                                                type: (selectedTicket.interventionType as InterventionType) || mapSubCategoryToType(selectedTicket.subCategory),
                                                                nature: (selectedTicket.subCategory as InterventionNature) || (mapSubCategoryToType(selectedTicket.subCategory) === 'INSTALLATION' ? 'Balise' : 'Dépannage'),
                                                    ...
                                                    {selectedTicket.status === 'IN_PROGRESS' && (
                                                        <button onClick={handleResolve} className="px-3 py-2 bg-green-600 text-white rounded-lg text-xs font-bold flex items-center gap-2">
                                            ...
                                            {selectedTicket.status === 'RESOLVED' && (
                                                <button onClick={() => handleStatusClick('CLOSED')} className="px-3 py-2 bg-slate-700 text-white rounded-lg text-xs font-bold flex items-center gap-2 hover:bg-slate-800 transition-colors">
                        ...
                        <div className="w-full lg:w-[380px] flex flex-col gap-4 overflow-y-auto overflow-x-hidden">
                            ...
                            <Card className="p-4 shrink-0">
                                <div className="flex justify-between items-center mb-3">
                                    <h4 className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2"><User className="w-3 h-3" /> Fiche Client</h4>
                                    {clientMetrics?.paymentStatus === 'OK' ? (
                                        <span className="px-2 py-0.5 bg-green-100 text-green-700 border border-green-200 rounded text-[10px] font-bold uppercase">À Jour</span>
                                    ) : (
                                        <span className="px-2 py-0.5 bg-red-100 text-red-700 border border-red-200 rounded text-[10px] font-bold uppercase">Impayés</span>
                                    )}
                                </div>
                                <div className="space-y-3">
                                    ...
                                    <div className="space-y-1.5">
                                        <div className="flex items-center gap-2 text-sm">
                                            ...
                                            <span className="text-slate-600 dark:text-slate-300">{clientDetails?.contactName || 'Non renseigné'}</span>
                                        </div>
                                        <div className="flex items-center gap-2 text-sm">
                                            <span className="w-3.5 h-3.5 text-slate-400 text-center">📞</span>
                                            <span className="text-slate-600 dark:text-slate-300">{clientDetails?.phone || 'Non renseigné'}</span>
                                        </div>
                                        <div className="flex items-center gap-2 text-sm">
                                            <span className="w-3.5 h-3.5 text-slate-400 text-center">✉️</span>
                                            <span className="text-slate-600 dark:text-slate-300">{clientDetails?.email || 'Non renseigné'}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                                        <div className="p-2 bg-slate-50 dark:bg-slate-800 rounded border border-slate-100 dark:border-slate-700">
                                            ...
                                            <p className="font-bold text-slate-700 dark:text-slate-300">{clientMetrics?.seniorityYears || 1} ans</p>
                                        </div>
                                        <div className="p-2 bg-slate-50 dark:bg-slate-800 rounded border border-slate-100 dark:border-slate-700">
                                            <div className="flex items-center gap-1.5 text-slate-400 text-[10px] uppercase font-bold mb-1">
                                                <Shield className="w-3 h-3" /> Flotte
                                            </div>
                                            <p className="font-bold text-slate-700 dark:text-slate-300">{clientMetrics?.vehicleCount || 0} véh.</p>
                                        </div>
                                        <div className="p-2 bg-slate-50 dark:bg-slate-800 rounded border border-slate-100 dark:border-slate-700 col-span-2">
                                            ...
                                            <p className="font-bold text-slate-700 dark:text-slate-300">{(clientMetrics?.revenueYTD || 0).toLocaleString()}</p>
                                        </div>
                                    </div>
                                </div>
                            </Card>
                            ...
                            {/* Interventions */}
                            <Card className="p-4 shrink-0">
                                <h4 className="text-xs font-bold text-slate-500 uppercase mb-3 flex items-center gap-2"><Shield className="w-3 h-3" /> Interventions</h4>
                                {relatedInterventions.length > 0 ? relatedInterventions.slice(0, 3).map(int => (
                                    <div key={int.id} className="p-2 bg-slate-50 dark:bg-slate-800 rounded mb-2">
                                        <span className="font-bold text-xs">{int.type}</span>
                                        <p className="text-[10px] text-slate-500">{new Date(int.scheduledDate).toLocaleDateString('fr-FR')}</p>
                                    </div>
                            ...
                            <Card className="p-4 max-h-48 overflow-y-auto shrink-0">
                                <h4 className="text-xs font-bold text-slate-500 uppercase mb-3 flex items-center gap-2 sticky top-0 bg-white dark:bg-slate-900 pb-1"><FileText className="w-3 h-3" /> Description</h4>
                                <p className="text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap leading-relaxed">{selectedTicket.description || <span className="italic text-slate-400">Aucune description</span>}</p>
                            </Card>

                            {/* Historique / Timeline */}
                            <Card className="p-4 max-h-64 overflow-y-auto shrink-0">
                                <h4 className="text-xs font-bold text-slate-500 uppercase mb-3 flex items-center gap-2 sticky top-0 bg-white dark:bg-slate-900 pb-1"><Clock className="w-3 h-3" /> Historique</h4>
                                <div className="relative pl-4 border-l-2 border-slate-200 dark:border-slate-700 space-y-3">
                                    ...
                                    {selectedTicket.firstResponseAt && (
                                        <div className="relative">
                                            ...
                                            <p className="text-xs text-slate-700 dark:text-slate-300"><span className="font-semibold">Première réponse</span></p>
                                            <p className="text-[10px] text-slate-400">{new Date(selectedTicket.firstResponseAt).toLocaleDateString('fr-FR')} à {new Date(selectedTicket.firstResponseAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}</p>
                                        </div>
                                    )}
                                    {/* Prise en charge */}
                                    {selectedTicket.startedAt && (
                                        <div className="relative">
                                            ...
                                            <p className="text-xs text-slate-700 dark:text-slate-300"><span className="font-semibold">Pris en charge</span>{selectedTicket.assignedUserName ? ` par ${selectedTicket.assignedUserName}` : ''}</p>
                                            <p className="text-[10px] text-slate-400">{new Date(selectedTicket.startedAt).toLocaleDateString('fr-FR')} à {new Date(selectedTicket.startedAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}</p>
                                        </div>
                                    )}
                                    {/* Escalade */}
                                    {selectedTicket.escalatedAt && (
                                        <div className="relative">
                                            ...
                                            <p className="text-xs text-slate-700 dark:text-slate-300"><span className="font-semibold">Escaladé</span> (×{selectedTicket.escalationCount || 1})</p>
                                            <p className="text-[10px] text-slate-400">{new Date(selectedTicket.escalatedAt).toLocaleDateString('fr-FR')} à {new Date(selectedTicket.escalatedAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}</p>
                                        </div>
                                    )}
                                    {/* Modification */}
                                    {selectedTicket.updatedAt && new Date(selectedTicket.updatedAt).getTime() !== new Date(selectedTicket.createdAt).getTime() && (
                                        <div className="relative">
                                            <div className="absolute -left-[21px] top-1 w-2.5 h-2.5 rounded-full bg-slate-400 border-2 border-white dark:border-slate-900" />
                                            <p className="text-xs text-slate-700 dark:text-slate-300"><span className="font-semibold">Modifié</span></p>
                                            <p className="text-[10px] text-slate-400">{new Date(selectedTicket.updatedAt).toLocaleDateString('fr-FR')} à {new Date(selectedTicket.updatedAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}</p>
                                        </div>
                                    )}
                                    {/* Résolu */}
                                    {selectedTicket.resolvedAt && (
                                        <div className="relative">
                                            ...
                                            <p className="text-xs text-slate-700 dark:text-slate-300"><span className="font-semibold">Résolu</span></p>
                                            <p className="text-[10px] text-slate-400">{new Date(selectedTicket.resolvedAt).toLocaleDateString('fr-FR')} à {new Date(selectedTicket.resolvedAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}</p>
                                        </div>
                                    )}
                                    {/* Clôturé */}
                                    {selectedTicket.closedAt && (
                                        <div className="relative">
                                            ...
                                            <p className="text-xs text-slate-700 dark:text-slate-300"><span className="font-semibold">Clôturé</span></p>
                                            <p className="text-[10px] text-slate-400">{new Date(selectedTicket.closedAt).toLocaleDateString('fr-FR')} à {new Date(selectedTicket.closedAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}</p>
                                        </div>
                                    )}
                                </div>
                            </Card>
                            ...
                            {/* Pièces jointes */}
                            <Card className="p-4 shrink-0">
                                <h4 className="text-xs font-bold text-slate-500 uppercase mb-3 flex items-center gap-2"><Paperclip className="w-3 h-3" /> Pièces jointes</h4>
                                <AttachmentUpload ticketId={selectedTicket.id} />
                            </Card>

                            {/* Tickets Récents */}
                            <Card className="p-4 shrink-0 max-h-64 overflow-y-auto">
                                <h4 className="text-xs font-bold text-slate-500 uppercase mb-3 flex items-center gap-2"><MessageSquare className="w-3 h-3" /> Tickets Récents du Client</h4>
                                {(() => {
                                    ...
                                    return clientTickets.length > 0 ? clientTickets.map(t => (
                                        <div key={t.id} onClick={() => setSelectedTicketId(t.id)} className="p-2 bg-slate-50 dark:bg-slate-800 rounded mb-2 cursor-pointer hover:bg-[var(--primary-dim)] dark:hover:bg-[var(--primary-dim)] transition-colors">
                                            ...
                                            <p className="text-xs font-medium text-slate-700 dark:text-slate-300 line-clamp-1">{t.subject}</p>
                                            <p className="text-[10px] text-slate-400 mt-1">{new Date(t.createdAt).toLocaleDateString('fr-FR')}</p>
                                        </div>
                ...
                activeTab === 'KANBAN' && (
                    <div className="flex-1 flex gap-4 overflow-x-auto pb-4">
                        {KANBAN_COLUMNS.map(col => {
                            ...
                            return (
                                <div key={col.id} className="flex-shrink-0 w-80 flex flex-col bg-slate-100 dark:bg-slate-800 rounded-xl"
                                    onDragOver={e => e.preventDefault()}
                                    onDrop={e => { const ticketId = e.dataTransfer.getData('ticketId'); handleKanbanDrop(ticketId, col.id); }}>
                                    <div className={`p-4 border-b-2 ${col.borderClass}`}>
                                        <div className="flex items-center justify-between">
                                            <h3 className="font-bold text-slate-800 dark:text-white">{col.label}</h3>
                                            <span className="px-2 py-0.5 bg-white dark:bg-slate-700 rounded-full text-sm font-bold">{columnTickets.length}</span>
                                        </div>
                                    </div>
                                    <div className="flex-1 p-2 space-y-2 overflow-y-auto">
                                        {columnTickets.map(ticket => {
                                            const agentName = ticket.assignedUserName || users.find(u => u.id === ticket.assignedTo)?.name || '';
                                            return (
                    ...
                    <div className="flex-1 overflow-y-auto space-y-6 pb-16 lg:pb-0">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <Card className="p-6 text-center">
                                ...
                                <p className={`text-4xl font-bold ${slaStats.respectRate >= 80 ? 'text-green-600' : slaStats.respectRate >= 60 ? 'text-orange-600' : 'text-red-600'}`}>
                        ...
                        <Card className="p-6">
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm">
                                    <thead className="bg-slate-50 dark:bg-slate-800 sticky top-0 z-10">
                                        <tr className="border-b border-slate-200 dark:border-slate-700">
                                            <th className="text-left px-3 py-3 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-300">ID</th>
                                            <th className="text-left px-3 py-3 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-300">Sujet</th>
                                            <th className="text-left px-3 py-3 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-300">Client</th>
                                            <th className="text-left px-3 py-3 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-300">Agent</th>
                                            <th className="text-center px-3 py-3 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-300">Priorité</th>
                                            <th className="text-center px-3 py-3 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-300">Créé le</th>
                                            <th className="text-center px-3 py-3 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-300">Temps restant</th>
                                ...
                                {[...slaStats.critical, ...slaStats.warning].map(t => {
                                    ...
                                    return (
                                    <tr key={t.id} className="density-row border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                        ...
                                        <td className="py-2 px-3 text-slate-700 dark:text-slate-300 max-w-[200px] truncate">{t.subject}</td>
                                        <td className="py-2 px-3 text-slate-600 dark:text-slate-400 text-xs">{clientName || <span className="italic text-slate-400">—</span>}</td>
                                        <td className="py-2 px-3 text-slate-600 dark:text-slate-400 text-xs">{agentName || <span className="italic text-slate-400">Non assigné</span>}</td>
                                        <td className="text-center py-2 px-3"><span className={`px-2 py-0.5 rounded text-xs font-bold ${getPriorityBadge(t.priority)}`}>{t.priority}</span></td>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <Card className="p-6">
                                ...
                                <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-lg mb-4 space-y-3">
                                    <input type="text" placeholder="Label (ex: Bonjour)" className="w-full px-3 py-2 border rounded-lg text-sm"
                                        value={newMacro.label} onChange={e => setNewMacro({ ...newMacro, label: e.target.value })} />
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        {isEditingMacro && (
                                            <button onClick={() => { setIsEditingMacro(null); setNewMacro({ label: '', text: '', category: '' }); }} className="px-4 py-2 text-slate-600">
                                                Annuler
                                            </button>
                                        )}
                                    </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    {macros.map(m => (
                                        <div key={m.id} className="flex items-center justify-between p-3 bg-white dark:bg-slate-900 border rounded-lg">
                                            <div>
                                                <p className="font-medium text-slate-800 dark:text-white">{m.label}</p>
                                                <p className="text-xs text-slate-500 line-clamp-1">{m.text}</p>
                                            </div>
                                            <div className="flex gap-2">
                                                <button onClick={() => { setIsEditingMacro(m); setNewMacro({ label: m.label, text: m.text, category: m.category || '' }); }} className="p-1 hover:bg-slate-100 rounded" title="Modifier la macro">
                                                    <Edit2 className="w-4 h-4 text-slate-400" />
                                                </button>
                                                <button onClick={() => deleteMacro(m.id)} className="p-1 hover:bg-red-50 rounded" title="Supprimer la macro">
                            ...
                            <Card className="p-6">
                                ...
                                <div className="space-y-3">
                                    {Object.entries(slaConfig || {}).map(([key, hours]) => {
                                        ...
                                        return (
                                            <div key={priority} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                                                <div className="flex items-center gap-3">
                                                    <span className={`px-2 py-0.5 rounded text-xs font-bold ${getPriorityBadge(priority)}`}>{priority}</span>
                                ...
                                <div className="space-y-3">
                                    {ticketCategories.map((cat: TicketCategory) => (
                                        <div key={cat.id} className="border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
                                            ...
                                            >
                                                <div className="flex items-center gap-2">
                                                    {expandedCategory === cat.id ? <ChevronDown className="w-4 h-4" /> : <Play className="w-3 h-3 text-slate-400" />}
                                                    <span className="font-medium text-slate-800 dark:text-white">{cat.name}</span>
                                                </div>
                                                <span className="text-xs text-slate-500">{expandedCategory === cat.id ? configSubCategories.length : '?'} sous-catégories</span>
                                            </div>
                                                            ...
                                                            configSubCategories.map((sub: SubCategoryConfig) => {
                                                                const saveSubCat = async (s: SubCategoryConfig) => {
                                                                    ...
                                                                    try {
                                                                        ...
                                                                        setConfigSubCategories(prev => prev.map((x: SubCategoryConfig) => x.id === s.id ? { ...x, _modified: false } : x));
                                                                    } catch (e) { showToast(mapError(e, 'sous-catégorie'), 'error'); }
                                                                };
                                                                return (
                                                                <div key={sub.id} className="flex items-center justify-between p-2 hover:bg-slate-50 dark:hover:bg-slate-800 rounded">
                                                                    ...
                                                                    <div className="flex items-center gap-2">
                                                                        <select
                                                                            ...
                                                                            onChange={(e) => {
                                                                                ...
                                                                                setConfigSubCategories(prev => {
                                                                                    const updated = prev.map((s: SubCategoryConfig) => s.id === sub.id ? { ...s, default_priority: val as SubCategoryConfig['default_priority'], _modified: true } : s);
                                                                        ...
                                                                        <div className="flex items-center gap-1">
                                                                            <input
                                                                                ...
                                                                                onChange={(e) => {
                                                                                    const newSubs = configSubCategories.map((s: SubCategoryConfig) => s.id === sub.id ? { ...s, sla_hours: parseInt(e.target.value), _modified: true } : s);
                                                                                    setConfigSubCategories(newSubs);
                                                                                }}
                                                                                onBlur={() => saveSubCat(sub)}
                                                                            />
            ...
            {/* MODALS */}
            <TicketFormModal
                isOpen={isCreateModalOpen} onClose={() => { setIsCreateModalOpen(false); setStagedAttachments([]); }}
                isEditMode={isEditMode} ticketForm={ticketForm} setTicketForm={setTicketForm}
                formErrors={formErrors} handleSaveTicket={handleSaveTicket} isSaving={isSavingTicket}
                clients={clients} vehicles={vehicles} technicians={ticketStaff} tiers={tiers}
                ticketCategories={ticketCategories} ticketSubcategories={ticketSubcategories}
                slaConfig={slaConfig} invoices={invoices}
                stagedFiles={stagedAttachments} onStagedFilesChange={setStagedAttachments}
            />
            ...
            <Modal isOpen={isImportModalOpen} onClose={() => { setIsImportModalOpen(false); setImportPreview([]); }} title="Importer des Tickets"
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2" htmlFor="import-tickets-file">Fichier CSV</label>
                        <input
                            type="file"
                    ...
                    {importPreview.length > 0 && (
                        <div>
                            <h4 className="font-bold text-slate-700 dark:text-slate-300 mb-2">Aperçu ({importPreview.length} lignes)</h4>
                                ...
                                <table className="w-full text-xs">
                                    ...
                                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                        ...
                                        {importPreview.length > 10 && (
                                            <tr><td colSpan={4} className="px-2 py-1 text-center text-slate-500">... et {importPreview.length - 10} autres</td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
            ...
            <Modal isOpen={isStatusModalOpen} onClose={() => setIsStatusModalOpen(false)} title="Changement de Statut"
                ...
                <div className="space-y-4">
                    ...
                    <textarea className="w-full px-3 py-2 border rounded-lg" rows={3} placeholder="Motif du changement..."
            ...
            <Modal isOpen={!!pendingKanbanDrop} onClose={() => setPendingKanbanDrop(null)} title="Motif du changement de statut"
                ...
                <div className="space-y-4">
                    <p className="text-slate-600 dark:text-slate-300">
                        Ticket <strong>{pendingKanbanDrop?.ticketId}</strong> → <span className="font-bold">{pendingKanbanDrop?.newStatus && getStatusInfo(pendingKanbanDrop.newStatus).label}</span>
                    </p>
                    <textarea className="w-full px-3 py-2 border rounded-lg dark:bg-slate-800 dark:border-slate-600" rows={3}
                        placeholder="Motif obligatoire du changement de statut..." required
                        value={kanbanDropReason} onChange={e => setKanbanDropReason(e.target.value)} />
            ...
            <InterventionForm
                ...
                onClose={() => { setIsInterventionModalOpen(false); setInterventionInitialData(null); }}
                onSave={(intervention) => {
