...
const STATUS_CONFIG = {
  ...
  assigned: { label: 'En cours', color: 'text-blue-500', bg: 'bg-blue-100 dark:bg-blue-900/30', dot: 'bg-blue-500' },
            ...
            <div className="flex items-center gap-1">
              {chatTab === 'internal' && (
                <button onClick={() => setShowNewInternalModal(true)}
                  className="p-1.5 bg-blue-600 text-white hover:bg-blue-700 rounded-lg" title="Nouvelle conversation">
            ...
            {(chatTab === 'support'
              ...
              <button key={f.value} onClick={() => setStatusFilter(f.value)}
                className={`px-3 py-1 text-xs rounded-full font-medium transition-colors ${statusFilter === f.value
                  ? (chatTab === 'support' ? 'bg-blue-600 text-white' : 'bg-green-600 text-white')
            ...
            filteredConversations.map(conv => {
              ...
              return (
                <button key={conv.id} onClick={() => openConversation({ ...conv, conversation_type: isInternal ? 'internal' : 'support' })}
                  className={`w-full text-left p-4 border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors ${activeConversation?.id === conv.id ? 'bg-blue-50 dark:bg-blue-900/20 border-l-2 border-l-blue-500' : ''
                    }`}>
          ...
          <>
            ...
            <div className="flex items-center justify-between px-4 py-3 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <div className={`w-9 h-9 rounded-full flex items-center justify-center ${activeConversation.conversation_type === 'internal' ? 'bg-green-100 dark:bg-green-900/30' : 'bg-blue-100 dark:bg-blue-900/30'}`}>
                  ...
                  return (
                    <div key={msg.id} className={`flex ${isSelf ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[75%] ${isSelf
                        ...
                        <p className={`text-[10px] mt-1 ${isSelf ? 'text-blue-200' : 'text-slate-400'}`}>
              ...
              <div className="p-3 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 px-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800 text-sm text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
