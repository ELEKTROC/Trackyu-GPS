...
export const NotificationComposer: React.FC<NotificationComposerProps> = ({
  ...
  }, [step, selectedType, selectedTemplate, subject, body, recipientType, selectedRecipients, channels, sendImmediately, scheduledAt, showToast]);
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      const formData: ManualNotificationFormData = {
        ...
        body,
        recipientType,
        recipientIds: recipientType === 'ALL_CLIENTS'
      ...
      // Envoyer si immédiat
      if (sendImmediately) {
        const recipients = recipientType === 'ALL_CLIENTS'
          ? availableRecipients
          : selectedRecipients;
        ...
        if (result.success) {
          ...
          showToast(
            `Envoyée avec ${result.failureCount} échec(s) sur ${result.totalRecipients}`,
            'warning'
          );
        }
      } else {
    ...
    user, selectedType, selectedTemplate, subject, body, recipientType,
    selectedRecipients, availableRecipients, channels, sendImmediately,
    scheduledAt, onSuccess, onClose, showToast
  ]);
      ...
      return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-4 gap-2">
              {Object.entries(NOTIFICATION_TYPE_CONFIG).map(([type, config]) => {
                ...
                return (
                  ...
                  >
                    <IconComponent className={`w-5 h-5 ${isSelected ? config.color : 'text-slate-400'}`} />
                    <span className={`text-xs font-medium ${isSelected ? 'text-slate-800' : 'text-slate-500'}`}>
                      {config.label}
                    </span>
                  </button>
                );
          ...
          <div>
            ...
            {errors.channels && (
              <p className="mt-1 text-sm text-red-500">{errors.channels}</p>
            )}
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-4">
            ...
            >
              ...
              <p className="font-medium text-slate-800">Tous les clients</p>
              <p className="text-xs text-slate-500">{availableRecipients.length} client(s)</p>
            </button>
          </div>
    ...
    return (
      <div className="space-y-6">
        <div className="bg-slate-50 p-4 rounded-lg space-y-4">
          <h4 className="font-bold text-slate-800">Récapitulatif</h4>
