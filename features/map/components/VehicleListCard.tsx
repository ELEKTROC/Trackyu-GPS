...
export const VehicleListCard: React.FC<VehicleListCardProps> = React.memo(({ 
    ...
    if (vehicle.status === VehicleStatus.MOVING) { statusText = "En mouvement"; statusColorClass = "text-green-600"; }
    else if (vehicle.status === VehicleStatus.IDLE) { statusText = "Ralenti"; statusColorClass = "text-orange-600"; }
    else if (vehicle.status === VehicleStatus.STOPPED) { statusText = "Arrêté"; statusColorClass = "text-red-600"; }

    // Fuel/Battery Color Logic
    let batteryColor = "text-slate-300 dark:text-slate-600";
    if (vehicle.fuelLevel > 50) batteryColor = "text-green-500";
    else if (vehicle.fuelLevel > 20) batteryColor = "text-orange-500";
    else batteryColor = "text-red-500";
    ...
    const getVehicleIcon = () => {
        if (vehicle.type) {
            switch (vehicle.type) {
                ...
                case 'VAN': return Truck; // Use Truck for Van for now or find better icon
                default: return Car;
            }
        }
    ...
    const getDisplayName = () => {
        ...
        return options.map(opt => {
            ...
            return null;
        }).filter(Boolean).join(' - ') || vehicle.name;
    };
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                    ...
                    {config.showStatusText && (
                        <span className={`text-[10px] font-bold ${statusColorClass}`}>{statusText}</span>
                    )}
                </div>
                
                <div className="flex items-center justify-between mt-2">
                    <div className="text-xs font-mono text-slate-500 dark:text-slate-400 w-16">
                        {config.showSpeed ? (vehicle.status === VehicleStatus.MOVING ? `${Math.round(vehicle.speed)} km/h` : '0 km/h') : ''}
                    </div>
                    ...
                    {config.showTime && (
                        <div className="flex flex-col items-end ml-auto">
                            <span className="text-[9px] text-slate-400 font-medium leading-tight">{getRelativeTime(vehicle.lastUpdated)}</span>
                            <span className="text-[9px] text-slate-500 font-mono leading-tight">{formatSyncDate(vehicle.lastUpdated)}</span>
                        </div>
                    )}
                </div>
            </div>
