...
import { Search, ChevronDown, Settings, LayoutTemplate, Box, Cpu, ArrowUpDown, FileSpreadsheet, Download, X, PackageOpen, Pencil, ArrowRightLeft, Trash2, SlidersHorizontal } from 'lucide-react';
...
export const StockTable: React.FC<StockTableProps> = ({
    ...
    const uniqueTypes = React.useMemo(() => Array.from(new Set(filteredData.map(d => d.type || 'BOX'))).filter(Boolean).sort(), [filteredData]);
    const uniqueModels = React.useMemo(() => Array.from(new Set(filteredData.map(d => d.model || ''))).filter(Boolean).sort(), [filteredData]);
    const uniqueResellers = React.useMemo(() => Array.from(new Set(filteredData.map(d => d.resellerName || ''))).filter(Boolean).sort(), [filteredData]);

    // Données filtrées mobile (applique les filtres supplémentaires localement)
    const mobileFilteredData = React.useMemo(() => filteredData.filter(d => {
    ...
    const mobileActiveCount = (statusFilter !== 'ALL' ? 1 : 0) + (selectedClient !== 'all' ? 1 : 0) + (mobileTypeFilter !== 'all' ? 1 : 0) + (mobileModelFilter !== 'all' ? 1 : 0) + (mobileResellerFilter !== 'all' ? 1 : 0);
    ...
    return (
        <Card className="flex-1 flex flex-col border-slate-200 dark:border-slate-700 shadow-sm" title={
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 w-full">
                <div className="flex items-center gap-4">
                    <span className="text-sm text-slate-500 dark:text-slate-400 hidden sm:block">{filteredData.length} éléments</span>
                </div>
                        ...
                        <div className="relative" ref={columnMenuRef}>
                            ...
                            {isColumnMenuOpen && (
                                <div className="absolute top-full right-0 mt-2 w-48 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                                    <div className="p-2 bg-slate-50 dark:bg-slate-900 border-b border-slate-100 dark:border-slate-700 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase">Colonnes</div>
            ...
            <div className="bg-white dark:bg-slate-900 flex flex-col relative">
                ...
                {filteredData.length === 0 ? (
                    <EmptyState
                        ...
                        description={searchTerm ? `Aucun résultat pour "${searchTerm}". Essayez une autre recherche.` : "Votre stock est vide. Commencez par ajouter des équipements."}
                    ...
                    <>
                        ...
                        <MobileCardList bordered={false}>
                            {mobileFilteredData.slice(0, mobileDisplayCount).map((item: DeviceStock) => {
                                const borderColor =
                                    item.status === 'IN_STOCK' ? 'border-l-blue-500' :
                                    item.status === 'INSTALLED' ? 'border-l-green-500' :
                                    item.status === 'RMA_PENDING' ? 'border-l-amber-500' :
                                    item.status === 'SENT_TO_SUPPLIER' ? 'border-l-purple-500' :
                                    (item.status === 'REMOVED' || item.status === 'LOST') ? 'border-l-red-500' :
                                    'border-l-slate-400';
                                const statusLabel =
                                    item.status === 'INSTALLED' ? 'Installé' :
                                    item.status === 'IN_STOCK' ? 'En Stock' :
                                    item.status === 'RMA_PENDING' ? 'SAV: Attente' :
                                    item.status === 'SENT_TO_SUPPLIER' ? 'Chez Fournisseur' :
                                ...
                                const locationLabel =
                                    item.location === 'CLIENT' ? 'Chez Client' :
                                    item.location === 'TECH' ? `Tech: ${item.technicianId || '?'}` :
                                    item.location === 'SIEGE' ? 'Siège' : 'Dépôt';
                                    ...
                                    <MobileCard key={item.id} borderColor={borderColor}>
                                        <div className="flex items-start justify-between gap-2">
                                            <div className="min-w-0">
                                                <p className="text-sm font-semibold font-mono text-slate-800 dark:text-slate-100 truncate">{item.imei || item.id}</p>
                                                <p className="text-xs text-slate-500 dark:text-slate-400">{item.model || item.type || '-'}</p>
                                            </div>
                                            <span className={`shrink-0 px-2 py-0.5 rounded-full text-[10px] font-semibold ${statusColors}`}>{statusLabel}</span>
                                        </div>
                                        ...
                                        <div className="mt-2 flex items-center gap-2">
                                            {onEditClick && <MobileCardAction icon={<Pencil className="w-3 h-3" />} color="blue" onClick={() => onEditClick!(item)}>Modifier</MobileCardAction>}
                                            {onTransferClick && <MobileCardAction icon={<ArrowRightLeft className="w-3 h-3" />} color="purple" onClick={() => onTransferClick!(item)}>Transférer</MobileCardAction>}
                                            <MobileCardAction icon={<Settings className="w-3 h-3" />} color="slate" onClick={() => onDetailClick(item)}>Détails</MobileCardAction>
                                        </div>
                                    </MobileCard>
                                );
                            })}
                        ...
                        <MobileFilterSheet
                            ...
                            onReset={() => { setStatusFilter('ALL'); setSelectedClient('all'); setMobileTypeFilter('all'); setMobileModelFilter('all'); setMobileResellerFilter('all'); }}
                            tabs={[
                                {
                                    id: 'status', label: 'Statut',
                                    ...
                                    content: (
                                        <>
                                            {[
                                                ...
                                                <FilterRadioRow key={opt.value} value={opt.value} label={opt.label} checked={statusFilter === opt.value} onChange={() => setStatusFilter(opt.value)} />
                                            ))}
                                        </>
                                    )
                                },
                                {
                                    ...
                                    content: (
                                        <>
                                            <FilterRadioRow value="all" label="Tous les clients" checked={selectedClient === 'all'} onChange={() => setSelectedClient('all')} />
                                            {uniqueClients.map(c => (
                                                <FilterRadioRow key={c} value={c} label={c} checked={selectedClient === c} onChange={() => setSelectedClient(c)} count={filteredData.filter(d => d.client === c).length} />
                                            ))}
                                        </>
                                    )
                                },
                                {
                                    ...
                                    content: (
                                        <>
                                            <FilterRadioRow value="all" label="Tous les types" checked={mobileTypeFilter === 'all'} onChange={() => setMobileTypeFilter('all')} />
                                            {uniqueTypes.map(t => (
                                                <FilterRadioRow key={t} value={t} label={t} checked={mobileTypeFilter === t} onChange={() => setMobileTypeFilter(t)} count={filteredData.filter(d => (d.type || 'BOX') === t).length} />
                                            ))}
                                        </>
                                    )
                                },
                                {
                                    id: 'model', label: 'Modèle GPS',
                                    activeCount: mobileModelFilter !== 'all' ? 1 : 0,
                                    content: (
                                        <>
                                            <FilterRadioRow value="all" label="Tous les modèles" checked={mobileModelFilter === 'all'} onChange={() => setMobileModelFilter('all')} />
                                            {uniqueModels.map(m => (
                                                <FilterRadioRow key={m} value={m} label={m} checked={mobileModelFilter === m} onChange={() => setMobileModelFilter(m)} count={filteredData.filter(d => d.model === m).length} />
                                            ))}
                                        </>
                                    )
                                },
                                {
                                    ...
                                    content: uniqueResellers.length === 0
                                        ? <p className="text-sm text-slate-400 text-center py-8">Aucun revendeur disponible</p>
                                        : (
                                            <>
                                                <FilterRadioRow value="all" label="Tous les revendeurs" checked={mobileResellerFilter === 'all'} onChange={() => setMobileResellerFilter('all')} />
                                                {uniqueResellers.map(r => (
                                                    <FilterRadioRow key={r} value={r} label={r} checked={mobileResellerFilter === r} onChange={() => setMobileResellerFilter(r)} count={filteredData.filter(d => d.resellerName === r).length} />
                                                ))}
                                            </>
                                        )
                                },
                            ...
                            <thead className="bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-300 sticky top-0 z-10">
                                <tr>
                                    ...
                                    {currentColumns.map(col => visibleColumns.includes(col.id) && (
                                        <th
                                            key={col.id}
                                            className={`px-6 py-3 text-xs font-bold uppercase border-b border-slate-200 dark:border-slate-700 ${col.id === 'actions' ? 'text-right' : ''} ${col.id !== 'actions' && col.id !== 'id' ? 'cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors' : ''}`}
                                ...
                                {paginatedData.map((item: DeviceStock) => (
                                    <tr key={item.id} className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group ${selectedIds.has(item.id) ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
                                        ...
                                        {visibleColumns.includes('id') && (
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="p-2 bg-slate-100 dark:bg-slate-700 rounded border border-slate-200 dark:border-slate-600 text-slate-500 dark:text-slate-300">
                                                        {activeTab === 'devices' ? <Cpu className="w-4 h-4" /> : activeTab === 'sims' ? <Cpu className="w-4 h-4" /> : <Box className="w-4 h-4" />}
                                                    </div>
                                                    <div>
                                                        <p className="text-sm font-semibold text-slate-700 dark:text-slate-200 font-mono">{item.id}</p>
                                                        {item.id !== item.imei && item.imei && <p className="text-xs font-mono text-slate-500 dark:text-slate-400">{item.imei}</p>}
                                                    </div>
                                                </div>
                                            </td>
                                        )}
                                        ...
                                        {visibleColumns.includes('model') && (
                                            <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-300 font-medium">{item.model || '-'}</td>
                                        )}

                                        {visibleColumns.includes('imei') && (
                                            <td className="px-6 py-4 text-sm font-mono text-slate-600 dark:text-slate-300">{item.imei || item.serialNumber || '-'}</td>
                                        )}
                                        ...
                                        {visibleColumns.includes('operator') && (
                                            <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-300 font-medium">
                                                <span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-xs font-semibold">
                                                    {item.operator || '-'}
                                                </span>
                                            </td>
                                        )}
                                                ...
                                                <span className={`px-2 py-1 rounded-full text-xs font-semibold border uppercase ${item.status === 'INSTALLED' ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 border-green-200 dark:border-green-800' :
                                                    ...
                                                    {item.status === 'INSTALLED' ? 'Installé' :
                                                        item.status === 'IN_STOCK' ? 'En Stock' :
                                                            item.status === 'RMA_PENDING' ? 'SAV: Attente' :
                                                                item.status === 'SENT_TO_SUPPLIER' ? 'Chez Fournisseur' :
                                                                    item.status === 'REMOVED' ? 'Retiré (Audit)' :
                                                                        item.status}
                                                </span>
                                            </td>
                                        )}
                                        ...
                                        {visibleColumns.includes('location') && (
                                            <td className="px-6 py-4">
                                                <span className={`px-2 py-1 rounded-full text-xs font-semibold border uppercase ${item.location === 'CLIENT' ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 border-green-200 dark:border-green-800' :
                                                    item.location === 'TECH' ? 'bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400 border-purple-200 dark:border-purple-800' :
                                                    item.location === 'SIEGE' ? 'bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400 border-orange-200 dark:border-orange-800' :
                                                        'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700'
                                                    }`}>
                                                    {item.location === 'CLIENT' ? 'Chez Client' : item.location === 'TECH' ? `Tech: ${item.technicianId || '?'}` : item.location === 'SIEGE' ? 'Siège' : 'Dépôt Central'}
                                                </span>
                                            </td>
                                        )}
                                        ...
                                        {visibleColumns.includes('client') && (
                                            <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-300">{resolveClientName(item)}</td>
                                        )}

                                        {visibleColumns.includes('sim') && (
                                            <td className="px-6 py-4 text-xs text-slate-500">
                                                {item.simCardId ? (
                                                    <span className="font-bold text-slate-700 dark:text-slate-300">{item.phoneNumber || item.simCardId}</span>
                                                ) : '-'}
                                            </td>
                                        )}

                                        {visibleColumns.includes('entryDate') && (
                                            <td className="px-6 py-4 text-xs text-slate-500">{item.entryDate ? new Date(item.entryDate).toLocaleDateString('fr-FR') : '-'}</td>
                                        )}
                                        {visibleColumns.includes('installationDate') && (
                                            <td className="px-6 py-4 text-xs text-slate-500">{item.installationDate ? new Date(item.installationDate).toLocaleDateString('fr-FR') : '-'}</td>
                                        )}
                                        {visibleColumns.includes('removalDate') && (
                                            <td className="px-6 py-4 text-xs text-slate-500">{item.removalDate ? new Date(item.removalDate).toLocaleDateString('fr-FR') : '-'}</td>
                                        )}

                                        {visibleColumns.includes('tech_info') && (
                                            <td className="px-6 py-4 text-xs text-slate-500">{item.simCardId}</td>
                                        )}
                                                ...
                                                <div className="flex items-center justify-end gap-1">
                                                        ...
                                                        <>
                                                            ...
                                                            {item.status === 'SENT_TO_SUPPLIER' && (
                                                                <div className="flex gap-1">
                                                                    <button onClick={() => onRmaAction(item, 'RECEIVE_OK')} className="px-2 py-1 bg-green-600 text-white text-[10px] font-bold rounded hover:bg-green-700 transition-colors">Reçu OK</button>
                                                                    <button onClick={() => onRmaAction(item, 'RECEIVE_REPLACE')} className="px-2 py-1 bg-purple-600 text-white text-[10px] font-bold rounded hover:bg-purple-700 transition-colors">Échange</button>
                                                                </div>
                                                            )}
                                                            {item.status === 'REMOVED' && (
                                                                <button onClick={() => onRmaAction(item, 'RESTORE')} className="px-2 py-1 bg-emerald-600 text-white text-[10px] font-bold rounded hover:bg-emerald-700 transition-colors">Réintégrer</button>
                                                            )}
                                                            {['RMA_PENDING', 'SENT_TO_SUPPLIER', 'REMOVED'].includes(item.status) && (
                                                                <button onClick={() => onRmaAction(item, 'SCRAP')} className="px-2 py-1 bg-slate-200 text-slate-700 text-[10px] font-bold rounded hover:bg-slate-300 transition-colors" title="Mettre au rebut">Pilon</button>
                                                            )}
                                                        </>
                                                    )}
                                                    {onEditClick && (
                                                    ...
                                                    {onTransferClick && (
                                                        <button
                                                            ...
                                                            className="p-1.5 hover:bg-purple-50 dark:hover:bg-purple-900/20 rounded-lg text-slate-400 hover:text-purple-600 dark:hover:text-purple-400 transition-colors"
                                                            title="Transférer"
                                                        >
                                                    ...
                                                    {onDeleteClick && ['IN_STOCK', 'REMOVED', 'SCRAPPED', 'LOST'].includes(item.status) && (
                                                        <button
                                                            ...
                                                            className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg text-slate-400 hover:text-red-600 dark:hover:text-red-400 transition-colors"
                                                            title="Supprimer"
                                                        >
            ...
            {filteredData.length > 0 && (
                <div className="p-3 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex justify-between items-center text-xs">
                    <div className="flex items-center gap-2">
                        <span className="text-slate-500 dark:text-slate-400">Lignes par page:</span>
                        <select value={itemsPerPage} onChange={(e) => setItemsPerPage(Number(e.target.value))} className="text-xs border border-slate-200 dark:border-slate-700 rounded bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 p-1">
