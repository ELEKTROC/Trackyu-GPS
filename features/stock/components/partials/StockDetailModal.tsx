...
export const StockDetailModal: React.FC<StockDetailModalProps> = ({ isOpen, onClose, item }) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Détail Équipement : ${item.id}`} footer={
            <button onClick={onClose} className="px-4 py-2 bg-slate-100 dark:bg-slate-700 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors">Fermer</button>
        }>
                ...
                <div className="flex items-start gap-4 p-4 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                    ...
                    <div className="flex-1">
                        <div className="flex justify-between items-start">
                            <div>
                                <h3 className="font-bold text-lg text-slate-800 dark:text-white">{item.model}</h3>
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
                            <div className="flex items-center gap-2 text-slate-600 dark:text-slate-300">
                                ...
                                <span>{item.location === 'TECH' ? `Tech: ${item.technicianId}` :
                                    item.location === 'SIEGE' ? 'Siège' : 'Dépôt Central'}</span>
                            </div>
                    ...
                    <div className="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-xl border border-purple-100 dark:border-purple-800">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                ...
                                <span className="font-mono font-bold text-purple-900 dark:text-purple-100">{item.phoneNumber || '-'}</span>
                            </div>
                            <div>
