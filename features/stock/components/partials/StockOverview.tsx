    ...
    return (
        <div className="flex-1 overflow-y-auto space-y-4 p-1 pb-16 lg:pb-1">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <Card className="p-4">
                    <h3 className="text-sm font-bold text-slate-800 dark:text-white mb-3">État Boîtiers</h3>
                    <div className="h-48">
                ...
                <Card className="p-4">
                    <h3 className="text-sm font-bold text-slate-800 dark:text-white mb-3">SIMs: Actives vs Stock</h3>
                    <div className="h-48">
                        <ResponsiveContainer width="100%" height="100%" minHeight={180} minWidth={180} initialDimension={{ width: 180, height: 180 }}>
                            <RePieChart>
                                <Pie
                ...
                {/* SIM par Opérateur */}
                <Card className="p-4">
                    <h3 className="text-sm font-bold text-slate-800 dark:text-white mb-3">SIMs par Opérateur</h3>
                    <div className="h-48">
                        <ResponsiveContainer width="100%" height="100%" minHeight={180} minWidth={180} initialDimension={{ width: 180, height: 180 }}>
                            <BarChart
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <Card className="p-4">
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <h3 className="text-sm font-bold text-slate-800 dark:text-white">Stock par Technicien</h3>
                    </div>
                    <div className="space-y-2 max-h-48 overflow-y-auto">
                        {Object.entries(stats.stockByTech).length === 0 ? (
                            <p className="text-sm text-slate-500 text-center py-4">Aucun technicien configuré</p>
                        ) : (
                ...
                <Card className="p-4">
                    <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="text-sm font-bold text-slate-800 dark:text-white">Historique récent</h3>
                        </div>
                        <div className="flex items-center gap-2">
                            <select
                                value={historyFilter}
                                onChange={(e) => { setHistoryFilter(e.target.value); setHistoryPage(1); }}
                                className="text-xs border border-slate-200 dark:border-slate-700 rounded px-2 py-1 bg-white dark:bg-slate-800 dark:text-white"
                            >
                    ...
                    <div className="space-y-1">
                        {paginatedHistory.length === 0 ? (
                            <p className="text-sm text-slate-500 text-center py-4">Aucun mouvement</p>
                        ) : (
                            paginatedHistory.map((mv: StockMovement) => {
                                ...
                                return (
                                    <div key={mv.id} className="flex items-center justify-between p-2 bg-slate-50 dark:bg-slate-800 rounded text-xs">
                                        <div className="flex items-center gap-2">
                                            <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${typeColors[mv.type] || 'bg-slate-100 text-slate-600'}`}>
                                                {mv.type}
                                            </span>
                                            <span className="text-slate-600 dark:text-slate-300">{device?.imei || device?.iccid || mv.deviceId}</span>
                                        </div>
                                        <span className="text-slate-500">{new Date(mv.date).toLocaleDateString('fr-FR')}</span>
                                    </div>
                                );
                            })
                        )}
                    ...
                    {totalHistoryPages > 1 && (
                        <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-200 dark:border-slate-700">
                            ...
                            <span className="text-xs text-slate-500">{historyPage}/{totalHistoryPages}</span>
                            <button
            ...
            <Card className="p-4">
                <h3 className="text-sm font-bold text-slate-800 dark:text-white mb-3">Répartition complète du Stock</h3>
                <div className="h-48" style={{ minHeight: 180, minWidth: 200 }}>
                    <ResponsiveContainer width="100%" height="100%" minHeight={180} minWidth={200} initialDimension={{ width: 200, height: 180 }}>
                        <BarChart
                            data={[
                                { name: 'Boîtiers GPS', enStock: stats.inStockDevices, installé: stats.installedDevices, rma: stats.rmaDevices },
                                { name: 'Cartes SIM', enStock: stats.inStockSims, installé: stats.activeSims, rma: 0 },
                                { name: 'Accessoires', enStock: stats.inStockAccessories, installé: stats.installedAccessories, rma: 0 },
                            ]}
