...
export const AddDeviceModal: React.FC<AddDeviceModalProps> = ({
    isOpen, onClose, newItem, setNewItem, catalogItems, deviceModels, tiers, onSave
}) => {
    // Label contextuel selon le type
    const productLabel = newItem.type === 'SIM' ? 'Opérateur'
        : newItem.type === 'BOX' ? 'Modèle GPS'
        : 'Produit';

    const selectClass = "w-full px-3 py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300";
        ...
        <Modal isOpen={isOpen} onClose={onClose} title="Ajouter un équipement" footer={
            ...
            <div className="p-4 space-y-4">
                <div>
                    ...
                    <select
                        value={newItem.type}
                        onChange={(e) => setNewItem({ ...newItem, type: e.target.value as DeviceStock['type'], model: '', operator: '' })}
                ...
                <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Revendeur (Propriétaire)</label>
                    <select
                    ...
                    >
                        ...
                        {tiers.filter(t => t.type === 'RESELLER').map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
                    </select>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        {newItem.type === 'SIM' ? (
                            ...
                            >
                                ...
                                {deviceModels.map(dm => (
                                    <option key={dm.id} value={`${dm.brand} ${dm.model}`}>{dm.brand} {dm.model}</option>
                                ))}
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className={selectClass}
                            placeholder={newItem.type === 'BOX' ? 'Ex: 860012345678901' : newItem.type === 'SIM' ? 'Ex: 8933...' : 'Ex: SN-001'}
                        />
                    </div>
                </div>
                ...
                {newItem.type === 'SIM' && (
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Numéro de Téléphone (MSISDN)</label>
                        <input
                            type="text"
                    ...
                    <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800">
                        ...
                        <div className="text-xs text-slate-600 dark:text-slate-300 font-mono">
                            {newItem.operator === 'Orange' ? 'APN: orangecidata (User: data, Pass: data)' :
                                ...
                                newItem.operator === 'Maroc Telecom' ? 'APN: internet1 (User: vide, Pass: vide)' :
                                newItem.operator === 'Inwi' ? 'APN: inwi (User: vide, Pass: vide)' :
...
export const AssignModal: React.FC<AssignModalProps> = ({
    isOpen, onClose, selectedItem, selectedClientForAssign, setSelectedClientForAssign, selectedVehicleForAssign, setSelectedVehicleForAssign, vehicles, stock, clients, onConfirm
}) => {
        ...
        <Modal isOpen={isOpen} onClose={onClose} title={isSim ? "Lier à un Boîtier GPS" : "Assigner à un Client"} footer={
            ...
            <div className="p-4 space-y-4">
                {isSim ? (
                    /* SIM → Boîtier */
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Sélectionner le Boîtier</label>
                        <select
                            value={selectedVehicleForAssign}
                            onChange={(e) => setSelectedVehicleForAssign(e.target.value)}
                            className="w-full p-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300"
                        >
                            <option value="">-- Sélectionner --</option>
                            {stock.filter(s => s.type === 'BOX').map(box => (
                    ...
                    <>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className="w-full p-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 mb-1"
                            />
                            <select
                        ...
                        {selectedClientForAssign && (
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
                                    Véhicule <span className="text-xs text-slate-400">(optionnel)</span>
                                </label>
                                <select
                                    value={selectedVehicleForAssign}
                                    onChange={(e) => setSelectedVehicleForAssign(e.target.value)}
                                    className="w-full p-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300"
                                >
                                    <option value="">{clientVehicles.length === 0 ? 'Aucun véhicule — création auto' : '-- Sélectionner un véhicule --'}</option>
...
export const TransferModal: React.FC<TransferModalProps> = ({
    isOpen, onClose, selectedIds, transferTarget, selectedTechId, setSelectedTechId, users, onConfirm
}) => {
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Transférer le stock" footer={
            ...
            <div className="p-4 space-y-4">
                <p className="text-sm text-slate-600 dark:text-slate-400">
                    Vous allez transférer <strong>{selectedIds.size}</strong> élément(s) vers <strong>{transferTarget === 'TECH' ? 'un Technicien' : transferTarget === 'SIEGE' ? 'le Siège' : 'le Dépôt Central'}</strong>.
                </p>

                {transferTarget === 'TECH' && (
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Sélectionner le technicien</label>
                        <select
                        ...
                        >
                            <option value="">Choisir...</option>
                            {users.filter(u => u.role.toLowerCase().includes('technicien') || u.role === 'TECH').map(tech => (
...
export const BulkImportModal: React.FC<BulkImportModalProps> = ({
    isOpen, onClose, bulkImportPreview, onDownloadTemplate, onFileUpload, onImport, fileInputRef
}) => {
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Import en bloc" footer={
            <>
                <button onClick={onClose} className="px-4 py-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors">Annuler</button>
                <button onClick={onImport} disabled={bulkImportPreview.length === 0} className="px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
            ...
            <div className="space-y-4">
                ...
                <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Fichier CSV</label>
                    <input
                        type="file"
                        ref={fileInputRef}
                        accept=".csv,.txt"
                ...
                {bulkImportPreview.length > 0 && (
                    <div>
                        <h4 className="font-bold text-slate-700 dark:text-slate-300 mb-2">Aperçu ({bulkImportPreview.length} lignes)</h4>
...
export const EditDeviceModal: React.FC<EditDeviceModalProps> = ({
    isOpen, onClose, item, catalogItems: _catalogItems, tiers, deviceModels, vehicles, allDevices, onSave
}) => {
    ...
    const statusLabels: Record<string, string> = {
        IN_STOCK: 'En Stock', INSTALLED: 'Installé', RMA_PENDING: 'SAV: Attente',
        SENT_TO_SUPPLIER: 'Chez Fournisseur', REMOVED: 'Retiré', LOST: 'Perdu', SCRAPPED: 'Rebut'
    };

    // Find linked SIM
    const linkedSim = item.type === 'BOX' ? allDevices.find(d => d.type === 'SIM' && d.assignedVehicleId === item.assignedVehicleId && item.assignedVehicleId) : null;

    const readOnlyClass = "w-full px-3 py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 cursor-not-allowed";
    const editableClass = "w-full px-3 py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300";
        ...
        <Modal isOpen={isOpen} onClose={onClose} title="Modifier l'équipement" footer={
            ...
            <div className="p-4 space-y-4 max-h-[70vh] overflow-y-auto">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Type</label>
                        <input type="text" value={editData.type === 'BOX' ? 'Boîtier GPS' : editData.type === 'SIM' ? 'Carte SIM' : editData.type || ''} disabled className={readOnlyClass} />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Statut</label>
                        <input type="text" value={statusLabels[editData.status || ''] || editData.status || '-'} disabled className={readOnlyClass} />
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input
                            type="text"
                            value={editData.type === 'BOX' ? (editData.imei || '') : editData.type === 'SIM' ? (editData.iccid || '') : (editData.serialNumber || '')}
                            onChange={(e) => {
                ...
                <div className="border-t border-slate-200 dark:border-slate-700 pt-3">
                    <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-3">Informations d'affectation</p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Affectation (Véhicule)</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    {editData.type === 'BOX' && (
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">SIM liée</label>
                            <input type="text" value={linkedSim ? `${linkedSim.phoneNumber || linkedSim.iccid || linkedSim.serialNumber}` : '-'} disabled className={readOnlyClass} />
                        </div>
                    )}
                </div>
                ...
                {/* Separator */}
                <div className="border-t border-slate-200 dark:border-slate-700 pt-3">
                    <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-3">Dates</p>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" value={editData.entryDate ? new Date(editData.entryDate).toLocaleDateString('fr-FR') : '-'} disabled className={readOnlyClass} />
                    </div>
                    <div>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Date de sortie</label>
                        <input
                            type="date"
...
export const IndividualTransferModal: React.FC<IndividualTransferModalProps> = ({
    isOpen, onClose, item, users, onConfirm
}) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Transférer l'équipement" footer={
            <>
                <button onClick={onClose} className="px-4 py-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors">Annuler</button>
                <button
            ...
            <div className="p-4 space-y-4">
                <p className="text-sm text-slate-600 dark:text-slate-400">
                    Transférer <strong className="text-slate-800 dark:text-white">{identifier}</strong> ({item.model})
                </p>
                <p className="text-xs text-slate-500">Localisation actuelle : <strong>{item.location === 'TECH' ? `Technicien (${item.technicianId || '?'})` : item.location === 'SIEGE' ? 'Siège' : 'Dépôt Central'}</strong></p>
