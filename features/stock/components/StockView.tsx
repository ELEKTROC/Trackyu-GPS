...
import {
    Box, Plus, Cpu, PackageOpen, PieChart,
    History, AlertTriangle, Upload, Smartphone, Activity
} from 'lucide-react';
...
const STOCK_TABS = [
    ...
    { id: 'movements',    label: 'Mouvements',     icon: History,      color: 'bg-slate-500',  description: 'Historique des mouvements' },
...
export const StockView: React.FC<StockViewProps> = ({ initialTab }) => {
    ...
    const visibleStockTabs = isMobile
        ? STOCK_TABS.filter(t => !STOCK_MOBILE_HIDDEN.has(t.id))
        : STOCK_TABS;
    const { vehicles, stock, updateDevice, addDevice, deleteDevice, users, catalogItems, tiers, stockMovements } = useDataContext();
    const { user } = useAuth();
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'devices' | 'sims' | 'accessories' | 'movements' | 'rma'>('overview');
    ...
    const uniqueClients = useMemo(() => {
        return Array.from(new Set(stock.map(item => item.client || ''))).filter(Boolean).sort();
    }, [stock]);

    const uniqueOperators = useMemo(() => {
        return Array.from(new Set(stock.filter(d => d.type === 'SIM').map(item => item.operator || ''))).filter(Boolean).sort();
    }, [stock]);
    ...
    useEffect(() => {
        api.techSettings.getDeviceModels().then((models: { id: string; brand: string; model: string; is_active?: boolean }[]) => {
            setDeviceModels(models.filter((m) => m.is_active !== false));
        }).catch(() => { /* device models optional */ });
    }, []);
...
ACCESSORY;;;;;"Capteur Température";SN-001;IN_STOCK;CENTRAL;Client ABC;Capteur`;
    ...
    const handleBulkImport = () => {
        ...
        if (rowErrors.length > 0) {
            ...
            showToast(`${successCount} importé(s), ${rowErrors.length} erreur(s) — ${preview}${suffix}`, successCount > 0 ? 'warning' : 'error');
        } else {
    ...
    const filteredData = useMemo(() => {
        if (activeTab === 'devices') {
            return stock.filter(d => {
                ...
                const matchesSearch = (d.imei || '').includes(searchTerm) || (d.model || '').toLowerCase().includes(searchTerm.toLowerCase()) || (d.assignedVehicleId?.toLowerCase().includes(searchTerm.toLowerCase()) ?? false);
            ...
            return stock.filter(d => {
                ...
                const matchesSearch = (d.iccid || '').includes(searchTerm) || (d.phoneNumber || '').includes(searchTerm) || (d.imei || '').includes(searchTerm) || (d.model || '').toLowerCase().includes(searchTerm.toLowerCase()) || (d.operator || '').toLowerCase().includes(searchTerm.toLowerCase());
                const matchesStatus = statusFilter === 'ALL' || d.status === statusFilter;
                const matchesClient = selectedClient === 'all' || d.client === selectedClient;
            ...
            return stock.filter(d => {
                ...
                const matchesSearch = (d.imei || d.serialNumber || '').includes(searchTerm) || (d.model || '').toLowerCase().includes(searchTerm.toLowerCase());
            ...
            return stock.filter(d => {
                ...
                const matchesSearch = (d.serialNumber || '').includes(searchTerm) || (d.model || '').toLowerCase().includes(searchTerm.toLowerCase());
                const matchesStatus = statusFilter === 'ALL' || d.status === statusFilter;
                const matchesClient = selectedClient === 'all' || d.client === selectedClient;
                return matchesSearch && matchesStatus && matchesClient;
            });
    ...
    const handleExportCSV = () => {
        const headers = activeTab === 'devices'
    ...
    const handleExportPDF = () => {
        const title = activeTab === 'devices' ? 'Inventaire Boîtiers GPS'
            : activeTab === 'sims' ? 'Inventaire Cartes SIM'
                : activeTab === 'rma' ? 'Équipements SAV / RMA'
    ...
    const confirmAssignment = async () => {
        ...
        try {
            if (selectedItem.type === 'SIM') {
                ...
                if (box.simCardId) {
                    const confirmSwap = await confirm({ message: `Ce boîtier a déjà une SIM (${box.simCardId}). Voulez-vous la remplacer ? L'ancienne SIM sera remise en stock.`, variant: 'warning', title: 'Remplacement de SIM', confirmLabel: 'Remplacer' });
    ...
    const handleBulkTransfer = async () => {
        ...
        showToast(TOAST.STOCK.BATCH_TRANSFERRED(updates.length, transferTarget === 'TECH' ? 'Technicien' : transferTarget === 'SIEGE' ? 'Siège' : 'Dépôt Central'), 'success');
        setIsTransferModalOpen(false);
        setSelectedIds(new Set());
    };

    const handleRmaAction = async (item: DeviceStock, action: 'SEND' | 'RECEIVE_OK' | 'RECEIVE_REPLACE' | 'SCRAP' | 'RESTORE') => {
    ...
    const handleIndividualTransfer = async (target: 'CENTRAL' | 'SIEGE' | 'TECH', techId?: string) => {
        ...
        try {
            ...
            showToast(TOAST.STOCK.TRANSFERRED(target === 'TECH' ? 'Technicien' : target === 'SIEGE' ? 'Siège' : 'Dépôt Central'), 'success');
    ...
    return (
        <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-500 sm:h-[calc(100vh-140px)] sm:flex sm:flex-col">
            ...
            <div className="flex items-center justify-between shrink-0">
                <h1 className="hidden sm:block text-xl sm:text-2xl font-bold text-slate-800 dark:text-white">Gestion du Stock</h1>
            ...
            <div className="hidden sm:grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 shrink-0">
                ...
                <Card className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 p-3">
                    <div className="flex items-center gap-2">
                        <div className="p-1.5 bg-green-100 dark:bg-green-900/20 text-green-600 rounded-lg"><Activity className="w-4 h-4" /></div>
                        <div>
                ...
                <Card className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 p-3">
                    <div className="flex items-center gap-2">
                        <div className="p-1.5 bg-red-100 dark:bg-red-900/20 text-red-600 rounded-lg"><AlertTriangle className="w-4 h-4" /></div>
                        <div>
                ...
                <Card className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 p-3">
                    <div className="flex items-center gap-2">
                        <div className="p-1.5 bg-purple-100 dark:bg-purple-900/20 text-purple-600 rounded-lg"><Smartphone className="w-4 h-4" /></div>
                        <div>
                ...
                <Card className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 p-3">
                    <div className="flex items-center gap-2">
                        <div className="p-1.5 bg-cyan-100 dark:bg-cyan-900/20 text-cyan-600 rounded-lg"><Smartphone className="w-4 h-4" /></div>
                        <div>
                ...
                <Card className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 p-3">
                    <div className="flex items-center gap-2">
                        <div className="p-1.5 bg-orange-100 dark:bg-orange-900/20 text-orange-600 rounded-lg"><PackageOpen className="w-4 h-4" /></div>
                        <div>
            ...
            {activeTab === 'overview' ? (
                ...
                <StockTable
                    ...
                    onDetailClick={(item) => { setDetailItem(item); setIsDetailModalOpen(true); }}
            ...
            <AddDeviceModal
                ...
                onSave={() => {
                    try {
                        ...
                        const resolvedModel = newItem.model === 'OTHER'
                            ? (newItem.customModel || 'Autre')
                            : newItem.model;

                        // For BOX/GPS_TRACKER: ID = IMEI (backend handles tenant via JWT)
                        const deviceId = (newItem.type === 'BOX' || !newItem.type)
                            ? newItem.imei || `DEV-${Date.now()}`
                            : `DEV-${Date.now()}`;
            ...
            <AssignModal
                ...
                vehicles={vehicles}
                stock={stock}
                clients={tiers.filter(t => t.type === 'CLIENT').map(t => ({ id: t.id, name: t.name, tenantId: t.tenantId }))}
            ...
            <IndividualTransferModal
                ...
                onClose={() => { setIsIndividualTransferOpen(false); setTransferItem(null); }}
                item={transferItem}
                users={users}
