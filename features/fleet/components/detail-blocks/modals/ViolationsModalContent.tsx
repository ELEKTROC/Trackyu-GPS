...
export const ViolationsModalContent: React.FC = () => {
  ...
  const violations = [
    ...
    },
    {
      ...
      details: '-0.6g décélération',
    },
    {
      id: 3,
      type: 'Accélération brusque',
      severity: 'low',
      date: '28/11/2025',
      location: 'Sortie Entrepôt',
      details: 'Départ arrêté agressif',
    },
    ...
    <div className="p-6 space-y-8">
      ...
      <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-6 text-white flex items-center justify-between shadow-lg">
        <div>
          ...
          <p className="text-sm text-[var(--text-muted)] mt-2">Bon conducteur, attention aux excès de vitesse.</p>
        </div>
            ...
            >
              <div className="flex justify-between items-start mb-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-[var(--text-primary)]">{violation.type}</h4>
                    <span
                      className={`text-xs font-bold px-2 py-0.5 rounded-full ${getSeverityColor(violation.severity)} bg-opacity-20 border-none`}
                    >
                      {getSeverityLabel(violation.severity)}
                    </span>
                  </div>
                </div>
                <div className="text-right text-xs text-[var(--text-secondary)] flex flex-col items-end gap-1">
                  <span className="flex items-center gap-1">
                    <Clock className="w-3 h-3" /> {violation.date}
                  </span>
                </div>
              </div>
