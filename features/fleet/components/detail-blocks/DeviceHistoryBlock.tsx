...
export const DeviceHistoryBlock: React.FC<DeviceHistoryBlockProps> = ({ vehicleId }) => {
  const { data: history = [], isLoading, isError } = useQuery<DeviceAssignment[]>({
  ...
  if (isError) {
    return (
      <p className="text-xs text-red-500 py-2">Impossible de charger l'historique des balises.</p>
    );
  }

  if (history.length === 0) {
    return (
      <p className="text-xs text-slate-400 py-2 italic">Aucun historique de balise disponible.</p>
    );
  }
    ...
    <div className="space-y-2">
      {history.map((entry, idx) => {
        ...
        return (
          <div
            key={entry.id}
            className={`relative pl-4 border-l-2 ${isActive ? 'border-emerald-400' : 'border-slate-200'} pb-3`}
          >
            {/* Indicateur de statut */}
            <span className={`absolute -left-[7px] top-0.5 w-3 h-3 rounded-full border-2 ${
            ...
            <div className="flex items-center gap-2 mb-0.5">
              ...
              <span className="font-mono text-xs font-semibold text-slate-700 tracking-tight">
                {entry.imei}
              </span>
              {isActive && (
            ...
            {/* Notes */}
            {entry.notes && (
              <p className="text-[10px] text-slate-400 mt-0.5 italic truncate" title={entry.notes}>
                {entry.notes}
              </p>
            )}
          </div>
