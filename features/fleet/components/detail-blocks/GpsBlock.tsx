...
export const GpsBlock: React.FC<GpsBlockProps> = ({
  ...
  return (
    <div className="space-y-2 text-xs">
        <ConfigurableRow id="gpsModel" isConfigMode={isConfigMode} isHidden={hiddenFields.has('gpsModel')} onToggle={() => toggleFieldVisibility('gpsModel')} className="flex justify-between py-1 border-b border-slate-100">
            <span className="text-slate-500">Modèle</span>
            <span className="font-mono font-medium">{mockData.deviceModel}</span>
        </ConfigurableRow>
        <ConfigurableRow id="imei" isConfigMode={isConfigMode} isHidden={hiddenFields.has('imei')} onToggle={() => toggleFieldVisibility('imei')} className="flex justify-between py-1 border-b border-slate-100">
            <span className="text-slate-500">IMEI</span>
            <span className="font-mono font-medium">{mockData.imei}</span>
        </ConfigurableRow>
        <ConfigurableRow id="sim" isConfigMode={isConfigMode} isHidden={hiddenFields.has('sim')} onToggle={() => toggleFieldVisibility('sim')} className="flex justify-between py-1 border-b border-slate-100">
            <span className="text-slate-500">Carte SIM</span>
