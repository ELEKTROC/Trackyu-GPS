...
export const ResellerForm = React.forwardRef<HTMLFormElement, ResellerFormProps>(
  ({ initialData, onFormSubmit }, ref) => {
              ...
              <FormSection icon={User} title="Administrateur Principal">
                <FormGrid columns={2}>
                  ...
                  <FormField label="Rôle">
                    <Input value="Administrateur" disabled className="bg-[var(--bg-elevated)] cursor-not-allowed" />
                  </FormField>
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <FormSection icon={Shield} title="Modules Autorisés">
                <div className="grid grid-cols-2 gap-3">
                  {['Fleet Management', 'CRM', 'Finance', 'Tech & Intervention', 'Stock', 'Reports', 'API Access'].map(
                    (mod) => (
                      <label
                        key={mod}
                        className="flex items-center gap-3 p-3 border border-[var(--border)] rounded-xl cursor-pointer tr-hover transition-colors"
                      >
                        <input
                          type="checkbox"
                          value={mod.toLowerCase()}
                          {...register('modules')}
                          className="w-4 h-4 rounded text-[var(--primary)] focus:ring-4 focus:ring-[var(--primary)]/20"
                        />
                        <span className="text-sm font-medium text-[var(--text-primary)]">{mod}</span>
                      </label>
                    )
                  )}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="p-4 border border-[var(--border)] rounded-xl bg-[var(--bg-surface)]">
                  ...
                  <p className="text-sm text-[var(--text-secondary)] mb-3">
                    Configuration des liens sociaux pour les emails et le portail client.
                  </p>
                  <button
                    type="button"
                    className="text-sm text-[var(--primary)] hover:text-[var(--primary)] font-medium hover:underline"
                  >
                  ...
                  <tbody className="divide-y divide-[var(--border)]">
                    {staffList.length === 0 ? (
                      ...
                      staffList.map((staff, idx) => (
                        <tr key={idx} className="bg-[var(--bg-surface)] tr-hover transition-colors">
                          <td className="px-4 py-3 font-medium text-[var(--text-primary)]">{staff.name}</td>
                          <td className="px-4 py-3 text-[var(--text-secondary)]">{staff.email}</td>
                          <td className="px-4 py-3">
                            <span className="px-2.5 py-1 bg-[var(--bg-elevated)] rounded-lg text-xs font-medium text-[var(--text-secondary)]">
                              {staff.role}
                            </span>
                          </td>
