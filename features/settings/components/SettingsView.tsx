...
import { 
  User, FileText, Receipt, Ticket, Lock, Map as MapIcon,
  Users, GitBranch, Box, Car, Wrench, Terminal,
  Hexagon, MapPin, Bell, Calendar, Leaf, HelpCircle,
  Info, RefreshCw, Save, Download, AlertTriangle, Clock, Smartphone,
  Search, Plus, LayoutTemplate, Settings, X, Trash2, Edit2,
  Phone, Eye, CheckCircle, Shield,
  History, FileCheck, CreditCard, MessageSquare, Building2, Globe2,
  Send, Filter, Mail, Paperclip, Check, RotateCcw,
  CheckSquare, Square, Database, ToggleLeft, ToggleRight, EyeOff, Camera, Sun, Moon, Layers, Activity, ChevronDown, ChevronUp, ChevronRight, ChevronLeft, Cpu, Fuel, MoreVertical, Power,
  Loader2
} from 'lucide-react';
...
import type { Client, Vehicle, Tier, Branch, Driver, Tech, Group, Command, POI, AlertConfig, MaintenanceRule, ScheduleRule, EcoDrivingProfile } from '../../../types';
...
const MyNotificationsView = lazy(() => import('./MyNotificationsView').then(m => ({ default: m.MyNotificationsView })));
...
const TierDetailModal = lazy(() => import('../../crm/components/TierDetailModal').then(m => ({ default: m.TierDetailModal })));
const SupportSettingsPanel = lazy(() => import('../../support/components/SupportSettingsPanel').then(m => ({ default: m.SupportSettingsPanel })));
...
type TabId =
  | 'profile' | 'operations' | 'contracts' | 'my_interventions' | 'my_notifications' | 'support'
  | 'admin' | 'users' | 'subaccounts' | 'groups' | 'branches' | 'objects' | 'drivers' | 'commands'
  | 'geofencing' | 'poi' | 'maintenance' | 'alerts' | 'expenses' | 'schedule' | 'ecodriving'
  | 'about' | 'privacy' | 'sync' | 'clients' | 'support_settings' | 'techs' | 'reseller';
...
const GenericTableContent: React.FC<GenericTableProps & { readOnly?: boolean }> = ({ 
  title, type, icon: Icon, columns, dataGenerator, 
  useRealVehicles, useRealClients, useRealBranches, useRealDrivers, useRealTechs, useRealGroups, useRealUsers, 
  useRealCommands, useRealPOIs, useRealAlertConfigs, useRealMaintenanceRules, useRealScheduleRules, useRealEcoDrivingProfiles,
  vehicles, clients, branches, drivers, techs, groups, users, 
  commands, pois, alertConfigs, maintenanceRules, scheduleRules, ecoDrivingProfiles,
  onAddClick, onEdit, readOnly 
}) => {
  ...
  const rawData = useMemo(() => {
      ...
      type, dataGenerator,
      useRealVehicles, vehicles, 
      useRealClients, clients, 
      useRealBranches, branches, 
      useRealDrivers, drivers, 
      useRealTechs, techs, 
      useRealGroups, groups, 
      useRealUsers, users,
      useRealCommands, commands,
      useRealPOIs, pois,
      useRealAlertConfigs, alertConfigs,
      useRealMaintenanceRules, maintenanceRules,
      useRealScheduleRules, scheduleRules,
      useRealEcoDrivingProfiles, ecoDrivingProfiles
  ]);
  ...
  useEffect(() => { setSettingsSortConfig(null); }, [type]);
  ...
  const getItemValue = (item: GenericItem, col: string): string | number => {
      ...
      if (cl.includes('véhicules')) return item.vehicules || item.vehicleCount || (item.vehicleIds ? item.vehicleIds.length : 0);
  ...
  const sortedFilteredData = useMemo(() => {
      ...
      return [...filteredData].sort((a: GenericItem, b: GenericItem) => {
          ...
          const numA = typeof valA === 'number' ? valA : parseFloat(String(valA).replace(/[^\d.,-]/g, '').replace(',', '.'));
          const numB = typeof valB === 'number' ? valB : parseFloat(String(valB).replace(/[^\d.,-]/g, '').replace(',', '.'));
          if (!isNaN(numA) && !isNaN(numB)) {
              return settingsSortConfig.direction === 'asc' ? numA - numB : numB - numA;
          }
          const strA = String(valA);
          const strB = String(valB);
          return settingsSortConfig.direction === 'asc'
  ...
  const isAllSelected = paginatedData.length > 0 && paginatedData.every((item: GenericItem) => selectedIds.has(item.id));
  ...
  const renderCell = (item: GenericItem, column: string) => {
      ...
      if (type === 'vehicle') {
          ...
          if (colLower.includes('plaque')) return <span className="font-mono bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded text-xs text-slate-600 dark:text-slate-300">{item.id}</span>;
      ...
      if (colLower.includes('nom') || colLower.includes('véhicule')) return <span className="font-bold text-slate-800 dark:text-white">{item.nom || item.name || item.vehicule || item.vehicleId}</span>;
      if (colLower.includes('email')) return <span className="text-slate-600 dark:text-slate-400 text-sm">{item.email}</span>;
      if (colLower.includes('clients')) return <span className="font-bold text-slate-800 dark:text-slate-400">{item.clients}</span>;
      if (colLower.includes('véhicules')) return <span className="font-bold text-slate-800 dark:text-slate-400">{item.vehicules || item.vehicleCount || (item.vehicleIds ? item.vehicleIds.length : 0)}</span>;
      if (colLower.includes('créé le')) return <span className="text-xs text-slate-500">{item.createdAt}</span>;
      if (colLower.includes('dernière connexion')) return <span className="text-xs text-slate-500">{item.lastLogin}</span>;
      if (colLower.includes('revendeur')) return <span className="text-slate-600 dark:text-slate-400 text-sm">{item.reseller || '--'}</span>;
      if (colLower.includes('client')) return <span className="font-medium text-slate-700 dark:text-slate-300">{item.client || item.clientId || '--'}</span>;
      if (colLower.includes('canal')) return <span className="text-xs font-mono bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded">{item.channel || item.transport || 'GPRS'}</span>;
      if (colLower.includes('réponse')) return <span className={`text-xs font-bold ${item.reponse === 'OK' || item.response === 'OK' ? 'text-green-600' : 'text-red-600'}`}>{item.reponse || item.response || '--'}</span>;
      if (colLower.includes('envoyé')) return <span className="text-xs text-slate-500">{item.envoye || item.sentAt}</span>;
      if (colLower.includes('erreurs')) return <span className="text-xs font-mono text-red-500">{item.passwordErrors || 0}</span>;
      if (colLower.includes('actions')) return (
        <div className="flex gap-2">
            <button className="p-1 hover:bg-slate-100 rounded text-slate-500" title="Voir logs"><FileText className="w-3 h-3" /></button>
            <button className="p-1 hover:bg-slate-100 rounded text-slate-500" title="Reset Password"><Lock className="w-3 h-3" /></button>
        </div>
      );
      
      // Maintenance specific columns
      if (type === 'maintenance') {
          if (colLower.includes('intervalle')) return <span className="text-xs font-mono">{item.intervalle || `${item.intervalValue} ${item.intervalUnit}`}</span>;
      }
      ...
      if (type === 'schedule') {
          ...
          if (colLower.includes('custom')) return item.enableCustomRestriction ? <span className="text-xs bg-purple-100 text-purple-700 px-2 py-1 rounded" title={item.customRestrictionName}>Oui</span> : <span className="text-xs text-slate-400">Non</span>;
      }
      ...
      if (type === 'ecodriving') {
          ...
          if (colLower.includes('accélération')) return <span className="text-xs">{item.harshAccelerationSensitivity}</span>;
      ...
      if (type === 'driver') {
          if (colLower.includes('permis')) return <span className="text-xs font-mono bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded">{item.permis || '--'}</span>;
          if (colLower.includes('rfid')) return <span className="text-xs font-mono text-slate-500">{item.rfidTag || '--'}</span>;
          if (colLower.includes('téléphone')) return <span className="text-sm text-slate-600 dark:text-slate-400">{item.telephone || '--'}</span>;
      }
                       ...
                       {isColumnMenuOpen && (
                           <div className="absolute top-full right-0 mt-2 w-48 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                               ...
                               <div className="max-h-48 overflow-y-auto custom-scrollbar p-1">
                                   {columns.map(col => (
                                       <label key={col} className="flex items-center gap-2 px-2 py-1.5 hover:bg-slate-50 dark:hover:bg-slate-700 rounded cursor-pointer text-sm">
                                           <input 
                                               type="checkbox" 
                                      ...
                                      <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity relative">
                                          {(type === 'reseller' || type === 'user') && (
                                              <div className="relative">
                                                  <button 
                                                      onClick={(e) => { e.stopPropagation(); setStatusMenuOpen(statusMenuOpen === item.id ? null : item.id); }}
                                                  ...
                                                  {statusMenuOpen === item.id && (
                                                      <div ref={statusMenuRef} className="absolute right-0 mt-2 w-32 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl z-50 animate-in fade-in zoom-in-95 duration-200">
...
export const SettingsView: React.FC<SettingsViewProps> = ({ initialAction, initialTab, initialId }) => {
  ...
  const { 
      vehicles, clients, users, branches, contracts, tickets, interventions, zones, 
      addClient, updateClient, addUser, updateUser, addVehicle, updateVehicle, addBranch, updateBranch, 
      tiers,
      drivers, addDriver, updateDriver, deleteDriver,
      techs, addTech, updateTech, deleteTech,
      groups, addGroup, updateGroup, deleteGroup,
      commands, addCommand, updateCommand, deleteCommand,
      pois, addPOI, updatePOI, deletePOI,
      alertConfigs, addAlertConfig, updateAlertConfig, deleteAlertConfig,
      maintenanceRules, addMaintenanceRule, updateMaintenanceRule, deleteMaintenanceRule,
      scheduleRules, addScheduleRule, updateScheduleRule, deleteScheduleRule,
      ecoDrivingProfiles, addEcoDrivingProfile, updateEcoDrivingProfile, deleteEcoDrivingProfile
  } = useDataContext();
  ...
  const { register, handleSubmit, setValue, getValues, formState: { errors, isDirty } } = useForm({
  ...
  const handleFormSubmit = async (data: Record<string, any>) => {
      if (activeTab === 'clients') {
          ...
          if (editingItem?.id) {
              ...
              addDriver({ id: `DRV-${Math.floor(Math.random() * 10000)}`, tenantId: 'tenant_default', statut: 'Actif', ...data } as unknown as Driver);
          ...
          if (editingItem?.id) {
              ...
              addTech({ id: `TCH-${Math.floor(Math.random() * 10000)}`, tenantId: 'tenant_default', statut: 'Actif', ...data } as unknown as Tech);
  ...
  const renderContent = () => {
      ...
      switch(activeTab) {
          case 'profile': return withSuspense(<MyAccountView />, 'Chargement du profil...');
          case 'operations': return withSuspense(<MyOperationsView />, 'Chargement des opérations...');
          case 'my_notifications': return withSuspense(<MyNotificationsView />, 'Chargement des notifications...');
          case 'support': return withSuspense(<HelpCenterView />, "Chargement de l'aide..."); 
          case 'support_settings': return withSuspense(<SupportSettingsPanel />, 'Chargement de la configuration...');
          case 'sync': return withSuspense(<SyncView />, 'Chargement...');
          case 'about': return withSuspense(<AboutView />, 'Chargement...');
          case 'drivers': return <GenericTableContent title="Conducteur" type="driver" icon={Car} columns={['ID', 'Nom', 'Permis', 'Téléphone', 'RFID Tag', 'Statut']} useRealDrivers drivers={drivers} {...commonProps} />;
          case 'techs': return <GenericTableContent title="Technicien" type="tech" icon={Wrench} columns={['ID', 'Nom', 'Spécialité', 'Zone', 'Niveau', 'Société', 'Statut']} useRealTechs techs={techs} {...commonProps} />;
          case 'users': {
            const clientUsers = users.filter((u: any) => {
              const r = (u.role || '').toUpperCase();
              return r === 'CLIENT' || r === 'SOUS_COMPTE';
            });
            return <GenericTableContent title="Utilisateur Client" type="users" icon={Users} columns={['ID', 'Nom', 'Email', 'Rôle', 'Dernière Connexion', 'Erreurs MDP', 'Actions', 'Statut']} useRealUsers users={clientUsers} {...commonProps} />;
          }
          case 'admin': return <GenericTableContent title="Administrateur" type="admin" icon={Shield} columns={['ID', 'Nom', 'Email', 'Rôle', 'Statut']} {...commonProps} />;
          case 'reseller': return withSuspense(<TierList type="RESELLER" onEdit={handleEdit} onViewDetail={(tier) => { setSelectedReseller(tier); setIsResellerDetailOpen(true); }} readOnly={true} />, 'Chargement des revendeurs...');
          case 'commands': return <GenericTableContent title="Commande Technique" type="command" icon={Terminal} columns={['ID', 'Véhicule', 'Type', 'Canal', 'Envoyé', 'Statut', 'Réponse']} useRealCommands commands={commands} {...commonProps} />;
          case 'subaccounts': {
            const subUsers = users.filter((u: any) => (u.role || '').toUpperCase() === 'SOUS_COMPTE');
            return <GenericTableContent title="Sous-utilisateur" type="subaccount" icon={Users} columns={['ID', 'Client', 'Nom', 'Email', 'Rôle', 'Statut']} useRealUsers users={subUsers} {...commonProps} />;
          }
          case 'objects': return <GenericTableContent title="Véhicule" type="vehicle" icon={Box} columns={['Nom', 'Plaque', 'Modèle', 'Groupe', 'Statut']} useRealVehicles vehicles={vehicles} {...commonProps} />;
          case 'groups': return <GenericTableContent title="Groupe" type="group" icon={Layers} columns={['ID', 'Nom', 'Description', 'Véhicules', 'Statut']} useRealGroups groups={groups} {...commonProps} />;
          case 'branches': return <GenericTableContent title="Branche" type="branch" icon={GitBranch} columns={['ID', 'Nom', 'Ville', 'Responsable', 'Statut']} useRealBranches branches={branches} {...commonProps} />;
          case 'geofencing': return <GenericTableContent title="Zone" type="geofence" icon={Hexagon} columns={['ID', 'Revendeur', 'Client', 'Nom', 'Type', 'Véhicules', 'Statut']} {...commonProps} />;
          case 'poi': return <GenericTableContent title="Point d'intérêt" type="poi" icon={MapPin} columns={['ID', 'Revendeur', 'Client', 'Nom', 'Type', 'Adresse', 'Rayon', 'Statut']} useRealPOIs pois={pois} {...commonProps} />;
          case 'maintenance': return <GenericTableContent title="Règle Maintenance" type="maintenance" icon={Wrench} columns={['ID', 'Revendeur', 'Client', 'Nom', 'Type', 'Intervalle', 'Véhicules', 'Statut']} useRealMaintenanceRules maintenanceRules={maintenanceRules} {...commonProps} />;
          case 'alerts': return <GenericTableContent title="Configuration Alerte" type="alert" icon={Bell} columns={['ID', 'Revendeur', 'Client', 'Nom', 'Type', 'Priorité', 'Destinataires', 'Statut']} useRealAlertConfigs alertConfigs={alertConfigs} {...commonProps} />;
          case 'schedule': return <GenericTableContent title="Règle de travail" type="schedule" icon={Calendar} columns={['ID', 'Nom', 'Horaires', 'Distance', 'Vitesse', 'Moteur', 'Custom', 'Véhicules', 'Statut']} useRealScheduleRules scheduleRules={scheduleRules} {...commonProps} />;
          case 'ecodriving': return <GenericTableContent title="Seuil Eco-conduite" type="ecodriving" icon={Leaf} columns={['ID', 'Nom', 'Score Cible', 'Vitesse Max', 'Accélération', 'Freinage', 'Ralenti', 'Statut']} useRealEcoDrivingProfiles ecoDrivingProfiles={ecoDrivingProfiles} {...commonProps} />;
          
          default: return <div className="p-4 text-center text-slate-500">Section en construction ({activeTab})</div>;
      }
  };
    ...
    <div className="flex flex-col h-full gap-2 sm:gap-6 animate-in fade-in duration-500">
        ...
        {isMobile && mobileShowList && (
          <div className="flex-1 overflow-y-auto pb-24 space-y-4">
            ...
            {filteredMenuGroups.filter(g => g.title !== 'Profil').map(group => (
              <div key={group.title}>
                <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase px-1 mb-1.5">{group.title}</p>
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden divide-y divide-slate-100 dark:divide-slate-700">
                  {group.items.map(item => {
                    ...
                    return (
                      <button
                        key={item.id}
                        onClick={() => { setActiveTab(item.id as TabId); setMobileShowList(false); }}
        ...
        <Modal
            ...
            footer={
              <div className="flex justify-end gap-3 w-full">
                  <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 border border-slate-300 dark:border-slate-600 rounded text-slate-600 dark:text-slate-300 text-sm font-bold hover:bg-white dark:hover:bg-slate-700 transition-colors">Annuler</button>
                  <button
                      type="button"
        ...
        >
            <Suspense fallback={<LoadingFallback label="Chargement du formulaire..." />}>
                {activeTab === 'objects' ? (
                    ...
                    <PoiForm ref={formRef} initialData={editingItem} onFormSubmit={handleFormSubmit} resellers={resellers} clients={clients} branches={branches} groups={groups} />
                ) : activeTab === 'geofencing' ? (
                    <GeofenceForm ref={formRef} initialData={editingItem} onFormSubmit={handleFormSubmit} resellers={resellers} clients={clients} branches={branches} groups={groups} />
                ) : activeTab === 'maintenance' ? (
                    <MaintenanceForm ref={formRef} initialData={editingItem} onFormSubmit={handleFormSubmit} resellers={resellers} clients={clients as unknown as Tier[]} branches={branches} groups={groups} vehicles={vehicles} users={users} />
                ) : activeTab === 'alerts' ? (
                    <AlertForm ref={formRef} initialData={editingItem} onFormSubmit={handleFormSubmit} resellers={resellers} clients={clients as unknown as Tier[]} branches={groups} vehicles={vehicles} users={users} zones={zones} />
                ) : activeTab === 'schedule' ? (
                    <ScheduleForm ref={formRef} initialData={editingItem} onFormSubmit={handleFormSubmit} resellers={resellers} clients={clients as unknown as Tier[]} />
                ) : activeTab === 'ecodriving' ? (
                    <EcoDrivingForm ref={formRef} initialData={editingItem} onFormSubmit={handleFormSubmit} resellers={resellers} clients={clients} />
                    ...
                    <UserForm ref={formRef} initialData={editingItem} onFormSubmit={handleFormSubmit} resellers={resellers} branches={branches as unknown as { id: string; nom: string; ville?: string }[]} vehicles={vehicles as unknown as { id: string; name: string; plate?: string }[]} clients={clients as unknown as { id: string; name: string }[]} />
                ) : activeTab === 'branches' ? (
        ...
        <Suspense fallback={<LoadingFallback />}>
            <TierDetailModal 
                ...
                onClose={() => { setIsResellerDetailOpen(false); setSelectedReseller(null); }}
                onEdit={(tier) => {
                    setEditingItem(tier);
                    setIsModalOpen(true);
                }}
