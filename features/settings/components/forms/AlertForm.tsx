...
export const AlertForm = React.forwardRef<
  HTMLFormElement,
  BaseFormProps & {
    resellers?: Tier[];
    clients?: Tier[];
...
>(({ initialData, onFormSubmit, resellers = [], clients = [], vehicles = [], users = [], zones = [] }, ref) => {
    ...
    <form ref={ref} onSubmit={handleSubmit(onSubmit)} className="space-y-4 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-4">
        ...
        <FormSection icon={Activity} title="Déclencheur">
          ...
          {type === 'Speeding' && (
            <FormGrid columns={2}>
              ...
              <FormField
                label="Durée tolérée (secondes)"
                hint="Déclencher seulement si la vitesse dépasse la limite pendant plus de X secondes."
              >
            ...
            <>
              ...
              <div className="p-3 bg-[var(--clr-caution-dim)] border border-[var(--clr-caution-border)] rounded-lg">
                <p className="text-xs text-[var(--clr-caution-strong)]">
                  ...
                  boîtier GPS. La sensibilité (seuils de G-force) est configurée directement sur le boîtier via
                  commandes AT/SMS.
                </p>
              </div>
            </>
          ...
          {type === 'Crash' && (
            <div className="p-3 bg-[var(--clr-danger-dim)] border border-[var(--clr-danger-border)] rounded-lg">
              <p className="text-xs text-[var(--clr-danger-strong)]">
                ...
                (G-force &gt; 2g sur plusieurs axes). Les alertes CRASH sont toujours de{' '}
                <strong>priorité CRITIQUE</strong> et génèrent une notification immédiate sur tous les canaux
                configurés.
              </p>
            </div>
          )}
          ...
          {type === 'SOS' && (
            <div className="p-3 bg-[var(--clr-danger-dim)] border border-[var(--clr-danger-border)] rounded-lg">
              <p className="text-xs text-[var(--clr-danger-strong)]">
                ...
                toujours de <strong>priorité CRITIQUE</strong> et nécessite une réponse immédiate.
              </p>
            </div>
          )}
          ...
          {(type === 'BatteryLevel' || type === 'DeviceBattery') && (
            <FormGrid columns={2}>
              ...
              <FormField
                label="Durée de confirmation (sec)"
                hint="Pour éviter les fausses alertes dues aux fluctuations."
              >
        ...
        <div className="relative">
          <div className="flex justify-between items-center mb-2">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-xs text-[var(--text-secondary)]">Tous les véhicules du client</span>
            </label>
          </div>
        ...
        <FormSection icon={Calendar} title="Calendrier d'activation">
          <div className="flex items-center justify-between mb-3">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="w-4 h-4 rounded-lg border-[var(--border)] text-[var(--primary)] focus:ring-[var(--primary)]"
              />
              <span className="text-sm text-[var(--text-secondary)]">Activer selon planning</span>
            </label>
          </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {['Lun', 'Mar', 'Mer', 'Jeu', 'Ven', 'Sam', 'Dim'].map((day) => (
                    <label
                      key={day}
                      className="flex items-center gap-1.5 text-xs cursor-pointer bg-[var(--bg-elevated)] px-2.5 py-1.5 rounded-lg border border-[var(--border)] hover:border-[var(--primary)] transition-colors"
                    >
                      <input
                        type="checkbox"
                        value={day}
          ...
          {!watch('isScheduled') && (
            <p className="text-xs text-[var(--text-secondary)] italic">L'alerte est active 24h/24 et 7j/7.</p>
          )}
        </FormSection>
          ...
          <div className="flex gap-4 mb-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="w-4 h-4 rounded-lg border-[var(--border)] text-[var(--primary)] focus:ring-[var(--primary)]"
              />
              <span className="text-sm text-[var(--text-primary)]">Web (Plateforme)</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
