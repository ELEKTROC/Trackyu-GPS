...
import { 
  Copy, Hexagon, Map, Circle, Trash2, Users, CheckSquare, 
  Square, ChevronDown, ChevronUp, Globe
} from 'lucide-react';
            ...
            <div className="p-4 space-y-4">
              <FormGrid columns={2}>
                <FormField label="Centre - Latitude">
                  <Input {...register('centerLat', { valueAsNumber: true })} type="number" step="any" placeholder="Ex: 5.316" />
                </FormField>
                <FormField label="Centre - Longitude">
                  <Input {...register('centerLng', { valueAsNumber: true })} type="number" step="any" placeholder="Ex: -4.008" />
                </FormField>
              </FormGrid>
              <FormField label="Rayon (mètres)">
                <Input {...register('radius', { valueAsNumber: true })} type="number" placeholder="1000" />
              </FormField>
              <div className="h-24 bg-slate-200 dark:bg-slate-900 rounded-lg flex items-center justify-center">
                <Circle className="w-12 h-12 text-slate-400" />
              </div>
            </div>
          ) : (
            <>
              <div className="h-40 bg-slate-200 dark:bg-slate-900 flex items-center justify-center relative">
                ...
                <p className="text-slate-500 text-sm z-10 flex flex-col items-center gap-2">
              ...
              {polygonPoints.length > 0 && (
                <div className="p-3 bg-slate-50 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 max-h-28 overflow-y-auto">
                  <p className="text-xs font-semibold text-slate-600 dark:text-slate-400 mb-2">Points ({polygonPoints.length})</p>
                  <div className="space-y-1">
                    {polygonPoints.map((point, index) => (
                      <div key={index} className="flex items-center justify-between text-xs bg-white dark:bg-slate-700 p-2 rounded-lg">
                        <span className="text-slate-600 dark:text-slate-300 font-mono">
                          P{index + 1}: [{point[0].toFixed(4)}, {point[1].toFixed(4)}]
                        </span>
                        <button type="button" onClick={() => removePoint(index)} className="text-red-500 hover:text-red-700 p-1">
