...
import {
    User, Settings, Shield, Users, FileText,
    Lock, Upload,
    Plus, Trash2, Download, GitBranch, Eye, EyeOff, Key, Pencil
} from 'lucide-react';
...
export const UserForm = React.forwardRef<HTMLFormElement, UserFormProps>(({ initialData, onFormSubmit, resellers, branches, vehicles = [], clients = [] }, ref) => {
    const { showToast } = useToast();
    const [activeTab, setActiveTab] = useState<number>(0);
    const [subUsersList, setSubUsersList] = useState<Partial<SubUserFormData>[]>((initialData?.subUsers as unknown as Partial<SubUserFormData>[]) || []);
    ...
    const onError = (errors: Record<string, unknown>) => {
        ...
        if (errorKeys.length > 0) {
            const fieldToTab: Record<string, number> = {
                // Tab 0: Personnel
                firstName: 0, lastName: 0, email: 0, phone: 0, address: 0, companyName: 0, resellerId: 0,
                // Tab 1: Sécurité
                password: 1, mustChangePassword: 1, isActive: 1,
                // Tab 2: Paramètres
                language: 2, timezone: 2, theme: 2, notifications: 2,
                // Tab 3: Permissions
                role: 3, accessLevel: 3, allowedVehicles: 3, allowedGroups: 3, allowedModules: 3,
            };
    ...
    return (
                            ...
                            <FormGrid columns={1}>
                                ...
                                <label className="flex items-center gap-3 p-3 border border-slate-200 dark:border-slate-700 rounded-xl cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-500">Le client devra changer son mot de passe à la première connexion</p>
                                    </div>
                                </label>
                                ...
                                <label className="flex items-center gap-3 p-3 border border-slate-200 dark:border-slate-700 rounded-xl cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-500">Désactiver pour bloquer l'accès sans supprimer le compte</p>
                                    </div>
                                </label>
                                ...
                                {isEditMode && (
                                    <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                                        <div className="flex justify-between items-center">
                                            <div>
                                                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Dernière connexion</p>
                                                <p className="text-sm text-slate-700 dark:text-slate-300">{initialData?.lastLogin || 'Jamais connecté'}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Erreurs MDP</p>
                                                <p className="text-sm text-slate-700 dark:text-slate-300">{initialData?.passwordErrors || 0}</p>
                                            </div>
                                            <button
                                                type="button"
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                        ...
                        <FormSection icon={Shield} title="Menus accessibles">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                {[
                                    { value: 'VIEW_DASHBOARD', label: 'Tableau de bord', description: 'Vue d\'ensemble et KPI', locked: true },
                                    ...
                                    {subUsersList.length === 0 ? (
                                        ...
                                        subUsersList.map((su, idx) => (
                                            <tr key={idx} className="bg-white dark:bg-slate-900">
                                                <td className="px-4 py-3 font-medium text-slate-800 dark:text-white">{su.nom}</td>
                                                <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{su.email}</td>
                                                <td className="px-4 py-3">
                                                    <span className="px-2 py-1 bg-slate-100 dark:bg-slate-800 rounded-lg text-xs font-medium text-slate-600 dark:text-slate-300">
                                                        {su.role}
                                                    </span>
                                                </td>
                ...
                {activeTab === 5 && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                        <div className="border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl p-8 text-center hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer relative">
                            ...
                            />
                            <div className="p-3 bg-slate-100 dark:bg-slate-700 rounded-xl inline-block mb-3">
                                <Upload className="w-8 h-8 text-slate-400" />
                            </div>
                            <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Cliquez pour uploader un document</h3>
                            <p className="text-xs text-slate-500 mt-1">CNI, Permis, Contrat, K-Bis (Max 5MB)</p>
                        </div>

                        <div className="grid grid-cols-1 gap-3">
                            {documentsList.length === 0 ? (
                                <p className="text-center text-slate-400 text-sm py-4">Aucun document associé</p>
                            ) : (
                                ...
                                <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                    {!branches || branches.length === 0 ? (
                                        ...
                                        branches.map((branch, idx) => (
                                            <tr key={idx} className="bg-white dark:bg-slate-900">
                                                <td className="px-4 py-3 font-medium text-slate-800 dark:text-white">{branch.nom}</td>
                                                <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{branch.ville}</td>
                                                <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{branch.responsable}</td>
                                                <td className="px-4 py-3">
        ...
        <Modal
            ...
            onClose={() => { setIsSubModalOpen(false); setEditingSubUser(null); setEditingSubUserIndex(null); }}
            ...
            footer={
                <div className="flex justify-end gap-3">
                    <button
                        type="button"
                        onClick={() => { setIsSubModalOpen(false); setEditingSubUser(null); setEditingSubUserIndex(null); }}
        ...
        >
            <SubUserForm
                ref={subFormRef}
                initialData={editingSubUser ? { ...editingSubUser, clientId: editingSubUser.clientId || parentClientId } : { clientId: parentClientId }}
