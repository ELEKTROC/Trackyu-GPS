...
>(({ initialData, onFormSubmit, clients = [], resellers = [], branches = [], groups = [], drivers = [] }, ref) => {
  ...
  return (
    <form
      ref={ref}
      onSubmit={handleSubmit(onSubmit as SubmitHandler<VehicleFormData>)}
    ...
    >
      ...
      {initialData?.id?.startsWith('ABO-') && (
        <div className="flex items-center gap-3 mb-3 px-3 py-2 bg-[var(--primary-dim)] dark:bg-[var(--primary-dim)] border border-[var(--border)] dark:border-[var(--primary)] rounded-lg">
          ...
          <span className="font-mono text-sm font-semibold text-[var(--primary)] dark:text-[var(--primary)] bg-[var(--primary-dim)] dark:bg-[var(--primary-dim)] px-2.5 py-0.5 rounded-md tracking-wider">
            {initialData.id}
          </span>
        </div>
      )}
