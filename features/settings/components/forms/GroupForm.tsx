...
export const GroupForm = React.forwardRef<HTMLFormElement, BaseFormProps>(
  ({ initialData, onFormSubmit, resellers = [] }, ref) => {
    const {
      ...
      defaultValues: initialData || { statut: 'ACTIVE', vehicleIds: [], clientId: '' },
    });
    ...
    const accessibleClients = useMemo(() => {
      ...
      if (normalizedRole === 'SUPERADMIN' || normalizedRole === 'ADMIN') return clients;
      if (user.role === 'RESELLER') return clients.filter((c) => c.resellerId === user.resellerId);
      if (user.role === 'CLIENT') return clients.filter((c) => c.id === user.clientId);
      return clients;
    }, [clients, user]);

    // Filtrer les véhicules selon le niveau d'habilitation ET le client sélectionné
    const accessibleVehicles = useMemo(() => {
      if (!user) return [];

      const role = user.role;
      let pool: typeof vehicles = vehicles;
      ...
      if (normalizedRole === 'SUPERADMIN' || normalizedRole === 'ADMIN') {
        ...
        pool = vehicles.filter((v) => clientNames.includes(v.client));
      } else if (role === 'CLIENT') {
        const clientName = clients.find((c) => c.id === user.clientId)?.name;
        pool = vehicles.filter((v) => v.client === clientName);
      } else if (user.branchId) {
        pool = vehicles.filter((v) => v.branchId === user.branchId);
      }

      // Cascade client : si un client est sélectionné dans le formulaire, filtrer davantage
      if (selectedClientId) {
        const clientName = clients.find((c) => c.id === selectedClientId)?.name;
        return pool.filter((v) => v.clientId === selectedClientId || v.client === clientName);
      }

      return pool;
    }, [vehicles, clients, user, selectedClientId]);
              ...
              (dynamicCriteriaOptions as Record<string, { values: { id: string; label: string }[]; label: string }>)[
                ...
                <FormField label="Valeur">
                  <Select {...register('criteriaValue')}>
                    ...
                    {(
                      dynamicCriteriaOptions as Record<
                        string,
                        { values: { id: string; label: string }[]; label: string }
                      >
