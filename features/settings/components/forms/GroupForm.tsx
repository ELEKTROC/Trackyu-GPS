...
export const GroupForm = React.forwardRef<HTMLFormElement, BaseFormProps>(({ initialData, onFormSubmit }, ref) => {
    ...
    const toggleSelectAll = () => {
        if (selectedVehicleIds.length === filteredVehicles.length) {
            ...
            setValue('vehicleIds', filteredVehicles.map(v => v.id));
        }
    };
        ...
        <form ref={ref} onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-5 bg-slate-50 dark:bg-slate-800/50">
                <div className="flex items-center gap-2 mb-4">
                    ...
                    <div>
                        <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Critère de groupement</h4>
                        <p className="text-xs text-slate-500">Optionnel - Filtrer automatiquement les véhicules</p>
                    </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {selectedCriteriaType && selectedCriteriaType !== 'CUSTOM' && dynamicCriteriaOptions[selectedCriteriaType]?.values.length > 0 && (
                    ...
                    <div className="p-4 bg-white dark:bg-slate-900 space-y-4">
                        ...
                        <div className="max-h-48 overflow-y-auto space-y-1">
                            {filteredVehicles.length > 0 ? filteredVehicles.map(vehicle => (
                                <label 
                                    key={vehicle.id}
                                    className="flex items-center gap-3 p-2.5 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg cursor-pointer transition-colors"
                                >
                                    <input
                                        type="checkbox"
                                    ...
                                    <span className={`text-xs px-2 py-0.5 rounded-lg font-medium ${
                                        ...
                                        vehicle.status === VehicleStatus.STOPPED ? 'bg-amber-100 text-amber-700' :
