...
export const RoleManager: React.FC = () => {
    ...
    const handleDeleteRole = async (roleId: string) => {
        if (await confirm({ message: 'Êtes-vous sûr de vouloir supprimer ce rôle ?', variant: 'danger', title: 'Confirmer la suppression', confirmLabel: 'Supprimer' })) {
        ...
        <div className="flex h-[calc(100vh-200px)] gap-6">
            ...
            <div className="w-1/3 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 flex flex-col overflow-hidden">
                <div className="p-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800">
                    <h3 className="font-bold text-slate-700 dark:text-white">Rôles & Profils</h3>
                    <button onClick={handleCreateRole} className="p-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                        ...
                        >
                            <div className="flex justify-between items-center">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className={`font-bold text-sm ${selectedRole?.id === role.id ? 'text-blue-700 dark:text-blue-300' : 'text-slate-700 dark:text-slate-300'}`}>
                                            {role.name}
                                        </p>
                                ...
                                {isEditing ? (
                                    <div className="flex flex-col gap-1">
                                        <input 
                                            ...
                                            className="text-lg font-bold bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded px-3 py-1.5 text-slate-800 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
                                        />
                                        <input 
                                            type="text" 
                                            ...
                                            className="text-sm bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded px-3 py-1 text-slate-600 dark:text-slate-400 focus:ring-2 focus:ring-blue-500 outline-none"
                                        />
                                    </div>
                                ) : (
                                    <div>
                                        <h3 className="text-lg font-bold text-slate-800 dark:text-white">{selectedRole.name}</h3>
                                        {selectedRole.description && (
                                            <p className="text-sm text-slate-500 dark:text-slate-400">{selectedRole.description}</p>
                                        )}
                                    </div>
                                )}
                                {selectedRole.isSystem && <span className="px-2 py-0.5 bg-purple-100 text-purple-700 text-xs font-bold rounded-full">Système</span>}
                            </div>
                            
                            {isEditing ? (
                                <div className="flex gap-2">
                                    <button onClick={() => setIsEditing(false)} className="px-3 py-1.5 text-slate-600 hover:bg-slate-100 rounded text-sm font-medium">Annuler</button>
                                    <button onClick={handleSaveRole} className="px-3 py-1.5 bg-blue-600 text-white rounded text-sm font-bold hover:bg-blue-700 flex items-center gap-2">
                                        <Save className="w-4 h-4" /> Enregistrer
                                    </button>
                                </div>
                            ) : (
                                !selectedRole.isSystem && (
                                    <button onClick={() => handleEditRole(selectedRole)} className="px-3 py-1.5 border border-slate-300 rounded text-slate-600 hover:bg-slate-50 text-sm font-medium flex items-center gap-2">
                                ...
                                {PERMISSION_STRUCTURE.map(group => (
                                    <div key={group.id} className="border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
                                        <div className="bg-slate-50 dark:bg-slate-800/50 p-3 border-b border-slate-200 dark:border-slate-700">
                                            <h4 className="font-bold text-slate-700 dark:text-slate-200 text-sm">{group.label}</h4>
                                        </div>
                                        ...
                                        {group.isBoolean ? (
                                            <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                                                {group.permissions?.map(perm => {
                                                    ...
                                                    return (
                                                        <label key={perm.id} className={`flex items-center gap-3 p-2 rounded transition-colors ${isEditing ? 'cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800' : ''}`}>
                                                            ...
                                                            <input 
                                                                ...
                                                                disabled={!isEditing}
                                                            />
                                                            <span className="text-sm text-slate-600 dark:text-slate-300 font-medium">{perm.label}</span>
                                                        </label>
                                                    );
                                                })}
                                            </div>
                                            ...
                                            <div className="overflow-x-auto">
                                                <table className="w-full text-sm text-left">
                                                    <thead className="bg-slate-50 dark:bg-slate-900 text-xs uppercase text-slate-500 font-semibold border-b border-slate-200 dark:border-slate-700">
                                                        <tr>
                                                    ...
                                                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                                        {group.resources?.map(res => (
                                                            <tr key={res.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                                                <td className="px-4 py-3 font-medium text-slate-700 dark:text-slate-300">{res.label}</td>
                                                                            ...
                                                                            <div 
                                                                                ...
                                                                                className={`w-5 h-5 mx-auto rounded border flex items-center justify-center transition-colors ${
                                                                                    isChecked 
                                                                                        ? 'bg-blue-600 border-blue-600 text-white' 
                                                                                        : 'bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-600'
                                                                                } ${isEditing ? 'cursor-pointer hover:ring-2 ring-blue-200' : 'cursor-default opacity-60'}`}
                                                                            >
                                                                                {isChecked && <Check className="w-3 h-3" />}
                                                                            </div>
                                                                        </td>
