...
import {
  Users, Search, Link2, Link2Off, Check, X, AlertTriangle,
  ChevronDown, ChevronRight, RefreshCw, Download, Upload,
  Merge, Plus, Edit2, Trash2, Eye, Filter, ArrowRight,
  Building2, CheckCircle2, XCircle, HelpCircle, Loader2,
  ArrowLeftRight, Save, FileSpreadsheet
} from 'lucide-react';
...
const findBestMatch = (
  ...
  for (const candidate of candidates) {
    ...
    if (client.email && candidate.email && 
        client.email.toLowerCase() === candidate.email.toLowerCase()) {
...
export const ClientReconciliation: React.FC<ClientReconciliationProps> = ({
  ...
  const runReconciliation = useCallback(async () => {
    ...
    nameGroups.forEach((clients, normalizedName) => {
      ...
      clients.forEach(c => {
        const key = c.source === 'TRAKZEE' ? 'trakzee' : 
                    c.source === 'ZOHO_BOOKS' ? 'zohoBooks' : 'zohoInvoice';
      ...
      const hasConflict = sourceCount > 1 && clients.some((c, i) => 
        clients.some((c2, j) => i !== j && calculateSimilarity(c.name, c2.name) < 95)
      );
      ...
      matchResults.push({
        ...
        status: hasConflict ? 'conflict' : 
                targetClient ? 'matched' : 
                sourceCount === 1 ? 'new' : 'review'
      });
    });
  ...
  const approveMatch = (id: string) => {
    setMatches(prev => prev.map(m => 
      m.id === id ? { ...m, status: 'matched' as const, matchType: 'manual' as const } : m
    ));
  };
  ...
  const markAsNew = (id: string) => {
    setMatches(prev => prev.map(m => 
      m.id === id ? { ...m, status: 'new' as const, targetClient: undefined } : m
    ));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Stats & Filters */}
      {matches.length > 0 && (
        <div className="flex flex-wrap items-center justify-between gap-4 bg-white dark:bg-slate-800 p-4 rounded-lg border border-slate-200 dark:border-slate-700">
          <div className="flex gap-4">
            <button
              onClick={() => setFilter('all')}
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                filter === 'all' ? 'bg-slate-200 text-slate-800' : 'text-slate-500 hover:bg-slate-100'
              }`}
            >
              Tous ({stats.total})
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                filter === 'matched' ? 'bg-green-100 text-green-700' : 'text-slate-500 hover:bg-slate-100'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                filter === 'review' ? 'bg-yellow-100 text-yellow-700' : 'text-slate-500 hover:bg-slate-100'
              }`}
            >
              <span className="flex items-center gap-1">
                <Eye className="w-3.5 h-3.5" />
                À vérifier ({stats.review})
              </span>
            </button>
            <button
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                filter === 'conflict' ? 'bg-red-100 text-red-700' : 'text-slate-500 hover:bg-slate-100'
              }`}
            >
              <span className="flex items-center gap-1">
                    ...
                    <React.Fragment key={match.id}>
                      <tr className="hover:bg-slate-50 dark:hover:bg-slate-900/50">
                        <td className="px-4 py-3">
                          <button onClick={() => toggleExpand(match.id)} className="p-1 hover:bg-slate-100 rounded">
                        ...
                        <tr className="bg-slate-50 dark:bg-slate-900/50">
                          <td colSpan={7} className="px-4 py-4">
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
                              ...
                              <div className={`p-3 rounded-lg border ${match.sources.trakzee ? 'bg-blue-50 border-blue-200' : 'bg-slate-100 border-slate-200'}`}>
                                ...
                                {match.sources.trakzee ? (
                                  <div className="space-y-1 text-slate-600">
                                    ...
                                    <p><strong>Email:</strong> {match.sources.trakzee.email || '-'}</p>
                                    <p><strong>Tél:</strong> {match.sources.trakzee.phone || '-'}</p>
                                    <p><strong>Ville:</strong> {match.sources.trakzee.city || '-'}</p>
                                    <p><strong>Véhicules:</strong> {match.sources.trakzee.vehicleCount || 0}</p>
                                  </div>
                                ) : (
                                  <p className="text-slate-400 italic">Pas de correspondance</p>
                                )}
                              </div>
                              
                              {/* Zoho Books Details */}
                              <div className={`p-3 rounded-lg border ${match.sources.zohoBooks ? 'bg-green-50 border-green-200' : 'bg-slate-100 border-slate-200'}`}>
                                <div className="font-semibold text-green-700 mb-2">Zoho Books (ABIDJAN GPS)</div>
                                {match.sources.zohoBooks ? (
                                  <div className="space-y-1 text-slate-600">
                                    <p><strong>Nom:</strong> {match.sources.zohoBooks.name}</p>
                                    <p><strong>Société:</strong> {match.sources.zohoBooks.company || '-'}</p>
                                    <p><strong>Email:</strong> {match.sources.zohoBooks.email || '-'}</p>
                                    <p><strong>Tél:</strong> {match.sources.zohoBooks.phone || '-'}</p>
                                    <p><strong>Solde:</strong> {match.sources.zohoBooks.balance?.toLocaleString() || 0} FCFA</p>
                                  </div>
                                ) : (
                                  <p className="text-slate-400 italic">Pas de correspondance</p>
                                )}
                              ...
                              <div className={`p-3 rounded-lg border ${match.sources.zohoInvoice ? 'bg-purple-50 border-purple-200' : 'bg-slate-100 border-slate-200'}`}>
                                <div className="font-semibold text-purple-700 mb-2">Zoho Invoice (SMARTRACK)</div>
                                {match.sources.zohoInvoice ? (
                                  <div className="space-y-1 text-slate-600">
                                    <p><strong>Nom:</strong> {match.sources.zohoInvoice.name}</p>
                                    <p><strong>Société:</strong> {match.sources.zohoInvoice.company || '-'}</p>
                                    <p><strong>Email:</strong> {match.sources.zohoInvoice.email || '-'}</p>
                                    <p><strong>Tél:</strong> {match.sources.zohoInvoice.phone || '-'}</p>
                                    <p><strong>Solde:</strong> {match.sources.zohoInvoice.balance?.toLocaleString() || 0} FCFA</p>
                                  </div>
                                ) : (
                                  <p className="text-slate-400 italic">Pas de correspondance</p>
                                )}
      ...
      {matches.length > 0 && (
        <div className="flex justify-between items-center pt-4 border-t border-slate-200 dark:border-slate-700">
          <div className="text-sm text-slate-500">
            {selectedIds.size > 0 ? `${selectedIds.size} client(s) sélectionné(s)` : `${stats.matched} correspondances validées`}
          </div>
