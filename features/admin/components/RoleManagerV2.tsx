...
import {
  Shield, Plus, Edit2, Trash2, Save, X, Check, ChevronDown, ChevronRight,
  Eye, Download, Upload, Lock, Copy, Search,
  LayoutDashboard, Map, Car, Users, Target, FileSignature, Receipt,
  CreditCard, Wrench, Package, LifeBuoy, BarChart3, Settings, Bell,
  UserCircle, Building, Info, Loader2, RefreshCw
} from 'lucide-react';
...
const ICON_MAP: Record<string, React.ElementType> = {
  LayoutDashboard, Map, Car, Users, Target, FileSignature, Receipt,
  CreditCard, Wrench, Package, LifeBuoy, BarChart3, Settings, Bell,
  UserCircle, Building, Eye, Plus, Edit2, Trash2, Download, Upload
};

// Icônes pour la config mobile tabs
import {
  Truck, Activity, Headset, Calculator, ShoppingCart, Briefcase, Calendar,
  FileText, Smartphone
} from 'lucide-react';
const MOBILE_ICON_MAP: Record<string, React.ElementType> = {
  LayoutDashboard, Map, Truck, Wrench, FileText, ShoppingCart, Briefcase,
  Calculator, Activity, Package, Headset, Settings, Calendar,
  ShieldCheck: Shield
};
...
const COLOR_CLASSES: Record<string, { dot: string; bg: string; text: string; badge: string }> = {
  ...
  red: { dot: 'bg-red-500', bg: 'bg-red-100 dark:bg-red-900/30', text: 'text-red-600', badge: 'bg-red-100 text-red-700' },
  amber: { dot: 'bg-amber-500', bg: 'bg-amber-100 dark:bg-amber-900/30', text: 'text-amber-600', badge: 'bg-amber-100 text-amber-700' },
  slate: { dot: 'bg-slate-500', bg: 'bg-slate-100 dark:bg-slate-900/30', text: 'text-slate-600', badge: 'bg-slate-100 text-slate-700' },
  indigo: { dot: 'bg-indigo-500', bg: 'bg-indigo-100 dark:bg-indigo-900/30', text: 'text-indigo-600', badge: 'bg-indigo-100 text-indigo-700' },
  teal: { dot: 'bg-teal-500', bg: 'bg-teal-100 dark:bg-teal-900/30', text: 'text-teal-600', badge: 'bg-teal-100 text-teal-700' },
  rose: { dot: 'bg-rose-500', bg: 'bg-rose-100 dark:bg-rose-900/30', text: 'text-rose-600', badge: 'bg-rose-100 text-rose-700' },
};
...
export const RoleManagerV2: React.FC<RoleManagerV2Props> = ({ onRoleSelect }) => {
  ...
  const handleDeleteRole = async (roleId: string) => {
    ...
    if (!await confirm({ message: 'Supprimer ce rôle ?', variant: 'danger', title: 'Confirmer la suppression', confirmLabel: 'Supprimer' })) return;
  ...
  const hasPermission = (
    ...
    const perms = (isEditingPermissions || selectedRole?.id === editingRole?.id) ? editingPermissions : selectedRole?.permissions || [];
    
    // Superadmin a tout
    if (selectedRole?.id === 'superadmin') return true;
    
    // Chercher permission exacte
    const exact = perms.find(p => 
      p.moduleId === moduleId && 
      p.tabId === tabId && 
      p.fieldId === fieldId
    );
  ...
  const countPermissions = (role: RoleWithPermissions): number => {
    if (role.id === 'superadmin') return PERMISSION_MODULES.reduce(
      (acc, m) => acc + m.globalActions.length + m.tabs.reduce(
        (tacc, t) => tacc + t.actions.length + t.fields.reduce(
          (facc, f) => facc + f.actions.length, 0
        ), 0
      ), 0
    );
  ...
  const ACTION_COLOR_CLASSES: Record<PermissionAction, { checked: string; unchecked: string }> = {
    ...
    EXPORT: { checked: 'bg-purple-100 text-purple-600 dark:bg-purple-900/30', unchecked: 'bg-slate-100 text-slate-400 dark:bg-slate-700' },
    IMPORT: { checked: 'bg-cyan-100 text-cyan-600 dark:bg-cyan-900/30', unchecked: 'bg-slate-100 text-slate-400 dark:bg-slate-700' },
  };
  ...
  return (
    <div className="flex gap-6 flex-1 min-h-0">
                      ...
                      <button
                        onClick={async () => {
                          try {
                            ...
                            const updatedRole = { ...selectedRole, permissions: editingPermissions, updatedAt: new Date().toISOString() };
              ...
              {selectedRole.id === 'superadmin' ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className="text-slate-500 max-w-md">
                    Ce rôle système dispose automatiquement de toutes les permissions 
                    sur tous les modules, onglets et champs. Il ne peut pas être modifié.
                  </p>
                </div>
              ) : (
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              ...
                              <span className={`text-xs px-2 py-0.5 rounded-full ${getColorClass(category?.color, 'badge')}`}>
                                {category?.label}
                              </span>
                            </div>
                          ...
                          <div className="flex gap-1" onClick={e => e.stopPropagation()}>
                            {(['VIEW', 'CREATE', 'EDIT', 'DELETE', 'EXPORT', 'IMPORT'] as PermissionAction[]).map(action => 
                              ...
                              return (
                                <div key={tab.id}>
                                  ...
                                  >
                                    ...
                                    <div className="flex gap-1" onClick={e => e.stopPropagation()}>
                                      {(['VIEW', 'CREATE', 'EDIT', 'DELETE', 'EXPORT', 'IMPORT'] as PermissionAction[]).map(action => 
                                        renderActionCheckbox(action, module.id, tab.actions as PermissionAction[], tab.id)
                                      )}
                                    </div>
                                  </div>
                                  ...
                                  {isTabExpanded && (
                                    <div className="bg-slate-50/50 dark:bg-slate-900/30">
                                      {tab.fields.map(field => (
                                        ...
                                        >
                                          <span className={`flex-1 text-xs ${field.sensitive ? 'text-amber-600 font-medium' : 'text-slate-600 dark:text-slate-400'}`}>
                                            {field.sensitive && '🔒 '}
                                            {field.label}
                                          ...
                                          <div className="flex gap-1">
                                            {(['VIEW', 'CREATE', 'EDIT', 'DELETE', 'EXPORT', 'IMPORT'] as PermissionAction[]).map(action => 
                                              renderActionCheckbox(action, module.id, field.actions as PermissionAction[], tab.id, field.id)
                                            )}
                                          </div>
                                        </div>
                                      ))}
      ...
      <Modal
        ...
        title={editingRole?.id && roles.find(r => r.id === editingRole.id) ? `Modifier ${editingRole.name}` : 'Nouveau Rôle'}
        maxWidth="max-w-lg"
        footer={
          <>
            <button 
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="pt-4 border-t dark:border-slate-700">
              ...
              <p className="text-xs text-slate-400 mb-3">
                Choisissez jusqu'à 4 onglets affichés dans la barre de navigation mobile. Les autres menus seront accessibles via "Plus".
              </p>
              {(() => {
                      ...
                      {allItems.map(item => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="font-medium leading-tight text-center">{item.mobileLabel || item.label}</span>
                          </button>
                        );
                      })}
                    </div>
              ...
              <div className="flex items-start gap-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                ...
                <div className="text-sm text-blue-800 dark:text-blue-300">
                  ...
                  <p className="text-xs opacity-80">
                    ...
                    à droite pour configurer les accès aux modules, onglets et champs.
                  </p>
                </div>
              </div>
            </div>
