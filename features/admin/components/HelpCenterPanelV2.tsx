...
import {
  BookOpen, Plus, Edit2, Trash2, Search, Save, X, Eye, EyeOff,
  ChevronDown, ChevronRight, Video, FileText, HelpCircle, Star,
  TrendingUp, Folder, BarChart2
} from 'lucide-react';
...
const DEFAULT_FAQS: FAQ[] = [
  ...
  },
  {
    ...
    answer: 'Vérifiez que le boîtier GPS est bien alimenté et que la carte SIM est active. Le voyant du boîtier doit clignoter vert.',
    category: 'tracking',
    order: 2,
    isPublished: true
  },
  {
    ...
    answer: 'Dans le module Rapports, sélectionnez le type de rapport puis cliquez sur le bouton "Exporter" en haut à droite. Vous pouvez choisir entre PDF et Excel.',
    category: 'reports',
    order: 3,
    isPublished: true
  },
  {
    ...
    answer: 'Le nombre de véhicules dépend de votre formule d\'abonnement. Contactez notre équipe commerciale pour augmenter votre quota.',
    category: 'billing',
    order: 4,
    isPublished: true
  },
...
export const HelpCenterPanelV2: React.FC = () => {
  ...
  const handleSaveArticle = () => {
    ...
    const slug = articleForm.title.toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/(^-|-$)/g, '');
  ...
  const handleDeleteArticle = async (id: string) => {
    if (await confirm({ message: 'Supprimer cet article ?', variant: 'danger', title: 'Confirmer la suppression', confirmLabel: 'Supprimer' })) {
  ...
  const handleDeleteFaq = async (id: string) => {
    if (await confirm({ message: 'Supprimer cette FAQ ?', variant: 'danger', title: 'Confirmer la suppression', confirmLabel: 'Supprimer' })) {
    ...
    <div className="space-y-6 h-full flex flex-col">
      ...
      <div className="hidden sm:grid grid-cols-2 sm:grid-cols-4 gap-4 shrink-0">
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-slate-500 uppercase font-bold">Articles</p>
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-slate-500 uppercase font-bold">Utile</p>
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-slate-500 uppercase font-bold">FAQ</p>
        ...
        <div className="flex gap-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2 border rounded-lg text-sm w-full sm:w-64 dark:bg-slate-900 dark:border-slate-700"
            />
          </div>
                ...
                return (
                  <Card key={article.id} className="p-4 hover:shadow-lg transition-shadow">
                    <div className="flex items-start justify-between mb-3">
                      <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-bold bg-${cat.color}-100 text-${cat.color}-700`}>
                        {cat.label}
                      </span>
          ...
          <Card className="divide-y dark:divide-slate-700">
            {filteredFaqs.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-slate-500 font-medium">Aucune FAQ trouvée</p>
              </div>
            ) : (
                ...
                <div key={faq.id} className="p-4">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                      <span className={`px-2 py-0.5 rounded text-xs bg-${getCategoryConfig(faq.category).color}-100 text-${getCategoryConfig(faq.category).color}-700`}>
                  ...
                  {expandedFaq === faq.id && (
                    <div className="mt-3 pl-8 text-sm text-slate-600 dark:text-slate-400">
                      {faq.answer}
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-3">
                {stats.topArticles.map((article, i) => (
                  <div key={article.id} className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-bold text-slate-500">{article.viewCount}</span>
                  </div>
                ))}
              </div>
            </Card>
      ...
      >
        <div className="p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase mb-1">URL Vidéo (optionnel)</label>
              <input
                type="url"
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 p-2 border rounded-lg dark:bg-slate-900 dark:border-slate-700 text-sm"
                placeholder="Ajouter un tag..."
              />
              <button onClick={addTag} className="px-3 py-2 border rounded-lg hover:bg-slate-50">
                <Plus className="w-4 h-4" />
              </button>
            </div>
          </div>
      ...
      >
        <div className="p-4 space-y-4">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Réponse</label>
            <textarea
              value={faqForm.answer}
              onChange={(e) => setFaqForm({ ...faqForm, answer: e.target.value })}
              className="w-full p-3 border rounded-lg dark:bg-slate-900 dark:border-slate-700 h-32"
            />
          </div>
