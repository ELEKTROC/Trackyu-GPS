...
export function useInterventionForm({ initialData, isOpen, technicians, onSave, onClose }: UseInterventionFormProps) {
  const { vehicles, clients, stock, addInvoice, catalogItems, tickets, updateTicket, contracts, users, branches } =
    useDataContext();
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    if (formData.clientId) {
      const activeContract = contracts.find(
        (c) =>
          c.clientId === formData.clientId && c.status === 'ACTIVE' && (!c.endDate || new Date(c.endDate) > new Date())
      );
      if (activeContract) {
  ...
  const availableVehicles = useMemo(() => {
    ...
    if (formData.clientId) {
      const clientVehicles = vehicles.filter((v) => v.clientId === formData.clientId || v.client === formData.clientId);
      // For INSTALLATION, also include internal/unassigned vehicles (stock)
      if (formData.type === 'INSTALLATION') {
        const internalVehicles = vehicles.filter(
          (v) => (v.client === 'Interne' || !v.client) && !clientVehicles.some((cv) => cv.id === v.id)
        );
  ...
  const hasMaterial = (keyword: string) =>
    (Array.isArray(formData.material) ? formData.material : []).some((m) =>
    ...
    switch (tabId) {
      case 'REQUEST':
        ...
        if (!formData.technicianId || formData.technicianId === 'UNASSIGNED')
          return 'Un technicien doit être assigné (Onglet 1)';
  ...
  const handleSubmit = async () => {
    ...
    if (!navigator.onLine) {
      ...
      const endpoint = formData.id ? `${API_URL}/tech/interventions/${formData.id}` : `${API_URL}/tech/interventions`;
...
export const getVehicleUpdates = (
  vehicleId: string,
  ...
  return {
    ...
    ...(contracts && contracts.length > 0
      ? (() => {
          ...
          if (v.clientId) {
            const clientContract = contracts.find(
              (c) =>
                c.clientId === v.clientId && c.status === 'ACTIVE' && (!c.endDate || new Date(c.endDate) > new Date())
            );
