...
export const InterventionPlanning: React.FC<InterventionPlanningProps> = ({ interventions, technicians, currentUserId, onEdit, onUpdate }) => {
    ...
    const getInterventionsForTechAndDate = useCallback((techId: string, date: Date) => {
    ...
    const hasConflict = useCallback((intervention: Intervention, techId: string, date: Date) => {
    ...
    const renderInterventionCard = (int: Intervention, compact = false) => {
        ...
        const time = int.scheduledDate ? new Date(int.scheduledDate).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }) : '';
        ...
        return (
            <>
                ...
                <div className="flex items-center gap-1 mt-0.5">
                    {int.licensePlate && <span className="font-mono text-[10px] bg-white/30 dark:bg-black/20 px-1 rounded truncate">{int.licensePlate}</span>}
    ...
    if (isMobile) {
        ...
        return (
            <Card className="flex-1 overflow-hidden flex flex-col p-0 border-slate-200 dark:border-slate-700">
                {/* Toolbar mobile */}
                <div className="p-3 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 shrink-0">
                    <div className="flex items-center gap-2">
                        <button onClick={goPrev} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg">
                            <ChevronLeft className="w-4 h-4" />
                        </button>
                        <button onClick={goToday} className="px-3 py-1.5 text-xs font-medium rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300">
                            Aujourd'hui
                        </button>
                        <button onClick={goNext} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg">
                    ...
                    {dayGroups.map(({ tech, ints }) => (
                        <div key={tech.id}>
                            <div className="px-4 py-2 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center gap-2">
                                ...
                                <span className="ml-auto text-xs text-slate-400">{ints.length} intervention{ints.length > 1 ? 's' : ''}</span>
                            </div>
                                    ...
                                    .map(int => {
                                        ...
                                        return (
                                            <div
                                                key={int.id}
                                                onClick={() => onEdit(int)}
                                                className="flex items-stretch cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50 active:bg-slate-100 dark:active:bg-slate-700"
                                            >
                                                <div className={`w-1 shrink-0 ${getStatusBgClass(int.status)}`} />
                                                <div className="flex-1 px-3 py-3 min-w-0">
                                                    <div className="flex items-center gap-2 mb-0.5">
                                                        <span className="text-xs font-mono font-bold text-slate-500">{time}</span>
                                                        {int.duration && <span className="text-[10px] text-slate-400">{int.duration}min</span>}
                                                        {conflict && <AlertTriangle className="w-3 h-3 text-red-500 shrink-0" />}
                                                    </div>
                                                    <div className="font-bold text-sm text-slate-800 dark:text-slate-100 truncate">{getClientName(int.clientId)}</div>
                                                    <div className="text-xs text-slate-500 dark:text-slate-400 truncate">{int.nature || int.type}</div>
                                                    {int.licensePlate && <div className="font-mono text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">{int.licensePlate}</div>}
                                                </div>
    ...
    return (
        <Card className="flex-1 overflow-hidden flex flex-col p-0 border-slate-200 dark:border-slate-700">
            {/* ======== TOOLBAR ======== */}
            <div className="p-3 border-b border-slate-200 dark:border-slate-700 flex flex-wrap justify-between items-center gap-2 bg-slate-50 dark:bg-slate-800">
            ...
            {/* ======== GRILLE PLANNING ======== */}
            <div className="flex-1 overflow-auto custom-scrollbar bg-white dark:bg-slate-900 pb-16 lg:pb-0">
                {viewMode === 'day' ? (
                    ...
                    <div className="min-w-[900px]">
                        ...
                        <div className="flex border-b border-slate-200 dark:border-slate-700 sticky top-0 bg-white dark:bg-slate-900 z-10">
                            <div className="w-36 shrink-0 p-2 font-bold text-slate-500 text-xs border-r border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 flex items-center gap-1">
                            ...
                            {filteredTechnicians.map(tech => {
                                ...
                                return (
                                    <div key={tech.id} className="flex" style={{ minHeight: '72px' }}>
                                        {/* Nom technicien */}
                                        <div className="w-36 shrink-0 p-2 border-r border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 flex flex-col justify-center">
                                            <div className="font-bold text-xs text-slate-700 dark:text-slate-300 truncate">{tech.name}</div>
                                        ...
                                        >
                                            ...
                                            {HOURS.map((hour, i) => (
                                                <div
                                                    key={hour}
                                                    className={`absolute top-0 bottom-0 border-l ${i === 0 ? 'border-transparent' : hour === 12 ? 'border-slate-300 dark:border-slate-600' : 'border-slate-100 dark:border-slate-800'}`}
                                            ...
                                            {isToday(currentDate) && (() => {
                                                ...
                                                if (nowH >= DAY_START && nowH <= DAY_END) {
                                                    ...
                                                    return <div className="absolute top-0 bottom-0 w-0.5 bg-red-500 z-20 pointer-events-none" style={{ left: `${pos}%` }}><div className="w-2 h-2 rounded-full bg-red-500 -translate-x-[3px] -translate-y-0.5" /></div>;
                                                }
                                                return null;
                                            })()}
                            ...
                            {filteredTechnicians.map(tech => {
                                ...
                                return (
                                    <div key={tech.id} className="flex min-h-[100px]">
                                        <div className="w-36 shrink-0 p-2 border-r border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 flex flex-col justify-center">
                                            <div className="font-bold text-xs text-slate-700 dark:text-slate-300 truncate">{tech.name}</div>
                                            <div className="text-[10px] text-slate-400 mt-0.5">{weekCount} intervention{weekCount !== 1 ? 's' : ''}</div>
                                        </div>
                                        {weekDates.map(date => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    {dayInts.length === 0 && (
                                                        <div className="flex-1 flex items-center justify-center text-[10px] text-slate-300 dark:text-slate-600 select-none">—</div>
                                                    )}
                                                </div>
                                            );
                                        })}
                ...
                <div className="border-t-2 border-amber-200 dark:border-amber-800 bg-amber-50/50 dark:bg-amber-900/10">
                    ...
                    <div className="p-2 flex flex-wrap gap-2 max-h-[160px] overflow-auto custom-scrollbar">
                        {unplannedInterventions.map(int => {
                            ...
                            const clientName = getClientName(int.clientId);
                            const techName = (() => {
                                if (!int.technicianId || int.technicianId === 'unassigned' || int.technicianId === 'UNASSIGNED') return null;
