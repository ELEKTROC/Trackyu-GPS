...
export const InterventionForm: React.FC<InterventionFormProps> = ({
    ...
    const [ticketForm, setTicketForm] = useState<Partial<Ticket>>({
        id: '', clientId: '', subject: '', category: 'Support Technique',
        subCategory: '', interventionType: 'Dépannage', priority: 'MEDIUM',
        vehicleId: '', description: '', assignedTo: '', source: 'TrackYu', receivedAt: new Date()
    });
    ...
    const handleOpenTicketModal = useCallback(() => {
        setTicketForm({
            id: '', clientId: formData.clientId || '', subject: '', category: "Demande d'intervention",
            subCategory: '', interventionType: formData.type === 'INSTALLATION' ? 'INSTALLATION' : 'DEPANNAGE',
            priority: 'MEDIUM', vehicleId: formData.vehicleId || '', description: '', assignedTo: '',
            source: 'TrackYu', receivedAt: new Date()
        });
        setTicketFormErrors({});
    ...
    const handleSaveTicketFromForm = useCallback(async () => {
        ...
        try {
            ...
            const createdTicket = await addTicket({
                ...ticketForm, id: newId, tenantId: user?.tenantId || '',
                status: 'OPEN', createdAt: new Date(), updatedAt: new Date(), messages: []
            } as Ticket);
    ...
    const modalContent = (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 md:p-6">
            ...
            {/* Modal Container */}
            <div className="relative bg-white dark:bg-slate-900 rounded-t-2xl sm:rounded-xl shadow-2xl w-full max-w-5xl h-[95vh] sm:h-[85vh] flex flex-col overflow-hidden animate-in slide-in-from-bottom sm:fade-in sm:zoom-in-95 duration-200 border-0 sm:border border-slate-200 dark:border-slate-700">
                ...
                {/* Header */}
                <div className="px-4 py-3 sm:px-6 sm:py-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800">
                    <div>
                        <div className="flex items-center gap-3">
                            <h3 className="text-xl font-bold text-slate-800 dark:text-white">
                                Intervention {formData.id || 'Nouvelle'}
                            </h3>
                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold border uppercase ${getStatusBgClass(formData.status || 'PENDING')}`}>
                                {INTERVENTION_STATUSES[formData.status as keyof typeof INTERVENTION_STATUSES] || formData.status}
                            </span>
                            {formData.type && (
                                <span className="bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-2 py-0.5 rounded text-[10px] font-bold text-slate-600 dark:text-slate-300 uppercase">
                                    {formData.type}
                                </span>
                            )}
                            {formData.nature && (
                ...
                {/* Tabs */}
                <div className="flex border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-2 sm:px-6 overflow-x-auto shrink-0">
                    {tabs.map(tab => (
                        <button
                            key={tab.id}
                ...
                {/* Body - Tab Content */}
                <div className="flex-1 overflow-y-auto bg-slate-50/50 dark:bg-slate-900/50 p-3 sm:p-6">
                    {activeTab === 'REQUEST' && (
                        <InterventionRequestTab
                            formData={formData}
                ...
                {/* Footer Actions */}
                <div className="p-3 sm:p-4 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex justify-end items-center shrink-0">
                    {/* Actions */}
                    <div className="flex flex-col-reverse sm:flex-row gap-2 w-full sm:w-auto">
                        <button
                            onClick={onClose}
                            className="px-4 py-2 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg text-slate-700 dark:text-slate-200 text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-600"
                        >
                            Annuler
                        </button>
                        ...
                        {(!formData.status || formData.status === 'PENDING' || formData.status === 'SCHEDULED' || formData.status === 'EN_ROUTE') && activeTab === 'REQUEST' && (
                            <button
                ...
                <TicketFormModal
                    ...
                    onClose={() => { setIsTicketModalOpen(false); setStagedAttachments([]); }}
                    isEditMode={false}
                    ticketForm={ticketForm}
                    setTicketForm={setTicketForm as unknown as (form: { subject: string; priority: string; description: string; clientId: string; vehicleId: string }) => void}
