...
export const TechRadarMap: React.FC<TechRadarMapProps> = ({ technicians, interventions, onInterventionClick }) => {
    ...
    const CENTER_LAT = 5.3600; // Côte d'Ivoire - Abidjan
    ...
    return (
        <div className="relative w-full h-full bg-slate-100 dark:bg-slate-900 z-0">
            {/* Live Indicator */}
            <div className="absolute top-4 right-4 z-[1000] bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm px-3 py-1.5 rounded-full border border-slate-200 dark:border-slate-700 shadow-sm flex items-center gap-2">
                <span className="relative flex h-3 w-3">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-3 w-3 bg-green-500"></span>
                </span>
                <span className="text-xs font-bold text-slate-700 dark:text-slate-200">Suivi GPS Live</span>
            </div>
            ...
            >
                ...
                {interventions.filter(i => i.status !== 'COMPLETED').map(int => {
                    const coords = getCoords(int.location);
                        ...
                        >
                            <Popup>
                                <div className="p-1 min-w-[140px]">
                                    ...
                                    <p className="text-xs text-slate-500">{int.nature || int.type}</p>
                                    {int.licensePlate && <p className="text-[10px] font-mono text-slate-400">{int.licensePlate}</p>}
                                    <div className="flex items-center justify-between mt-1 gap-2">
                                        <span className="text-[10px] font-medium px-1.5 py-0.5 rounded" style={{ backgroundColor: getStatusColorHex(int.status) + '20', color: getStatusColorHex(int.status) }}>{getStatusLabel(int.status)}</span>
                                        {time && <span className="text-[10px] font-mono text-slate-400">{time}</span>}
                                    </div>
                                    {techName && <p className="text-[10px] text-slate-400 mt-0.5">Tech: {techName}</p>}
                ...
                {/* Technicians Markers */}
                {technicians.filter(t => t.location).map(tech => {
            ...
            {/* Legend Overlay */}
            <div className="absolute bottom-4 right-4 bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm text-xs space-y-2 z-[1000]">
                <p className="font-bold text-slate-600 dark:text-slate-300 mb-1 uppercase text-[10px]">Légende</p>
                <div className="flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-green-500"></span> <span className="text-slate-600 dark:text-slate-400">Tech Dispo</span>
                </div>
                <div className="flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-orange-500"></span> <span className="text-slate-600 dark:text-slate-400">Tech Occupé</span>
                </div>
                ...
                <div className="flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-orange-500"></span> <span className="text-slate-600 dark:text-slate-400">En cours</span>
                </div>
            </div>
        </div>
    );
