...
import { 
  Calendar, Map as MapIcon, Table, PieChart,
  Filter, Search, Plus, ChevronLeft, ChevronRight, Box, ClipboardCheck, Users,
  Wrench, CheckCircle, Clock, Activity, ArrowRightLeft
} from 'lucide-react';
...
const InterventionDetailModal = React.lazy(() => import('../../agenda/components/partials/InterventionDetailModal').then(m => ({ default: m.InterventionDetailModal })));
...
const STOCK_STATUS_LABELS: Record<string, { label: string; bg: string; text: string }> = {
  ...
  SCRAPPED: { label: 'Mis au rebut', bg: 'bg-slate-200', text: 'text-slate-600' },
...
const getStockStatusStyle = (status: string) => {
  return STOCK_STATUS_LABELS[status] || { label: status, bg: 'bg-slate-100', text: 'text-slate-700' };
};
...
export const TechView: React.FC<TechViewProps> = ({ initialViewMode = 'LIST' }) => {
  ...
  const { interventions, updateIntervention, deleteIntervention, addIntervention, users, stock, updateDevice, tiers, tickets, updateTicket } = useDataContext();
  const { user } = useAuth();
  ...
  const kpis = useMemo(() => {
    ...
    const inProgressInterventions = interventions.filter(i => i.status === 'IN_PROGRESS' || i.status === 'EN_ROUTE').length;
    ...
    if (completedWithDuration.length > 0) {
        const avgMinutes = completedWithDuration.reduce((sum, i) => sum + (i.duration || 0), 0) / completedWithDuration.length;
  ...
  const technicians = useMemo(() => {
    ...
    if (techs.length === 0) {
      return [{ id: 'unassigned', name: 'Non assigné', email: '', role: 'TECHNICIAN' } as unknown as typeof users[number]];
    }
    return techs;
  }, [users]);
  ...
  useEffect(() => { setStockPage(1); }, [stockSearch, stockTypeFilter]);
  ...
  const { sortedItems: sortedMyStock, sortConfig: stockSortConfig, handleSort: handleStockSort } = useTableSort(
      filteredMyStock,
  ...
  const paginatedStock = sortedMyStock.slice(
  ...
  const HISTORY_SORT_ACCESSORS: Record<string, (l: { id: string; date: string; interventionId: string; user: string; action: string; details: string }) => string> = {
      date: (l) => l.date,
  ...
  const { sortedItems: sortedHistory, sortConfig: historySortConfig, handleSort: handleHistorySort } = useTableSort(
      historyLogs,
      { key: 'date', direction: 'desc' },
      HISTORY_SORT_ACCESSORS
  );
  ...
  const visibleTechTabs = useMemo(() => {
    const baseTabs = isMobile
      ? TECH_TABS.filter(t => !TECH_MOBILE_HIDDEN.has(t.id))
      : TECH_TABS;
    return isMobile ? filterTabsForView('techView', baseTabs) : baseTabs;
  }, [isMobile, filterTabsForView]);

  const [viewMode, setViewMode] = useState<'LIST' | 'PLANNING' | 'MAP' | 'STATS' | 'HISTORY' | 'STOCK' | 'TEAM' | 'OVERVIEW' | 'CONFIG'>(initialViewMode);
  ...
  const mobileKpiFilteredInterventions = useMemo(() => {
    ...
    if (mobileKpiFilter === 'pending') return filteredInterventions.filter(i => i.status === 'PENDING' || i.status === 'SCHEDULED');
    if (mobileKpiFilter === 'in_progress') return filteredInterventions.filter(i => i.status === 'IN_PROGRESS' || i.status === 'EN_ROUTE');
  ...
  const renderStock = () => (
      <div className="flex-1 flex flex-col gap-4 overflow-hidden">
                              ...
                              {pendingStock.map(item => (
                                  <tr key={item.id} className="border-b border-orange-100 dark:border-orange-800/50 last:border-0">
                                      ...
                                      <td className="py-2 text-right space-x-2">
                                          <button onClick={() => handleAcceptTransfer(item)} className="px-2 py-1 bg-green-600 text-white rounded text-xs hover:bg-green-700">Accepter</button>
                                          <button onClick={() => handleRejectTransfer(item)} className="px-2 py-1 bg-red-600 text-white rounded text-xs hover:bg-red-700">Refuser</button>
                                      </td>
                                  </tr>
                              ))}
                          </tbody>
          ...
          {isMobile ? (
                    ...
                    <div key={item.id} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-3 shadow-sm">
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${typeColor}`}>{item.type}</span>
                          <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${s.bg} ${s.text}`}>{s.label}</span>
                        </div>
                        <span className="text-[10px] text-slate-400 shrink-0">{item.entryDate ? new Date(item.entryDate).toLocaleDateString('fr-FR') : '-'}</span>
                      </div>
                      <p className="font-bold text-sm text-slate-800 dark:text-white">{item.model}</p>
                      <p className="font-mono text-xs text-slate-500 dark:text-slate-400 mt-0.5">{item.serialNumber || item.imei || item.iccid || '-'}</p>
                      {item.status === 'IN_STOCK' && (
                        <button onClick={() => handleTransfer(item)} className="mt-2 w-full py-1.5 bg-indigo-600 text-white text-xs font-bold rounded-lg flex items-center justify-center gap-1 hover:bg-indigo-700 active:scale-95 transition-all">
            ...
            /* ── DESKTOP STOCK TABLE ── */
            <Card className="flex-1 flex flex-col min-h-0 border-slate-200 dark:border-slate-700 p-0 overflow-hidden"
              title={
              ...
              <div className="flex-1 overflow-auto custom-scrollbar pb-16 lg:pb-0">
                <table className="w-full text-left border-collapse min-w-[700px]">
                  <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 sticky top-0 z-10">
                    <tr>
                      <SortableHeader label="Type" sortKey="type" currentSortKey={stockSortConfig.key} currentDirection={stockSortConfig.direction} onSort={handleStockSort} className="text-xs font-bold text-slate-500 uppercase" />
                      <SortableHeader label="Modèle" sortKey="model" currentSortKey={stockSortConfig.key} currentDirection={stockSortConfig.direction} onSort={handleStockSort} className="text-xs font-bold text-slate-500 uppercase" />
                      <SortableHeader label="S/N (IMEI/ICCID)" sortKey="serialNumber" currentSortKey={stockSortConfig.key} currentDirection={stockSortConfig.direction} onSort={handleStockSort} className="text-xs font-bold text-slate-500 uppercase" />
                      <SortableHeader label="Statut" sortKey="status" currentSortKey={stockSortConfig.key} currentDirection={stockSortConfig.direction} onSort={handleStockSort} className="text-xs font-bold text-slate-500 uppercase" />
                      <SortableHeader label="Date Entrée" sortKey="receivedDate" currentSortKey={stockSortConfig.key} currentDirection={stockSortConfig.direction} onSort={handleStockSort} className="text-xs font-bold text-slate-500 uppercase" />
                      <th className="px-4 py-2 text-xs font-bold text-slate-500 uppercase text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-sm">
                    {paginatedStock.map(item => (
                      <tr key={item.id} className="density-row hover:bg-slate-50 dark:hover:bg-slate-800/50">
                        ...
                        <td className="px-4 py-2 text-slate-500 text-xs">{item.entryDate ? new Date(item.entryDate).toLocaleDateString('fr-FR') : '-'}</td>
                        <td className="px-4 py-2 text-right">
                          {item.status === 'IN_STOCK' && (
                            <button onClick={() => handleTransfer(item)} title="Transférer à un technicien" className="px-2 py-1 bg-indigo-600 text-white rounded text-xs hover:bg-indigo-700 inline-flex items-center gap-1">
                              <ArrowRightLeft className="w-3 h-3" /> Transférer
                            </button>
                          )}
                        </td>
                    ...
                    {paginatedStock.length === 0 && (
                      <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-500">Aucun matériel en stock.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
              {sortedMyStock.length > stockItemsPerPage && (
                <div className="p-3 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-slate-500">Lignes par page:</span>
                    <select title="Lignes par page" className="p-1 border border-slate-200 dark:border-slate-600 rounded text-xs bg-white dark:bg-slate-900" value={stockItemsPerPage} onChange={e => { setStockItemsPerPage(Number(e.target.value)); setStockPage(1); }}>
                      <option value={10}>10</option><option value={15}>15</option><option value={25}>25</option><option value={50}>50</option>
                    </select>
                    <span className="text-xs text-slate-500 ml-2">Page {stockPage} sur {totalStockPages} ({sortedMyStock.length} éléments)</span>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => setStockPage(p => Math.max(1, p - 1))} disabled={stockPage === 1} className="p-2 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded hover:bg-slate-50 dark:hover:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"><ChevronLeft className="w-4 h-4" /></button>
                    <button onClick={() => setStockPage(p => Math.min(totalStockPages, p + 1))} disabled={stockPage === totalStockPages} className="p-2 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded hover:bg-slate-50 dark:hover:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"><ChevronRight className="w-4 h-4" /></button>
                  </div>
                </div>
              )}
            </Card>
  ...
  const renderHistory = () => {
      ...
      <Card className="flex-1 flex flex-col min-h-0 p-0 overflow-hidden border-slate-200 dark:border-slate-700">
          <div className="flex-1 overflow-auto custom-scrollbar pb-16 lg:pb-0">
              <table className="w-full text-left border-collapse min-w-[600px]">
                  <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 sticky top-0 z-10">
                      <tr>
                          <SortableHeader label="Date" sortKey="date" currentSortKey={historySortConfig.key} currentDirection={historySortConfig.direction} onSort={handleHistorySort} className="text-xs font-bold text-slate-500 uppercase" />
                          <SortableHeader label="Réf" sortKey="ref" currentSortKey={historySortConfig.key} currentDirection={historySortConfig.direction} onSort={handleHistorySort} className="text-xs font-bold text-slate-500 uppercase" />
                          <SortableHeader label="Utilisateur" sortKey="user" currentSortKey={historySortConfig.key} currentDirection={historySortConfig.direction} onSort={handleHistorySort} className="text-xs font-bold text-slate-500 uppercase" />
                          <SortableHeader label="Action" sortKey="action" currentSortKey={historySortConfig.key} currentDirection={historySortConfig.direction} onSort={handleHistorySort} className="text-xs font-bold text-slate-500 uppercase" />
                          <SortableHeader label="Détails" sortKey="details" currentSortKey={historySortConfig.key} currentDirection={historySortConfig.direction} onSort={handleHistorySort} className="text-xs font-bold text-slate-500 uppercase" />
                      </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-sm">
          ...
          <div className="p-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 flex justify-between items-center">
              ...
              <div className="flex gap-2">
                  <button 
                      ...
                      className="p-2 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded hover:bg-slate-50 dark:hover:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                  >
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50 dark:bg-slate-900 overflow-hidden">
      ...
      >
       {/* TOP BAR */}
       <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white dark:bg-slate-800 p-2 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm shrink-0">
          {!isMobile && <Tabs tabs={visibleTechTabs} activeTab={viewMode} onTabChange={(id) => setViewMode(id as typeof viewMode)} />}
          ...
          <div className="flex items-center gap-2 w-full md:w-auto">
              ...
              <div className="relative hidden sm:block" ref={filterMenuRef}>
                  ...
                  {isFilterMenuOpen && (() => {
                    const filterContent = (
                      <>
                        <h4 className="text-xs font-bold text-slate-500 uppercase mb-3">Filtres actifs</h4>
                        <div className="space-y-3">
                          <div className="space-y-1">
                            <label className="text-xs text-slate-600 dark:text-slate-300">Statut</label>
                            <select className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-900 text-sm" value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}>
                          ...
                          <div className="space-y-1">
                            ...
                            <select className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-900 text-sm" value={filterType} onChange={(e) => setFilterType(e.target.value)}>
                              <option value="ALL">Tous les types</option>
                          ...
                          <div className="space-y-1">
                            <label className="text-xs text-slate-600 dark:text-slate-300">Technicien</label>
                            <select className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-900 text-sm" value={filterTech} onChange={(e) => setFilterTech(e.target.value)}>
                    ...
                    return isMobile ? (
                      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" onClick={() => setIsFilterMenuOpen(false)}>
                        <div className="w-full max-w-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl p-4" onClick={e => e.stopPropagation()}>
                          {filterContent}
                        </div>
                      </div>
                    ) : (
      ...
      {/* KPI CARDS - Only visible for Intervention views - Hidden on mobile */}
       ...
       <div className="grid grid-cols-1 md:grid-cols-5 gap-4 shrink-0">
        ...
        <Card className="p-4 border-l-4 border-l-green-500">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-bold text-slate-500 uppercase">Terminées</p>
              <p className="text-2xl font-bold text-slate-800 dark:text-white mt-1">{kpis.completed}</p>
            </div>
        ...
        <Card className="p-4 border-l-4 border-l-amber-500">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-slate-800 dark:text-white mt-1">{kpis.pending}</p>
            </div>
        ...
        <Card className="p-4 border-l-4 border-l-orange-500">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-slate-800 dark:text-white mt-1">{kpis.inProgress}</p>
            </div>
        ...
        <Card className="p-4 border-l-4 border-l-purple-500">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-slate-800 dark:text-white mt-1">{kpis.avgTime}</p>
            </div>
       ...
       <div className="flex-1 flex flex-col min-h-0">
           {viewMode === 'OVERVIEW' && (
               interventions.length === 0 ? (
                   <Card className="flex-1 flex flex-col items-center justify-center p-12 text-center border-slate-200 dark:border-slate-700">
                       <ClipboardCheck className="w-16 h-16 text-slate-200 dark:text-slate-700 mb-4" />
                       <p className="text-lg font-semibold text-slate-500 dark:text-slate-400">Aucune intervention</p>
                       <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">Créez votre première intervention pour voir les statistiques</p>
                   </Card>
               ) : (
                   <Card className="flex-1 border-slate-200 dark:border-slate-700 p-0 overflow-hidden">
           ...
           {viewMode === 'LIST' && (() => {
               ...
               return listData.length === 0 ? (
                   <Card className="flex-1 flex flex-col items-center justify-center p-12 text-center border-slate-200 dark:border-slate-700">
                       <Wrench className="w-16 h-16 text-slate-200 dark:text-slate-700 mb-4" />
                       <p className="text-lg font-semibold text-slate-500 dark:text-slate-400">Aucune intervention trouvée</p>
                       <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">Modifiez les filtres ou créez une nouvelle intervention</p>
                   </Card>
               ) : (
                   <InterventionList
           ...
           {viewMode === 'MAP' && (
               <Card className="flex-1 border-slate-200 dark:border-slate-700 p-0 overflow-hidden flex flex-col relative">
                   <TechRadarMap 
                       technicians={technicians} 
                       interventions={filteredInterventions} 
       ...
       {typeof document !== 'undefined' && isDetailModalOpen && createPortal(
             ...
             <InterventionDetailModal
                 ...
                 onStatusChange={(int, newStatus) => {
                     ...
                     if (newStatus === 'EN_ROUTE' && int.ticketId && updateTicket) {
                         const ticket = tickets.find((t) => t.id === int.ticketId);
                         if (ticket && ticket.status !== 'IN_PROGRESS' && ticket.status !== 'RESOLVED' && ticket.status !== 'CLOSED') {
                             updateTicket({ ...ticket, status: 'IN_PROGRESS', updatedAt: new Date(), startedAt: ticket.startedAt || new Date() });
                         }
                     }
                 }}
      ...
      <Modal isOpen={isAuditModalOpen} onClose={() => setIsAuditModalOpen(false)} title="Inventaire du Stock" maxWidth="max-w-2xl">
          <div className="p-4">
              ...
              <div className="max-h-96 overflow-y-auto border rounded-lg">
                  {myStock.filter(s => s.transferStatus !== 'PENDING_RECEIPT').map(item => (
                      <div key={item.id} className="flex items-center gap-3 p-3 border-b last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800">
                          <input 
                              type="checkbox" 
                          ...
                          <div className="flex-1">
                              <div className="flex justify-between">
                                  <p className="font-bold text-sm text-slate-800 dark:text-white">{item.type} - {item.model}</p>
                                  {(() => {
                                      const s = getStockStatusStyle(item.status);
                                      return <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${s.bg} ${s.text}`}>{s.label}</span>;
                                  })()}
                              </div>
                              <p className="text-xs font-mono text-slate-500">{item.serialNumber || item.imei || item.iccid}</p>
                          </div>
                      </div>
                  ))}
                  {myStock.filter(s => s.transferStatus !== 'PENDING_RECEIPT').length === 0 && (
                      <div className="p-8 text-center text-slate-500">
                          Aucun matériel en stock à auditer.
                      </div>
                  )}
              </div>
          </div>
      ...
      {/* Transfer Modal */}
      <Modal isOpen={isTransferModalOpen} onClose={() => { setIsTransferModalOpen(false); setTransferTarget(null); setTransferTechId(''); }} title="Transférer un matériel" maxWidth="max-w-md">
          <div className="p-4 flex flex-col gap-4">
              {transferTarget && (
                  <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-3 text-sm">
                      <p className="font-bold text-slate-700 dark:text-slate-200">{transferTarget.type} — {transferTarget.model}</p>
                      <p className="font-mono text-xs text-slate-500 mt-1">{transferTarget.serialNumber || transferTarget.imei || transferTarget.iccid}</p>
                  </div>
              )}
              <div>
                  <label htmlFor="transfer-tech" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Technicien destinataire</label>
                  <select
                      id="transfer-tech"
              ...
              <div className="flex justify-end gap-2 pt-2">
                  <button
                      onClick={() => { setIsTransferModalOpen(false); setTransferTarget(null); setTransferTechId(''); }}
                      className="px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg text-sm hover:bg-slate-50 dark:hover:bg-slate-800"
                  >
                      Annuler
                  </button>
                  <button
