    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-500 uppercase">
                            {isTransfer() ? 'Nouvelle Plaque *' : 'Plaque'}
                        </label>
                        <input 
                            title="Plaque"
                            className={`w-full px-3 py-2.5 border rounded-lg bg-white dark:bg-slate-900 text-sm font-bold ${isTransfer() ? 'border-orange-300 ring-2 ring-orange-100' : 'border-slate-200 dark:border-slate-600'}`}
                    ...
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-500 uppercase">
                            {isTransfer() ? 'Nouvelle Marque *' : 'Marque'}
                        </label>
                        <input 
                            list="vehicle-brands"
                            title="Marque" 
                        ...
                        <datalist id="vehicle-brands">
                            {['Toyota', 'Mitsubishi', 'Isuzu', 'Hino', 'Mercedes-Benz', 'Volvo', 'Scania', 'MAN', 'Renault', 'Peugeot', 'Volkswagen', 'Ford', 'Hyundai', 'Kia', 'Nissan', 'Mazda', 'Suzuki', 'Dacia', 'Iveco', 'DAF'].map(b => (
                    ...
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-500 uppercase">
                            {isTransfer() ? 'Nouveau Modèle *' : 'Modèle'}
                        </label>
                        <input 
                            list="vehicle-models"
                            title="Modèle" 
                        ...
                        <datalist id="vehicle-models">
                            ...
                            {formData.vehicleBrand === 'Toyota' && ['Hilux', 'Land Cruiser', 'Corolla', 'Fortuner', 'Hiace', 'Prado'].map(m => <option key={m} value={m} />)}
                            {formData.vehicleBrand === 'Mitsubishi' && ['L200', 'Pajero', 'Canter', 'Fuso'].map(m => <option key={m} value={m} />)}
                            {formData.vehicleBrand === 'Isuzu' && ['D-Max', 'D-Max Single Cab', 'N-Series', 'F-Series'].map(m => <option key={m} value={m} />)}
                            {['Berline', 'SUV', 'Pick-up', 'Camionnette', 'Poids Lourd', 'Remorque'].map(m => <option key={m} value={m} />)}
                        </datalist>
                    </div>

                    {/* Vehicle Type */}
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-500 uppercase">Type d'engin</label>
                        <select title="Type d'engin" className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-900 text-sm" value={formData.vehicleType || ''} onChange={e => setFormData({...formData, vehicleType: e.target.value})}>
                    ...
                    {/* Mileage */}
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-500 uppercase">Kilométrage</label>
                        <input title="Kilométrage" type="number" className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-900 text-sm" value={formData.vehicleMileage || ''} onChange={e => setFormData({...formData, vehicleMileage: parseInt(e.target.value)})} />
                    </div>

                    {/* VIN */}
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-500 uppercase">VIN</label>
                        <input title="VIN" className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-900 text-sm font-mono uppercase" value={formData.vin || ''} onChange={e => setFormData({...formData, vin: e.target.value})} maxLength={17} />
                    </div>

                    {/* GPS Status (Read-only) */}
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-500 uppercase">Statut GPS</label>
                        <div className="relative">
                            <Activity className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                            <input 
                                title="Statut GPS" 
                                className="w-full pl-10 p-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-100 dark:bg-slate-800 text-sm font-bold" 
                                value={(() => {
                                    const status = availableVehicles.find(v => v.id === formData.vehicleId)?.status;
                                    switch(status) {
                                        case 'MOVING': return 'EN MOUVEMENT';
                                        case 'STOPPED': return 'ARRÊTÉ';
                                        case 'IDLE': return 'RALENTI';
                        ...
                        <div className="md:col-span-4 p-4 bg-orange-50 dark:bg-orange-900/10 rounded-lg border border-orange-200 dark:border-orange-800 flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-orange-600">
                                    ...
                                    <strong> Veuillez saisir ci-dessous les informations du NOUVEL engin physique</strong> (Plaque, Marque, Modèle). 
                                    L'ancien véhicule sera automatiquement mis à jour à la clôture.
                                </p>
                            </div>
                        </div>
                    )}
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-500 uppercase">Type de Balise</label>
                        <div className="relative">
                            {loadingModels && (
                        ...
                        <p className="text-[10px] text-slate-400 mt-0.5">Modèles depuis Admin &gt; Paramètres Boîtiers</p>
                    </div>
                    ...
                    <div className="space-y-1 relative" ref={materialMenuRef}>
                        <label className="text-xs font-bold text-slate-500 uppercase">Matériel à utiliser</label>
                        <div className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-900 text-sm cursor-pointer flex justify-between items-center" onClick={() => setIsMaterialMenuOpen(!isMaterialMenuOpen)}>
                    ...
                    */}
                    ...
                    {hasSensor() && (
                        <>
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slate-500 uppercase">
                                    {(formData.nature as string) === 'Remplacement' ? 'Nouvelle Sonde (S/N)' : 'Accessoire / Capteur (S/N)'}
                                </label>
                                <div className="relative">
                                    <input 
                                        list="stock-sensor"
                                        className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-900 text-sm font-mono" 
                            ...
                            {(formData.nature as string) === 'Remplacement' && (
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-slate-500 uppercase">Ancienne Sonde</label>
                                    <input 
                                        className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-100 dark:bg-slate-800 text-sm font-mono text-slate-500 cursor-not-allowed" 
                                        value={(() => {
                            ...
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slate-500 uppercase">MAC</label>
                                <input className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-900 text-sm font-mono" value={formData.macAddress || ''} onChange={e => setFormData({...formData, macAddress: e.target.value})} placeholder="AA:BB:CC:DD:EE:FF" />
                            </div>

                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slate-500 uppercase">Type de Sonde</label>
                                <select className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-900 text-sm" value={formData.probeType || ''} onChange={e => setFormData({...formData, probeType: e.target.value})}>
                                    <option value="">-- Sélectionner --</option>
                    ...
                    {(currentNatureConfig?.stock_impact?.action === 'SWAP' || currentNatureConfig?.stock_impact?.action === 'IN') && (
                        <div className="space-y-1">
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                ...
                <div className="mt-4">
                    <label className="text-xs font-bold text-slate-500 uppercase mb-1 block">Observations</label>
                    <textarea className="w-full p-3 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-900 text-sm min-h-[80px] resize-none" placeholder="Défauts constatés..." value={formData.observations || ''} onChange={e => setFormData({...formData, observations: e.target.value})} />
                </div>
            </div>
        </div>
    );
