...
export const InterventionRequestTab: React.FC<RequestTabProps> = ({
    ...
    const dynamicNatures = currentTypeObj 
        ? allNatures.filter(n => n.typeId === currentTypeObj.id)
        : [];
            ...
            <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-500 uppercase">Ticket Lié</label>
                        {isTicketLocked ? (
                            <input
                                className="w-full p-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-100 dark:bg-slate-800 text-sm font-mono text-slate-500 cursor-not-allowed"
                                value={(() => {
                                ...
                                <select
                                    ...
                                    onChange={e => {
                                        ...
                                        if (ticket) {
                                            ...
                                            if (ticket.vehicleId) {
                                                const vehicleUpdates = getVehicleUpdates(ticket.vehicleId, availableVehicles, stock, formData, contracts);
                        ...
                        {!isTicketLocked && formData.clientId && tickets.filter(t => t.clientId === formData.clientId && t.status !== 'CLOSED').length === 0 && (
                    ...
                    <div className="space-y-1">
                        ...
                        <input
                            className="w-full p-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-100 dark:bg-slate-800 text-sm text-slate-500"
                            value={(() => {
                    ...
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-500 uppercase">Contact Client</label>
                        <div className="relative">
                            <Phone className="absolute left-2 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                            <input
                                className="w-full pl-8 p-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-100 dark:bg-slate-800 text-sm text-slate-500"
                                value={(() => {
                                    const client = clients.find(c => c.id === formData.clientId);
                    ...
                    <FormField label="Type" required>
                        <Select 
                            ...
                            onChange={e => { 
                                ...
                                const firstNature = allNatures.find(n => n.typeId === interventionTypes.find(t => t.code === val)?.id)?.label || 'Balise';
                        ...
                        <Select
                            ...
                            onChange={e => {
                                ...
                                setFormData({
                                    ...
                                    removalReason: (val === 'Retrait' || val?.includes('Remplacement') || ['REMPLACEMENT', 'RETRAIT'].includes(formData.type || '')) ? formData.removalReason : undefined
                                });
                            }}
                            className="font-bold"
                        >
                            ...
                            {dynamicNatures.length > 0 ? (
                                dynamicNatures.map(n => <option key={n.id} value={n.label}>{n.label}</option>)
                            ) : (
                    ...
                    <FormField label="Date & Heure">
                        <Input title="Date & Heure" type="datetime-local" value={formData.scheduledDate?.slice(0, 16) || ''} onChange={e => setFormData({ ...formData, scheduledDate: e.target.value ? new Date(e.target.value).toISOString() : null })} />
                    </FormField>

                    {/* Adresse / Lieu */}
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-500 uppercase">Adresse / Lieu</label>
                        <div className="relative">
                            <MapPin className="absolute left-2 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                            <input title="Adresse / Lieu" className="w-full pl-8 p-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-900 text-sm" value={formData.location || ''} onChange={e => setFormData({ ...formData, location: e.target.value })} />
                        </div>
                    </div>

                    {/* Contact Sur Site - Name */}
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-500 uppercase">Nom Contact Sur Site</label>
                        <div className="relative">
                            <Building2 className="absolute left-2 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                            <input 
                                title="Nom Contact Sur Site" 
                                type="text" 
                                className="w-full pl-8 p-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-900 text-sm" 
                    ...
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-500 uppercase">Tél Contact Sur Site</label>
                        <div className="relative">
                            <Phone className="absolute left-2 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                            <input 
                ...
                {((formData.nature as string) === 'Retrait' || (formData.type as string) === 'RETRAIT') && (
                    <div className="mt-4 p-3 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800">
                        ...
                        <select
                            className="w-full max-w-md p-2 border border-red-200 dark:border-red-800 rounded-lg bg-white dark:bg-slate-900 text-sm font-bold"
                            ...
                            onChange={e => {
                                ...
                                setFormData({
                                    ...
                                    contractRemovalReason: shouldRemoveContract ? (reason === 'FIN_CONTRAT' ? 'Fin de contrat / Désabonnement' : 'Retrait simple') : ''
                                });
                            }}
                            required
                        >
            ...
            <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <FormField label="Sélection Matériel / Véhicule">
                        <Select title="Sélection Matériel / Véhicule" className="font-mono" value={formData.vehicleId || ''} onChange={e => {
                            ...
                            {formData.type === 'INSTALLATION' ? (
                                <>
                                    <optgroup label="En Stock">
                                        {stock.filter(s => s.status === 'IN_STOCK' && s.location === 'TECH' && s.technicianId === formData.technicianId).map(s => (
                                    ...
                                    <optgroup label="Client">
                                        {availableVehicles.map(v => (
                                            <option key={v.id} value={v.id}>{v.plate || v.name} (IMEI actuel: {v.imei || 'Aucun'})</option>
                                        ))}
                                    </optgroup>
                                </>
                            ) : (
                                <optgroup label="Client">
                                    {availableVehicles.map(v => (
                                        <option key={v.id} value={v.id}>{v.plate || v.name} (IMEI: {v.imei || 'Aucun'})</option>
                                    ))}
                                </optgroup>
                            )}
                        </Select>
                    ...
                    <FormField label="Branche">
                        ...
                        >
                            <option value="">-- Sélectionner --</option>
                            {branches.filter(b => formData.clientId ? b.clientId === formData.clientId || b.client_id === formData.clientId : true).map(b => (
                    ...
                    <FormField label="Nom du Véhicule">
                        <Input title="Nom du Véhicule" value={formData.vehicleName || ''} onChange={e => setFormData({ ...formData, vehicleName: e.target.value })} placeholder="Ex: Camion 1" />
                    </FormField>
                    ...
                    <FormField label="Plaque WW">
                        <Input title="Plaque WW" className="font-mono uppercase" value={formData.tempPlate || ''} onChange={e => setFormData({ ...formData, tempPlate: e.target.value })} placeholder="WW-..." />
                    </FormField>
                        ...
                        <div className="space-y-1">
                            <label className="text-xs font-bold text-slate-500 uppercase">
                                {(() => {
                                    ...
                                    if (nature.includes('Remplacement') || formData.type === 'REMPLACEMENT') return 'Nouveau Boîtier (IMEI)';
                                    return 'Boîtier GPS (IMEI)';
                                })()}
                            </label>
                            <div className="relative">
                                {(formData.type === 'TRANSFERT' || formData.type === 'RETRAIT' || formData.type === 'REINSTALLATION' || (formData.nature as string) === 'Transfert' || (formData.nature as string) === 'Retrait' || (formData.nature as string) === 'Réinstallation') ? (
                                    ...
                                    <select
                                        className="w-full p-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-900 text-sm font-mono"
                                        value={formData.imei || ''}
                                    ...
                                    >
                                        ...
                                        {stock
                                            ...
                                            .filter(d => d.location === 'TECH' && d.technicianId === formData.technicianId && d.status === 'IN_STOCK')
                                            .map(d => (
                                        ...
                                        <datalist id="stock-box">
                                            {stock
                                                ...
                                                .filter(d => {
                                                    ...
                                                    return d.status === 'IN_STOCK' || (d.location === 'TECH' && d.technicianId === formData.technicianId);
                                                })
                                                .map(d => (
                                                    <option key={d.id} value={d.serialNumber || d.imei || d.iccid}>
                    ...
                    {formData.nature?.includes('Remplacement') && hasTracker() && (
                        <div className="space-y-1">
                            <label className="text-xs font-bold text-slate-500 uppercase">Ancien Boîtier</label>
                            <input
                                className="w-full p-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-100 dark:bg-slate-800 text-sm font-mono text-slate-500"
                    ...
                    {formData.nature?.includes('Remplacement') && (
                        <div className="space-y-1">
                            <label className="text-xs font-bold text-slate-500 uppercase text-orange-600">État Matériel Retiré</label>
                            <select
                                ...
                                onChange={e => setFormData({ ...formData, removedMaterialStatus: e.target.value as Intervention['removedMaterialStatus'] })}
                            >
                        ...
                        <>
                            <div className="space-y-1 col-span-2">
                                <label className="text-xs font-bold text-purple-600 uppercase flex items-center gap-1">
                                    <Navigation className="w-3 h-3" /> Nouvelle Plaque d'Immatriculation <span className="text-red-500">*</span>
                                </label>
                                <input
                                    type="text"
                                    ...
                                    required
                                />
                                <p className="text-[10px] text-purple-500">La plaque du véhicule sera mise à jour avec cette nouvelle immatriculation.</p>
                            </div>
                        </>
                    )}
                    ...
                    {hasSim() && (
                        <>
                            <div className="space-y-1">
                                ...
                                <div className="relative">
                                    {(['Retrait', 'Transfert', 'Réinstallation'].includes(formData.nature || '') || (formData.nature || '').includes('Remplacement')) ? (
                            ...
                            {formData.nature?.includes('Remplacement') && (
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-slate-500 uppercase">Ancienne SIM</label>
                                    <input
                                        className="w-full p-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-100 dark:bg-slate-800 text-sm font-mono text-slate-500"
                            ...
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slate-500 uppercase">N° Tel SIM</label>
                                <div className="relative">
                                    <Phone className="absolute left-2 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                                    <input
                                        title="Numéro de Téléphone"
                                        className="w-full pl-8 p-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-100 dark:bg-slate-800 text-sm font-mono text-slate-500"
                ...
                {/* Scheduling Row */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-4 border-t border-slate-100 dark:border-slate-700 pt-4">
                    <FormField label="Durée (min)">
                        <Input title="Durée (min)" type="number" value={formData.duration || ''} onChange={e => setFormData({ ...formData, duration: parseInt(e.target.value) })} />
                    </FormField>
                    <FormField label="Montant">
                        <Input type="text" className="font-mono text-slate-500 bg-slate-100 dark:bg-slate-800" value={formatPrice(formData.cost || 0)} readOnly placeholder="0" />
                    </FormField>
