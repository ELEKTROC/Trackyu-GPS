...
export const TechSettingsPanel: React.FC = () => {
  ...
  const isSuperAdmin =
    ['SUPERADMIN', 'SUPER_ADMIN'].includes(user?.role?.toUpperCase() || '') || user?.tenantId === 'tenant_trackyu';
  ...
  const handleDeleteType = async (id: string, isSystem: boolean) => {
    ...
    if (
      !(await confirm({
        message: "Supprimer ce type d'intervention ?",
        variant: 'danger',
  ...
  const handleDeleteNature = async (id: string, isSystem: boolean) => {
    ...
    if (
      !(await confirm({
        message: 'Supprimer cette nature ?',
        variant: 'danger',
        title: 'Confirmer la suppression',
        confirmLabel: 'Supprimer',
      }))
            ...
            <div className="space-y-4">
              ...
              {DEVICE_TYPES.map((deviceType) => {
                const filteredDevices = deviceModels.filter(
                  (d) => d.type === deviceType.value && (!deviceTypeFilter || deviceTypeFilter === deviceType.value)
                );
