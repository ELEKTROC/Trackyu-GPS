...
import { Edit2, Trash2, LayoutTemplate, X, Download, Upload, Search, Filter, FileText, FileSpreadsheet, FileDown, Clock, MapPin, User, ChevronRight, Wrench, SearchX } from 'lucide-react';
...
export const InterventionList: React.FC<InterventionListProps> = ({ interventions, technicians, onEdit, onDelete, onDepart }) => {
    const isMobile = useIsMobile();
    const { hasPermission } = useAuth();
    const { showToast } = useToast();
    const { types: interventionTypes, natures: allNatures } = useInterventionTypes();
    const { addInvoice, invoices, addIntervention, updateIntervention, clients, tickets, updateTicket } = useDataContext();
    ...
    const hasActiveFilters = filterNature !== 'ALL' || filterClient !== 'ALL' || filterReseller !== 'ALL' || filterInvoiced !== 'ALL';
    const activeFilterCount = [filterNature, filterClient, filterReseller, filterInvoiced].filter(f => f !== 'ALL').length;
    ...
    const handleBulkInvoice = async () => {
        ...
        const alreadyInvoiced = selectedInterventions.filter(i => i.invoiceId || invoices.some(inv => inv.interventionId === i.id));
        if (alreadyInvoiced.length > 0) {
             if (!await confirm({ message: `${alreadyInvoiced.length} intervention(s) semblent déjà facturée(s). Voulez-vous continuer quand même ?`, variant: 'warning', title: 'Interventions déjà facturées', confirmLabel: 'Continuer' })) {
                 return;
             }
        }
        ...
        const totalAmount = allItems.reduce((sum, item) => sum + ((item as { total?: number; quantity: number; price: number }).total || (item.quantity * item.price)), 0);

        // Check: total amount should not be zero
        if (totalAmount === 0) {
            if (!await confirm({ message: 'Le montant total est de 0. Voulez-vous quand même créer la facture ?', variant: 'warning', title: 'Montant nul', confirmLabel: 'Continuer' })) {
                return;
            }
        }
        ...
        const invoiceData: Partial<Invoice> = {
            ...
            amount: totalAmount,
            status: 'DRAFT',
            items: allItems.map(i => ({ description: i.description, quantity: i.quantity, price: (i as { unitPrice?: number; price: number }).unitPrice || i.price })),
            vatRate: 0,
            invoiceType: 'FACTURE'
        };
    ...
    const getResellerName = (i: Intervention) => i.resellerName || clients.find(c => c.id === i.clientId)?.resellerName || '';
    ...
    }), [interventions, globalSearch, technicians, clients, tickets, invoices, filterNature, filterClient, filterReseller, filterInvoiced]);

    const { sortedItems: sortedInterventions, sortConfig: intSortConfig, handleSort: handleIntSort } = useTableSort(filteredInterventions);

    const paginatedInterventions = sortedInterventions.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
    ...
    useEffect(() => { setMobileDisplayCount(20); }, [globalSearch, filterNature, filterClient, filterStatusMobile, filterTechMobile, filterResellerMobile]);
    ...
    const handleExportCSV = () => {
        try {
            const headers = ['Référence', 'Statut', 'Date', 'Client', 'Technicien', 'Type', 'Nature', 'Lieu', 'Véhicule', 'Montant'];
    ...
    const handleDownloadTemplate = () => {
        const headers = ['clientId', 'technicianId', 'type', 'nature', 'scheduledDate', 'location', 'licensePlate', 'notes', 'cost'];
        const example = ['CLIENT-001', 'TECH-001', 'INSTALLATION', 'Installation', '2025-01-15', 'Abidjan, Cocody', 'AB-1234-CI', 'Installation GPS tracker', '50000'];
    ...
    const handleImportCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = async (event) => {
            try {
                ...
                queryClient.invalidateQueries({ queryKey: ['interventions'] });
                showToast(errorCount > 0 ? TOAST.IO.IMPORT_PARTIAL(importCount, errorCount) : TOAST.IO.IMPORT_SUCCESS(importCount), importCount > 0 ? 'success' : 'error');
    ...
    const getStatusBadge = (status: string) => {
        const labels: Record<string, string> = {
            'PENDING': 'À planifier', 'SCHEDULED': 'Planifié', 'EN_ROUTE': 'En route',
    ...
    const MOBILE_STATUS_LABELS: Record<string, string> = {
        ALL: 'Tous', PENDING: 'À planifier', SCHEDULED: 'Planifié',
        EN_ROUTE: 'En route', IN_PROGRESS: 'En cours',
        COMPLETED: 'Terminé', CANCELLED: 'Annulé', POSTPONED: 'Reporté',
    };

    // ── MOBILE RENDER ──────────────────────────────────────────────────────────
    if (isMobile) {
        const mobileFilterCount = (filterStatusMobile !== 'ALL' ? 1 : 0) + (filterNature !== 'ALL' ? 1 : 0) + (filterClient !== 'ALL' ? 1 : 0) + (filterTechMobile !== 'ALL' ? 1 : 0) + (filterResellerMobile !== 'ALL' ? 1 : 0);

        return (
            <div className="flex-1 flex flex-col min-h-0 bg-slate-50 dark:bg-slate-900/50">
                {/* Mobile toolbar */}
                <div className="flex flex-col gap-1.5 px-3 py-2 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                    ...
                    {/* Export buttons row */}
                    <div className="flex items-center gap-2">
                        <span className="text-[10px] text-slate-400 font-medium uppercase tracking-wide">Exporter :</span>
                        <button onClick={handleExport} className="flex items-center gap-1 px-2 py-1 text-[10px] font-bold rounded border border-red-200 text-red-600 bg-red-50 dark:bg-red-900/20 dark:border-red-800 dark:text-red-400 hover:bg-red-100 transition-colors">
                ...
                <div className="flex-1 overflow-y-auto p-3 pb-40 space-y-2" style={{ WebkitOverflowScrolling: 'touch' }}>
                    {mobileFilteredInterventions.slice(0, mobileDisplayCount).map(int => {
                        ...
                        const canDepart = (int.status === 'SCHEDULED' || (int.status === 'PENDING' && int.scheduledDate && int.technicianId)) && !!onDepart;
                            ...
                            >
                                ...
                                <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <span className="text-[10px] font-mono text-slate-400 dark:text-slate-500">#{int.id.slice(-8).toUpperCase()}</span>
                                    </div>
                                    <span className="text-[11px] text-slate-500 dark:text-slate-400">{formatDate(int.scheduledDate)}</span>
                                </div>
                                {/* Client */}
                                <p className="font-bold text-sm text-slate-900 dark:text-white truncate mb-1">{clientName !== '-' ? clientName : 'Client non défini'}</p>
                                {/* Type + Nature */}
                                ...
                                <div className="flex items-center gap-3 text-[11px] text-slate-500 dark:text-slate-400">
                                    {int.location && (
                                        <span className="flex items-center gap-0.5 truncate">
                                            <MapPin className="w-3 h-3 shrink-0" />{int.location}
                                        </span>
                                    )}
                                    {techName && (
                                ...
                                {canDepart && (
                                    <div className="mt-2 pt-2 border-t border-black/5 dark:border-white/10">
                                        <button
                                            onClick={e => { e.stopPropagation(); onDepart!(int); }}
                    ...
                    {mobileFilteredInterventions.length === 0 && (
                        interventions.length === 0
                            ? <EmptyState compact icon={Wrench} title="Aucune intervention" description="Aucune intervention n'est encore planifiée." />
                            : <EmptyState compact icon={SearchX} title="Aucun résultat" description="Aucune intervention ne correspond aux filtres actifs." />
                    )}
                ...
                <MobileFilterSheet
                    ...
                    onReset={() => { setFilterStatusMobile('ALL'); setFilterNature('ALL'); setFilterClient('ALL'); setFilterTechMobile('ALL'); setFilterResellerMobile('ALL'); }}
                    tabs={[
                        {
                            id: 'status', label: 'Statut', activeCount: filterStatusMobile !== 'ALL' ? 1 : 0,
                            content: Object.entries(MOBILE_STATUS_LABELS).map(([val, label]) => (
                                <FilterRadioRow key={val} value={val} label={label} checked={filterStatusMobile === val} onChange={() => setFilterStatusMobile(val)} count={val === 'ALL' ? sortedInterventions.length : sortedInterventions.filter(i => i.status === val).length} />
                            ))
                        },
                        {
                            id: 'nature', label: 'Nature', activeCount: filterNature !== 'ALL' ? 1 : 0,
                            content: <>
                                <FilterRadioRow value="ALL" label="Toutes" checked={filterNature === 'ALL'} onChange={() => setFilterNature('ALL')} count={sortedInterventions.length} />
                                {[...new Set(sortedInterventions.map(i => i.nature).filter(Boolean))].sort().map(n => (
                                    <FilterRadioRow key={n} value={n!} label={n!} checked={filterNature === n} onChange={() => setFilterNature(n!)} count={sortedInterventions.filter(i => i.nature === n).length} />
                                ))}
                            </>
                        },
                        {
                            id: 'client', label: 'Client', activeCount: filterClient !== 'ALL' ? 1 : 0,
                            content: <>
                                <FilterRadioRow value="ALL" label="Tous" checked={filterClient === 'ALL'} onChange={() => setFilterClient('ALL')} count={sortedInterventions.length} />
                                {clientOptions.map(([id, name]) => (
                                    <FilterRadioRow key={id} value={id} label={name} checked={filterClient === id} onChange={() => setFilterClient(id)} count={sortedInterventions.filter(i => i.clientId === id).length} />
                                ))}
                            </>
                        },
                        {
                            id: 'technicien', label: 'Tech', activeCount: filterTechMobile !== 'ALL' ? 1 : 0,
                            content: <>
                                <FilterRadioRow value="ALL" label="Tous" checked={filterTechMobile === 'ALL'} onChange={() => setFilterTechMobile('ALL')} count={sortedInterventions.length} />
                                {technicians.map(t => (
                                    <FilterRadioRow key={t.id} value={t.id} label={t.name} checked={filterTechMobile === t.id} onChange={() => setFilterTechMobile(t.id)} count={sortedInterventions.filter(i => i.technicianId === t.id).length} />
                                ))}
                            </>
                        },
                        {
                            id: 'revendeur', label: 'Revendeur', activeCount: filterResellerMobile !== 'ALL' ? 1 : 0,
                            content: <>
                                <FilterRadioRow value="ALL" label="Tous" checked={filterResellerMobile === 'ALL'} onChange={() => setFilterResellerMobile('ALL')} count={sortedInterventions.length} />
                                {resellerOptions.map(r => (
                                    <FilterRadioRow key={r} value={r} label={r} checked={filterResellerMobile === r} onChange={() => setFilterResellerMobile(r)} count={sortedInterventions.filter(i => getResellerName(i) === r).length} />
                                ))}
                            </>
                        },
                    ]}
    ...
    return (
        <Card className="flex-1 overflow-hidden p-0 border-slate-200 dark:border-slate-700 flex flex-col relative">
            {/* Toolbar */}
            <div className="p-2 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800">
                {selectedIds.size > 0 ? (
                    <div className="flex items-center gap-2 w-full animate-in fade-in duration-200">
                        <span className="text-sm font-bold text-slate-700 dark:text-slate-200 bg-slate-200 dark:bg-slate-700 px-2 py-1 rounded">
                        ...
                        <button onClick={() => setSelectedIds(new Set())} className="p-1 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-full" aria-label="Effacer la sélection" title="Effacer la sélection">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="relative" ref={filterMenuRef}>
                        ...
                        {isFilterMenuOpen && (
                            <div className="absolute top-full right-0 mt-2 w-72 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl z-50 p-4 animate-in fade-in slide-in-from-top-2">
                                <h4 className="text-xs font-bold text-slate-500 uppercase mb-3">Filtres avancés</h4>
                                <div className="space-y-3">
                                    <div className="space-y-1">
                                        <label className="text-xs text-slate-600 dark:text-slate-300">Nature</label>
                                        <select className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-900 text-sm" value={filterNature} onChange={e => setFilterNature(e.target.value)}>
                                            ...
                                            {allNatures.length > 0 ? (
                                                ...
                                                INTERVENTION_NATURES.map(n => <option key={n} value={n}>{n}</option>)
                                            )}
                                        </select>
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-xs text-slate-600 dark:text-slate-300">Client</label>
                                        <select className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-900 text-sm" value={filterClient} onChange={e => setFilterClient(e.target.value)}>
                                            <option value="ALL">Tous les clients</option>
                                            {clientOptions.map(([id, name]) => <option key={id} value={id}>{name}</option>)}
                                        </select>
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-xs text-slate-600 dark:text-slate-300">Revendeur</label>
                                        <select className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-900 text-sm" value={filterReseller} onChange={e => setFilterReseller(e.target.value)}>
                                            <option value="ALL">Tous les revendeurs</option>
                                            {resellerOptions.map(r => <option key={r} value={r}>{r}</option>)}
                                        </select>
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-xs text-slate-600 dark:text-slate-300">Facturation</label>
                                        <select className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-900 text-sm" value={filterInvoiced} onChange={e => setFilterInvoiced(e.target.value as 'ALL' | 'YES' | 'NO')}>
                    ...
                    <button onClick={() => setIsImportModalOpen(true)} className="p-2 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300" title="Importer"><Upload className="w-4 h-4" /></button>
                    <div className="relative" ref={exportMenuRef}>
                        <button onClick={() => setIsExportMenuOpen(!isExportMenuOpen)} className="p-2 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300" title="Exporter"><Download className="w-4 h-4" /></button>
                        {isExportMenuOpen && (
                            <div className="absolute top-full right-0 mt-2 w-48 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl z-50 p-2 animate-in fade-in slide-in-from-top-2">
                                <button onClick={handleExport} className="w-full flex items-center gap-2 px-3 py-2 hover:bg-slate-50 dark:hover:bg-slate-700 rounded text-sm text-left">
                                    <FileDown className="w-4 h-4 text-red-500" /> Export PDF
                                </button>
                                <button onClick={handleExportCSV} className="w-full flex items-center gap-2 px-3 py-2 hover:bg-slate-50 dark:hover:bg-slate-700 rounded text-sm text-left">
                    ...
                    <div className="relative" ref={columnMenuRef}>
                        <button onClick={() => setIsColumnMenuOpen(!isColumnMenuOpen)} className="p-2 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300" title="Colonnes"><LayoutTemplate className="w-4 h-4" /></button>
                        {isColumnMenuOpen && (
                            <div className="absolute top-full right-0 mt-2 w-56 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl z-50 p-2 animate-in fade-in slide-in-from-top-2 max-h-80 overflow-y-auto custom-scrollbar">
                    ...
                    <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 sticky top-0 z-10">
                        <tr>
                            ...
                            {INTERVENTION_COLUMNS.map(col => visibleColumns.includes(col.id) && (
                                col.id === 'actions' ? (
                                    <th key={col.id} className="px-4 py-3 text-xs font-bold text-slate-500 dark:text-slate-300 uppercase whitespace-nowrap border-b border-slate-200 dark:border-slate-700 text-right" style={{ minWidth: col.minWidth }}>{col.label}</th>
                                ) : (
                                    <SortableHeader key={col.id} label={col.label} sortKey={col.id} currentSortKey={intSortConfig.key} currentDirection={intSortConfig.direction} onSort={handleIntSort} className="whitespace-nowrap" />
                                )
                            ))}
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-sm bg-white dark:bg-slate-900">
                            ...
                            <tr key={int.id} className={`density-row hover:bg-[var(--primary-dim)] dark:hover:bg-[var(--primary-dim)] transition-colors cursor-pointer ${selectedIds.has(int.id) ? 'bg-[var(--primary-dim)] dark:bg-[var(--primary-dim)]' : ''}`} onClick={() => onEdit(int)}>
                                ...
                                {visibleColumns.includes('scheduledDate') && (
                                    <td className="px-4 py-3">
                                        <div className="font-medium text-slate-700 dark:text-slate-300">{formatDate(int.scheduledDate)}</div>
                                        <div className="text-slate-500 text-xs">{formatTime(int.scheduledDate)} ({int.duration}m)</div>
                                    </td>
                                )}
                                {visibleColumns.includes('client') && <td className="px-4 py-3 font-semibold text-slate-800 dark:text-white">{getClientName(int.clientId)}</td>}
                                {visibleColumns.includes('ticket') && <td className="px-4 py-3 font-mono text-violet-600 dark:text-violet-400">{getTicketNumber(int.ticketId)}</td>}
                                ...
                                {visibleColumns.includes('location') && <td className="px-4 py-3 text-slate-600 dark:text-slate-400 truncate max-w-[150px]">{int.location}</td>}
                                {visibleColumns.includes('vehicle') && <td className="px-4 py-3 font-mono text-slate-700 dark:text-slate-300">{cleanPlate(int.licensePlate) || '-'}</td>}
                                {visibleColumns.includes('resolutionTime') && (
                                    <td className="px-4 py-3">
                                        {int.status === 'COMPLETED' ? (
                                            <span className={`font-medium flex items-center gap-1 ${getResolutionTimeColor(calculateResolutionTime(int), int.type)}`}>
                                ...
                                {visibleColumns.includes('cost') && <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{int.cost ? formatPrice(int.cost) : '-'}</td>}
                                
                                {visibleColumns.includes('actions') && (
                                    <td className="px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                                        <div className="flex items-center justify-end gap-1">
                                            {/* Bouton "Partir" visible si SCHEDULED, ou si PENDING avec date et technicien */}
                                            {(int.status === 'SCHEDULED' || (int.status === 'PENDING' && int.scheduledDate && int.technicianId)) && onDepart && (
                                                <button 
                                                    onClick={(e) => { e.stopPropagation(); onDepart(int); }} 
                                            ...
                                            <button onClick={() => onEdit(int)} className="p-1.5 hover:bg-slate-200 dark:hover:bg-slate-700 rounded text-slate-500 transition-colors" aria-label="Modifier" title="Modifier"><Edit2 className="w-4 h-4" /></button>
                                        </div>
                                    </td>
                                )}
                            </tr>
                        ...
                        {paginatedInterventions.length === 0 && (
                            <tr>
                                <td colSpan={20}>
                                    {interventions.length === 0
                                        ? <EmptyState icon={Wrench} title="Aucune intervention" description="Aucune intervention n'est encore planifiée." />
                                        : <EmptyState icon={SearchX} title="Aucun résultat" description="Aucune intervention ne correspond aux filtres actifs." />
                                    }
                                </td>
                            </tr>
                        )}
            ...
            <div className="p-3 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex justify-between items-center text-xs">
                <div className="flex items-center gap-2">
                    ...
                    <select 
                        className="p-1 border border-slate-200 dark:border-slate-600 rounded text-xs bg-white dark:bg-slate-900"
                        value={itemsPerPage}
                    ...
                    <span className="text-slate-500 ml-2">Affichage {paginatedInterventions.length} sur {filteredInterventions.length}</span>
                </div>
                <Pagination
            ...
            >
                <div className="p-4 space-y-4">
                    ...
                    <button
                        ...
                        className="w-full flex items-center justify-center gap-2 px-4 py-3 border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-lg text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
                    >
