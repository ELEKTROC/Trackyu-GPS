...
import { 
  Wrench, Clock, TrendingUp, CheckCircle, LayoutTemplate, AlertTriangle, AlertCircle, CalendarClock, XCircle, Timer,
  Banknote, Ban, Target, Hourglass
} from 'lucide-react';
import { 
...
export const TechStats: React.FC<TechStatsProps> = ({ interventions, technicians }) => {
    ...
    const stats = useMemo(() => {
        ...
        switch (statsPeriod) {
            ...
            case 'THIS_WEEK': periodStart = startOfWeek(now, { weekStartsOn: 1 }); break;
            case 'THIS_YEAR': periodStart = startOfYear(now); break;
            case 'THIS_MONTH':
            default: periodStart = startOfMonth(now); break;
        }
        ...
        const revenue = filtered
            ...
            .reduce((sum, i) => {
                const itemsTotal = (i.invoiceItems || []).reduce((s, item) => s + (item.total ?? item.unitPrice * item.quantity), 0);
        ...
        const punctualityRate = completedFiltered.length > 0 
            ? Math.round((onTimeCount / completedFiltered.length) * 100) 
            : 0;
        ...
        const unassigned = filtered.filter(i => 
            !i.technicianId || i.technicianId === 'UNASSIGNED'
        );
        ...
        const natureData = filtered.reduce((acc, curr) => { 
            acc[curr.nature] = (acc[curr.nature] || 0) + 1; return acc; 
        ...
        const materialCounts = filtered.flatMap(i => i.invoiceItems || []).reduce((acc, item) => {
        ...
        const vehicleTypeData = filtered.reduce((acc, curr) => {
        ...
        const monthCounts = filtered.reduce((acc, curr) => {
        ...
        const STATUS_LABELS: Record<string, string> = {
            'PENDING': 'En attente', 'SCHEDULED': 'Planifiée', 'EN_ROUTE': 'En route',
            'IN_PROGRESS': 'En cours', 'COMPLETED': 'Terminée', 'CANCELLED': 'Annulée', 'POSTPONED': 'Reportée'
        };
        const STATUS_COLORS: Record<string, string> = {
            'PENDING': '#94a3b8', 'SCHEDULED': '#3b82f6', 'EN_ROUTE': '#f59e0b',
            'IN_PROGRESS': '#f97316', 'COMPLETED': '#22c55e', 'CANCELLED': '#ef4444', 'POSTPONED': '#a855f7'
        };
        const statusCounts = filtered.reduce((acc, curr) => {
            acc[curr.status] = (acc[curr.status] || 0) + 1; return acc;
        }, {} as Record<string, number>);
        ...
        // 8. Top 5 Clients
        const clientCounts = filtered.reduce((acc, curr) => {
        ...
        return {
            totalInterventions, pending, completed, cancelled, postponed, successRate,
            revenue, avgResponseTime, punctualityRate,
            overdue, atRisk, unassigned, inProgress, resolutionStats,
            techData, pieNatureData, materialData, barVehicleData, installData, trendData,
            statusData, topClientsData
        };
    ...
    return (
        <div className="space-y-6 overflow-y-auto h-full custom-scrollbar p-2">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                <div>
                    <h3 className="font-bold text-lg text-slate-800 dark:text-white">Performance Technique</h3>
                    <p className="text-xs text-slate-500">Analyse dynamique des interventions.</p>
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                    <div className="relative" ref={chartMenuRef}>
                        <button 
                            onClick={() => setIsChartMenuOpen(!isChartMenuOpen)} 
                            className="flex items-center gap-2 px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                        >
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                ...
                <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-xs text-slate-500 uppercase font-bold">En Attente</p>
                            <p className="text-2xl font-bold text-slate-600 mt-1">{stats.pending}</p>
                        </div>
                        <div className="p-2 bg-slate-100 dark:bg-slate-700 rounded text-slate-500"><Hourglass className="w-5 h-5" /></div>
                    </div>
                </div>
                <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-xs text-slate-500 uppercase font-bold">En Cours</p>
                            <p className="text-2xl font-bold text-orange-600 mt-1">{stats.inProgress.length}</p>
                        </div>
                        <div className="p-2 bg-orange-50 dark:bg-orange-900/20 rounded text-orange-600"><Clock className="w-5 h-5" /></div>
                    </div>
                </div>
                <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-xs text-slate-500 uppercase font-bold">Terminées</p>
                            <p className="text-2xl font-bold text-green-600 mt-1">{stats.completed}</p>
                        </div>
                        <div className="p-2 bg-green-50 dark:bg-green-900/20 rounded text-green-600"><CheckCircle className="w-5 h-5" /></div>
                    </div>
                </div>
                <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-xs text-slate-500 uppercase font-bold">Annulées</p>
                            <p className="text-2xl font-bold text-red-500 mt-1">{stats.cancelled}</p>
                            {stats.postponed > 0 && (
                                <p className="text-[10px] text-slate-400 mt-0.5">{stats.postponed} reportée{stats.postponed > 1 ? 's' : ''}</p>
                            )}
                        </div>
                        <div className="p-2 bg-red-50 dark:bg-red-900/20 rounded text-red-500"><Ban className="w-5 h-5" /></div>
                    </div>
                </div>
                <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-xs text-slate-500 uppercase font-bold">Taux Succès</p>
                            <p className="text-2xl font-bold text-purple-600 mt-1">{stats.successRate}%</p>
                        </div>
                        <div className="p-2 bg-purple-50 dark:bg-purple-900/20 rounded text-purple-600"><TrendingUp className="w-5 h-5" /></div>
                    </div>
                </div>
                <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-xs text-slate-500 uppercase font-bold">Ponctualité</p>
                            <p className="text-2xl font-bold text-teal-600 mt-1">{stats.punctualityRate}%</p>
                        </div>
                        <div className="p-2 bg-teal-50 dark:bg-teal-900/20 rounded text-teal-600"><Target className="w-5 h-5" /></div>
                    </div>
                </div>
                <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-xs text-slate-500 uppercase font-bold">Temps Moyen</p>
                        ...
                        <div className="p-2 bg-cyan-50 dark:bg-cyan-900/20 rounded text-cyan-600"><Timer className="w-5 h-5" /></div>
                    </div>
                </div>
                <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-xs text-slate-500 uppercase font-bold">Réactivité</p>
                            <p className="text-lg font-bold text-indigo-600 mt-1">
                                {stats.avgResponseTime !== null ? formatDuration(stats.avgResponseTime) : '-'}
                            </p>
                            <p className="text-[10px] text-slate-400 mt-0.5">Création → Début</p>
                        </div>
                        <div className="p-2 bg-indigo-50 dark:bg-indigo-900/20 rounded text-indigo-600"><CalendarClock className="w-5 h-5" /></div>
                    </div>
                </div>
                <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-xs text-slate-500 uppercase font-bold">Chiffre Aff.</p>
                            <p className="text-lg font-bold text-emerald-600 mt-1">
                                {stats.revenue > 0 ? formatPrice(stats.revenue) : '0'}
                            </p>
                            <p className="text-[10px] text-slate-400 mt-0.5">{currency}</p>
                        </div>
                        <div className="p-2 bg-emerald-50 dark:bg-emerald-900/20 rounded text-emerald-600"><Banknote className="w-5 h-5" /></div>
                    </div>
                </div>
            </div>
                            ...
                            <div className="mt-3 space-y-1 max-h-20 overflow-y-auto">
                                {stats.atRisk.slice(0, 3).map(i => (
                                    <p key={i.id} className="text-xs text-amber-700 dark:text-amber-300 truncate">
                                        {i.id} - {new Date(i.scheduledDate).toLocaleTimeString('fr-FR', {hour: '2-digit', minute: '2-digit'})}
                                    </p>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="bg-slate-100 dark:bg-slate-700/50 p-4 rounded-xl border border-slate-200 dark:border-slate-600">
                            <div className="flex items-center gap-3">
                                <div className="p-2 bg-slate-200 dark:bg-slate-600 rounded-lg">
                                    <AlertCircle className="w-6 h-6 text-slate-600 dark:text-slate-300" />
                                </div>
                                <div>
                                    <p className="text-2xl font-bold text-slate-700 dark:text-slate-200">{stats.unassigned.length}</p>
                                    <p className="text-xs font-bold text-slate-600 dark:text-slate-400 uppercase">Non Assignées</p>
                                </div>
                            </div>
                            <div className="mt-3 space-y-1 max-h-20 overflow-y-auto">
                                {stats.unassigned.slice(0, 3).map(i => (
                                    <p key={i.id} className="text-xs text-slate-600 dark:text-slate-400 truncate">
                                        {getClientName(i.clientId)} - {i.nature || i.type}
                                    </p>
                                ))}
                            </div>
                ...
                {visibleCharts.nature && (
                    <Card title="Répartition par Nature">
                        <div className="h-[250px] w-full">
                            <ResponsiveContainer width="100%" height="100%" minHeight={200} minWidth={200} initialDimension={{ width: 200, height: 200 }}>
                                <RePieChart>
                                    <Pie data={stats.pieNatureData} cx="50%" cy="50%" outerRadius={80} fill="#8884d8" dataKey="value" label>
                                        {stats.pieNatureData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                                    </Pie>
                                    <Tooltip /><Legend />
                                </RePieChart>
                            </ResponsiveContainer>
                ...
                {visibleCharts.status && stats.statusData.length > 0 && (
                    <Card title="Répartition par Statut">
                        <div className="h-[250px] w-full">
                            <ResponsiveContainer width="100%" height="100%" minHeight={200} minWidth={200} initialDimension={{ width: 200, height: 200 }}>
                                <RePieChart>
                                    <Pie data={stats.statusData} cx="50%" cy="50%" innerRadius={40} outerRadius={80} dataKey="value" label>
                                        {stats.statusData.map((entry, index) => <Cell key={`s-${index}`} fill={entry.fill} />)}
                                    </Pie>
                                    <Tooltip /><Legend />
                                </RePieChart>
                            </ResponsiveContainer>
