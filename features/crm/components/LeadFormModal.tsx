...
export const LeadFormModal: React.FC<LeadFormModalProps> = ({
    ...
    const checkDuplicate = (email: string, companyName: string): string | null => {
        if (email) {
            const emailDuplicate = existingLeads.find(l =>
                l.id !== editingLead.id &&
        ...
        if (companyName) {
            const companyDuplicate = existingLeads.find(l =>
                l.id !== editingLead.id &&
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const calculatedValue = editingLead.interestedProducts?.reduce((sum, p) => sum + (p.price * (p.quantity || 1)), 0) || editingLead.potentialValue || 0;
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={editingLead.id ? "Modifier le Lead" : "Nouveau Lead"} maxWidth="max-w-6xl" isDirty={hasChanges}>
            <div className="space-y-4 p-1">
                <FormGrid columns={3}>
                ...
                <FormField label="Produits / Services Intéressés">
                    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-3 bg-slate-50 dark:bg-slate-900 space-y-3">
                        <Select
                            onChange={(e) => {
                        ...
                        >
                            ...
                            {catalogItems.map(item => (
                                <option key={item.id} value={item.id}>{item.name} ({(item.price ?? 0).toLocaleString('fr-FR')})</option>
                            ))}
                        </Select>

                        <div className="space-y-2">
                            {(editingLead.interestedProducts || []).map((product, idx) => (
                                <div key={product.id} className="flex items-center gap-2 bg-white dark:bg-slate-800 p-2.5 rounded-xl border border-slate-200 dark:border-slate-700">
                                    <div className="flex-1">
                                        <div className="text-sm font-bold text-slate-700 dark:text-slate-200">{product.name}</div>
                                        <div className="text-xs text-slate-500">{(product.price ?? 0).toLocaleString('fr-FR')} / unité</div>
                                    </div>
                            ...
                            {(!editingLead.interestedProducts || editingLead.interestedProducts.length === 0) && (
                                <div className="text-center text-xs text-slate-400 italic py-2">Aucun produit sélectionné</div>
                            )}
                        </div>
                    </div>
