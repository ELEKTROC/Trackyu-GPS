...
export const TiersView: React.FC<{ onNavigate?: any; dateRange?: { start: string; end: string } }> = ({
  onNavigate,
  ...
  const kpis = useMemo(() => {
    ...
    const newClients = filteredTiers.filter(
      (t) => t.type === 'CLIENT' && new Date(t.createdAt).getMonth() === new Date().getMonth()
    ).length;
        ...
        <div className="flex gap-2 items-center flex-wrap sm:flex-nowrap">
          ...
          <div className="relative" ref={filterMenuRef}>
            <button
              ...
              className={`p-2 border border-[var(--border)] rounded-lg hover:bg-[var(--bg-elevated)] dark:hover:bg-slate-700 text-[var(--text-secondary)] transition-colors ${isFilterMenuOpen || activeFilters.length > 0 ? 'bg-[var(--bg-elevated)] ring-2 ring-[var(--primary-dim)] text-[var(--primary)] dark:text-[var(--primary)]' : ''}`}
                ...
                <div className="p-1">
                  {[
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-between px-3 py-2 text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-elevated)] dark:hover:bg-slate-700 rounded-md transition-colors"
                    >
                      <span>{opt.label}</span>
          ...
          <div className="hidden sm:block relative" ref={columnMenuRef}>
            <button
              ...
              className={`p-2 border border-[var(--border)] rounded-lg hover:bg-[var(--bg-elevated)] dark:hover:bg-slate-700 text-[var(--text-secondary)] transition-colors ${isColumnMenuOpen ? 'bg-[var(--bg-elevated)] ring-2 ring-[var(--primary-dim)] text-[var(--primary)] dark:text-[var(--primary)]' : ''}`}
              ...
              <div className="absolute top-full right-0 mt-2 w-56 bg-[var(--bg-elevated)] border border-[var(--border)] rounded-lg shadow-xl z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                <div className="p-1 max-h-48 overflow-y-auto custom-scrollbar">
                  {['Nom', 'Type', 'Email', 'Téléphone', 'Ville', 'Statut', 'Date Création'].map((col) => (
                    <label
                      key={col}
                      className="flex items-center gap-2 px-3 py-2 hover:bg-[var(--bg-elevated)] dark:hover:bg-slate-700 rounded cursor-pointer"
                    >
                      <input
                        type="checkbox"
                        defaultChecked
          ...
          <div className="hidden sm:block h-4 w-px bg-slate-200 bg-[var(--bg-elevated)] mx-1 self-center"></div>

          {/* EXPORT BUTTON — desktop only */}
          <button
            onClick={handleExportExcel}
            className="hidden sm:block p-2 border border-[var(--border)] rounded-lg hover:bg-[var(--bg-elevated)] dark:hover:bg-slate-700 text-[var(--text-secondary)] transition-colors active:bg-slate-100 dark:active:bg-slate-800"
          ...
          <div className="hidden sm:block relative" ref={importMenuRef}>
            <button
              ...
              className={`p-2 border border-[var(--border)] rounded-lg hover:bg-[var(--bg-elevated)] dark:hover:bg-slate-700 text-[var(--text-secondary)] transition-colors ${isImportMenuOpen ? 'bg-[var(--bg-elevated)] ring-2 ring-[var(--primary-dim)] text-[var(--primary)] dark:text-[var(--primary)]' : ''}`}
              title="Importer des données"
            >
              <Upload className="w-4 h-4" />
            </button>
            {isImportMenuOpen && (
              <div className="absolute right-0 top-full mt-2 w-56 bg-[var(--bg-elevated)] rounded-lg shadow-xl border border-[var(--border)] z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                <div className="p-2 bg-[var(--bg-elevated)] border-b border-[var(--border)] border-[var(--border)] text-[10px] font-bold text-[var(--text-secondary)] uppercase">
                  Import de données
                </div>
                <button
                  ...
                  className="w-full text-left px-4 py-3 text-sm hover:bg-[var(--bg-elevated)] dark:hover:bg-slate-700 flex items-center gap-3 text-[var(--text-primary)] transition-colors"
                >
                ...
                <button
                  ...
                  className="w-full text-left px-4 py-3 text-sm hover:bg-[var(--bg-elevated)] dark:hover:bg-slate-700 flex items-center gap-3 border-t border-[var(--border)] border-[var(--border)] text-[var(--text-primary)] transition-colors"
                >
