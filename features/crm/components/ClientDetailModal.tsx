...
import {
    Users, FileText, MoreHorizontal, Plus, Calendar, Building2, Mail, Phone, X,
    Truck, Search, LayoutGrid, ChevronLeft, ChevronRight,
    Edit2, MapPin, CreditCard, MessageSquare, FileSpreadsheet, Send,
    Languages, Briefcase as BriefcaseIcon, Bookmark, BarChart3, CheckSquare,
    Receipt, FileCheck, Wrench, LifeBuoy, Book, Download, Trash2
} from 'lucide-react';
import {
...
const generateMockPayments = (count: number) => Array.from({ length: count }).map((_, i) => ({
    id: `PMT-${900 + i}`, date: new Date(Date.now() - i * 50000000).toLocaleDateString('fr-FR'),
    method: ['Virement', 'Chèque', 'Carte'][i % 3], amount: (Math.random() * 5000).toFixed(2), ref: `REF-${Math.floor(Math.random() * 10000)}`
}));

const generateMockJournals = (count: number) => Array.from({ length: count }).map((_, i) => ({
    id: `JRN-${i}`, date: new Date(Date.now() - i * 5000000).toLocaleDateString('fr-FR'),
    code: ['411100', '706000', '445710'][i % 3], label: `Ecriture comptable ${i}`, debit: i % 2 === 0 ? (Math.random() * 1000).toFixed(2) : 0, credit: i % 2 !== 0 ? (Math.random() * 1000).toFixed(2) : 0
}));

// --- STATIC DATA ---
const MOCK_MAILS = [
    { id: 'MAIL-01', date: '2023-10-01', subject: 'Bienvenue chez Trackyu GPS', type: 'Automatique', status: 'Lu', content: "Bonjour, \n\nNous sommes ravis de vous compter parmi nos nouveaux clients. Votre espace est désormais configuré.\n\nCordialement,\nL'équipe Trackyu GPS." },
    { id: 'MAIL-02', date: '2023-10-25', subject: 'Votre facture #FAC-2023-899', type: 'Facturation', status: 'Lu', content: "Cher client,\n\nVeuillez trouver ci-joint votre facture du mois d'octobre. Le montant est à régler avant le 30/11.\n\nService Comptabilité." },
    { id: 'MAIL-03', date: '2023-11-20', subject: 'Rappel de paiement', type: 'Relance', status: 'Envoyé', content: "Sauf erreur de notre part, nous n'avons pas reçu le règlement de la facture #FAC-2023-899. Merci de régulariser la situation." },
];

const INITIAL_COMMENTS = [
    { id: 'COM-01', date: '2023-09-15 10:30', author: 'Sophie Vente', text: 'Client rencontré au salon Logistique. Très intéressé par le module carburant.' },
...
export const ClientDetailModal: React.FC<ClientDetailModalProps> = ({
    client, isOpen, onClose, onEdit, invoices, quotes, contracts, interventions, tickets
}) => {
    ...
    const renderTransactionsTab = () => {
        ...
        const filteredData = currentData.filter((item: Record<string, unknown>) =>
            ...
            switch (activeTrxSubTab) {
                    ...
                    return (
                        <>
                            <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 text-xs text-slate-500 uppercase">
                                <tr><th>Numéro</th><th>Date</th><th>Échéance</th><th className="text-right">Montant</th><th className="text-right">Statut</th></tr>
                            </thead>
                            <tbody className="text-sm divide-y divide-slate-100 dark:divide-slate-800">
                                {paginatedData.map((i: Record<string, unknown>) => (
                                    <tr key={i.id as string} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                        ...
                                        <td className="py-3 px-4 text-right"><span className={`px-2 py-1 rounded text-[10px] font-bold border ${i.status === 'PAYÉ' ? 'bg-green-100 text-green-700 border-green-200' : i.status === 'RETARD' ? 'bg-red-100 text-red-700 border-red-200' : 'bg-orange-100 text-orange-700 border-orange-200'}`}>{i.status as string}</span></td>
                                    </tr>
                                ))}
                            </tbody>
                        </>
                        ...
                        <>
                            ...
                            <tbody className="text-sm divide-y divide-slate-100 dark:divide-slate-800">
                                {paginatedData.map((p: Record<string, unknown>) => (
                                    <tr key={p.id as string} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                        <td className="py-3 px-4 font-mono text-slate-600">{p.ref as string}</td>
                ...
                case 'INTERVENTIONS':
                    return (
                        <>
                            <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 text-xs text-slate-500 uppercase">
                                <tr><th>ID</th><th>Date</th><th>Type</th><th>Technicien</th><th className="text-right">Statut</th></tr>
                            </thead>
                            <tbody className="text-sm divide-y divide-slate-100 dark:divide-slate-800">
                                {paginatedData.map((int: Record<string, unknown>) => (
                                    <tr key={int.id as string} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                        ...
                                        <td className="py-3 px-4 text-right"><span className="px-2 py-1 bg-slate-100 rounded text-xs font-bold">{int.status as string}</span></td>
                                    </tr>
                                ))}
                            </tbody>
                        </>
                ...
                case 'JOURNALS':
                    return (
                        <>
                            <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 text-xs text-slate-500 uppercase">
                                <tr><th>Date</th><th>Compte</th><th>Libellé</th><th className="text-right">Débit</th><th className="text-right">Crédit</th></tr>
                            </thead>
                            <tbody className="text-sm divide-y divide-slate-100 dark:divide-slate-800">
                                {paginatedData.map((j: Record<string, unknown>) => (
                                    <tr key={j.id as string} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                        <td className="py-3 px-4 text-slate-500">{j.date as string}</td>
                                        <td className="py-3 px-4 font-mono text-slate-700 dark:text-slate-300">{j.code as string}</td>
                                        <td className="py-3 px-4">{j.label as string}</td>
                                        <td className="py-3 px-4 text-right font-mono text-slate-600">{(j.debit as number) > 0 ? (j.debit as number) : '-'}</td>
                                        <td className="py-3 px-4 text-right font-mono text-slate-600">{(j.credit as number) > 0 ? (j.credit as number) : '-'}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </>
                ...
                default:
                    return (
                        <>
                            <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 text-xs text-slate-500 uppercase">
                                <tr>
                                    {Object.keys(paginatedData[0] || {}).slice(0, 5).map(k => <th key={k} className="capitalize">{k}</th>)}
                                </tr>
                            </thead>
                            <tbody className="text-sm divide-y divide-slate-100 dark:divide-slate-800">
                                {paginatedData.map((item: any, i: number) => (
                                    <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                        {(item ? Object.values(item).slice(0, 5) : []).map((val: any, idx) => (
                                            <td key={idx} className="py-3 px-4 text-slate-600 dark:text-slate-300">{val}</td>
                                        ))}
                                    </tr>
                                ))}
                            </tbody>
            ...
            <div className="relative w-full max-w-6xl h-[90vh] bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-2xl flex overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-slate-200 dark:border-slate-700">
                ...
                <div className="w-64 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 flex flex-col z-10">
                    <div className="p-6 border-b border-slate-100 dark:border-slate-700">
                        ...
                        <h2 className="text-lg font-bold text-slate-800 dark:text-white leading-tight mb-1">{client.name}</h2>
                        <p className="text-xs text-slate-500 font-medium uppercase tracking-wider">{client.type}</p>
                    ...
                    <div className="p-4 border-t border-slate-100 dark:border-slate-700">
                        <button
                            onClick={async () => {
                                if (await confirm({ message: 'Êtes-vous sûr de vouloir supprimer ce client ?', title: 'Supprimer le client', variant: 'danger', confirmLabel: 'Supprimer' })) {
                ...
                <div className="flex-1 flex flex-col min-w-0 bg-slate-50/50 dark:bg-slate-900/50">
                    ...
                    <div className="h-16 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between px-6 shrink-0">
                        <div className="flex items-center gap-2 text-sm text-slate-500">
                            ...
                            <ChevronRight className="w-4 h-4" />
                            <span className="font-bold text-slate-800 dark:text-white">{activeDetailTab === 'OVERVIEW' ? "Vue d'ensemble" : activeDetailTab}</span>
                        </div>
                            ...
                            <div className="grid grid-cols-12 gap-6 animate-in fade-in slide-in-from-right-4 duration-300">
                                ...
                                <div className="col-span-12 lg:col-span-8 space-y-6">
                                    ...
                                    <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden">
                                        ...
                                        <div className="relative z-10">
                                            ...
                                            <div className="grid grid-cols-2 gap-6">
                                                ...
                                                <div className="p-3 bg-slate-50 dark:bg-slate-900 rounded-lg border border-slate-100 dark:border-slate-700">
                                                    <p className="text-[10px] font-bold text-slate-400 uppercase mb-2">Contact Secondaire</p>
                                                    {client.secondContactName ? (
                                                        <div className="flex items-start gap-3">
                                                            <div className="w-8 h-8 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center font-bold text-xs">{client.secondContactName.charAt(0)}</div>
                                                            <div>
                                                                <p className="font-bold text-slate-700 dark:text-slate-200 text-sm">{client.secondContactName}</p>
                                                                <p className="text-xs text-slate-400 italic mt-0.5">Données contact non renseignées</p>
                                                            </div>
                                                        </div>
                                                    ) : (
                                                        <p className="text-xs text-slate-400 italic">Aucun contact secondaire</p>
                                                    )}
                                                </div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-2 gap-4 text-sm mt-6">
                                                <div className="col-span-2">
                                                    ...
                                                    <div className="font-medium text-slate-800 dark:text-slate-200 flex items-start gap-2">
                                                        ...
                                                        <span>
                                                            {client.address}<br />
                                                            {client.city && <span className="text-slate-600 dark:text-slate-400">{client.city}, </span>}
                                                ...
                                                <div>
                                                    ...
                                                    <div className="font-medium text-slate-800 dark:text-slate-200 flex items-center gap-2">
                                                        <Calendar className="w-3 h-3 text-slate-400" /> {new Date(client.createdAt).toLocaleDateString('fr-FR')}
                                                    </div>
                                                </div>
                                                <div>
                                                    <label className="block text-xs text-slate-500 mb-1">Langue</label>
                                                    <div className="font-medium text-slate-800 dark:text-slate-200 flex items-center gap-2">
                                                        <Languages className="w-3 h-3 text-slate-400" /> {client.language || 'Français'}
                                                    </div>
                                                </div>
                                                <div>
                                                    <label className="block text-xs text-slate-500 mb-1">Secteur</label>
                                                    <div className="font-medium text-slate-800 dark:text-slate-200 flex items-center gap-2">
                                                        <BriefcaseIcon className="w-3 h-3 text-slate-400" /> {client.sector || 'Non défini'}
                                                    </div>
                                                </div>
                                                <div>
                                                    <label className="block text-xs text-slate-500 mb-1">Segment</label>
                                                    <div className="font-medium text-slate-800 dark:text-slate-200 flex items-center gap-2">
                                                        <Bookmark className="w-3 h-3 text-slate-400" /> {client.segment || 'Standard'}
                                                    </div>
                                                </div>
                                                <div>
                                                    <label className="block text-xs text-slate-500 mb-1">Revendeur</label>
                                                    <div className="font-medium text-slate-800 dark:text-slate-200 flex items-center gap-2">
                                                        <Users className="w-3 h-3 text-slate-400" /> {client.resellerName || 'Global'}
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        {/* Infos Financières */}
                                        <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-700 grid grid-cols-3 gap-4">
                                            <div>
                                                <label className="block text-xs text-slate-500 mb-1">Conditions Paiement</label>
                                                <div className="font-bold text-slate-700 dark:text-slate-300 text-sm">{client.paymentTerms || '30 jours'}</div>
                                            </div>
                                            <div>
                                                <label className="block text-xs text-slate-500 mb-1">Devise</label>
                                                <div className="font-bold text-slate-700 dark:text-slate-300 text-sm">{client.currency || 'XOF'}</div>
                                            </div>
                                            <div>
                                                <label className="block text-xs text-slate-500 mb-1">Encours Autorisé</label>
                                                <div className="font-bold text-slate-700 dark:text-slate-300 text-sm">1 000 000 {client.currency}</div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="col-span-12 lg:col-span-4 space-y-6">
                                        ...
                                        <div className="space-y-3">
                                            {client.contacts && client.contacts.length > 0 ? (
                                                client.contacts.map(contact => (
                                                    <div key={contact.id} className="flex items-center gap-3 p-2 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-lg transition-colors border border-transparent hover:border-slate-100 dark:hover:border-slate-600">
                                                ...
                                                <p className="text-xs text-slate-400 italic text-center py-4">Aucun contact additionnel.</p>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    {/* BLOC FLOTTE */}
                                    <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                                        <h4 className="text-xs font-bold text-slate-400 uppercase mb-3 flex items-center gap-2"><Truck className="w-4 h-4" /> Flotte</h4>
                                        <div className="grid grid-cols-2 gap-3 mb-3">
                                            <div className="p-3 bg-slate-50 dark:bg-slate-900 rounded text-center">
                                    ...
                                    <div className="bg-[var(--primary-dim)] p-4 rounded-xl border border-[var(--border)]">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <button
                                                ...
                                                className="w-full py-2 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-xs font-bold rounded border border-slate-200 dark:border-slate-600 hover:shadow-sm transition-all flex items-center justify-center gap-2"
                                            >
                                    ...
                                    {MOCK_MAILS.map((mail, i) => (
                                        <div key={mail.id} className={`p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/50 group relative cursor-pointer ${i !== MOCK_MAILS.length - 1 ? 'border-b border-slate-100 dark:border-slate-800' : ''}`}>
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <p className="font-bold text-slate-800 dark:text-white text-sm">{mail.subject}</p>
                                                    <p className="text-xs text-slate-500">{mail.date} • {mail.type}</p>
                                                </div>
                                            </div>
                                            <span className="px-2 py-1 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-[10px] font-bold rounded uppercase">{mail.status}</span>

                                            {/* Tooltip Aperçu du Mail au survol */}
                                            <div className="hidden group-hover:block absolute top-full left-12 right-12 z-20 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-xl rounded-lg p-3 mt-1 animate-in fade-in slide-in-from-top-1">
                        ...
                        {activeDetailTab === 'COMMENTS' && (
                            <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-4 duration-300">
                                <div className="flex-1 overflow-y-auto space-y-4 mb-4">
                                    {comments.length === 0 ? (
                                        <div className="text-center text-slate-400 italic py-10">Aucun commentaire. Soyez le premier à en ajouter un.</div>
                                    ) : (
                                        comments.map(com => (
                                                ...
                                                <div className="flex-1">
                                                    <div className="bg-white dark:bg-slate-800 p-3 rounded-lg rounded-tl-none border border-slate-200 dark:border-slate-700 shadow-sm">
                                                        <div className="flex justify-between items-center mb-1">
                                                            <span className="font-bold text-xs text-slate-700 dark:text-slate-200">{com.author}</span>
                                                            <span className="text-[10px] text-slate-400">{com.date}</span>
                                                        </div>
                                                        <p className="text-sm text-slate-600 dark:text-slate-300 whitespace-pre-wrap">{com.text}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        ))
                                ...
                                <div className="relative mt-auto bg-white dark:bg-slate-800 p-2 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm">
                                    <textarea
                                        className="w-full p-2 bg-transparent text-sm resize-none focus:outline-none text-slate-700 dark:text-slate-200 placeholder-slate-400"
                        ...
                        {activeDetailTab === 'STATEMENT' && (
                            <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300 h-full flex flex-col">
                                <div className="flex flex-col sm:flex-row justify-between items-center bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm gap-4">
                                    <div>
                                        <p className="text-xs font-bold text-slate-500 uppercase mb-1">Solde au {new Date().toLocaleDateString('fr-FR')}</p>
                                        <p className="text-3xl font-bold text-slate-800 dark:text-white mt-1">- 45 000 <span className="text-sm font-normal text-slate-500">{currency}</span></p>
                                    </div>
                                    <div className="flex items-center gap-3 w-full sm:w-auto">
                                        ...
                                        <button
                                            ...
                                            className="p-2 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition-colors" title="Envoyer par Email"
                                        >
                                            <Mail className="w-5 h-5" />
                                        </button>
                                        <button
                                ...
                                <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden flex-1 flex flex-col">
                                    <div className="p-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 flex justify-between items-center">
                                        <h4 className="font-bold text-slate-700 dark:text-slate-200 text-sm">Détail des écritures ({statementPeriod === 'THIS_YEAR' ? '2024' : 'Période'})</h4>
                                        <span className="text-xs text-slate-500">{MOCK_STATEMENT_LINES.length} lignes</span>
                                    </div>
                                    <div className="flex-1 overflow-auto custom-scrollbar">
                                        <table className="w-full text-left text-sm">
                                            <thead className="bg-slate-50 dark:bg-slate-900 text-xs text-slate-500 uppercase font-bold sticky top-0 z-10">
                                                <tr>
                                                    <th className="px-4 py-3 border-b dark:border-slate-700">Date</th>
                                                    <th className="px-4 py-3 border-b dark:border-slate-700">Réf</th>
                                                    <th className="px-4 py-3 border-b dark:border-slate-700">Libellé</th>
                                                    <th className="px-4 py-3 border-b dark:border-slate-700 text-right">Débit</th>
                                                    <th className="px-4 py-3 border-b dark:border-slate-700 text-right">Crédit</th>
                                                    <th className="px-4 py-3 border-b dark:border-slate-700 text-right">Solde</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                                {MOCK_STATEMENT_LINES.map((line, i) => (
                                                    <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                                        ...
                                                        <td className={`px-4 py-3 text-right font-bold ${(line.balance ?? 0) < 0 ? 'text-red-600' : 'text-green-600'}`}>{(line.balance ?? 0).toLocaleString('fr-FR')}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
