...
export const CRMView: React.FC<CRMViewProps> = ({ mode = 'LEADS', onNavigate, onCreateQuote, dateRange }) => {
  ...
  // Filter state
  const [filters, setFilters] = useState<{ status: string; plan: string; reseller: string; category: string }>({
    status: 'ALL',
    plan: 'ALL',
    reseller: 'ALL',
    category: 'ALL',
    ...
    try {
      if (mode === 'CLIENTS') {
        ...
        const data = clients.map((c) => ({
          ...
          resellerName: c.resellerName || 'Global',
        }));
        generatePDF('Liste des Clients', columns, data, `clients_${new Date().toISOString().split('T')[0]}.pdf`, {
        ...
        const data = filteredLeadsByDate.map((l) => ({
          ...
          source: l.source || '',
          resellerName: l.resellerName || 'Global',
        }));
        generatePDF('Liste des Leads', columns, data, `leads_${new Date().toISOString().split('T')[0]}.pdf`, {
          orientation: 'landscape',
          branding,
        });
      } else if (mode === 'CATALOG') {
        ...
        generatePDF('Catalogue', columns, data, `catalog_${new Date().toISOString().split('T')[0]}.pdf`, {
          orientation: 'landscape',
          branding,
        });
      }
    ...
    <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-500 sm:h-[calc(100vh-140px)] sm:flex sm:flex-col">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {mode === 'LEADS' && (
            <>
              ...
              <button
                ...
                className="bg-[var(--primary)] hover:bg-[var(--primary-light)] text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-colors shadow-sm"
              >
          ...
          {mode === 'CATALOG' && (
            <button
              ...
              className="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-colors shadow-sm"
            >
      ...
      {selectedLead && (
        <Modal
          ...
          title={`Détail Lead: ${selectedLead.companyName}`}
        >
      ...
      <Modal
        ...
        title={editingCatalogItem?.id ? "Modifier l'article" : 'Ajouter un article'}
        maxWidth="max-w-6xl"
      >
        <CatalogForm
