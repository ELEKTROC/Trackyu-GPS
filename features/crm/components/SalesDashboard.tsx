...
export const SalesDashboard: React.FC<{
...
}> = ({ dateRange: externalDateRange, onNavigate }) => {
  ...
  const stats = useMemo(() => {
    ...
    const avgGrowth =
      last3Months.length >= 2 ? (((last3Months[2] - last3Months[0]) / (last3Months[0] || 1)) * 100) / 2 : 0;
  ...
  const contractStatusData = useMemo(() => {
    const statusCounts = filteredContracts.reduce(
      (acc, c) => {
    ...
    <div className="space-y-6 h-full overflow-y-auto custom-scrollbar p-1">
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          {isMobile && (
            <button
              ...
              className="relative flex items-center gap-1.5 px-3 py-2 bg-[var(--bg-elevated)] border border-[var(--border)] rounded-lg text-sm text-[var(--text-secondary)]"
            >
          ...
          {!isMobile && resellers.length > 1 && (
            <div className="relative">
              ...
              <select
                ...
                className="pl-8 pr-3 py-2 rounded-lg text-sm border border-[var(--border)] bg-[var(--bg-elevated)] text-[var(--text-primary)] appearance-none cursor-pointer"
              >
      ...
      <div className="hidden sm:grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        ...
        <Card className="p-4 border-[var(--border)]">
          ...
          <p className="text-xs text-[var(--text-secondary)] font-medium uppercase">Impayés</p>
          <h3 className="text-xl font-bold text-red-600 mt-1">{(stats.totalUnpaid ?? 0).toLocaleString('fr-FR')}</h3>
          <p className="text-[10px] text-[var(--text-muted)]">Factures en attente</p>
        </Card>
        ...
        <Card className="p-4 border-[var(--border)]">
          ...
          <p className="text-[10px] text-[var(--text-muted)]">Actifs</p>
        </Card>
        ...
        <Card className="p-4 border-l-4 border-l-violet-500 bg-gradient-to-r from-violet-50 to-white dark:from-[var(--bg-elevated)] dark:to-[var(--bg-primary)]">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-[var(--text-secondary)]">{(leads || []).length} leads en cours</p>
            </div>
            <div className="flex gap-1">
              {pipelineData.map((p) => (
                <div key={p.name} className="text-center px-2">
                  ...
                  <p className="text-[9px] text-[var(--text-secondary)] mt-1">{p.name}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-[var(--primary)] mt-1">
                {formatPrice(stats.forecastNextMonth ?? 0)}
              </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 p-6 border-[var(--border)]">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-bold text-[var(--text-primary)]">Évolution du Chiffre d'Affaires</h3>
        ...
        <Card className="p-6 border-[var(--border)]">
          <h3 className="text-lg font-bold text-[var(--text-primary)] mb-4">Répartition Contrats</h3>
          <div className="h-48">
            <ResponsiveContainer
              width="100%"
              height="100%"
          ...
          <p className="text-center text-xs text-[var(--text-secondary)] mt-2">
            {filteredContracts.length} contrats total
          </p>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* UPCOMING RENEWALS */}
        <Card className="p-6 border-[var(--border)] border-l-4 border-l-orange-500">
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-bold text-[var(--text-primary)]">
                        {c.clientName || tiers.find((t) => t.id === c.clientId)?.name || 'Client'}
                      </p>
                      <p className="text-xs text-[var(--text-secondary)]">
                        {c.contractNumber || c.id} - {formatPrice(c.monthlyFee ?? 0)}
                      </p>
                    </div>
                    <button
        ...
        {/* RECENT ACTIVITY */}
        <Card className="p-6 border-[var(--border)]">
          <h3 className="text-lg font-bold text-[var(--text-primary)] mb-4">Activités Récentes</h3>
          <div className="space-y-3 max-h-48 overflow-y-auto custom-scrollbar">
            {invoices.slice(0, 5).map((inv) => (
              <div key={inv.id} className="flex items-center gap-3 p-3 rounded-lg tr-hover/50 transition-colors">
                <div
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-[var(--text-primary)] truncate">Facture {inv.number}</p>
                  <p className="text-xs text-[var(--text-secondary)] truncate">{inv.clientName}</p>
                </div>
