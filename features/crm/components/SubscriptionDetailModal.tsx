...
import { X, Truck, FileText, Calendar, RefreshCw, CreditCard, AlertTriangle, History, ChevronDown, ChevronUp, ExternalLink, Edit2, PauseCircle, XCircle } from 'lucide-react';
...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
  ...
  CANCELED:  { label: 'Résilié', color: 'bg-slate-100 text-slate-700 dark:bg-slate-900/30 dark:text-slate-400' },
  SUSPENDED: { label: 'Suspendu', color: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400' },
  TERMINATED: { label: 'Terminé', color: 'bg-slate-100 text-slate-700 dark:bg-slate-900/30 dark:text-slate-400' },
  DRAFT: { label: 'Brouillon', color: 'bg-slate-100 text-slate-700 dark:bg-slate-900/30 dark:text-slate-400' },
};
...
export const SubscriptionDetailModal: React.FC<SubscriptionDetailModalProps> = ({
  ...
  const handleSuspend = async () => {
    ...
    try { await onSuspend(subscriptionId); onClose(); }
  ...
  const handleRésilier = async () => {
    ...
    setActionLoading(true);
    try { await onRésilier(subscriptionId, résilierReason || undefined); onClose(); }
    finally { setActionLoading(false); setConfirmRésilier(false); }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="relative bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-4xl h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        {/* Header */}
        ...
        {/* Tabs */}
        <div className="flex gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl mx-6">
          {[
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-slate-50 dark:bg-slate-900/50 rounded-lg p-4 space-y-3">
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-500 text-sm">Contrat</span>
                    <button 
                      onClick={() => {
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-500 text-sm">Revendeur</span>
                ...
                <div className="bg-slate-50 dark:bg-slate-900/50 rounded-lg p-4 space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-green-600">
                      {formatPrice(monthlyFee)} <span className="text-slate-400 font-normal text-xs">/ {cycleLabel}</span>
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500 text-sm">Date d'installation</span>
                    <span className="font-medium">{formatDate(installationDate)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500 text-sm">Dernière facturation</span>
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-500 text-sm">Renouvellement auto</span>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-slate-50 dark:bg-slate-900/50 rounded-lg p-4 space-y-3">
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-500 text-sm">Marque / Modèle</span>
                    <span className="font-medium">{vehicle?.brand} {vehicle?.model}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500 text-sm">Plaque</span>
                    <span className="font-mono bg-slate-200 dark:bg-slate-700 px-2 py-0.5 rounded text-sm">
                      {vehicle?.licensePlate || '-'}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500 text-sm">IMEI Balise</span>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-slate-50 dark:bg-slate-900/50 rounded-lg p-4 space-y-3">
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-500 text-sm">Date de fin</span>
                    <span className={`font-medium ${
                      subEndDate && new Date(subEndDate) < new Date() ? 'text-red-600' : ''
                    }`}>
                      {subEndDate ? formatDate(subEndDate) : <span className="text-green-600 text-xs">N'expire jamais</span>}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500 text-sm">Contrat parent</span>
                    <span className="font-medium text-slate-600 dark:text-slate-300">{formatDate(contract?.startDate)} → {contract?.endDate ? formatDate(contract.endDate) : '∞'}</span>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead className="bg-slate-50 dark:bg-slate-900">
                        <tr>
                          <th className="px-4 py-2 text-left text-xs font-bold text-slate-500 uppercase">N° Facture</th>
                          <th className="px-4 py-2 text-left text-xs font-bold text-slate-500 uppercase">Date</th>
                          <th className="px-4 py-2 text-right text-xs font-bold text-slate-500 uppercase">Montant</th>
                          <th className="px-4 py-2 text-center text-xs font-bold text-slate-500 uppercase">Statut</th>
                          <th className="px-4 py-2 text-center text-xs font-bold text-slate-500 uppercase">Actions</th>
                        </tr>
                      </thead>
                    ...
                    <div key={index} className="relative pl-10 pb-4">
                      ...
                      <div className="bg-slate-50 dark:bg-slate-900/50 rounded-lg p-3">
                        <div className="flex justify-between items-start">
                          <p className="text-sm font-medium text-slate-800 dark:text-white">{event.description}</p>
        ...
        {confirmRésilier && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/40 rounded-xl">
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl p-6 w-full max-w-sm mx-4">
              ...
              <p className="text-sm text-slate-500 mb-4">Cette action est irréversible. L'abonnement sera résilié immédiatement.</p>
              <textarea
                value={résilierReason}
                onChange={e => setRésilierReason(e.target.value)}
                placeholder="Motif de résiliation (optionnel)"
                rows={3}
                className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 bg-white dark:bg-slate-700 text-sm resize-none mb-4"
              />
              <div className="flex gap-2">
                <button
                  onClick={() => setConfirmRésilier(false)}
                  className="flex-1 px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg text-sm hover:bg-slate-50 dark:hover:bg-slate-700"
                >
                  Annuler
                </button>
                <button
        ...
        <div className="flex justify-between items-center px-6 py-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={onClose}
              className="px-3 py-2 text-sm border border-slate-300 dark:border-slate-600 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700"
            >
              Fermer
            </button>
          </div>
