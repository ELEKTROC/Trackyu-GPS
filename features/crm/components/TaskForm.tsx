      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <FormField label="Rappel avant échéance">
          <Select
            ...
            onChange={(e) => {
              setFormData({ ...formData, reminder: e.target.value as Task['reminder'] });
              setHasChanges(true);
            }}
          >
                  ...
                  onChange={(e) => {
                    ...
                    setFormData({
                      ...
                      relatedTo: {
                        type: 'QUOTE',
                        id: e.target.value,
                        name: quote?.number || `Devis #${e.target.value.slice(0, 8)}`,
                      },
                    });
                    setHasChanges(true);
                  }}
        ...
        <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-[var(--border)]">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2.5 text-[var(--text-secondary)] hover:bg-[var(--bg-elevated)] rounded-lg text-[var(--text-secondary)] hover:bg-[var(--bg-elevated)] font-medium transition-colors"
          >
            Annuler
          </button>
          <button
