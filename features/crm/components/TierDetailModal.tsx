...
import { 
  Users, FileText, DollarSign, Briefcase, CheckCircle, 
  Plus, Calendar, Building2, Mail, Phone, X, 
  Truck, Loader2, Search,
  LayoutGrid, Tag, ChevronLeft, ChevronRight,
  Edit2, MapPin, CreditCard, MessageSquare, FileSpreadsheet, Send, Clock, AlertCircle, Globe2,
  Languages, Briefcase as BriefcaseIcon, Bookmark, AlertTriangle, BarChart3,
  Receipt, FileCheck, Wrench, LifeBuoy, Book, Download, Trash2, Settings,
  Car, Eye, WifiOff, Activity, CircleDot, RefreshCw, History, PhoneIncoming, StickyNote
} from 'lucide-react';
import { 
...
export const TierDetailModal: React.FC<TierDetailModalProps> = ({
    tier, isOpen, onClose, onEdit, onQuickAction
}) => {
    const { showToast } = useToast();
    const { formatPrice, currency } = useCurrency();
    const { invoices, quotes, contracts, interventions, tickets, deleteTier, updateTier, payments, vehicles } = useDataContext();
    ...
    const [comments, setComments] = useState<{id: string; date: string; author: string; text: string; type: 'note' | 'appel' | 'email' | 'sms' | 'système'}[]>([]);
    ...
    useEffect(() => {
        if (isOpen && tier) {
            ...
            setTrxData({
                ...
                PAYMENTS: tierPayments.length > 0 ? tierPayments : tierInvoices.filter((i: Invoice) => i.status === 'PAID' || i.status === 'PAYÉ').map((i: Invoice) => ({ id: `PMT-${i.id}`, date: i.paidDate || i.date, method: 'Virement', amount: i.amount, ref: i.id })),
                QUOTES: tierQuotes,
    ...
    const handleStatusChange = async () => {
        ...
        try {
            ...
            showToast(TOAST.FINANCE.STATUS_CHANGED(newStatus === 'ACTIVE' ? 'Actif' : newStatus === 'INACTIVE' ? 'Inactif' : 'Suspendu'), 'success');
    ...
    const getInvoiceStatusLabel = (status: string): string => {
        const map: Record<string, string> = {
            'DRAFT': 'Brouillon', 'SENT': 'Envoyée', 'PAID': 'Payée', 'PAYÉ': 'Payée',
            'PARTIALLY_PAID': 'Partiel', 'PARTIAL': 'Partiel', 'OVERDUE': 'En retard', 'RETARD': 'En retard',
    ...
    const getInvoiceStatusStyle = (status: string): string => {
        switch (status?.toUpperCase()) {
            ...
            case 'CANCELLED': return 'bg-slate-100 text-slate-500 border-slate-200 dark:bg-slate-800 dark:text-slate-500 dark:border-slate-700 line-through';
            default: return 'bg-orange-50 text-orange-700 border-orange-200 dark:bg-orange-900/30 dark:text-orange-400 dark:border-orange-800';
        }
    };
            ...
            switch(activeTrxSubTab) {
                            ...
                            <tr>
                              <th colSpan={7} className="p-0">
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 p-4 bg-gradient-to-r from-slate-50 to-white dark:from-slate-900 dark:to-slate-800 border-b border-slate-200 dark:border-slate-700">
                                    ...
                                    <div className={`bg-white dark:bg-slate-800 p-3 rounded-lg border shadow-sm ${totalBalance > 0 ? 'border-red-100 dark:border-red-800' : 'border-slate-100 dark:border-slate-700'}`}>
                                        <p className="text-[10px] font-bold text-slate-400 uppercase mb-1 flex items-center gap-1">
                                            Solde restant {overdueCount > 0 && <span className="text-red-500">({overdueCount} en retard)</span>}
                                        </p>
                                        <p className={`text-lg font-bold ${totalBalance > 0 ? 'text-red-600' : 'text-slate-800 dark:text-white'}`}>{formatPrice(totalBalance)}</p>
                                    </div>
                                </div>
                              </th>
                            </tr>
                              ...
                              {paginatedData.map((inv: Invoice) => {
                                  ...
                                  return (
                                      <tr key={inv.id} className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors ${isOverdue ? 'bg-red-50/30 dark:bg-red-900/10' : ''}`}>
                                          ...
                                          <td className="py-3 px-4 text-right text-slate-600 dark:text-slate-300 whitespace-nowrap">{formatPrice(Number(inv.amountHT || 0))}</td>
                                          <td className="py-3 px-4 text-right font-bold text-slate-800 dark:text-white whitespace-nowrap">{formatPrice(Number(inv.amount || 0))}</td>
                                          <td className="py-3 px-4 text-center">
                                              <span className={`inline-flex items-center gap-1 px-2 py-1 rounded text-[10px] font-bold border ${getInvoiceStatusStyle(effectiveStatus)}`}>
                    ...
                    return (
                        <>
                          <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 text-xs text-slate-500 uppercase">
                              <tr><th>Réf</th><th>Date</th><th>Méthode</th><th className="text-right">Montant</th></tr>
                          </thead>
                          <tbody className="text-sm divide-y divide-slate-100 dark:divide-slate-800">
                ...
                case 'QUOTES':
                    return (
                        <>
                          <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 text-xs text-slate-500 uppercase">
                              <tr><th className="py-3 px-4">Numéro</th><th className="py-3 px-4">Date</th><th className="py-3 px-4">Client</th><th className="py-3 px-4 text-right">Montant</th><th className="py-3 px-4 text-right">Statut</th></tr>
                          </thead>
                          <tbody className="text-sm divide-y divide-slate-100 dark:divide-slate-800">
                ...
                case 'CONTRACTS':
                    return (
                        <>
                          <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 text-xs text-slate-500 uppercase">
                              <tr><th className="py-3 px-4">N° Contrat</th><th className="py-3 px-4">Début</th><th className="py-3 px-4">Fin</th><th className="py-3 px-4">Cycle</th><th className="py-3 px-4 text-right">Mensualité</th><th className="py-3 px-4 text-right">Statut</th></tr>
                          </thead>
                          <tbody className="text-sm divide-y divide-slate-100 dark:divide-slate-800">
                              {paginatedData.map((c: Contract) => {
                                  const cStatusMap: Record<string, string> = { 'ACTIVE': 'Actif', 'EXPIRED': 'Expiré', 'CANCELLED': 'Annulé', 'DRAFT': 'Brouillon', 'SUSPENDED': 'Suspendu' };
                                  ...
                                  <tr key={c.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                      ...
                                      <td className="py-3 px-4 text-right"><span className={`px-2 py-1 rounded text-[10px] font-bold border ${c.status === 'ACTIVE' ? 'bg-green-100 text-green-700 border-green-200' : c.status === 'EXPIRED' ? 'bg-red-100 text-red-700 border-red-200' : 'bg-orange-100 text-orange-700 border-orange-200'}`}>{cStatusMap[c.status?.toUpperCase()] || c.status}</span></td>
                                  </tr>
                                  );
                              })}
                          </tbody>
                ...
                case 'INTERVENTIONS':
                    return (
                        <>
                          <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 text-xs text-slate-500 uppercase">
                              <tr><th className="py-3 px-4">ID</th><th className="py-3 px-4">Type</th><th className="py-3 px-4">Date</th><th className="py-3 px-4">Technicien</th><th className="py-3 px-4 text-right">Statut</th></tr>
                          </thead>
                          <tbody className="text-sm divide-y divide-slate-100 dark:divide-slate-800">
                              {paginatedData.map((itv: Intervention) => {
                                  const itvStatusMap: Record<string, string> = { 'COMPLETED': 'Terminée', 'IN_PROGRESS': 'En cours', 'PLANNED': 'Planifiée', 'CANCELLED': 'Annulée', 'PENDING': 'En attente' };
                ...
                case 'TICKETS':
                    return (
                        <>
                          <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 text-xs text-slate-500 uppercase">
                              <tr><th className="py-3 px-4">ID</th><th className="py-3 px-4">Sujet</th><th className="py-3 px-4">Priorité</th><th className="py-3 px-4 text-right">Statut</th></tr>
                          </thead>
                          <tbody className="text-sm divide-y divide-slate-100 dark:divide-slate-800">
                              {paginatedData.map((t: Ticket) => {
                                  const tStatusMap: Record<string, string> = { 'OPEN': 'Ouvert', 'IN_PROGRESS': 'En cours', 'RESOLVED': 'Résolu', 'CLOSED': 'Fermé', 'PENDING': 'En attente' };
                ...
                case 'JOURNALS':
                    return (
                        <>
                          <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 text-xs text-slate-500 uppercase">
                              <tr><th className="py-3 px-4" colSpan={5}>Journaux comptables</th></tr>
                          </thead>
                          <tbody><tr><td colSpan={5} className="py-8 text-center text-sm text-slate-400">Module en cours de développement</td></tr></tbody>
                        </>
                    );
                // Generic fallback
                default:
                    return (
                        <>
                          <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 text-xs text-slate-500 uppercase">
                              <tr>
                                  {Object.keys(paginatedData[0] || {}).slice(0, 5).map(k => <th key={k} className="py-3 px-4 capitalize">{k}</th>)}
                              </tr>
                          </thead>
                          <tbody className="text-sm divide-y divide-slate-100 dark:divide-slate-800">
                              {paginatedData.map((item: any, i: number) => (
                                  <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                      {Object.values(item).slice(0, 5).map((val: any, idx) => (
                                          <td key={idx} className="py-3 px-4 text-slate-600 dark:text-slate-300">{String(val ?? '')}</td>
                                      ))}
                                  </tr>
                              ))}
                          </tbody>
                ...
                <div className="flex-1 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden flex flex-col">
                    ...
                    {totalPages > 1 && (
                        <div className="p-3 border-t border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
                            ...
                            <span className="text-xs font-bold text-slate-500">Page {trxPage} / {totalPages}</span>
                            <button 
            ...
            <div className="relative w-full max-w-6xl h-[90vh] bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-2xl flex overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-slate-200 dark:border-slate-700">
                ...
                <div className="w-64 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 flex flex-col z-10">
                    <div className="p-6 border-b border-slate-100 dark:border-slate-700">
                        <div className={`w-16 h-16 rounded-2xl flex items-center justify-center text-white text-2xl font-bold shadow-lg mb-4 ${
                            tier.type === 'CLIENT' ? 'bg-gradient-to-br from-blue-500 to-indigo-600' :
                            tier.type === 'RESELLER' ? 'bg-gradient-to-br from-purple-500 to-pink-600' :
                        ...
                        <div className="mt-4 flex flex-wrap gap-2">
                            <button
                                onClick={() => { setNewStatus(tier.status === 'ACTIVE' ? 'SUSPENDED' : 'ACTIVE'); setShowStatusModal(true); }}
                            ...
                            >
                                ...
                                {tier.status === 'ACTIVE' ? 'Actif' : tier.status === 'INACTIVE' ? 'Inactif' : tier.status === 'SUSPENDED' ? 'Suspendu' : tier.status}
                            </button>
                            ...
                            {tier.type === 'CLIENT' && (
                                <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase border flex items-center gap-1 ${
                                    ...
                                    {(tier.clientData?.balance || 0) < 0 ? <AlertCircle className="w-3 h-3"/> : <CheckCircle className="w-3 h-3"/>}
                            ...
                            {tier.type === 'SUPPLIER' && (
                                <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase border flex items-center gap-1 ${
                                    ...
                                    {(tier.supplierData?.balance || 0) < 0 ? <AlertTriangle className="w-3 h-3"/> : <CheckCircle className="w-3 h-3"/>}
                    ...
                    <div className="p-4 border-t border-slate-100 dark:border-slate-700">
                        <button 
                            onClick={async () => {
                                if(await confirm({ message: 'Êtes-vous sûr de vouloir supprimer ce tiers ? Cette action est irréversible.', variant: 'danger', title: 'Confirmer la suppression', confirmLabel: 'Supprimer' })) {
                                    try {
                ...
                <div className="flex-1 flex flex-col min-w-0 bg-slate-50/50 dark:bg-slate-900/50">
                    ...
                    <div className="h-16 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between px-6 shrink-0">
                        <div className="flex items-center gap-2 text-sm text-slate-500">
                            ...
                            <ChevronRight className="w-4 h-4"/>
                            <span className="font-bold text-slate-800 dark:text-white">{getTabs().find(t => t.id === activeDetailTab)?.label}</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                            ...
                            <button onClick={() => handleQuickAction('intervention')} className="p-2 hover:bg-orange-50 dark:hover:bg-orange-900/20 rounded-lg text-slate-400 hover:text-orange-600 transition-colors" title="Créer une intervention">
                                <Wrench className="w-4 h-4"/>
                            </button>
                            <button onClick={() => handleQuickAction('paiement')} className="p-2 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-lg text-slate-400 hover:text-green-600 transition-colors" title="Enregistrer un paiement">
                                <CreditCard className="w-4 h-4"/>
                            </button>
                            <button onClick={() => handleQuickAction('mail')} className="p-2 hover:bg-sky-50 dark:hover:bg-sky-900/20 rounded-lg text-slate-400 hover:text-sky-600 transition-colors" title="Envoyer un e-mail">
                                <Mail className="w-4 h-4"/>
                            </button>
                            <div className="w-px h-6 bg-slate-200 dark:bg-slate-700 mx-1"/>
                            <button onClick={onClose} className="p-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg text-slate-400 hover:text-red-500 transition-colors" title="Fermer">
                                <X className="w-5 h-5"/>
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                                ...
                                <div className="col-span-12 lg:col-span-8 space-y-6">
                                    ...
                                    <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden">
                                        ...
                                        <div className="relative z-10">
                                            <div className="flex items-start justify-between mb-6">
                                                <div>
                                                    <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-1">Informations Générales</h3>
                                                    <p className="text-sm text-slate-500">Données administratives et légales</p>
                                                </div>
                                                <button 
                                                    onClick={() => onEdit ? onEdit(tier) : showToast(TOAST.CRM.FEATURE_COMING_SOON('Modification'), 'info')}
                                            ...
                                            <div className="grid grid-cols-2 gap-4 text-sm mt-6">
                                                <div className="col-span-2">
                                                    ...
                                                    <div className="font-medium text-slate-800 dark:text-slate-200 flex items-start gap-2">
                                                        ...
                                                        <span>
                                                            {tier.address || 'Non renseignée'}<br/>
                                                ...
                                                <div>
                                                    ...
                                                    <div className="font-medium text-slate-800 dark:text-slate-200 flex items-center gap-2">
                                                        <Calendar className="w-3 h-3 text-slate-400"/> {new Date(tier.createdAt).toLocaleDateString('fr-FR')}
                                                    </div>
                                                </div>
                                                <div>
                                                ...
                                                {tier.type === 'CLIENT' && (
                                                    <>
                                                        <div>
                                                            ...
                                                            <div className="font-medium text-slate-800 dark:text-slate-200 flex items-center gap-2">
                                                                <Bookmark className="w-3 h-3 text-slate-400"/> {tier.clientData?.subscriptionPlan || 'Standard'}
                                                            </div>
                                                        </div>
                                                        <div>
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="font-medium text-slate-800 dark:text-slate-200 flex items-center gap-2">
                                                                <Languages className="w-3 h-3 text-slate-400"/> {tier.clientData?.language || 'Français'}
                                                            </div>
                                                        </div>
                                                        <div>
                                                        ...
                                                        <div className="col-span-2 mt-2 pt-2 border-t border-slate-100 dark:border-slate-700 grid grid-cols-2 gap-4">
                                                            ...
                                                            <div>
                                                                ...
                                                                <div className="font-medium text-slate-800 dark:text-slate-200 flex items-center gap-2">
                                                                    <Clock className="w-3 h-3 text-slate-400"/> {tier.clientData?.paymentTerms || '30 jours'}
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </>
                                    ...
                                    <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                                        ...
                                        <div className="h-[250px] w-full" style={{ minHeight: 240, minWidth: 200 }}>
                                            <ResponsiveContainer width="100%" height="100%" minHeight={240} minWidth={200} initialDimension={{ width: 200, height: 240 }}>
                                                <BarChart data={(() => {
                                                    const MONTHS = ['Jan','Fév','Mar','Avr','Mai','Juin','Juil','Aoû','Sep','Oct','Nov','Déc'];
                                                    const tierInvs = invoices.filter(inv => inv.clientId === tier.id || inv.clientId === tier.name);
                                                    const currentYear = chartPeriod === 'THIS_YEAR' ? new Date().getFullYear() : new Date().getFullYear() - 1;
                                    ...
                                    <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                                        ...
                                        <div className="space-y-3">
                                            <div className="p-2 bg-slate-50 dark:bg-slate-900 rounded border border-slate-100 dark:border-slate-700 flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="text-sm font-bold text-slate-700 dark:text-slate-200">{tier.contactName || tier.name}</p>
                                            ...
                                            {tier.secondContactName && (
                                                <div className="p-2 bg-slate-50 dark:bg-slate-900 rounded border border-slate-100 dark:border-slate-700 flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <p className="text-sm font-bold text-slate-700 dark:text-slate-200">{tier.secondContactName}</p>
                        ...
                        {activeDetailTab === 'VEHICLES' && tier.type === 'CLIENT' && (() => {
                            const tierVehicles = vehicles.filter(v => v.clientId === tier.id);
                            // Calculer les infos de contrat/abonnement pour chaque véhicule
                            const getVehicleContract = (v: Vehicle) => contracts.find(c => c.id === v.contractId && c.clientId === tier.id);
                            const getVehicleInvoiceStatus = (v: Vehicle) => {
                                const vInvoices = invoices.filter(inv => (inv.clientId === tier.id) && (inv.status === 'OVERDUE' || inv.status === 'DRAFT'));
                                ...
                                <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                                    <div className="p-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 flex justify-between items-center">
                                        ...
                                        <div className="flex items-center gap-3 text-xs">
                                            <span className="flex items-center gap-1 text-green-600"><CircleDot className="w-3 h-3"/> {tierVehicles.filter(v => v.status === VehicleStatus.MOVING).length} en mouvement</span>
                                            <span className="flex items-center gap-1 text-slate-400"><WifiOff className="w-3 h-3"/> {tierVehicles.filter(v => v.status === VehicleStatus.OFFLINE).length} hors ligne</span>
                                        </div>
                                    </div>
                                                    ...
                                                    {tierVehicles.map((vehicle: Vehicle) => {
                                                        ...
                                                        return (
                                                            <tr key={vehicle.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                                                <td className="py-3 px-4">
                                                                    <div className="flex items-center gap-2">
                                                                        <div className={`w-2 h-2 rounded-full shrink-0 ${
                                                                            vehicle.status === VehicleStatus.MOVING ? 'bg-green-500 animate-pulse' : 
                                                                            vehicle.status === VehicleStatus.IDLE ? 'bg-yellow-500' : 
                                                                            vehicle.status === VehicleStatus.STOPPED ? 'bg-orange-500' : 'bg-slate-300'
                                                                        }`}/>
                                                                ...
                                                                <td className="py-3 px-4 font-mono text-xs">{vehicle.plate || vehicle.licensePlate || '-'}</td>
                                                                <td className="py-3 px-4 text-xs">
                                                                    {vehicle.installDate ? (
                                                                        <span className="flex items-center gap-1"><Calendar className="w-3 h-3 text-slate-400"/> {new Date(vehicle.installDate).toLocaleDateString('fr-FR')}</span>
                                                                ...
                                                                <td className="py-3 px-4">
                                                                    <span className={`text-xs px-2 py-1 rounded font-bold ${
                                                                        vehicle.status === VehicleStatus.MOVING ? 'bg-green-100 text-green-700' :
                                                                        vehicle.status === VehicleStatus.IDLE ? 'bg-yellow-100 text-yellow-700' :
                                                                        vehicle.status === VehicleStatus.STOPPED ? 'bg-orange-100 text-orange-700' :
                                                                        'bg-slate-100 text-slate-600'
                                                                    }`}>
                                                                        {vehicle.status === VehicleStatus.MOVING ? 'En mouvement' : 
                                                                         vehicle.status === VehicleStatus.IDLE ? 'Au ralenti' : 
                                                                         vehicle.status === VehicleStatus.STOPPED ? 'Arrêté' : 'Hors ligne'}
                                                                    </span>
                                                                </td>
                                ...
                                <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                                    ...
                                    {auditLoading ? (
                                        ...
                                        <div className="p-8 text-center text-slate-400">
                                            ...
                                            <p className="text-xs mt-1">Les notes, appels, emails et actions utilisateur seront affichés ici</p>
                                        </div>
                                    ) : (
                                        <div className="divide-y divide-slate-100 dark:divide-slate-700 max-h-[500px] overflow-y-auto custom-scrollbar">
                                            ...
                                            {(() => {
                                                const auditActionConfig: Record<string, { icon: React.ElementType; color: string; label: string }> = {
                                                    ...
                                                    'PAYMENT_RECEIVED': { icon: CreditCard, color: 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400', label: 'Paiement reçu' },
                                                ...
                                                const commentTypeConfig: Record<string, { icon: React.ElementType; color: string; label: string }> = {
                                                    ...
                                                    système: { icon: Activity, color: 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400', label: 'Système' },
                                                };
                                                
                                                // Normalize all entries to a common format
                                                const allEntries: { id: string; date: string; sortDate: number; author: string; text: string; type: string; source: 'audit' | 'comment'; details?: any }[] = [];
                                                ...
                                                auditLogs.forEach((log: AuditLog) => {
                                                    ...
                                                    if (details?.description) {
                                                        ...
                                                        const changes = Array.isArray(details.changes) ? details.changes : Object.keys(details.changes).map(k => `${k}: ${details.changes[k]}`);
                                                    ...
                                                    allEntries.push({
                                                        ...
                                                        date: d.toLocaleString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }),
                                                        sortDate: d.getTime(),
                                                ...
                                                return allEntries.map(entry => {
                                                    const config = entry.source === 'audit' 
                                                        ? (auditActionConfig[entry.type] || { icon: Activity, color: 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400', label: entry.type })
                                                        : (commentTypeConfig[entry.type] || { icon: StickyNote, color: 'bg-slate-100 text-slate-500', label: 'Note' });
                                                    const EntryIcon = config.icon;
                                                    return (
                                                        <div key={entry.id} className="p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors flex gap-3">
                                                            ...
                                                            <div className="flex-1 min-w-0">
                                                                <div className="flex justify-between items-start mb-1">
                                                                    <div className="flex items-center gap-2 flex-wrap">
                                                                        <span className="font-bold text-sm text-slate-800 dark:text-white">{entry.author}</span>
                                                                        <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold uppercase ${config.color}`}>{config.label}</span>
                                                                        {entry.source === 'audit' && (
                                                                            <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-700 text-slate-500 font-mono">auto</span>
                                                                        )}
                                                                    </div>
                                                                    <span className="text-xs text-slate-400 flex items-center gap-1 flex-shrink-0 ml-2"><Clock className="w-3 h-3"/> {entry.date}</span>
                                                                </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md border border-slate-200 dark:border-slate-700 animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="p-6 space-y-4">
                            <div>
                                ...
                                <div className="flex gap-2">
                                    {[
                                        { value: 'ACTIVE', label: 'Actif', color: 'bg-emerald-50 text-emerald-700 border-emerald-300 ring-emerald-500' },
                                        { value: 'INACTIVE', label: 'Inactif', color: 'bg-slate-50 text-slate-700 border-slate-300 ring-slate-500' },
                                        { value: 'SUSPENDED', label: 'Suspendu', color: 'bg-amber-50 text-amber-700 border-amber-300 ring-amber-500' }
                                    ].filter(s => s.value !== tier.status).map(s => (
                                        <button
                                            key={s.value}
                                            onClick={() => setNewStatus(s.value)}
                                            className={`flex-1 px-3 py-2.5 rounded-lg text-sm font-bold border-2 transition-all ${
                                                newStatus === s.value ? `${s.color} ring-2` : 'bg-white dark:bg-slate-800 text-slate-500 border-slate-200 dark:border-slate-700 hover:border-slate-300'
                                            }`}
                                        >
                                            {s.label}
                                        </button>
                            ...
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Motif du changement <span className="text-red-500">*</span></label>
                                <textarea
                                    value={statusMotif}
                        ...
                        <div className="p-4 border-t border-slate-100 dark:border-slate-700 flex justify-end gap-2">
                            <button
                                onClick={() => { setShowStatusModal(false); setStatusMotif(''); setNewStatus(''); }}
