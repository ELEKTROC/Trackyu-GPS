...
import {
  FileText, Truck,
  CheckCircle, AlertCircle, PauseCircle, PlayCircle, StopCircle,
  Download, Receipt, User, MapPin, FileCheck,
  TrendingUp, AlertTriangle, Edit2,
  Calendar, RefreshCw, FileSignature
} from 'lucide-react';
...
const STATUS_LABELS: Record<string, string> = {
  DRAFT: 'Brouillon', ACTIVE: 'Actif', SUSPENDED: 'Suspendu', EXPIRED: 'Expiré', TERMINATED: 'Résilié',
};
const BILLING_CYCLE_LABELS: Record<string, string> = {
  MONTHLY: 'Mensuel', QUARTERLY: 'Trimestriel', SEMESTRIAL: 'Semestriel', ANNUAL: 'Annuel', YEARLY: 'Annuel',
  monthly: 'Mensuel', quarterly: 'Trimestriel', semi_annual: 'Semestriel', yearly: 'Annuel',
};
...
export const ContractDetailModal: React.FC<ContractDetailModalProps> = ({
    contract, isOpen, onClose, client, invoices, vehicles, onStatusChange, onEdit
}) => {
    ...
    useEffect(() => {
        ...
        api.subscriptions.list()
            .then((subs: any[]) => setContractSubs(subs.filter((s: any) => s.contract_id === contract.id || s.contractId === contract.id)))
            .catch(() => {})
    ...
    const averagePaymentDelay = paidInvoicesWithDate.length > 0
        ? Math.round(paidInvoicesWithDate.reduce((sum, inv) => {
    ...
    const PaginationControls = ({ total, current, onPageChange }: { total: number, current: number, onPageChange: (page: number) => void }) => (
        <Pagination
            currentPage={current}
    ...
    const getStatusIcon = (status: string) => {
        switch (status) {
            ...
            case 'SUSPENDED': return <PauseCircle className="w-4 h-4" />;
            case 'TERMINATED': return <StopCircle className="w-4 h-4" />;
    ...
    const handleDownloadContract = () => {
        try {
            ...
            if (companyPhone || companyEmail) doc.text([companyPhone, companyEmail].filter(Boolean).join('  |  '), 18, y + 20);
            ...
            if (clientPhone || clientEmail) doc.text([clientPhone, clientEmail].filter(Boolean).join('  |  '), clientX + 4, y + 20);
            ...
            const subRows = contractSubs.map(s => {
                ...
                const statusLabel = status === 'ACTIVE' ? 'Actif' : status === 'SUSPENDED' ? 'Suspendu' : status === 'CANCELLED' || status === 'CANCELED' ? 'Résilié' : status;
                return [plate, vehicle, cycle, `${fee.toLocaleString('fr-FR', { minimumFractionDigits: 0 })} FCFA`, statusLabel];
            });
            ...
            autoTable(doc, {
                ...
                body: subRows,
                foot: [['', '', 'TOTAL', `${subsTotal.toLocaleString('fr-FR', { minimumFractionDigits: 0 })} FCFA`, `${contractSubs.filter(s => (s.status||'').toUpperCase()==='ACTIVE').length} actif(s)`]],
                theme: 'grid',
            ...
            // ── Legal articles ───────────────────────────────────────────────────
            const articles = [
                { title: 'Article 1 – Objet du contrat', body: `Le présent contrat a pour objet la fourniture par ${companyName} d'un service de géolocalisation et de suivi GPS de véhicules au profit du CLIENT, selon les modalités définies aux présentes.` },
                { title: 'Article 2 – Durée', body: `Le contrat prend effet à la date de début indiquée ci-dessus. Sauf résiliation dans les conditions prévues à l'article 5, il est reconduit tacitement pour des périodes identiques sauf mention contraire.` },
                { title: 'Article 3 – Obligations du Prestataire', body: `${companyName} s'engage à assurer la disponibilité de la plateforme, à fournir l'assistance technique et à notifier le CLIENT en cas d'interruption planifiée.` },
                { title: 'Article 4 – Obligations du Client', body: `Le CLIENT s'engage à utiliser les services conformément aux présentes, à régler les factures dans les délais convenus et à signaler toute anomalie dans les meilleurs délais.` },
                { title: 'Article 5 – Résiliation', body: `Chaque partie peut résilier le contrat par lettre recommandée avec accusé de réception en respectant un préavis de 30 jours. En cas de manquement grave non corrigé sous 15 jours après mise en demeure, la résiliation peut être immédiate.` },
                { title: 'Article 6 – Confidentialité', body: `Les parties s'engagent à garder confidentielles toutes informations échangées dans le cadre du présent contrat, pendant toute sa durée et 3 ans après son terme.` },
            ...
            // ── Footer on each page ──────────────────────────────────────────────
            const pageCount = doc.getNumberOfPages?.() ?? (doc.internal as typeof doc.internal & { getNumberOfPages?: () => number }).getNumberOfPages?.() ?? 1;
            for (let p = 1; p <= pageCount; p++) {
                doc.setPage(p);
                doc.setFontSize(7);
                doc.setTextColor(148, 163, 184);
                doc.setFont('helvetica', 'normal');
                doc.text(`${companyName}  •  Contrat ${contractRef}  •  Page ${p}/${pageCount}`, pageW / 2, doc.internal.pageSize.getHeight() - 6, { align: 'center' });
            }
    ...
    const handleDownloadInvoice = async (invoice: Invoice) => {
        try {
            ...
            const data: InvoiceData = {
                ...
                client: { name: client?.name || invoice.clientId || '', address: client?.address || '', city: client?.city || '', email: client?.email || '', phone: client?.phone || '' },
                items, subtotal, taxRate, taxAmount, total, status: invoice.status,
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Contrat : ${contract.contractNumber || contract.id.slice(0, 8).toUpperCase()}`} maxWidth="max-w-6xl">
            <div className="flex flex-col md:flex-row h-[80vh] overflow-hidden bg-slate-50 dark:bg-slate-900">
                ...
                <div className="w-full md:w-72 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 p-5 flex flex-col gap-5 overflow-y-auto shrink-0">
                    ...
                    <div className="text-center relative">
                        ...
                        <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border ${getStatusColor(contract.status)}`}>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        ...
                        <div className="p-2.5 bg-slate-50 dark:bg-slate-900 rounded-lg border border-slate-100 dark:border-slate-700 text-center">
                            ...
                            <p className="font-mono font-bold text-green-700 dark:text-green-400">{contractSubs.filter(s => (s.status || '').toUpperCase() === 'ACTIVE').length}</p>
                        </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-2.5">
                            ...
                            <div>
                                <p className="text-sm font-bold text-slate-700 dark:text-slate-200">{client?.name || contract.clientId}</p>
                                <p className="text-xs text-slate-500">{client?.contactName || ''}</p>
                            </div>
                        </div>
                        {client?.phone && <div className="flex items-center gap-2 text-xs text-slate-500"><User className="w-3 h-3" /> {client.phone}</div>}
                        {client?.address && <div className="flex items-center gap-2 text-xs text-slate-500"><MapPin className="w-3 h-3" /> {client.address}</div>}
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="space-y-1.5 text-sm">
                            <div className="flex justify-between"><span className="text-slate-500 text-xs">Début</span><span className="font-medium text-xs">{new Date(contract.startDate).toLocaleDateString('fr-FR')}</span></div>
                            <div className="flex justify-between"><span className="text-slate-500 text-xs">Fin</span><span className="font-medium text-xs">{contract.endDate ? new Date(contract.endDate).toLocaleDateString('fr-FR') : 'Indéterminée'}</span></div>
                            <div className="flex justify-between"><span className="text-slate-500 text-xs">Renouv. Auto</span><span className="font-medium text-xs">{contract.autoRenew ? 'Oui' : 'Non'}</span></div>
                        </div>
                    </div>
                    ...
                    <div className="p-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl border border-indigo-100 dark:border-indigo-800 text-center">
                        ...
                        <p className="text-xl font-bold text-indigo-700 dark:text-indigo-300">{averagePaymentDelay} <span className="text-xs font-normal">jours</span></p>
                    </div>

                    {/* ACTIONS */}
                    <div className="mt-auto space-y-2 pt-3 border-t border-slate-200 dark:border-slate-700">
                        {contract.status === 'ACTIVE' && (
                            <button onClick={() => onStatusChange(contract, 'SUSPENDED')} className="w-full py-2 px-3 bg-orange-50 text-orange-700 hover:bg-orange-100 rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition-colors">
                                <PauseCircle className="w-3.5 h-3.5" /> Suspendre
                            </button>
                        )}
                        {contract.status === 'SUSPENDED' && (
                            <button onClick={() => onStatusChange(contract, 'ACTIVE')} className="w-full py-2 px-3 bg-green-50 text-green-700 hover:bg-green-100 rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition-colors">
                                <PlayCircle className="w-3.5 h-3.5" /> Reprendre
                            </button>
                        )}
                        {(contract.status === 'ACTIVE' || contract.status === 'SUSPENDED') && (
                            <button onClick={() => onStatusChange(contract, 'TERMINATED')} className="w-full py-2 px-3 bg-red-50 text-red-700 hover:bg-red-100 rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition-colors">
                                                    ...
                                                    {contractSubs.slice(0, 5).map((s: any) => {
                                                        ...
                                                        return (
                                                            <tr key={s.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                                                <td className="px-3 py-2 font-mono font-bold text-slate-800 dark:text-white text-xs">{plate}</td>
                                                                <td className="px-3 py-2 text-xs text-slate-600 dark:text-slate-400">{[brand, model].filter(Boolean).join(' ') || '—'}</td>
                                                                <td className="px-3 py-2 text-xs">{BILLING_CYCLE_LABELS[(s.billing_cycle || s.billingCycle || '').toUpperCase()] || '—'}</td>
                                                                <td className="px-3 py-2 text-right text-xs font-bold">{formatPrice(parseFloat(s.monthly_fee ?? s.monthlyFee ?? 0))}</td>
                                                                <td className="px-3 py-2 text-center">
                                                                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${status === 'ACTIVE' ? 'bg-green-100 text-green-700' : status === 'SUSPENDED' ? 'bg-orange-100 text-orange-700' : 'bg-slate-100 text-slate-600'}`}>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    ...
                                    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            {contract.notes && (
                                                <div className="p-2.5 bg-yellow-50 border border-yellow-100 rounded text-xs text-yellow-800 whitespace-pre-wrap">{contract.notes}</div>
                                            )}
                                    ...
                                    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
                                        <div className="overflow-x-auto">
                                            <table className="w-full text-sm text-left">
                                                ...
                                                <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                                    {contractSubs.length > 0 ? paginate(contractSubs).map((s: any) => {
                                                        const plate = s.vehicle_plate || s.vehiclePlate || '—';
                                                        const brand = s.vehicle_brand || s.vehicleBrand || '';
                                                        const model = s.vehicle_model || s.vehicleModel || '';
                                                        ...
                                                        return (
                                                            <tr key={s.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                                                <td className="px-4 py-3 font-mono font-bold text-slate-800 dark:text-white">{plate}</td>
                                                        ...
                                                        <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-500 text-sm">Aucun abonnement associé à ce contrat.</td></tr>
                                                    )}
                                                </tbody>
                                                {contractSubs.length > 0 && (
                                                    <tfoot>
                                        ...
                                        <div className="p-3 border-t border-slate-100 dark:border-slate-700">
                                            <PaginationControls total={contractSubs.length} current={currentPage} onPageChange={setCurrentPage} />
                                        </div>
                                    </div>
                                )}
                            </div>
                                            ...
                                            {contractInvoices.length > 0 ? paginate(contractInvoices).map(inv => (
                                                <tr key={inv.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                                    ...
                                                    <td className="px-5 py-3 text-center">
                                                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${inv.status === 'PAID' || inv.status === 'paid' ? 'bg-green-100 text-green-700 border-green-200' : inv.status === 'OVERDUE' ? 'bg-red-100 text-red-700 border-red-200' : 'bg-orange-100 text-orange-700 border-orange-200'}`}>
                                                ...
                                                <tr><td colSpan={6} className="px-5 py-8 text-center text-slate-500 text-sm">Aucune facture générée pour ce contrat.</td></tr>
                                            )}
                                        </tbody>
                                    </table>
                                </div>
                                <div className="p-3 border-t border-slate-100 dark:border-slate-700">
                                    <PaginationControls total={contractInvoices.length} current={currentPage} onPageChange={setCurrentPage} />
                                </div>
                            </div>
                        )}
                                ...
                                <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                                    ...
                                    <div className="bg-blue-700 px-8 py-5 text-white">
                                        ...
                                        <div className="mt-3 flex flex-wrap gap-4 text-xs opacity-80">
                                            ...
                                            <span>Fin : {contract.endDate ? new Date(contract.endDate).toLocaleDateString('fr-FR') : 'Indéterminée'}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-8 space-y-6 text-sm text-slate-700 dark:text-slate-300">
                                        ...
                                        <div>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                ...
                                                <div className="p-4 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-xl">
                                                    ...
                                                    <p className="font-bold text-slate-800 dark:text-white">{client?.name || contract.clientId}</p>
                                        ...
                                        <div>
                                            ...
                                            {subsLoading ? (
                                                <div className="text-center py-4 text-slate-400 text-xs"><RefreshCw className="w-4 h-4 animate-spin inline mr-2" />Chargement…</div>
                                            ) : (
                                                <div className="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-700">
                                                    <table className="w-full text-xs">
                                                        ...
                                                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                                            {contractSubs.length > 0 ? contractSubs.map((s: any) => {
                                                                const plate = s.vehicle_plate || s.vehiclePlate || '—';
                                                                const brand = s.vehicle_brand || s.vehicleBrand || '';
                                                                const model = s.vehicle_model || s.vehicleModel || '';
                                                                const status = (s.status || '').toUpperCase();
                                                                return (
                                                                    <tr key={s.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                                                                        ...
                                                                        <td className="px-3 py-2 text-slate-600 dark:text-slate-400">{[brand, model].filter(Boolean).join(' ') || '—'}</td>
                                                                        <td className="px-3 py-2">{BILLING_CYCLE_LABELS[(s.billing_cycle || s.billingCycle || '').toUpperCase()] || '—'}</td>
                                                                        <td className="px-3 py-2 text-right font-bold">{formatPrice(parseFloat(s.monthly_fee ?? s.monthlyFee ?? 0))}</td>
                                                                        <td className="px-3 py-2 text-center">
                                                                            <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'}`}>
                                                                ...
                                                                <tr><td colSpan={5} className="px-3 py-4 text-center text-slate-500 italic">Aucun abonnement.</td></tr>
                                                            )}
                                                        </tbody>
                                                        {contractSubs.length > 0 && (
                                                            <tfoot>
                                        ...
                                        <div className="space-y-4 pt-2 border-t border-slate-200 dark:border-slate-700">
                                            ...
                                            {[
                                                { title: 'Article 1 – Objet du contrat', body: `Le présent contrat a pour objet la fourniture par ${branding?.name || 'le Prestataire'} d'un service de géolocalisation et de suivi GPS de véhicules au profit du CLIENT, selon les modalités définies aux présentes.` },
                                                { title: 'Article 2 – Durée', body: 'Le contrat prend effet à la date de début indiquée ci-dessus. Sauf résiliation dans les conditions prévues à l\'article 5, il est reconduit tacitement pour des périodes identiques sauf mention contraire.' },
                                                { title: 'Article 3 – Obligations du Prestataire', body: `${branding?.name || 'Le Prestataire'} s'engage à assurer la disponibilité de la plateforme de géolocalisation, à fournir l'assistance technique nécessaire et à notifier le CLIENT en cas d'interruption planifiée de service.` },
                                                { title: 'Article 4 – Obligations du Client', body: 'Le CLIENT s\'engage à utiliser les services conformément aux présentes, à régler les factures dans les délais convenus et à signaler toute anomalie dans les meilleurs délais.' },
                                                { title: 'Article 5 – Résiliation', body: 'Chaque partie peut résilier le contrat par lettre recommandée avec accusé de réception en respectant un préavis de 30 jours. En cas de manquement grave non corrigé sous 15 jours après mise en demeure, la résiliation peut être immédiate.' },
                                                { title: 'Article 6 – Confidentialité', body: 'Les parties s\'engagent à garder confidentielles toutes informations échangées dans le cadre du présent contrat, pendant toute sa durée et 3 ans après son terme.' },
                                                { title: 'Article 7 – Loi applicable et litiges', body: 'Le présent contrat est régi par le droit applicable localement. En cas de litige, les parties s\'efforceront de trouver une solution amiable. À défaut, les tribunaux compétents du siège social du Prestataire seront seuls compétents.' },
                                        ...
                                        <div className="pt-4 border-t border-slate-200 dark:border-slate-700">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                ...
                                                <div className="p-5 border-2 border-dashed border-purple-200 dark:border-purple-800 rounded-xl bg-purple-50/40 dark:bg-purple-900/10">
                                                    ...
                                                    <p className="font-bold text-sm text-slate-800 dark:text-white mb-3">{client?.name || contract.clientId}</p>
                                            ...
                                            <p className="text-[10px] text-slate-400 text-center mt-4 italic">Fait en deux exemplaires originaux.</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="space-y-4 flex-1 overflow-auto">
                                    {paginate(historyItems).map((item, i) => (
                                        <div key={i} className="flex gap-4">
                                            ...
                                            <div className="pb-5">
                                                <p className={`text-sm ${item.type === 'CREATION' ? 'font-bold text-slate-800 dark:text-white' : 'text-slate-700 dark:text-slate-300'}`}>{item.text}</p>
