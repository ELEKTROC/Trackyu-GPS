...
export const PresalesView: React.FC<PresalesViewProps> = ({ initialTab }) => {
  ...
  useEffect(() => {
    if (
      initialTab &&
      (initialTab === 'OVERVIEW' || initialTab === 'LEADS' || initialTab === 'QUOTES' || initialTab === 'CATALOG')
    ) {
  ...
  const kpis = useMemo(() => {
    ...
    const scoredLeads = filteredLeads
  ...
  const PRESALES_TABS_ALL = [
    ...
    },
    {
      ...
      icon: Zap,
      color: 'bg-yellow-500',
      description: 'Règles automatiques',
    },
    {
      id: 'REGISTRATION',
      label: 'Inscriptions',
            ...
            <div className="space-y-6 overflow-y-auto h-full custom-scrollbar p-1">
              ...
              <div className="hidden sm:grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                ...
                <Card className="p-4 border-l-4 border-l-cyan-500">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="section-title">Temps Conv.</p>
                ...
                <Card className="p-4 border-l-4 border-l-violet-500 bg-gradient-to-r from-violet-50 to-white dark:from-slate-800 dark:to-slate-900">
                  <div>
                    ...
                    <p className="text-xl font-bold text-violet-600 mt-1">
                      {(kpis.pipelineValue ?? 0).toLocaleString('fr-FR')}
                    </p>
                    <p className="text-[10px] text-[var(--text-muted)]">{currency} en cours</p>
                  </div>
                </Card>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* Leads Dormants */}
                <Card
                  className={`p-4 border-l-4 ${kpis.dormantLeads > 0 ? 'border-l-amber-500 bg-amber-50/50 dark:bg-amber-900/10' : 'border-l-slate-300'}`}
                >
                  <h3 className="text-sm font-bold text-[var(--text-primary)] mb-3 flex items-center gap-2">
                    <AlertCircle
                      className={`w-4 h-4 ${kpis.dormantLeads > 0 ? 'text-amber-500 animate-pulse' : 'text-[var(--text-muted)]'}`}
                    />
                  ...
                  {kpis.dormantLeads === 0 ? (
                    <p className="text-sm text-[var(--text-muted)] text-center py-4">✓ Aucun lead dormant</p>
                  ) : (
                      ...
                      {kpis.dormantList.map((lead: Lead) => (
                        ...
                        >
                          <div className="w-8 h-8 rounded-full bg-amber-100 text-amber-600 flex items-center justify-center text-xs font-bold">
                            {Math.ceil(
                              (new Date().getTime() - new Date(lead.updatedAt || lead.createdAt).getTime()) /
                                (1000 * 60 * 60 * 24)
                            )}
                            j
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-bold text-[var(--text-primary)] truncate">{lead.companyName}</p>
                            <p className="text-[10px] text-[var(--text-secondary)]">{lead.contactName}</p>
                          </div>
                          <button
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {/* PIE CHART */}
                <Card className="p-6">
                  <h3 className="text-lg font-bold text-[var(--text-primary)] mb-4">Répartition par Statut</h3>
                  <div className="h-64" style={{ minHeight: 200, minWidth: 200 }}>
                    <ResponsiveContainer
                      width="100%"
                      height="100%"
