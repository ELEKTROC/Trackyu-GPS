...
interface InvoiceFormState extends Omit<Partial<Invoice>, 'status' | 'category'>, Omit<Partial<Quote>, 'status' | 'category'> {
...
export const InvoiceForm: React.FC<InvoiceFormProps> = ({
    initialData, clients, tiers: rawTiers = [], tenants: rawTenants = [], vehicles = [], catalogItems = [], onSave, onCancel, onDirtyChange, mode
}) => {
    ...
    const { data: previewNumber, isLoading: isLoadingNumber } = usePreviewNumber(numberingModule, isNew, formState.resellerId || undefined);
    ...
    const clientVehicles = useMemo(() => {
        ...
        return safeVehicles.filter(v => v.clientId === formState.clientId || (v as Vehicle & { tier_id?: string }).tier_id === formState.clientId);
    ...
    const selectedPlates = useMemo(() => {
        ...
        return formState.licensePlate.split(',').map((p: string) => p.trim()).filter(Boolean);
    ...
    const handleItemSelect = (index: number, selectedName: string) => {
        ...
        const newItems = [...(formState.items || [])];
        if (selectedItem) {
            newItems[index] = { ...newItems[index], description: selectedItem.description || selectedItem.name, price: selectedItem.price };
    ...
    const addItem = () => setFormState({...formState, items: [...(formState.items || []), {description: '', quantity: 1, price: 0}]});
    const removeItem = (index: number) => setFormState({...formState, items: (formState.items || []).filter((_: InvoiceLineItem, i: number) => i !== index)});

    const subtotal = useMemo(() => formState.items ? formState.items.reduce((sum: number, i: InvoiceLineItem) => sum + i.quantity * i.price, 0) : 0, [formState.items]);
        ...
        <div className="space-y-6 text-sm">
                ...
                <FormGrid cols={3}>
                    ...
                    <FormField 
                        label={
                            <>
                                Revendeur (Tenant)
                                {formState.resellerId && formState._resellerAuto && <span className="text-green-500 text-[10px] ml-1">(Auto)</span>}
                    ...
                    >
                        ...
                        {isLocked && <p className="text-[10px] text-orange-500 mt-0.5">Non modifiable (facture {formState.status === 'PAID' ? 'payée' : 'annulée'})</p>}
                    </FormField>
                    ...
                    <FormField label="Numéro">
                        <div className="relative">
                            <Input 
                                ...
                                readOnly 
                                className="bg-slate-100 dark:bg-slate-700 text-slate-500 cursor-not-allowed font-mono" 
                            />
                            {isNew && formState.number && (
                                <span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-green-500 font-medium">Auto</span>
                            )}
                        </div>
                    </FormField>
                    ...
                    >
                        <Input
                            ...
                            placeholder={mode === 'QUOTES' ? "Ex: Devis Abonnement - AA-123-BB" : "Ex: Facture Abonnement - AA-123-BB"}
                        />
                    </FormField>
                    <FormField label="Date">
                        <Input
                            type="date"
                            value={mode === 'INVOICES'
                                ? (formState.date ? (typeof formState.date === 'string' ? formState.date.split('T')[0] : new Date(formState.date).toISOString().split('T')[0]) : '')
                                : (formState.createdAt ? (typeof formState.createdAt === 'string' ? formState.createdAt.split('T')[0] : new Date(formState.createdAt).toISOString().split('T')[0]) : '')}
                            onChange={e => mode === 'INVOICES' ? setFormState({...formState, date: e.target.value}) : setFormState({...formState, createdAt: e.target.value})}
                            aria-label="Date"
                        />
                    </FormField>
                    ...
                    <div className="md:col-span-3 grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <Select
                                ...
                                onChange={e => {
                                    ...
                                    if (newCategory === 'INSTALLATION' && !formState.installationDate) {
                                        const invoiceDate = formState.date ? (typeof formState.date === 'string' ? formState.date.split('T')[0] : new Date(formState.date).toISOString().split('T')[0]) : new Date().toISOString().split('T')[0];
                        ...
                        <FormField label="Statut" hint="Géré automatiquement">
                            <Input
                                ...
                                value={
                                    mode === 'INVOICES'
                                        ? ({ DRAFT: 'Brouillon', SENT: 'Envoyée', PAID: 'Payée', PARTIAL: 'Partiel', OVERDUE: 'En retard', CANCELLED: 'Annulée' } as Record<string, string>)[formState.status || 'DRAFT'] || formState.status
                                        : ({ DRAFT: 'Brouillon', SENT: 'Envoyé', ACCEPTED: 'Accepté', REJECTED: 'Refusé' } as Record<string, string>)[formState.status || 'DRAFT'] || formState.status
                                }
                                readOnly
                                className="bg-slate-100 dark:bg-slate-700 text-slate-500 cursor-not-allowed"
                            />
                        </FormField>
                        <FormField label="Échéance / Validité">
                            <Input
                                type="date"
                                value={mode === 'INVOICES'
                                    ? (formState.dueDate ? (typeof formState.dueDate === 'string' ? formState.dueDate.split('T')[0] : new Date(formState.dueDate).toISOString().split('T')[0]) : '')
                                    : (formState.validUntil ? (typeof formState.validUntil === 'string' ? formState.validUntil.split('T')[0] : new Date(formState.validUntil).toISOString().split('T')[0]) : '')}
                                onChange={e => mode === 'INVOICES' ? setFormState({...formState, dueDate: e.target.value}) : setFormState({...formState, validUntil: e.target.value})}
                                aria-label="Échéance"
                            />
                        </FormField>
                    </div>
                    ...
                    <FormGrid cols={3}>
                        <FormField label="Plaque(s) d'immatriculation">
                            <div className="relative" ref={plateDropdownRef}>
                                <button
                                    ...
                                    className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-left flex items-center justify-between min-h-[40px]"
                                >
                                        ...
                                        {selectedPlates.length > 0 ? selectedPlates.map(plate => (
                                            <span key={plate} className="inline-flex items-center gap-1 px-2 py-0.5 bg-[var(--primary-dim)] dark:bg-[var(--primary-dim)] text-[var(--primary)] dark:text-[var(--primary)] text-xs rounded-full">
                                                ...
                                                <X 
                                                    ...
                                                    onClick={(e) => { e.stopPropagation(); togglePlate(plate); }} 
                                                />
                                            </span>
                                        )) : (
                                            <span className="text-slate-400 text-sm">Sélectionner des véhicules...</span>
                                        )}
                                    </div>
                                    {isPlateDropdownOpen ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
                                </button>
                                {isPlateDropdownOpen && (
                                    <div className="absolute top-full left-0 w-full bg-white dark:bg-slate-800 border dark:border-slate-600 shadow-lg z-20 rounded-b-lg max-h-48 overflow-y-auto">
                                        <div className="p-2 border-b dark:border-slate-600">
                                            <input
                                                ...
                                                className="w-full p-1.5 text-xs border rounded dark:bg-slate-700 dark:border-slate-600"
                                                autoFocus
                                            />
                                        </div>
                                        {clientVehicles.length === 0 ? (
                                            ...
                                            clientVehicles
                                                .filter(v => {
                                                    ...
                                                    return plate.toLowerCase().includes(plateSearchTerm.toLowerCase()) ||
                                                ...
                                                .map(v => {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            {v.name && <span className="text-slate-400">— {v.name}</span>}
                                                        </div>
                                                    );
                                                })
                                        )}
                                        {/* Option saisie libre */}
                                        {plateSearchTerm && !clientVehicles.some(v => (v.licensePlate || v.plate || '') === plateSearchTerm.toUpperCase()) && (
                                            <div
                                                onClick={() => { togglePlate(plateSearchTerm.toUpperCase()); setPlateSearchTerm(''); }}
                                                className="p-2 text-xs cursor-pointer hover:bg-green-50 dark:hover:bg-green-900/30 border-t dark:border-slate-600 text-green-600"
                                            >
                                                <Plus className="w-3 h-3 inline mr-1" /> Ajouter "{plateSearchTerm.toUpperCase()}" manuellement
                                            </div>
                                        )}
                                    </div>
                                )}
                        ...
                        {formState.category === 'INSTALLATION' && (
                            <FormField
                                ...
                                hint={formState.installationDate ? "Définie à la clôture de l'intervention" : "Non renseignée — clôturer une intervention d'installation"}
                            >
                                <Input
                                    type="date"
                                    ...
                                    readOnly
                                    disabled
                                    className="border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-500 cursor-not-allowed"
                                />
                            </FormField>
                        )}
            ...
            <FormSection title="Lignes">
                <div className="space-y-2">
                    <div className="grid grid-cols-12 gap-2 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-2">
                        ...
                        <div key={index} className="grid grid-cols-12 gap-2 items-center">
                            <div className="col-span-6 relative" ref={openCatalogIndex === index ? catalogDropdownRef : undefined}>
                                <Input 
                                    ...
                                    onFocus={() => { setOpenCatalogIndex(index); setCatalogSearchTerm(item.description || ''); }}
                                    placeholder="Rechercher un article..." 
                                />
                                {openCatalogIndex === index && filteredCatalogItems.length > 0 && (
                                    <div className="absolute top-full left-0 w-full bg-white dark:bg-slate-800 border dark:border-slate-600 shadow-lg z-20 max-h-48 overflow-y-auto rounded-b-lg">
                                        {filteredCatalogItems
                                            .filter(ci => !catalogSearchTerm || ci.name.toLowerCase().includes(catalogSearchTerm.toLowerCase()))
                                            .map(ci => (
                                            <div
                                                key={ci.id}
                                        ...
                                        {filteredCatalogItems.filter(ci => !catalogSearchTerm || ci.name.toLowerCase().includes(catalogSearchTerm.toLowerCase())).length === 0 && (
                                            <div className="p-2 text-xs text-slate-400 text-center">Aucun article trouvé</div>
                                        )}
                                    </div>
                                )}
                            </div>
                            <div className="col-span-2">
                                <Input type="number" value={item.quantity} onChange={e => handleItemChange(index, 'quantity', e.target.value)} className="text-center" />
                            </div>
                            <div className="col-span-3">
                                <Input type="number" value={item.price} onChange={e => handleItemChange(index, 'price', e.target.value)} className="text-right" />
                            </div>
                            <div className="col-span-1 text-center">
                                <button onClick={() => removeItem(index)} className="text-red-500 hover:text-red-700"><Trash2 className="w-4 h-4" /></button>
                            </div>
                        </div>
                    ))}
            ...
            <div className="flex justify-end">
                <div className="w-64 space-y-2 text-sm bg-slate-50 dark:bg-slate-800 p-4 rounded-lg">
                    <div className="flex justify-between"><span className="text-slate-500">Sous-total HT:</span> <span className="font-bold font-mono">{formatPrice(subtotal)}</span></div>
                    <div className="flex justify-between items-center">
                        <span className="text-slate-500">Remise:</span> 
                        <input type="number" value={formState.discount || 0} onChange={e => setFormState({...formState, discount: parseFloat(e.target.value)})} className="w-20 p-1.5 text-right border border-slate-200 dark:border-slate-600 rounded-lg dark:bg-slate-700 bg-white text-xs" />
                    </div>
                    <div className="flex justify-between items-center">
            ...
            <FormSection title="Notes & Conditions">
                <div className="space-y-4">
                    <FormField label="Notes (Visible sur le document)">
                        <Textarea value={formState.notes || ''} onChange={e => setFormState({...formState, notes: e.target.value})} rows={2} />
                    </FormField>
                    <FormField label="Conditions Générales">
                        <Textarea value={formState.generalConditions || ''} onChange={e => setFormState({...formState, generalConditions: e.target.value})} rows={2} />
                    </FormField>
                </div>
            </FormSection>
