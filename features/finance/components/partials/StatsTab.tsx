...
import { 
    ...
    PieChart as RePieChart, Pie, Cell, Legend, BarChart, Bar, Line, ComposedChart
} from 'recharts';
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                ...
                <div className="bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex justify-between items-start mb-1.5">
                        ...
                        <span className="text-[9px] font-bold text-slate-400 uppercase">Encaissé</span>
                    </div>
                    <p className="text-base font-bold text-slate-800 dark:text-white">{formatPrice(financeKPIs.totalEncaissements || 0)}</p>
                ...
                <div className="bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex justify-between items-start mb-1.5">
                        ...
                        <span className="text-[9px] font-bold text-slate-400 uppercase">Charges</span>
                    </div>
                    <p className="text-base font-bold text-slate-800 dark:text-white">{formatPrice(financeKPIs.totalCharges || 0)}</p>
                ...
                <div className="bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex justify-between items-start mb-1.5">
                        <div className={`p-1.5 rounded-lg ${financeKPIs.resultatNet >= 0 ? 'bg-green-100 dark:bg-green-900/20 text-green-600' : 'bg-red-100 dark:bg-red-900/20 text-red-600'}`}>
                            <DollarSign className="w-4 h-4" />
                        </div>
                        <span className="text-[9px] font-bold text-slate-400 uppercase">Résultat</span>
                    </div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">Marge {financeKPIs.margePercent}%</p>
                </div>

                {/* DSO (Délai Moyen Paiement) */}
                <div className="bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex justify-between items-start mb-1.5">
                        <div className="p-1.5 bg-slate-100 dark:bg-slate-900/20 rounded-lg text-slate-600">
                            <Clock className="w-4 h-4" />
                        </div>
                        <span className="text-[9px] font-bold text-slate-400 uppercase">DSO</span>
                    </div>
                    <p className="text-base font-bold text-slate-800 dark:text-white">{financeKPIs.dso || 0} Jours</p>
                    <p className="text-xs text-slate-500 mt-0.5">Délai moyen paiement</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <Card title="📊 Encaissements vs Dépenses (6 mois)" className="h-[320px]">
                    <div className="h-[250px] w-full">
                        ...
                        <ResponsiveContainer width="100%" height="100%" minHeight={230} minWidth={200} initialDimension={{ width: 200, height: 230 }}>
                            <ComposedChart data={monthlyRevenueData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                                ...
                                <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: chartTextColor, fontSize: 11 }} />
                                ...
                                <Line type="monotone" dataKey="solde" stroke="#3b82f6" strokeWidth={2} dot={{ fill: '#3b82f6', r: 4 }} name="Solde net" />
                            </ComposedChart>
                        </ResponsiveContainer>
                        )}
                    </div>
                        ...
                        <ResponsiveContainer width="100%" height="100%" minHeight={230} minWidth={200} initialDimension={{ width: 200, height: 230 }}>
                            <AreaChart data={bankBalanceData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                                ...
                                <Tooltip 
                                    ...
                                    formatter={(value: number) => formatPrice(value)}
                                />
                                <Area type="monotone" dataKey="solde" stroke="#6366f1" strokeWidth={2} fill="url(#colorSoldeBancaire)" name="Solde" />
                            </AreaChart>
                        </ResponsiveContainer>
                        )}
                    </div>
                    ...
                    <div className="h-[250px] w-full">
                        ...
                        <ResponsiveContainer width="100%" height="100%" minHeight={230} minWidth={200} initialDimension={{ width: 200, height: 230 }}>
                            <BarChart data={budgetData} layout="vertical" margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                                ...
                                <XAxis type="number" axisLine={false} tickLine={false} tick={{ fill: chartTextColor, fontSize: 10 }} />
                                <YAxis type="category" dataKey="category" axisLine={false} tickLine={false} tick={{ fill: chartTextColor, fontSize: 10 }} width={80} />
                                <Tooltip 
                                    contentStyle={{ backgroundColor: tooltipBg, borderRadius: '8px', border: `1px solid ${tooltipBorder}` }}
                                    formatter={(value: number) => formatPrice(value)}
                                />
                                ...
                                {top5ClientsImpayes.map((client, index) => (
                                    ...
                                    >
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                <p className="font-medium text-xs text-slate-800 dark:text-white truncate max-w-[100px]">
                                                    {client.clientName}
                                                </p>
                                        ...
                                        <span className={`text-xs font-bold ${index === 0 ? 'text-red-600' : 'text-slate-700 dark:text-slate-300'}`}>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                {recentActivity.slice(0, 10).map((activity) => (
                                    <tr key={activity.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                        ...
                                        <td className="py-2 text-slate-700 dark:text-slate-300 max-w-[200px] truncate">
                                            {activity.description}
                                        </td>
                                        <td className={`py-2 text-right font-mono font-medium ${getActivityColor(activity.type)}`}>
                                            {activity.type === 'expense' ? '-' : '+'}{formatPrice(activity.amount)}
                                        </td>
