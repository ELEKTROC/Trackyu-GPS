...
export const AccountingContent: React.FC<AccountingContentProps> = ({
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300 h-full flex flex-col">
          ...
          {accountStats.map((cl, i) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span
                  className="font-medium text-[var(--text-primary)] text-sm truncate max-w-[120px]"
                  title={cl.label}
                >
                  {cl.label.split(':')[1]}
                </span>
              </div>
              <span className="font-mono font-bold text-[var(--text-primary)] text-sm">{formatPrice(cl.balance)}</span>
            </div>
          ))}
        </div>
        {selectedClassFilter && (
      ...
      {/* JOURNAL */}
      <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl overflow-hidden flex flex-col flex-1 min-h-[300px]">
        <div className="p-3 sm:p-4 border-b border-[var(--border)] flex flex-wrap sm:flex-nowrap justify-between items-center gap-2 bg-[var(--bg-elevated)]">
          <div className="flex items-center gap-2">
            <FileSpreadsheet className="w-5 h-5 text-[var(--primary)] dark:text-[var(--primary)]" />
            <h3 className="font-bold text-[var(--text-primary)]">Journal Général</h3>
          </div>
          <div className="flex gap-2 flex-wrap sm:flex-nowrap">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-9 pr-4 py-2 border border-[var(--border)] rounded-lg text-sm bg-[var(--bg-surface)] text-[var(--text-primary)] focus:ring-2 focus:ring-[var(--primary)] outline-none"
                value={searchTerm}
            ...
            {showLockDateInput && (
              <input
                ...
                className="px-3 py-2 border border-[var(--border)] rounded-lg text-sm bg-[var(--bg-surface)] text-[var(--text-primary)] focus:ring-2 focus:ring-red-500 outline-none"
              />
            )}
            <button
          ...
          <table className="w-full text-left border-collapse">
              ...
              <tr>
                {visibleColumns.includes('date') && (
                  <SortableHeader
                    label="Date"
                    sortKey="date"
                    currentSortKey={entrySortConfig.key}
                    currentDirection={entrySortConfig.direction}
            ...
            <tbody className="divide-y divide-[var(--border)] dark:divide-slate-800 text-sm">
              {sortedEntries.length === 0 && (
                <tr>
                  ...
                  >
                    ...
                    {searchTerm
                      ? `Aucune écriture pour "${searchTerm}"`
              ...
              {sortedEntries.map((entry, i) => (
                ...
                >
                  {visibleColumns.includes('date') && (
                    <td className="px-4 py-2 text-[var(--text-secondary)]">{entry.date}</td>
                  )}
                  {visibleColumns.includes('ref') && (
                    <td className="px-4 py-2 font-mono text-xs text-[var(--text-secondary)]">{entry.ref}</td>
                  )}
                  {visibleColumns.includes('reseller') && (
                  ...
                  {visibleColumns.includes('account') && (
                    <td className="px-4 py-2 font-bold text-[var(--text-primary)]">{entry.account}</td>
                  )}
                  {visibleColumns.includes('label') && (
                    <td className="px-4 py-2 text-[var(--text-primary)]">{entry.label}</td>
                  )}
                  {visibleColumns.includes('debit') && (
