...
export const DocumentPreview: React.FC<DocumentPreviewProps> = ({ item, onEdit, onAction }) => {
    ...
    const client = tiers.find(t => t.id === item.clientId || t.name === item.clientId) ||
        clients.find(c => c.name === item.clientId);

    // Check if reseller is from Côte d'Ivoire (no VAT)
    const resellerName = doc.resellerName || '';
    const isNoVatReseller = resellerName.toUpperCase().includes('ABIDJAN') ||
    ...
    const itemsTotal = hasItems
        ? item.items.reduce((sum, line) => sum + (line.quantity || 1) * (line.price || 0), 0)
        : 0;
    // Use stored amountHT as subtotal when it's consistent (within 1% of items total or no items)
    const subtotal = storedHT > 0
        ? storedHT
        : (hasItems ? itemsTotal : storedTTC);
    ...
    return (
        <div className="bg-slate-50 dark:bg-slate-900/50 p-4 md:p-8 rounded-lg text-slate-800 dark:text-slate-200 shadow-sm">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-6 md:mb-8 border-b border-slate-200 dark:border-slate-700 pb-4 md:pb-6">
                    ...
                    <div className="text-xs md:text-sm text-slate-600 dark:text-slate-400 whitespace-pre-line">
                        {tenantInfo ? (
                            <>
                                ...
                                {(tenantInfo.city || tenantInfo.country) && <p>{[tenantInfo.city, tenantInfo.country].filter(Boolean).join(', ')}</p>}
                ...
                <div className="text-left md:text-right w-full md:w-auto">
                    ...
                    {item.status && (() => {
                        ...
                        const Icon = cfg.icon;
                        return (
                            <span className={`inline-flex items-center gap-1 mt-2 px-2.5 py-1 text-xs font-bold rounded-full border ${cfg.color}`}>
                    ...
                    {/* Type d'Opération */}
                    {doc.category && (
                        <p className="mt-2 text-xs text-slate-500">Type Op. : <span className="font-medium text-slate-700 dark:text-slate-300">{categoryLabels[doc.category] || doc.category}</span></p>
                    )}
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 mb-8">
                <div>
                    <h3 className="text-xs font-bold uppercase text-slate-500 mb-2">{isInvoice ? 'Facturé à' : 'Destinataire'}</h3>
                ...
                <div className="flex flex-col items-start md:items-end gap-2 md:gap-4 text-sm text-left md:text-right bg-white dark:bg-slate-800 p-3 md:p-0 md:bg-transparent rounded md:rounded-none border md:border-0 border-slate-200 dark:border-slate-700">
                    <div>
                        <p className="text-xs font-bold uppercase text-slate-500">{isInvoice ? 'Date de la facture' : 'Date du devis'}</p>
                        ...
                        <p className="font-medium">
                            {isInvoice
                                ...
                                : ((item as Quote).validUntil
                                    ...
                                    : (() => { try { const d = new Date((item as Quote).createdAt); d.setDate(d.getDate() + 30); return fmtDate(d); } catch { return '-'; } })())}
                        </p>
                    </div>
            ...
            {((item as Quote).subject || (item as Invoice).subject) && (
                <div className="mb-6">
                    ...
                    <p className="font-medium text-slate-800 dark:text-slate-200">{(item as Quote).subject || (item as Invoice).subject}</p>
                </div>
            )}
            ...
            <div className="overflow-x-auto -mx-4 md:mx-0">
                <table className="w-full text-xs md:text-sm mb-4 min-w-[400px]">
                    ...
                    <tbody className="bg-white dark:bg-slate-800 divide-y divide-slate-100 dark:divide-slate-700">
                        {hasItems ? item.items.map((line, i) => (
                            <tr key={i}>
                                ...
                                <td className="p-2 text-right font-mono font-bold">{formatPrice((line.quantity || 1) * (line.price || 0))}</td>
                            </tr>
                        )) : (
                            <tr>
            ...
            <div className="flex flex-wrap justify-center md:justify-end gap-2 pt-6 border-t border-slate-200 dark:border-slate-700 mt-6 no-print">
                <button onClick={onEdit} className="p-2 px-3 flex items-center gap-1 text-xs font-bold bg-white dark:bg-slate-700 border rounded hover:bg-slate-100 dark:hover:bg-slate-600">
                    <Edit2 className="w-3 h-3" /> Modifier
                </button>
                <button onClick={() => onAction('download')} className="p-2 px-3 flex items-center gap-1 text-xs font-bold bg-white dark:bg-slate-700 border rounded hover:bg-slate-100 dark:hover:bg-slate-600">
                    <Download className="w-3 h-3" /> PDF
                </button>
                <button onClick={() => onAction('send')} className="p-2 px-3 flex items-center gap-1 text-xs font-bold bg-white dark:bg-slate-700 border rounded hover:bg-slate-100 dark:hover:bg-slate-600">
                    <Mail className="w-3 h-3" /> Envoyer
                </button>
                {item.status === 'DRAFT' && (
                    <button onClick={() => onAction('mark_sent')} className="p-2 px-3 flex items-center gap-1 text-xs font-bold bg-amber-100 dark:bg-amber-700 text-amber-800 dark:text-amber-100 border border-amber-300 dark:border-amber-600 rounded hover:bg-amber-200 dark:hover:bg-amber-600">
                        <Send className="w-3 h-3" /> Marquer envoyé
                    </button>
                )}
                {!isInvoice && item.status === 'ACCEPTED' && (
                    <button onClick={() => onAction('convert_to_invoice')} className="p-2 px-3 flex items-center gap-1 text-xs font-bold bg-green-100 dark:bg-green-700 text-green-800 dark:text-green-100 border border-green-300 dark:border-green-600 rounded hover:bg-green-200 dark:hover:bg-green-600">
