...
import { DollarSign, Plus, ArrowUpRight, ArrowDownLeft, FileCheck, Lock, X, History, CheckCircle, AlertTriangle, Download } from 'lucide-react';
...
export const CashView: React.FC<CashViewProps> = ({
  ...
  const counterpartOptions = useMemo(() =>
    PLAN_COMPTABLE.filter(a => a.code !== '530000' && (
  ...
  const cashEntries = useMemo(() => {
    return journalEntries
      .filter(entry => entry.account === '530000');
  }, [journalEntries]);

  const { sortedItems: sortedCashEntries, sortConfig: cashSortConfig, handleSort: handleCashSort } = useTableSort(
    cashEntries,
  ...
  const handleSubmit = async () => {
    ...
    await createGroupedJournalEntry({ date, description: label, reference: reference || undefined, journalCode: 'CA', lines });
  ...
  const handleGenerateClosing = async () => {
      ...
      await generateCashClosingPDF({
        ...
        gap,
        entries: dailyStats.entriesForDate.map(e => ({ ref: e.ref, label: e.label, debit: e.debit, credit: e.credit })),
        notes: closingNotes || undefined,
        formatPrice,
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-x-auto">
        <table className="w-full text-sm text-left text-slate-500 dark:text-slate-400 min-w-[600px]">
            <thead className="text-xs text-slate-700 uppercase bg-slate-50 dark:bg-slate-700 dark:text-slate-400">
                <tr>
                    <SortableHeader label="Date" sortKey="date" currentSortKey={cashSortConfig.key} currentDirection={cashSortConfig.direction} onSort={handleCashSort} />
                    <SortableHeader label="Référence" sortKey="reference" currentSortKey={cashSortConfig.key} currentDirection={cashSortConfig.direction} onSort={handleCashSort} />
                    <SortableHeader label="Libellé" sortKey="label" currentSortKey={cashSortConfig.key} currentDirection={cashSortConfig.direction} onSort={handleCashSort} />
                    {isSuperAdmin && <th className="px-6 py-3">Revendeur</th>}
                    <SortableHeader label="Entrée" sortKey="debit" currentSortKey={cashSortConfig.key} currentDirection={cashSortConfig.direction} onSort={handleCashSort} className="text-right" />
                    <SortableHeader label="Sortie" sortKey="credit" currentSortKey={cashSortConfig.key} currentDirection={cashSortConfig.direction} onSort={handleCashSort} className="text-right" />
                ...
                {sortedCashEntries.length === 0 ? (
                    ...
                    (() => {
                        ...
                        return sortedCashEntries.map((entry, index) => {
                            ...
                            return (
                                <tr key={entry.id} className="bg-white border-b dark:bg-slate-800 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50">
                    ...
                    <div className="space-y-6 max-w-3xl mx-auto">
                        <div className="flex gap-4 p-1 bg-slate-100 dark:bg-slate-700 rounded-lg">
                            <button
                                ...
                                onClick={() => { setTransactionType('WITHDRAWAL'); setCounterpartAccount('606000'); }}
                            >
                                Sortie (Dépense)
                            </button>
                            <button
                                ...
                                onClick={() => { setTransactionType('DEPOSIT'); setCounterpartAccount('411100'); }}
                            >
                                Entrée (Recette)
                            </button>
                        </div>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Compte de contrepartie</label>
                            <select
                        ...
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Référence (Optionnel)</label>
                            <input 
                                type="text" 
                                value={reference}
      ...
      >
        <div className="space-y-4">
                ...
                <div className="bg-orange-50 dark:bg-orange-900/20 p-4 rounded-lg border border-orange-200 dark:border-orange-800 flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-orange-700 dark:text-orange-400 mt-1">
                            Cette journée a été clôturée le {new Date(getClosingForDate(closingDate)?.closedAt || '').toLocaleString('fr-FR')}.
                            Une nouvelle clôture remplacera la précédente.
                        </p>
                    </div>
                </div>
            ) : (
            ...
            <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Date de l'arrêté</label>
                <input 
                    type="date" 
                    value={closingDate}
            ...
            <div className="grid grid-cols-2 gap-4 p-4 bg-slate-50 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
                <div>
                    ...
                    <p className="text-lg font-mono font-bold text-slate-700 dark:text-slate-300">{formatPrice(dailyStats.openingBalance)}</p>
                </div>
                <div>
            ...
            <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Solde Réel Constaté (Espèces)</label>
                <div className="relative">
                    <input 
                        type="number" 
                        step="0.01"
                ...
                {actualClosingBalance && (
                    <p className={`text-sm mt-1 font-bold ${parseFloat(actualClosingBalance) - dailyStats.theoreticalClosing === 0 ? 'text-emerald-600' : 'text-red-600'}`}>
            ...
            <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Notes / Observations</label>
                <textarea 
                    value={closingNotes}
                            ...
                            >
                                <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="font-bold text-slate-800 dark:text-white">
                                            {new Date(closing.date).toLocaleDateString('fr-FR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                                        </span>
                                    </div>
                                    <span className="text-xs text-slate-500">
                                        Clôturé à {new Date(closing.closedAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
                                    </span>
                                </div>
                                ...
                                {closing.gap !== 0 && (
                                    <div className="mt-2 pt-2 border-t border-orange-200 dark:border-orange-700">
                                        <p className="text-sm">
                                            <span className="text-orange-700 dark:text-orange-400 font-bold">Écart: {formatPrice(closing.gap)}</span>
