...
export const AccountingView: React.FC = () => {
  ...
  const accountStats = useMemo(() => {
    ...
    (journalEntries || []).forEach((entry) => {
      ...
      if (statIndex !== -1) {
        const amount = ['2', '3', '5', '6'].includes(classId) ? entry.debit - entry.credit : entry.credit - entry.debit;
  ...
  const balanceStructure = useMemo(() => {
    ...
    const circulating =
      (accountStats.find((s) => s.id === '3')?.balance || 0) + (accountStats.find((s) => s.id === '4')?.balance || 0);
  ...
  const top5ClientsImpayes = useMemo(() => {
    const unpaidByClient: Record<
      string,
      { clientId: string; clientName: string; totalUnpaid: number; invoiceCount: number }
    > = {};
    ...
    return categories
      .map((cat) => {
        ...
        const matchingBudget = (budgets || []).find(
          (b) => b.category.includes(cat.name) || b.accountPrefix === cat.code
        );
  ...
  const recentActivity = useMemo(() => {
    ...
    (journalEntries || [])
      .filter((e) => e.account.startsWith('6'))
      .slice(0, 5)
      .forEach((e) => {
        activities.push({
          id: `exp-${e.id}`,
  ...
  const toggleColumn = useCallback((id: JournalColumnId) => {
    setVisibleColumns((prev) => {
      ...
      try {
        localStorage.setItem('accounting_visible_columns', JSON.stringify(next));
      } catch {
        /* ignore */
      }
      return next;
  ...
  const togglePaymentColumn = useCallback((id: PaymentColumnId) => {
    setVisiblePaymentColumns((prev) => {
      const next = prev.includes(id) ? prev.filter((c) => c !== id) : [...prev, id];
      try {
        localStorage.setItem('accounting_visible_payment_columns', JSON.stringify(next));
      } catch {
        /* ignore */
      }
      return next;
  ...
  const handleRemoveEntryLine = useCallback(
    (index: number) => {
    ...
    setEntryForm((prev) => ({
      ...
      lines: prev.lines.map((line, i) => {
        ...
        if (field === 'account') {
          ...
          const newLabel =
            (!line.label || line.label === prev.label) && accountInfo?.suggestions?.[0]
    ...
    <div className="sm:h-[calc(100vh-140px)] sm:flex sm:flex-col animate-in fade-in duration-500 gap-3 sm:gap-6">
        ...
        {isSuperAdmin && (
          <div className="flex items-center gap-2 shrink-0">
            ...
            >
              ...
              {resellers.map((r) => (
                <option key={r.id} value={r.tenantId}>
                  {r.name}
                </option>
              ))}
            </select>
      ...
      >
        ...
        <div className="flex-1 min-h-0">
          ...
          {activeTab === 'CASH' && (
            <CashView journalEntries={journalEntries || []} isSuperAdmin={isSuperAdmin} resellers={resellers} />
          )}
          {activeTab === 'REPORTS' && (
            <ReportsView journalEntries={journalEntries || []} isSuperAdmin={isSuperAdmin} resellers={resellers} />
          )}
        </div>
