...
import { AlertTriangle, Phone, Mail, FileWarning, CheckCircle, Clock, ShieldAlert, Search, Filter, Columns, X, Calendar, Download, Settings, MoreVertical, Eye } from 'lucide-react';
...
export const RecoveryView: React.FC<RecoveryViewProps> = ({
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>(['client', 'reseller', 'invoice', 'amount', 'overdue', 'level', 'reminders', 'lastReminder', 'type', 'notes', 'actions']);
  ...
  const executeBulkReminder = async () => {
    ...
    for (const inv of selectedInvoices) {
      try {
        ...
        const channel: 'EMAIL' | 'SMS' | 'CALL' = 
          bulkSendMode === 'WHATSAPP' ? 'EMAIL' : bulkSendMode;
  ...
  const handleExport = (format: 'CSV' | 'EXCEL' | 'PDF') => {
      if (format === 'CSV') {
          const headers = ['Client', 'Facture', 'Montant', 'Retard (Jours)', 'Niveau', 'Relances', 'Dernière Relance', 'Notes'];
  ...
  const overdueInvoices = useMemo(() => {
    ...
    if (searchTerm) {
        filtered = filtered.filter(inv => 
            ...
            clients.find(c => c.id === inv.clientId)?.name.toLowerCase().includes(searchTerm.toLowerCase())
        );
    }
  ...
  const { sortedItems: sortedOverdue, sortConfig: recoverySortConfig, handleSort: handleRecoverySort } = useTableSort(
    overdueInvoices,
    { key: 'daysOverdue', direction: 'desc' }
  );
  ...
  const handleCreateRecoveryTask = (invoice: Invoice & { daysOverdue: number }, action: 'CALL' | 'EMAIL' | 'LETTER', sendMode?: 'EMAIL' | 'SMS' | 'WHATSAPP' | 'CALL') => {
    const client = clients.find(c => c.id === invoice.clientId);
    let title = '';
    let desc = '';
    let priority: 'LOW' | 'MEDIUM' | 'HIGH' = 'MEDIUM';

    const modeLabel = sendMode === 'SMS' ? 'SMS' : sendMode === 'WHATSAPP' ? 'WhatsApp' : sendMode === 'CALL' ? 'Téléphonique' : 'Email';
  ...
  const handleDeclareLost = async (invoice: Invoice) => {
      if (await confirm({ 
          message: 'Êtes-vous sûr de vouloir déclarer cette facture comme irrécouvrable ? Une écriture comptable de type Pertes et Profits sera générée automatiquement.', 
          variant: 'danger', 
          ...
          await createGroupedJournalEntry({
              ...
              journalCode: 'OD',
              lines: [
                  { account_code: '654000', debit: invoice.amount, credit: 0, description: `Perte sur créance - Facture ${invoice.number}` },
                  { account_code: '411000', debit: 0, credit: invoice.amount, description: `Solde client - Facture ${invoice.number}` },
              ],
          });
  ...
  const getLevelBadge = (level: string) => {
      switch(level) {
          case 'LEVEL_1': return <span className="px-2 py-1 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300 rounded text-xs font-bold">Relance 1</span>;
          case 'LEVEL_2': return <span className="px-2 py-1 bg-orange-100 dark:bg-orange-900/30 text-orange-800 dark:text-orange-300 rounded text-xs font-bold">Relance 2</span>;
          case 'LEVEL_3': return <span className="px-2 py-1 bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 rounded text-xs font-bold">Contentieux</span>;
          case 'LITIGATION': return <span className="px-2 py-1 bg-slate-800 text-white rounded text-xs font-bold">Juridique</span>;
          default: return <span className="px-2 py-1 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded text-xs">À traiter</span>;
      }
  };
  ...
  const getInvoiceHistory = (invoiceId: string) => {
      return tasks.filter(t => t.relatedTo?.type === 'INVOICE' && t.relatedTo?.id === invoiceId).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  };

  return (
    <div className="h-full flex flex-col p-6 space-y-6 overflow-hidden" onClick={() => openMenuId && setOpenMenuId(null)}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="p-4 border-l-4 border-l-red-500 bg-red-50 dark:bg-red-900/10">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-2xl font-bold text-slate-800 dark:text-white">{formatPrice(kpis.totalOverdue)}</p>
                </div>
        ...
        <Card className="p-4 border-l-4 border-l-orange-500">
            <div className="flex justify-between items-center">
                <div>
                    <p className="text-sm font-medium text-slate-500">Factures en Retard</p>
                    <p className="text-2xl font-bold text-slate-800 dark:text-white">{kpis.count}</p>
                </div>
        ...
        <Card className="p-4 border-l-4 border-l-slate-500">
            <div className="flex justify-between items-center">
                <div>
                    <p className="text-sm font-medium text-slate-500">Dossiers Critiques (&gt;60j)</p>
                    <p className="text-2xl font-bold text-slate-800 dark:text-white">{kpis.criticalCount}</p>
                </div>
      ...
      {/* Toolbar */}
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
          <div className="flex flex-wrap items-center gap-2 flex-1">
              ...
              <div className="flex items-center gap-2">
                  <button 
                      ...
                      className="flex items-center gap-2 px-3 py-2 bg-purple-600 text-white hover:bg-purple-700 disabled:bg-slate-300 disabled:text-slate-500 rounded-lg text-sm font-medium transition-colors"
                      title="Relancer les factures sélectionnées"
                  >
                      <Mail className="w-4 h-4" /> 
                      {isProcessingBulk ? 'Traitement...' : selectedIds.size > 0 ? `Relancer (${selectedIds.size})` : 'Relancer Sélection'}
                  </button>
                  <button 
                  ...
                  <button 
                      ...
                      className="flex items-center gap-2 px-3 py-2 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg text-sm font-medium transition-colors"
                  >
                      <Columns className="w-4 h-4" /> Colonnes
                  </button>
                  {isColumnMenuOpen && (
                      <div className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 z-20 p-2">
                          {['client', 'invoice', 'amount', 'overdue', 'level', 'reminders', 'lastReminder', 'type', 'notes', 'actions'].map(col => (
        ...
        {isMobile ? (
          <MobileCardList>
            {paginatedInvoices.length === 0 ? (
              ...
              const client = clients.find(c => c.id === inv.clientId);
              const borderColor = inv.daysOverdue > 60 ? 'border-l-red-600'
                : inv.daysOverdue > 30 ? 'border-l-red-400'
                : 'border-l-orange-500';
              return (
                <MobileCard key={inv.id} borderColor={borderColor}>
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <p className="text-sm font-bold text-slate-800 dark:text-slate-100 truncate">{client?.name || 'Client Inconnu'}</p>
                      <p className="text-xs font-mono text-slate-500">{inv.number} · Éch. {new Date(inv.dueDate).toLocaleDateString('fr-FR')}</p>
                    </div>
                    <p className="shrink-0 font-bold text-slate-800 dark:text-white text-sm">{formatPrice(inv.amount)}</p>
                  </div>
                  <div className="mt-1 flex items-center gap-2 flex-wrap">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${inv.daysOverdue > 30 ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300' : 'bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300'}`}>
                      {inv.daysOverdue} jours
                    </span>
                    {getLevelBadge(inv.recoveryLevel as string)}
                    {inv.isDisputed && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-red-100 text-red-700">EN LITIGE</span>}
                  </div>
                  <div className="mt-2 flex items-center gap-2">
                    <MobileCardAction icon={<Mail className="w-3 h-3" />} color="blue" onClick={() => handleCreateRecoveryTask(inv, 'EMAIL')}>Relancer</MobileCardAction>
                    <MobileCardAction color="slate" onClick={() => { setSelectedInvoice(inv); setIsModalOpen(true); }}>Voir détails</MobileCardAction>
                  </div>
                </MobileCard>
              );
            })}
            ...
            <table className="w-full text-left border-collapse min-w-[800px]">
                <thead className="bg-slate-50 dark:bg-slate-900/50 sticky top-0 z-10">
                    <tr>
                        ...
                        {visibleColumns.includes('client') && <SortableHeader label="Client" sortKey="clientId" currentSortKey={recoverySortConfig.key} currentDirection={recoverySortConfig.direction} onSort={handleRecoverySort} />}
                        {visibleColumns.includes('reseller') && <SortableHeader label="Revendeur" sortKey="tenantId" currentSortKey={recoverySortConfig.key} currentDirection={recoverySortConfig.direction} onSort={handleRecoverySort} />}
                        {visibleColumns.includes('invoice') && <SortableHeader label="Facture" sortKey="number" currentSortKey={recoverySortConfig.key} currentDirection={recoverySortConfig.direction} onSort={handleRecoverySort} />}
                        {visibleColumns.includes('type') && <SortableHeader label="Type" sortKey="invoiceType" currentSortKey={recoverySortConfig.key} currentDirection={recoverySortConfig.direction} onSort={handleRecoverySort} />}
                        {visibleColumns.includes('amount') && <SortableHeader label="Montant" sortKey="amount" currentSortKey={recoverySortConfig.key} currentDirection={recoverySortConfig.direction} onSort={handleRecoverySort} />}
                        {visibleColumns.includes('overdue') && <SortableHeader label="Retard" sortKey="daysOverdue" currentSortKey={recoverySortConfig.key} currentDirection={recoverySortConfig.direction} onSort={handleRecoverySort} />}
                        {visibleColumns.includes('level') && <SortableHeader label="Niveau" sortKey="recoveryLevel" currentSortKey={recoverySortConfig.key} currentDirection={recoverySortConfig.direction} onSort={handleRecoverySort} />}
                        {visibleColumns.includes('reminders') && <th className="p-4 text-xs font-semibold text-slate-500 uppercase text-center">Relances</th>}
                        {visibleColumns.includes('lastReminder') && <SortableHeader label="Dernière Relance" sortKey="lastReminderDate" currentSortKey={recoverySortConfig.key} currentDirection={recoverySortConfig.direction} onSort={handleRecoverySort} />}
                        {visibleColumns.includes('notes') && <th className="p-4 text-xs font-semibold text-slate-500 uppercase">Notes</th>}
                        {visibleColumns.includes('actions') && <th className="p-4 text-xs font-semibold text-slate-500 uppercase text-right">Actions</th>}
                    </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                    {paginatedInvoices.length === 0 ? (
                        ...
                        paginatedInvoices.map(inv => {
                            const client = clients.find(c => c.id === inv.clientId);
                            return (
                                <tr key={inv.id} className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors ${selectedIds.has(inv.id) ? 'bg-purple-50 dark:bg-purple-900/10' : ''}`}>
                                    <td className="p-4">
                                        <input 
                                            type="checkbox"
                                    ...
                                    {visibleColumns.includes('invoice') && <td className="p-4 text-sm text-slate-600 dark:text-slate-300">
                                        <div className="flex items-center gap-2">
                                            <button
                                        ...
                                        <div className="text-xs text-slate-400">Échéance: {new Date(inv.dueDate).toLocaleDateString('fr-FR')}</div>
                                    </td>}
                                    {visibleColumns.includes('type') && <td className="p-4 text-sm text-slate-600 dark:text-slate-300">
                                        <span className="px-2 py-1 bg-slate-100 dark:bg-slate-700 rounded text-xs">{inv.category || 'STANDARD'}</span>
                                    </td>}
                                    {visibleColumns.includes('amount') && <td className="p-4 font-bold text-slate-800 dark:text-white">
                                        {formatPrice(inv.amount)}
                                    </td>}
                                    {visibleColumns.includes('overdue') && <td className="p-4">
                                        <span className={`font-bold ${inv.daysOverdue > 30 ? 'text-red-600' : 'text-orange-600'}`}>
                                            {inv.daysOverdue} jours
                                        </span>
                                    </td>}
                                    {visibleColumns.includes('level') && <td className="p-4">
                                    ...
                                    {visibleColumns.includes('reminders') && <td className="p-4 text-center font-mono font-bold text-slate-600 dark:text-slate-300">
                                        {inv.reminderCount || 0}
                                    </td>}
                                    {visibleColumns.includes('lastReminder') && <td className="p-4 text-sm text-slate-500">
                                        {inv.lastReminderDate ? new Date(inv.lastReminderDate).toLocaleDateString('fr-FR') : '-'}
                                    </td>}
                                        ...
                                        <td className="p-4 text-sm text-slate-500 max-w-xs">
                                            {editingNoteId === inv.id ? (
                                                <div className="flex flex-col gap-2">
                                                    <textarea 
                                                        className="w-full p-2 text-xs border border-slate-200 dark:border-slate-700 rounded bg-white dark:bg-slate-900 text-slate-800 dark:text-white focus:ring-2 focus:ring-purple-500 focus:outline-none"
                                                        value={tempNoteValue}
                                                ...
                                                <div 
                                                    className="group flex items-start gap-2 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 p-1 rounded"
                                                    onClick={() => { setEditingNoteId(inv.id); setTempNoteValue(inv.notes || ''); }}
                                    ...
                                    {visibleColumns.includes('actions') && <td className="p-4 text-right">
                                        <div className="relative">
                                            ...
                                            {openMenuId === inv.id && (
                                                <div className="absolute right-0 z-50 mt-1 w-52 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg py-1">
                                                    <button onClick={() => { setSelectedInvoice(inv); setIsModalOpen(true); setOpenMenuId(null); }} className="w-full text-left px-4 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2">
                                                    ...
                                                    <button onClick={() => { handleToggleDispute(inv); setOpenMenuId(null); }} className={`w-full text-left px-4 py-2 text-sm hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2 ${inv.isDisputed ? 'text-red-600' : 'text-slate-700 dark:text-slate-200'}`}>
                                                        <ShieldAlert className={`w-4 h-4 ${inv.isDisputed ? 'text-red-500' : 'text-slate-400'}`} /> {inv.isDisputed ? 'Lever le litige' : 'Marquer en litige'}
                                                    </button>
                                                    {inv.daysOverdue > 30 && (
                                                        <button onClick={() => { handleCreateRecoveryTask(inv, 'LETTER'); setOpenMenuId(null); }} className="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 flex items-center gap-2">
                                                            <FileWarning className="w-4 h-4" /> Mise en demeure
                                                        </button>
                                                    )}
                                                    <div className="border-t border-slate-100 dark:border-slate-700 my-1" />
                                                    <button onClick={() => { handleDeclareLost(inv); setOpenMenuId(null); }} className="w-full text-left px-4 py-2 text-sm text-red-700 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 flex items-center gap-2">
      ...
      {isModalOpen && selectedInvoice && (
          <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={`Détail Recouvrement - ${selectedInvoice.number}`}>
              <div className="space-y-6">
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-lg col-span-2 md:col-span-1">
                          ...
                          {(() => {
                              ...
                              return (tier || client) ? (
                                  ...
                                  <p className="font-bold text-slate-800 dark:text-white">Inconnu</p>
                              );
                          })()}
                      </div>
                      <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-lg">
                          <p className="text-xs text-slate-500 uppercase font-bold">Montant Dû</p>
                          <p className="text-xl font-bold text-red-600">{formatPrice(selectedInvoice.amount)}</p>
                      </div>
                      <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-lg">
                          <p className="text-xs text-slate-500 uppercase font-bold">Retard</p>
                          <p className="text-xl font-bold text-slate-800 dark:text-white">{selectedInvoice.daysOverdue} jours</p>
                      </div>
                      <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-lg">
                      ...
                      <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-lg">
                          <p className="text-xs text-slate-500 uppercase font-bold">Relances</p>
                          <p className="text-xl font-bold text-slate-800 dark:text-white">{selectedInvoice.reminderCount || 0}</p>
                      </div>
                      <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-lg">
                          <p className="text-xs text-slate-500 uppercase font-bold">Dernière Relance</p>
                          <p className="text-sm font-bold text-slate-800 dark:text-white">
                              {selectedInvoice.lastReminderDate ? new Date(selectedInvoice.lastReminderDate).toLocaleDateString('fr-FR') : '-'}
                          </p>
                      </div>
                  </div>
                  ...
                  <div>
                      ...
                      <div className="border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden max-h-60 overflow-y-auto">
                          <table className="w-full text-sm text-left">
                              <thead className="bg-slate-50 dark:bg-slate-900 font-bold text-xs uppercase text-slate-500">
                                  <tr>
                              ...
                              <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                  {getInvoiceHistory(selectedInvoice.id).length === 0 ? (
                                      <tr><td colSpan={3} className="p-4 text-center text-slate-500">Aucune action enregistrée</td></tr>
                                  ) : (
                                      getInvoiceHistory(selectedInvoice.id).map(task => (
                                          <tr key={task.id}>
                                              ...
                                              <td className="p-3">
                                                  <span className={`px-2 py-1 rounded text-xs ${task.status === 'DONE' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300' : 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300'}`}>
                                                      {task.status}
                                                  </span>
                                              </td>
                                          </tr>
                  ...
                  <div className="pt-4 border-t border-slate-200 dark:border-slate-700 space-y-3">
                      <div className="flex flex-wrap gap-2">
                          ...
                          <button
                              onClick={() => { setCallNote(''); setIsCallModalOpen(true); }}
                          ...
                          <button
                              onClick={() => { setIsModalOpen(false); setIsExtendModalOpen(true); }}
                          ...
                          <button
                              onClick={() => { handleToggleDispute(selectedInvoice); }}
                              className={`px-3 py-2 rounded-lg text-sm font-bold flex items-center gap-2 ${selectedInvoice.isDisputed ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 hover:bg-red-200' : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200'}`}
                          >
                      ...
                      <div className="flex justify-end gap-3">
                          ...
                          <button
                              ...
                              className="px-4 py-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg font-bold text-sm"
                          >
                              Fermer
                          </button>
                      </div>
      ...
      {isAutomationModalOpen && (
          <Modal isOpen={isAutomationModalOpen} onClose={() => setIsAutomationModalOpen(false)} title="⚙️ Automatisation du Recouvrement">
              <div className="space-y-4">
                  ...
                  {/* Niveau 1 */}
                  <div className={`flex items-center justify-between p-3 border rounded-lg transition-all ${automationSettings.level1Enabled ? 'border-green-300 bg-green-50 dark:bg-green-900/10' : 'border-slate-200 dark:border-slate-700'}`}>
                      <div className="flex-1">
                          <p className="font-medium text-slate-800 dark:text-white">Relance Niveau 1</p>
                          <div className="flex items-center gap-2 mt-1">
                              <span className="text-xs text-slate-500">Après</span>
                              <input 
                                  type="number" 
                                  min="1" 
                                  max="30"
                                  value={automationSettings.level1Days}
                                  onChange={e => setAutomationSettings({...automationSettings, level1Days: parseInt(e.target.value)})}
                                  title="Jours avant relance niveau 1"
                                  className="w-14 px-2 py-1 border rounded text-sm text-center"
                              />
                              <span className="text-xs text-slate-500">jours → Email de courtoisie</span>
                          </div>
                      </div>
                      <button
                          onClick={() => setAutomationSettings({...automationSettings, level1Enabled: !automationSettings.level1Enabled})}
                          title="Activer/désactiver relance niveau 1"
                          className={`w-12 h-6 rounded-full relative transition-colors ${automationSettings.level1Enabled ? 'bg-green-500' : 'bg-slate-300 dark:bg-slate-600'}`}
                      >
                          <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${automationSettings.level1Enabled ? 'right-1' : 'left-1'}`}></div>
                      </button>
                  </div>
                  
                  {/* Niveau 2 */}
                  <div className={`flex items-center justify-between p-3 border rounded-lg transition-all ${automationSettings.level2Enabled ? 'border-orange-300 bg-orange-50 dark:bg-orange-900/10' : 'border-slate-200 dark:border-slate-700'}`}>
                      <div className="flex-1">
                          <p className="font-medium text-slate-800 dark:text-white">Relance Niveau 2</p>
                          <div className="flex items-center gap-2 mt-1">
                              <span className="text-xs text-slate-500">Après</span>
                              <input 
                                  type="number" 
                                  ...
                                  onChange={e => setAutomationSettings({...automationSettings, level2Days: parseInt(e.target.value)})}
                                  title="Jours avant relance niveau 2"
                                  className="w-14 px-2 py-1 border rounded text-sm text-center"
                              />
                              <span className="text-xs text-slate-500">jours → Email + Tâche d'appel</span>
                          </div>
                      </div>
                      <button
                          onClick={() => setAutomationSettings({...automationSettings, level2Enabled: !automationSettings.level2Enabled})}
                          title="Activer/désactiver relance niveau 2"
                          className={`w-12 h-6 rounded-full relative transition-colors ${automationSettings.level2Enabled ? 'bg-orange-500' : 'bg-slate-300 dark:bg-slate-600'}`}
                      >
                          <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${automationSettings.level2Enabled ? 'right-1' : 'left-1'}`}></div>
                      </button>
                  </div>
                  
                  {/* Niveau 3 */}
                  <div className={`flex items-center justify-between p-3 border rounded-lg transition-all ${automationSettings.level3Enabled ? 'border-red-300 bg-red-50 dark:bg-red-900/10' : 'border-slate-200 dark:border-slate-700'}`}>
                      <div className="flex-1">
                          <p className="font-medium text-slate-800 dark:text-white">Contentieux Niveau 3</p>
                          <div className="flex items-center gap-2 mt-1">
                              <span className="text-xs text-slate-500">Après</span>
                              <input 
                                  type="number" 
                                  ...
                                  onChange={e => setAutomationSettings({...automationSettings, level3Days: parseInt(e.target.value)})}
                                  title="Jours avant contentieux niveau 3"
                                  className="w-14 px-2 py-1 border rounded text-sm text-center"
                              />
                              <span className="text-xs text-slate-500">jours → Mise en demeure</span>
                          </div>
                      </div>
                      <button
                          onClick={() => setAutomationSettings({...automationSettings, level3Enabled: !automationSettings.level3Enabled})}
                          title="Activer/désactiver contentieux niveau 3"
                          className={`w-12 h-6 rounded-full relative transition-colors ${automationSettings.level3Enabled ? 'bg-red-500' : 'bg-slate-300 dark:bg-slate-600'}`}
                      >
                          <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${automationSettings.level3Enabled ? 'right-1' : 'left-1'}`}></div>
                      </button>
                  </div>

                  {/* Stats */}
                  <div className="p-3 bg-slate-100 dark:bg-slate-800 rounded-lg">
                      <p className="text-sm font-medium text-slate-600 dark:text-slate-300 mb-2">📊 Factures concernées :</p>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-center">
                          <div className="bg-white dark:bg-slate-700 rounded p-2">
                              <p className="text-lg font-bold text-yellow-600">{overdueInvoices.filter(i => i.daysOverdue >= automationSettings.level1Days && i.daysOverdue < automationSettings.level2Days).length}</p>
                              <p className="text-xs text-slate-500">Niveau 1</p>
                          </div>
                          <div className="bg-white dark:bg-slate-700 rounded p-2">
                              <p className="text-lg font-bold text-orange-600">{overdueInvoices.filter(i => i.daysOverdue >= automationSettings.level2Days && i.daysOverdue < automationSettings.level3Days).length}</p>
                              <p className="text-xs text-slate-500">Niveau 2</p>
                          </div>
                          <div className="bg-white dark:bg-slate-700 rounded p-2">
                              <p className="text-lg font-bold text-red-600">{overdueInvoices.filter(i => i.daysOverdue >= automationSettings.level3Days).length}</p>
                              <p className="text-xs text-slate-500">Contentieux</p>
                          </div>
                      </div>
                  </div>
              ...
              <div className="space-y-4">
                  <div className="bg-blue-50 dark:bg-blue-900/20 text-blue-800 dark:text-blue-300 p-4 rounded-lg text-sm flex items-start gap-2">
                      ...
                      <div>
                          ...
                          <p>Vous êtes sur le point de modifier la date d'échéance de cette facture. Cette action impactera le calcul des retards.</p>
                      </div>
                  </div>

                  <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Nouvelle Date d'Échéance</label>
                      <input 
                          type="date" 
                  ...
                  <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Motif de la prorogation</label>
                      <textarea 
                          className="w-full p-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 h-24"
              ...
              <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                      ...
                      <button
                          ...
                          className={`p-4 rounded-lg border-2 text-left transition-all ${bulkSendMode === 'SMS' ? 'border-green-500 bg-green-50 dark:bg-green-900/20' : 'border-slate-200 dark:border-slate-700 hover:border-green-300'}`}
                      >
                          <Phone className={`w-6 h-6 mb-2 ${bulkSendMode === 'SMS' ? 'text-green-600' : 'text-slate-400'}`} />
                          <p className="font-bold text-slate-800 dark:text-white">SMS</p>
                          <p className="text-xs text-slate-500">Message texte court</p>
                      </button>
                      
                      <button
                          onClick={() => setBulkSendMode('WHATSAPP')}
                          className={`p-4 rounded-lg border-2 text-left transition-all ${bulkSendMode === 'WHATSAPP' ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20' : 'border-slate-200 dark:border-slate-700 hover:border-emerald-300'}`}
                      >
                          <svg className={`w-6 h-6 mb-2 ${bulkSendMode === 'WHATSAPP' ? 'text-emerald-600' : 'text-slate-400'}`} fill="currentColor" viewBox="0 0 24 24">
                          ...
                          <p className="text-xs text-slate-500">Message WhatsApp Business</p>
                      </button>
                      
                      <button
                          onClick={() => setBulkSendMode('CALL')}
                          className={`p-4 rounded-lg border-2 text-left transition-all ${bulkSendMode === 'CALL' ? 'border-orange-500 bg-orange-50 dark:bg-orange-900/20' : 'border-slate-200 dark:border-slate-700 hover:border-orange-300'}`}
                      >
                          <Phone className={`w-6 h-6 mb-2 ${bulkSendMode === 'CALL' ? 'text-orange-600' : 'text-slate-400'}`} />
                          <p className="font-bold text-slate-800 dark:text-white">Appel</p>
                          <p className="text-xs text-slate-500">Créer tâche d'appel</p>
                      </button>
                  </div>
          ...
          <Modal isOpen={isPTPModalOpen} onClose={() => setIsPTPModalOpen(false)} title="🤝 Promesse de Paiement">
              <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                      ...
                      <div>
                          <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Montant promis (Optionnel)</label>
                          <div className="relative">
                              <input 
                                  type="number" 
      ...
      {isCallModalOpen && selectedInvoice && (
          <Modal isOpen={isCallModalOpen} onClose={() => setIsCallModalOpen(false)} title="📞 Compte-rendu d'Appel">
              <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                      <div>
                          <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Résumé de l'appel</label>
                          <textarea 
                              className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-900 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-green-500"
                              value={callNote}
      ...
      {isInvoicePreviewOpen && selectedInvoice && (
          <Modal isOpen={isInvoicePreviewOpen} onClose={() => setIsInvoicePreviewOpen(false)} title={`Aperçu — ${selectedInvoice.number}`}>
              <DocumentPreview
      ...
      <TierDetailModal
          ...
          onClose={() => { setIsTierModalOpen(false); setSelectedTierForModal(null); }}
      />
    </div>
  );
};
