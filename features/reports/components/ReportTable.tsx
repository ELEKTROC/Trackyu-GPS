  ...
  return (
    <div className="relative" ref={containerRef}>
      <button
        ...
        className={`flex items-center gap-2 px-3 py-2 border rounded-lg text-sm font-medium transition-colors ${isOpen || totalSelected < totalVehicles ? 'bg-[var(--primary-dim)] border-[var(--border)] text-[var(--primary)] dark:bg-[var(--primary-dim)] dark:border-[var(--primary)] dark:text-[var(--primary)]' : 'bg-[var(--bg-elevated)] border-[var(--border)] text-[var(--text-secondary)] hover:bg-[var(--bg-elevated)] dark:hover:bg-slate-700'}`}
      >
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 w-80 bg-[var(--bg-elevated)] border border-[var(--border)] rounded-lg shadow-xl z-50 flex flex-col max-h-[500px] animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="p-3 border-b border-[var(--border)] border-[var(--border)] space-y-2">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-2 py-1.5 text-sm border border-[var(--border)] rounded-lg bg-[var(--bg-elevated)] text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--primary)]"
              />
            </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
                ...
                >
                  <div className="flex items-center bg-[var(--bg-elevated)] p-2 hover:bg-[var(--bg-elevated)]/50 transition-colors">
                    <button
                      ...
                      className="p-1 mr-1 hover:bg-[var(--bg-elevated)] dark:hover:bg-slate-600 rounded"
                    >
                      {isExpanded ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
                    </button>
                    <div
                      className="flex items-center gap-2 flex-1 cursor-pointer"
                      onClick={() => handleClientCheckbox(client, vehicles)}
                    >
                      <div
                        className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${isAllSelected || isIndeterminate ? 'bg-[var(--primary)] border-[var(--primary)]' : 'border-[var(--border)] bg-[var(--bg-surface)]'}`}
                      >
                        {isAllSelected && <Check className="w-3 h-3 text-white" />}
                        {isIndeterminate && <div className="w-2 h-0.5 bg-white rounded-full" />}
                      </div>
                      <span className="text-sm font-medium text-[var(--text-primary)] select-none">{client}</span>
                      <span className="text-xs text-[var(--text-muted)] ml-auto">
                        {selectedCount}/{vehicleList.length}
                      </span>
                    </div>
                  </div>
                  ...
                  {isExpanded && (
                    <div className="pl-9 pr-2 py-1 space-y-0.5 bg-[var(--bg-surface)] border-t border-[var(--border)] border-[var(--border)]/50">
                      {vehicleList.map((vehicle) => (
                        <label
                          key={vehicle}
                          className="flex items-center gap-2 py-1.5 cursor-pointer tr-hover rounded px-1"
                        >
                          <input
                            type="checkbox"
                          ...
                          />
                          <span className="text-sm text-[var(--text-secondary)] truncate select-none">{vehicle}</span>
                        </label>
                      ))}
                    </div>
                  )}
            ...
            {filteredMap.size === 0 && (
              <div className="text-center py-8 text-[var(--text-muted)] text-sm">Aucun résultat trouvé</div>
            )}
          </div>
        </div>
      )}
...
export const ReportTable: React.FC<ReportTableProps> = ({ title, columns, data, onExport, onAiAnalysis }) => {
  ...
  const clientColIdx = useMemo(
    () => columns.findIndex((c) => ['Client', 'Société', 'Compte'].some((k) => c.includes(k))),
    [columns]
  );
  const vehicleColIdx = useMemo(
    () => columns.findIndex((c) => ['Véhicule', 'Plaque', 'Immatriculation'].some((k) => c.includes(k))),
    [columns]
  );
  ...
  const sortedRows = useMemo(() => {
    ...
    return [...filteredRows].sort((a, b) => {
      ...
      return reportSortConfig.direction === 'asc' ? valA.localeCompare(valB, 'fr') : valB.localeCompare(valA, 'fr');
    });
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-[var(--bg-elevated)] rounded-xl border border-[var(--border)] shadow-sm overflow-hidden">
      {/* Toolbar */}
      <div className="p-4 border-b border-[var(--border)] flex flex-col gap-4">
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-bold text-[var(--text-primary)]">{title}</h2>
            <p className="text-sm text-[var(--text-secondary)]">{filteredRows.length} enregistrements</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 flex-wrap">
            ...
            <div className="relative w-full sm:w-auto">
              ...
              <select
                ...
                className="w-full sm:w-auto pl-9 pr-8 py-2 bg-[var(--bg-elevated)] border border-[var(--border)] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[var(--primary)] appearance-none cursor-pointer shadow-sm font-medium text-[var(--text-primary)] min-w-[160px]"
              >
            ...
            <div className="relative w-full sm:w-48">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 border border-[var(--border)] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[var(--primary)] bg-[var(--bg-elevated)] text-[var(--text-primary)] shadow-sm"
                value={filter}
            ...
            <div className="flex gap-2 w-full sm:w-auto relative" ref={columnMenuRef}>
              ...
              <button
                ...
                className={`justify-center p-2 border rounded-lg shadow-sm transition-colors ${isColumnMenuOpen ? 'bg-[var(--primary-dim)] border-[var(--border)] text-[var(--primary)]' : 'bg-[var(--bg-elevated)] border-[var(--border)] text-[var(--text-secondary)] hover:bg-[var(--bg-elevated)] dark:hover:bg-slate-700'}`}
              ...
              {/* Dropdown Colonnes */}
              {isColumnMenuOpen && (
                <div className="absolute top-full right-0 mt-2 w-56 bg-[var(--bg-elevated)] border border-[var(--border)] rounded-lg shadow-lg z-20 p-2 animate-in fade-in slide-in-from-top-2 duration-200">
                  <p className="text-xs font-semibold text-[var(--text-secondary)] px-2 py-1 mb-1 uppercase">
                    Colonnes affichées
                  </p>
                  <div className="max-h-60 overflow-y-auto custom-scrollbar space-y-1">
                    {columns.map((col) => (
                      <label
                        key={col}
                        className="flex items-center gap-2 px-2 py-1.5 hover:bg-[var(--bg-elevated)] dark:hover:bg-slate-700 rounded cursor-pointer"
                      >
                        <input
                          type="checkbox"
                        ...
                        />
                        <span className="text-sm text-[var(--text-primary)] truncate">{col}</span>
                      </label>
                    ))}
                  </div>
                </div>
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 bg-slate-800 bg-[var(--bg-elevated)] text-white rounded-lg hover:bg-slate-700 dark:hover:bg-slate-600 transition-colors shadow-sm"
              >
                ...
                (col, idx) =>
                  visibleColumns.includes(col) && (
                    <SortableHeader
                      ...
                      className="px-4 py-3 text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wider border-b border-[var(--border)] whitespace-nowrap"
                    />
                  )
              )}
            </tr>
          ...
          <tbody className="divide-y divide-[var(--border)]">
            {paginatedRows.length > 0 ? (
              paginatedRows.map((row, rowIdx) => (
                <tr key={rowIdx} className="tr-hover/50 transition-colors group">
                  {row.map(
                    (cell, cellIdx) =>
              ...
              <tr>
                <td colSpan={visibleColumns.length} className="px-4 py-12 text-center text-[var(--text-muted)]">
      ...
      {/* Pagination */}
      <div className="p-4 border-t border-[var(--border)] flex flex-col sm:flex-row items-center justify-between gap-4 bg-[var(--bg-elevated)]">
        <div className="flex items-center gap-2 text-sm text-[var(--text-secondary)]">
          <span>Afficher</span>
          <select
            value={itemsPerPage}
            onChange={(e) => setItemsPerPage(Number(e.target.value))}
            className="bg-[var(--bg-elevated)] border border-[var(--border)] rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-[var(--primary)]"
          >
