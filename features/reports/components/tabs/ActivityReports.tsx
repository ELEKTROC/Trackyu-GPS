...
export const ActivityReports: React.FC<ActivityReportsProps> = ({ vehicles, onAiAnalysis, initialItem }) => {
    ...
    // --- DATA FUNCTIONS ---
    const getTripsData = () => ({
        columns: ['Véhicule', 'Client', 'Conducteur', 'Départ', 'Arrivée', 'Durée', 'Distance', 'Lieu Départ', 'Lieu Arrivée'],
        data: vehicles.flatMap(v => [
            [v.name, v.client || 'Non assigné', 'Jean Dupont', '08:00', '09:30', '1h 30m', '120 km', 'Dépôt Central', 'Client A'],
    ...
    const getTripsByDriverData = () => ({
        ...
        data: vehicles.map(v => [
            'Jean Dupont', '5', `${vehicles.indexOf(v) * 50 + 200} km`, '6h 15m', '72 km/h', v.name,
        ]),
    });
    ...
    const getStopsByVehicleData = () => ({
        ...
        data: vehicles.map(v => [
            v.name, v.client || 'Non assigné', '4', '2h 30m', '37m', '1h 00m',
        ]),
    });
    ...
    const getIdlingCostData = () => ({
        ...
        data: vehicles.map(v => [
            v.name, v.client || 'Non assigné', '30m', '1.5 L', '2.10 €', '8%',
        ]),
    });

    const getOfflineData = () => ({
        columns: ['Véhicule', 'Client', 'Début', 'Fin', 'Durée', 'Dernière Position'],
        data: vehicles.filter(v => v.status === 'OFFLINE').map(v => [
            v.name, v.client || 'Non assigné', 'Hier 18:00', "Aujourd'hui 08:00", '14h 00m', 'Garage Central',
        ]),
    });
    ...
    const getOfflineHistoryData = () => ({
        ...
        data: vehicles.map(v => [
            v.name, v.client || 'Non assigné',
            String(Math.floor(Math.random() * 3) + 1), '2h 30m', '50m', 'Hier 18:00',
        ]),
    });
    ...
    const getSpeedData = () => ({
        ...
        data: vehicles.map(v => [
            v.name, v.client || 'Non assigné', '110 km/h', '65 km/h',
    ...
    const getSpeedViolationsData = () => ({
        ...
        data: vehicles.flatMap(v => [
            [new Date().toLocaleDateString('fr-FR'), v.name, v.client || 'Non assigné', '110 km/h', '90 km/h', '+20 km/h', 'Autoroute A10 - km 45', 'Jean Dupont'],
        ]),
    });
    ...
    const getSpeedRankingData = () => ({
        ...
        data: vehicles.map((v, i) => [
            String(i + 1), 'Jean Dupont', `${Math.max(50, 95 - i * 8)}/100`,
            String(Math.floor(Math.random() * 5)), `${100 + i * 5} km/h`, `${(i + 1) * 180} km`, v.name,
        ]),
    });
    ...
    const getCurrentReportData = (): { columns: string[]; data: string[][]; title: string } => {
        switch (activeItem) {
            case 'trips':
                ...
                if (selectedSubReport === 'by_day') return { ...getTripsByDayData(), title: 'Synthèse journalière des trajets' };
            ...
            case 'offline':
                if (selectedSubReport === 'history') return { ...getOfflineHistoryData(), title: 'Historique des déconnexions' };
    ...
    const handleExport = (title: string, columns: string[], data: string[][]) => {
        generatePDF(
            ...
            `rapport_${activeItem}_${new Date().toISOString().slice(0, 10)}.pdf`,
            { branding }
        );
    };
                ...
                {!isGenerated ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <Card title="Arrêts Clients">
                            ...
                            <p className="text-sm text-slate-500">Visites effectuées</p>
                        </Card>
                    </div>
                ) : columns.length > 0 ? (
                    <ReportTable
