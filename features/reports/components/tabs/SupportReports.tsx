...
export const SupportReports: React.FC<SupportReportsProps> = ({ vehicles, onAiAnalysis, initialItem }) => {
    ...
    const MOCK_TICKETS = [
        ...
        ['T-1005', 'Connexion perdue balise', 'Transport Express', 'TRK-003', 'Haute', 'Résolu', '2026-03-24', '2026-03-25'],
    ];
    ...
    const getTicketsByPriorityData = () => {
        ...
        return {
            ...
            data: priorities.map(p => {
                ...
                return [p, String(pTickets.length), String(pTickets.filter(t => t[5] === 'Ouvert').length), String(pTickets.filter(t => t[5] === 'En cours').length), String(pTickets.filter(t => t[5] === 'Résolu' || t[5] === 'Fermé').length), p === 'Urgente' ? '2h' : p === 'Haute' ? '8h' : '48h'];
            }),
        };
    };
    ...
    const getResolvedListData = () => ({
        ...
        data: MOCK_TICKETS.filter(t => t[5] === 'Résolu' || t[5] === 'Fermé').map(t => [...t.slice(0, 4), t[6], t[7], '1j 2h', 'Support L1']),
    });
    ...
    const getPendingListData = () => ({
        ...
        data: MOCK_TICKETS.filter(t => t[5] === 'Ouvert' || t[5] === 'En attente').map(t => [t[0], t[1], t[2], t[3], t[4], '2j 4h', t[4] === 'Urgente' ? '4h' : '48h']),
    });

    const getOverdueSLAData = () => ({
        columns: ['ID', 'Sujet', 'Client', 'Priorité', 'SLA Limite', 'Dépassement', 'Agent Assigné'],
        data: [
            ['T-1003', 'Alerte répétée vitesse', 'ACME SA', 'Urgente', '4h', '+20h', 'Non assigné'],
        ],
    });
    ...
    const getCurrentReportData = (): { columns: string[]; data: string[][]; title: string } => {
        switch (activeItem) {
            case 'tickets':
                if (selectedSubReport === 'by_priority') return { ...getTicketsByPriorityData(), title: 'Tickets par priorité' };
                if (selectedSubReport === 'by_category') return { ...getTicketsByCategoryData(), title: 'Tickets par catégorie' };
    ...
    const handleGenerate = (mode: 'view' | 'csv' | 'excel' | 'pdf') => {
        ...
        else exportReportData(columns, data, `rapport_support_${activeItem}_${selectedSubReport}_${new Date().toISOString().slice(0, 10)}`, mode);
    };
                ...
                {!isGenerated ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <Card title="Satisfaction Client">
                            ...
                            <p className="text-sm text-slate-500">Moyenne période</p>
                        </Card>
                    </div>
                ) : columns.length > 0 ? (
