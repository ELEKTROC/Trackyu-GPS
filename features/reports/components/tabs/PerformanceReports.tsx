...
export const PerformanceReports: React.FC<PerformanceReportsProps> = ({ vehicles, onAiAnalysis, initialItem }) => {
  ...
  const filteredVehicles = useMemo(
    () => vehicles.filter((v) => selectedVehicles.has(v.name)),
  ...
  const getEcoScoresData = () => ({
    ...
    data: vehicles.map((v) => [
      ...
      `${(8 + Math.random() * 4).toFixed(1)} L/100km`,
    ]),
  });
  ...
  const getEngineHoursData = () => ({
    ...
    data: vehicles.map((v) => [
      ...
      v.client || 'Non assigné',
      `${v.engineHours || 0} h`,
      `${Math.floor((v.engineHours || 0) * 0.2)} h`,
      `${Math.floor((v.engineHours || 0) * 0.75)} h`,
      `${Math.floor((v.engineHours || 0) * 0.05)} h`,
    ]),
  });
  ...
  const getEngineUtilizationData = () => ({
    ...
    data: vehicles.map((v) => {
      ...
      return [
        ...
        `${rate}%`,
        '160 h',
        `${Math.floor((160 * rate) / 100)} h`,
        `${160 - Math.floor((160 * rate) / 100)} h`,
        '80%',
      ];
    }),
  });
  ...
  const getCurrentReportData = (): { columns: string[]; data: string[][]; title: string } => {
    switch (activeItem) {
      case 'productivity':
        if (selectedSubReport === 'by_vehicle')
          return { ...getProductivityByVehicleData(), title: 'Productivité par véhicule' };
      ...
      case 'schedule':
        return {
          ...getProductivityByDriverData(),
          title: selectedSubReport === 'overtime' ? 'Heures supplémentaires' : 'Emplois du temps',
        };
      case 'expenses':
        return {
          ...getProductivityByVehicleData(),
          title: selectedSubReport === 'by_category' ? 'Dépenses par catégorie' : 'Dépenses par véhicule',
        };
      case 'engine':
        if (selectedSubReport === 'utilization')
          return { ...getEngineUtilizationData(), title: "Taux d'utilisation moteur" };
  ...
  const renderContent = () => {
    ...
    const avgScore =
      filteredVehicles.length > 0
        ? filteredVehicles.reduce((s, v) => s + (v.driverScore ?? 0), 0) / filteredVehicles.length
        : 0;
        ...
        {!isGenerated ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <Card title="Score de Flotte Moyen">
              ...
              <p className="text-sm text-[var(--text-secondary)]">Performance globale</p>
            </Card>
            <Card title="Heures Moteur Totales">
              <div className="text-3xl font-bold text-green-600">
                {filteredVehicles.reduce((s, v) => s + (v.engineHours || 0), 0).toLocaleString()} h
              </div>
              <p className="text-sm text-[var(--text-secondary)]">Cumul flotte</p>
            </Card>
            <Card title="Taux Utilisation Moyen">
              <div className="text-3xl font-bold text-orange-600">72%</div>
              <p className="text-sm text-[var(--text-secondary)]">Flotte active</p>
            </Card>
          </div>
        ) : columns.length > 0 ? (
          <ReportTable
