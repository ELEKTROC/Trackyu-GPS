...
export const FuelReports: React.FC<FuelReportsProps> = ({ vehicles, onAiAnalysis, initialItem }) => {
  ...
  const getConsumptionByVehicleData = () => ({
    ...
    data: vehicles.map((v) => {
      ...
      return [
        ...
        `${((totalFuel / distance) * 100).toFixed(1)} L/100km`,
        `${totalCost.toFixed(2)}`,
      ];
    }),
  });
  ...
  const getConsumptionByPeriodData = () => {
    ...
    return {
      ...
      data: days.map((day) => [
        day,
        `${vehicles.length * 100} km`,
        `${(vehicles.length * 10).toFixed(1)} L`,
        '10.0 L/100km',
  ...
  const getConsumptionComparisonData = () => ({
    ...
    data: vehicles.map((v) => {
      ...
      return [
        ...
        parseFloat(diff) > 0 ? '↑ Hausse' : '↓ Baisse',
      ];
    }),
  });
  ...
  const getRefillsByStationData = () => ({
    ...
    data: [
      [
        'Total Énergies - Paris',
        String(filteredRecords.filter((r) => r.type === 'REFILL').length),
        `${filteredRecords.reduce((s, r) => s + r.volume, 0).toFixed(0)} L`,
        `${filteredRecords.reduce((s, r) => s + r.cost, 0).toFixed(2)}`,
        '1.85 €',
      ],
  ...
  const getTheftAnalysisData = () => ({
    ...
    data: vehicles.map((v) => {
      ...
      return [
        ...
        `${incidents.reduce((s, r) => s + r.volume, 0).toFixed(0)} L`,
        `${(incidents.reduce((s, r) => s + r.volume, 0) * 1.85).toFixed(2)} €`,
        incidents.length ? new Date(incidents[0].date).toLocaleDateString('fr-FR') : '--',
      ];
    }),
  });
  ...
  const getEfficiencyRankingData = () => ({
    ...
    data: vehicles.map((v, i) => [
      ...
      v.name,
      v.client || 'Non assigné',
      `${(7 + i * 0.5).toFixed(1)} L/100km`,
      `${Math.max(60, 95 - i * 5)}/100`,
      `${(i + 1) * 800} km`,
      `${(0.13 + i * 0.01).toFixed(2)} €`,
    ]),
  });
  ...
  const getCurrentReportData = (): { columns: string[]; data: string[][]; title: string } => {
    switch (activeItem) {
      case 'consumption':
        if (selectedSubReport === 'by_period')
          return { ...getConsumptionByPeriodData(), title: 'Consommation par période' };
        if (selectedSubReport === 'comparison')
          return { ...getConsumptionComparisonData(), title: 'Comparaison consommation' };
        return { ...getConsumptionByVehicleData(), title: 'Consommation par véhicule' };
      case 'refills':
        if (selectedSubReport === 'by_station')
          return { ...getRefillsByStationData(), title: 'Ravitaillements par station' };
        return { ...getRefillsData(), title: 'Détail des ravitaillements' };
      case 'theft':
        if (selectedSubReport === 'analysis')
          return { ...getTheftAnalysisData(), title: 'Analyse des pertes suspectes' };
        return { ...getTheftData(), title: 'Alertes vol de carburant' };
      case 'efficiency':
        return {
          ...getEfficiencyRankingData(),
          title: selectedSubReport === 'trends' ? "Tendances d'efficacité" : 'Classement efficacité carburant',
        };
      default:
  ...
  const renderContent = () => {
    ...
    const chartData = vehicles
      .map((v) => {
        ...
        return {
          name: v.name,
          volume: vRecords.reduce((s, r) => s + r.volume, 0),
          cost: vRecords.reduce((s, r) => s + r.cost, 0),
        };
      })
      .slice(0, 10);
        ...
        {!isGenerated ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <Card title="Coût Total Carburant">
              ...
              <p className="text-sm text-[var(--text-secondary)]">Période sélectionnée</p>
            </Card>
            ...
            <Card title="Alertes Vol">
              <div className="text-3xl font-bold text-red-600">
                {filteredRecords.filter((r) => r.type === 'THEFT_ALERT').length}
              </div>
              <p className="text-sm text-[var(--text-secondary)]">Incidents détectés</p>
            </Card>
          </div>
        ) : activeItem === 'charts' ? (
          <Card
            title={
              selectedSubReport === 'cost_chart'
                ? 'Coûts carburant par véhicule (Top 10)'
                : 'Consommation par véhicule (Top 10)'
            }
          >
            <div className="h-[400px]">
              <ResponsiveContainer
                ...
                minHeight={350}
                minWidth={200}
                initialDimension={{ width: 200, height: 350 }}
              >
