...
export const LogReports: React.FC<LogReportsProps> = ({ onAiAnalysis, initialItem }) => {
  ...
  const getEventsData = (type?: 'login' | 'data_changes') => {
    ...
    return {
      columns: ['Date', 'Action', 'Cible', 'Utilisateur', 'IP / Détail'],
      data: [...login.map((r) => r.slice(0, 5)), ...changes.map((r) => r.slice(0, 5))],
    };
  };
  ...
  const getAuditLogsData = (filter?: 'admin' | 'user') => {
    ...
    const data =
      filter === 'admin'
        ? all.filter((r) => r[4] === 'ADMIN')
        : filter === 'user'
          ? all.filter((r) => r[4] === 'USER')
          : all;
  ...
  const getCurrentReportData = (): { columns: string[]; data: string[][]; title: string } => {
    switch (activeItem) {
      ...
      case 'events':
        ...
        if (selectedSubReport === 'data_changes')
          return { ...getEventsData('data_changes'), title: 'Modifications de données' };
      ...
      case 'audit':
        if (selectedSubReport === 'admin')
          return { ...getAuditLogsData('admin'), title: "Piste d'audit — Actions admin" };
        if (selectedSubReport === 'user')
          return { ...getAuditLogsData('user'), title: "Piste d'audit — Actions utilisateur" };
        ...
        {!isGenerated ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <Card title="Erreurs (24h)">
              ...
              <p className="text-sm text-[var(--text-secondary)]">Système stable</p>
            </Card>
            ...
            <Card title="Connexions Utilisateurs">
              ...
              <p className="text-sm text-[var(--text-secondary)]">Aujourd'hui</p>
            </Card>
          </div>
        ) : columns.length > 0 ? (
          <ReportTable
