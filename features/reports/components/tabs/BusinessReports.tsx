...
export const BusinessReports: React.FC<BusinessReportsProps> = ({ onAiAnalysis, initialItem }) => {
    ...
    const summaryStats = useMemo(() => {
        ...
        const monthlyRevenue = payments.filter(p => new Date(p.date) >= startOfMonth && p.status === 'COMPLETED').reduce((s, p) => s + (p.amount || 0), 0);
        ...
        const activeClientIds = new Set(invoices.filter(inv => new Date(inv.date) >= startOfMonth).map(inv => inv.clientId));
        return { monthlyRevenue, pendingAmount, overdueCount, pendingQuotes, conversionRate, activeClients: activeClientIds.size, totalInvoices: invoices.length, totalPayments: payments.length };
    }, [invoices, payments, quotes]);

    // --- DATA FUNCTIONS ---
    const getQuotesData = (filter?: 'pending' | 'accepted') => {
        const src = filter === 'pending'
    ...
    const getPaymentsData = () => ({
        ...
        data: payments.map(p => {
            ...
            return [
                ...
                p.method === 'BANK_TRANSFER' ? 'Virement' : p.method === 'CASH' ? 'Espèces' : p.method === 'MOBILE_MONEY' ? 'Mobile Money' : p.method,
    ...
    const getPaymentsByMethodData = () => {
        ...
        payments.forEach(p => {
            const method = p.method === 'BANK_TRANSFER' ? 'Virement' : p.method === 'CASH' ? 'Espèces' : p.method === 'MOBILE_MONEY' ? 'Mobile Money' : p.method;
        ...
        return {
            ...
            data: Array.from(byMethod.entries()).map(([method, { count, total }]) => [
                method, String(count), total.toLocaleString('fr-FR'),
    ...
    const getCurrentReportData = (): { columns: string[]; data: string[][]; title: string } => {
        switch (activeItem) {
            ...
            case 'accounting':
                return { ...getInvoicesData(), title: selectedSubReport === 'balance' ? 'Bilan financier' : 'Journal comptable' };
            default:
    ...
    const handleGenerate = (mode: 'view' | 'csv' | 'excel' | 'pdf') => {
        ...
        else exportReportData(columns, data, `rapport_business_${activeItem}_${selectedSubReport}_${new Date().toISOString().slice(0, 10)}`, mode);
    };
                ...
                {!isGenerated ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <Card title="Chiffre d'affaires (Mois)">
                            ...
                            <p className="text-sm text-slate-500">{summaryStats.totalPayments} paiements reçus</p>
                        </Card>
                        ...
                        <Card title="Total Factures">
                            <div className="text-3xl font-bold text-slate-700 dark:text-slate-200">{summaryStats.totalInvoices}</div>
                            <p className="text-sm text-slate-500">Toutes périodes</p>
                        </Card>
                    </div>
                ) : columns.length > 0 ? (
