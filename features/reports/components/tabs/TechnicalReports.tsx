...
export const TechnicalReports: React.FC<TechnicalReportsProps> = ({ vehicles, onAiAnalysis, initialItem }) => {
    ...
    const filteredInterventions = useMemo(() => interventions.filter(int => {
    ...
    const getGeofencingViolationsData = () => ({
        ...
        data: vehicles.flatMap(v => [
            [new Date().toLocaleDateString('fr-FR'), v.name, v.client || 'Non assigné', 'Zone Interdite A', 'Entrée non autorisée', '14:30', 'Jean Dupont'],
        ]),
    });
    ...
    const getAlertsData = (filter?: 'critical') => {
        ...
        return {
            ...
            data: src.map(a => {
                const vehicle = vehicles.find(v => v.name === a.vehicleName);
                return [
                    a.id.toString(), a.vehicleName || '--', vehicle?.client || 'Non assigné',
                    a.type, a.message, a.severity,
                    new Date(a.createdAt).toLocaleString(), a.isRead ? 'Lu' : 'Non lu',
                ];
            }),
        };
    };
    ...
    const getCurrentReportData = (): { columns: string[]; data: string[][]; title: string } => {
        switch (activeItem) {
            case 'geofencing':
                ...
                if (selectedSubReport === 'violations') return { ...getGeofencingViolationsData(), title: 'Violations de geofencing' };
                return { ...getGeofencingData(), title: 'Entrées et sorties de zones' };
            case 'poi':
                return { ...getGeofencingData(), title: 'Visites Points d\'Intérêt' };
            case 'maintenance':
                if (selectedSubReport === 'upcoming') return { ...getMaintenanceData('upcoming'), title: 'Maintenances à venir' };
                if (selectedSubReport === 'completed') return { ...getMaintenanceData('completed'), title: 'Maintenances effectuées' };
                return { ...getMaintenanceData(), title: 'Suivi de maintenance' };
            case 'sensors':
                return { ...getSensorsData(), title: selectedSubReport === 'history' ? 'Historique capteurs' : 'État actuel des capteurs' };
            case 'immobilization':
                return { ...getGeofencingData(), title: selectedSubReport === 'history' ? 'Historique immobilisations' : 'Immobilisations en cours' };
            case 'alerts':
                ...
                {!isGenerated ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <Card title="Alertes Critiques">
                            <div className="text-3xl font-bold text-red-600">{filteredAlerts.filter(a => a.severity === 'CRITICAL').length}</div>
                        ...
                        <Card title="Capteurs Actifs">
                            ...
                            <p className="text-sm text-slate-500">Temp, Poids, Fuel</p>
                        </Card>
                    </div>
                ) : columns.length > 0 ? (
