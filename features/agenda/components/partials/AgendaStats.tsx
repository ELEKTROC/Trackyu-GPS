...
export const AgendaStats: React.FC<AgendaStatsProps> = ({ 
    ...
    const occupationRate = useMemo(() => {
        ...
        const agentCount = selectedAgentId !== 'ALL' 
            ? 1 
    ...
    const selectedAgentName = selectedAgentId !== 'ALL' 
        ? users.find(u => u.id === selectedAgentId)?.name 
        : null;
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Card className="p-4 flex items-center gap-4 border-l-4 border-l-orange-500">
                ...
                <div>
                    <p className="text-xs font-bold text-slate-500 uppercase">Interventions</p>
                    <p className="text-xl font-bold text-slate-800 dark:text-white">
                        {monthInterventionsCount} <span className="text-sm font-normal text-slate-500">ce mois</span>
                    </p>
                </div>
            </Card>
            ...
            <Card className="p-4 flex items-center gap-4 border-l-4 border-l-indigo-500">
                ...
                <div>
                    <p className="text-xs font-bold text-slate-500 uppercase">Tâches / RdV</p>
                    <p className="text-xl font-bold text-slate-800 dark:text-white">
                        {monthTasksCount} <span className="text-sm font-normal text-slate-500">ce mois</span>
                    </p>
                </div>
            </Card>
            ...
            <Card className="p-4 flex items-center gap-4 border-l-4 border-l-emerald-500">
                ...
                <div>
                    <p className="text-xs font-bold text-slate-500 uppercase">Taux d'occupation</p>
                    <p className="text-xl font-bold text-slate-800 dark:text-white">
                        {occupationRate.rate}%
                    </p>
            ...
            <Card className="p-4 flex items-center gap-4 border-l-4 border-l-purple-500">
                ...
                <div>
                    <p className="text-xs font-bold text-slate-500 uppercase">
                        {selectedAgentName ? 'Agent' : 'Agents'}
                    </p>
                    <p className="text-xl font-bold text-slate-800 dark:text-white">
                        {selectedAgentName || `${occupationRate.agentCount} actifs`}
                    </p>
                    <p className="text-[10px] text-slate-400">
                        {occupationRate.workDays} jours ouvrés
                    </p>
                </div>
            </Card>
        </div>
