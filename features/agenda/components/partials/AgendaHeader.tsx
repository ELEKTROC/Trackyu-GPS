    ...
    return (
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <div className="shrink-0">
                <h2 className="text-xl sm:text-2xl font-bold text-slate-800 dark:text-white">Agenda Unifié</h2>
                <p className="text-xs sm:text-sm text-slate-500 hidden sm:block">Planification des interventions et rendez-vous commerciaux</p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto">
                ...
                <div className="relative" ref={dropdownRef}>
                    ...
                    {dropdownOpen && (
                        <div className="absolute right-0 top-full mt-2 w-52 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl z-50 overflow-hidden">
                            <button
                                onClick={() => { onNewEvent('TECH'); setDropdownOpen(false); }}
                                className="w-full text-left px-4 py-3 text-sm hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 border-b border-slate-100 dark:border-slate-700"
                            >
                                Intervention Technique
                            </button>
                            <button
                                onClick={() => { onNewEvent('BUSINESS'); setDropdownOpen(false); }}
                                className="w-full text-left px-4 py-3 text-sm hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 border-b border-slate-100 dark:border-slate-700"
                            >
                                Rendez-vous Commercial
                            </button>
                            <button
                                onClick={() => { onNewEvent('TASK'); setDropdownOpen(false); }}
                                className="w-full text-left px-4 py-3 text-sm hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200"
                            >
                                Tâche
                            </button>
                        </div>
