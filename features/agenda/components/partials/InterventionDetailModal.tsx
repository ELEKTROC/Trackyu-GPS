      ...
      <div className="relative bg-[var(--bg-surface)] rounded-t-2xl sm:rounded-xl shadow-2xl w-full sm:max-w-2xl h-[92vh] sm:h-auto sm:max-h-[90vh] flex flex-col border-0 sm:border border-[var(--border)] z-[101] animate-in slide-in-from-bottom sm:zoom-in-95 duration-200">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className="text-[10px] text-[var(--text-muted)] bg-white/50 dark:bg-black/20 px-1.5 py-0.5 rounded border border-[var(--border)] dark:border-white/10">
        ...
        >
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span
              className={`px-2.5 py-1 rounded-lg text-xs font-bold border ${getStatusBgClass(intervention.status || 'PENDING')}`}
            >
              {getStatusLabel(intervention.status || 'PENDING')}
            </span>
            <span className={`px-2.5 py-1 rounded-lg text-xs font-medium ${priority.color}`}>{priority.label}</span>
          ...
          <div className="bg-[var(--bg-elevated)] rounded-xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
              <div>
                <p className="text-xs text-[var(--text-secondary)] mb-1">Date prévue</p>
              ...
              {(intervention.address || intervention.location) && (
                <div>
                  ...
                  <p className="text-sm font-medium text-[var(--text-primary)] flex items-start gap-1.5">
                    <MapPin className="w-3.5 h-3.5 text-[var(--text-muted)] mt-0.5" />
                    {intervention.address || intervention.location}
                  </p>
                </div>
              )}
              {intervention.estimatedDuration && (
                <div className="col-span-1 sm:col-span-2">
                  <p className="text-xs text-[var(--text-secondary)] mb-1">Durée estimée</p>
            ...
            {(intervention.startedAt || intervention.completedAt) && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3 border-t border-[var(--border)]">
                {intervention.startedAt && (
                  <div>
                    <p className="text-xs text-[var(--text-secondary)] mb-1">Démarré le</p>
                    <p className="text-sm font-medium text-green-600">{formatShortDate(intervention.startedAt)}</p>
                  </div>
                )}
                {intervention.completedAt && (
                  <div>
                    <p className="text-xs text-[var(--text-secondary)] mb-1">Terminé le</p>
                    <p className="text-sm font-medium text-[var(--primary)]">
                      {formatShortDate(intervention.completedAt)}
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="bg-[var(--bg-elevated)] rounded-xl p-4 space-y-3">
            ...
            <div className="space-y-4">
                ...
                {intervention.clientName && (
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm font-medium text-[var(--text-primary)]">{intervention.clientName}</p>
                    </div>
                  </div>
                )}
                {displayContactPhone && (
                  <div className="flex items-start gap-3 group">
                    ...
                    <div className="flex-1">
                      <p className="text-xs text-[var(--text-secondary)]">Téléphone Client</p>
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-medium text-[var(--text-primary)]">{displayContactPhone}</p>
                        <a
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3 border-t border-[var(--border)]/50 border-[var(--border)]/50">
                {intervention.siteContactName && (
                  <div className="flex items-start gap-3">
                    <User className="w-4 h-4 text-[var(--text-muted)] mt-0.5" />
                    <div className="flex-1">
                      <p className="text-xs text-[var(--text-secondary)]">Contact sur site</p>
                      <p className="text-sm font-medium text-[var(--text-primary)]">{intervention.siteContactName}</p>
                    </div>
                  </div>
                )}
                {intervention.siteContactPhone && (
                  <div className="flex items-start gap-3 group">
                    <Phone className="w-4 h-4 text-[var(--text-muted)] mt-0.5" />
                    <div className="flex-1">
                      <p className="text-xs text-[var(--text-secondary)]">Téléphone sur site</p>
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-medium text-[var(--text-primary)]">
                          {intervention.siteContactPhone}
                        </p>
            ...
            <div className="bg-[var(--bg-elevated)] rounded-xl p-4 space-y-3">
              ...
              <div className="flex items-center gap-4">
                ...
                {intervention.vehiclePlate && (
                  <div>
                    <p className="text-xs text-[var(--text-secondary)]">Immatriculation</p>
                    <p className="text-sm font-bold font-mono text-[var(--text-primary)] bg-yellow-100 dark:bg-yellow-900/30 px-2 py-0.5 rounded">
                      {intervention.vehiclePlate}
                    </p>
                  </div>
                )}
              ...
              <div className="space-y-2">
                {intervention.materialsUsed.map((mat: any, idx: number) => (
                  <div key={idx} className="flex items-center justify-between text-sm">
                    <span className="text-[var(--text-primary)]">{mat.name || mat.itemName}</span>
                    <span className="font-mono text-[var(--text-secondary)]">x{mat.quantity || 1}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {intervention.description && (
            <div className="bg-[var(--bg-elevated)] rounded-xl p-4 space-y-3">
              ...
              <p className="text-sm text-[var(--text-primary)] whitespace-pre-wrap">{intervention.description}</p>

              {/* Numéro de ticket déplacé en dessous de la description */}
              {intervention.ticketId && (
                <div className="mt-4 pt-3 border-t border-[var(--border)]/50 flex items-center gap-2">
                  <span className="text-xs text-[var(--text-secondary)]">Référence Ticket:</span>
        ...
        <div className="flex-shrink-0 px-6 py-4 border-t border-[var(--border)] bg-[var(--bg-elevated)] flex justify-between items-center rounded-b-xl">
          <div className="flex gap-2">
            {/* Download Buttons */}
            <button
              onClick={handleDownloadBon}
              className="flex items-center gap-2 px-3 py-2 bg-[var(--bg-elevated)] text-[var(--text-primary)] rounded-lg hover:bg-[var(--bg-elevated)] transition-colors text-sm font-medium border border-[var(--border)]"
            >
