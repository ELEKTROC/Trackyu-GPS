...
interface DataContextType {
  ...
  createGroupedJournalEntry: (payload: {
    date: string;
    description: string;
    reference?: string;
    journalCode?: string;
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const { data: ticketCategories = [] } = useQuery<TicketCategory[]>({
    ...
    queryFn: () =>
      api.adminFeatures.supportSettings.getCategories(true).then((res: unknown) => (res as TicketCategory[]) || []),
  ...
  const { data: ticketSubcategories = [] } = useQuery<TicketSubCategory[]>({
    ...
    queryFn: () =>
      api.adminFeatures.supportSettings
        .getSubCategories(undefined, true)
        .then((res: unknown) => (res as TicketSubCategory[]) || []),
    enabled: !!user && (!!tenantId || user.role === 'SUPERADMIN' || user.role === 'SUPER_ADMIN') && deferredEnabled,
  });
  ...
  const { data: slaConfig = {} } = useQuery<Record<string, unknown>>({
    ...
    queryFn: () =>
      api.adminFeatures.supportSettings.getSlaConfig().then((res: unknown) => (res as Record<string, unknown>) || {}),
    enabled: !!user && (!!tenantId || user.role === 'SUPERADMIN' || user.role === 'SUPER_ADMIN') && deferredEnabled,
  });
  ...
  const addTierMutation = useMutation({
    ...
    onError: (error: unknown) => {
      logger.error('[DataContext] addTier failed:', error);
    },
  });
  ...
  const updateTierMutation = useMutation({
    ...
    onSuccess: (updatedTier) => {
      queryClient.setQueryData(['tiers', tenantId], (old: Tier[] = []) =>
        old.map((t) => (t.id === updatedTier.id ? updatedTier : t))
      );
      // Invalider le cache pour forcer un refetch
      queryClient.invalidateQueries({ queryKey: ['tiers', tenantId] });
    },
    onError: (error: unknown) => {
      logger.error('[DataContext] updateTier failed:', error);
    },
  });
  ...
  const deleteTierMutation = useMutation({
    ...
    onError: (error: unknown) => {
      logger.error('[DataContext] deleteTier failed:', error);
    },
  });

  const addClientMutation = useMutation({
    mutationFn: (client: Client) =>
      api.clients.create({ ...client, tenantId: tenantId || client.tenantId || 'tenant_default' }),
    onSuccess: (newClient) => {
      queryClient.setQueryData(['clients', tenantId], (old: Client[] = []) => [...old, newClient]);
    },
    onError: (error: unknown) => {
      logger.error('[DataContext] addClient failed:', error);
    },
  });
  ...
  const updateClientMutation = useMutation({
    ...
    onSuccess: (updatedClient) => {
      queryClient.setQueryData(['clients', tenantId], (old: Client[] = []) =>
        old.map((c) => (c.id === updatedClient.id ? updatedClient : c))
      );
    },
    onError: (error: unknown) => {
      logger.error('[DataContext] updateClient failed:', error);
    },
  });
  ...
  const deleteClientMutation = useMutation({
    ...
    onError: (error: unknown) => {
      logger.error('[DataContext] deleteClient failed:', error);
    },
  });
  ...
  const addContractMutation = useMutation({
    mutationFn: (contract: Contract) =>
      api.contracts.create({ ...contract, tenantId: tenantId || contract.tenantId || 'tenant_default' }),
    onSuccess: (newContract) => {
      queryClient.setQueryData(['contracts', tenantId], (old: Contract[] = []) => [...old, newContract]);
    },
    onError: (error: unknown) => {
      logger.error('[DataContext] addContract failed:', error);
    },
  });
  ...
  const updateContractMutation = useMutation({
    ...
    onSuccess: (updatedContract) => {
      queryClient.setQueryData(['contracts', tenantId], (old: Contract[] = []) =>
        old.map((c) => {
    ...
    onError: (error: unknown) => {
      logger.error('[DataContext] updateContract failed:', error);
    },
  });
  ...
  const deleteContractMutation = useMutation({
    ...
    onError: (error: unknown) => {
      logger.error('[DataContext] deleteContract failed:', error);
    },
  });
  ...
  const markAlertAsReadMutation = useMutation({
    ...
    onSuccess: (_, id) => {
      queryClient.setQueryData(['alerts', tenantId], (old: Alert[] = []) =>
        old.map((a) => (a.id === Number(id) ? { ...a, isRead: true } : a))
      );
    },
  });
  ...
  const bulkUpdateClientStatusMutation = useMutation({
    mutationFn: ({ ids, status }: { ids: string[]; status: 'ACTIVE' | 'SUSPENDED' | 'CHURNED' }) =>
      api.clients.bulkUpdateStatus(ids, status),
  ...
  const addStockMovementMutation = useMutation({
    mutationFn: (movement: StockMovement) =>
      api.stockMovements.create({ ...movement, tenantId: tenantId || movement.tenantId }),
  ...
  const updateDeviceMutation = useMutation({
    ...
    onSuccess: (updatedDevice) => {
      ...
      queryClient.setQueryData(['stock', tenantId], (old: DeviceStock[] = []) =>
  ...
  const deleteTaskMutation = useMutation({
    ...
    onSuccess: (deletedId, inputId) => {
      ...
      queryClient.setQueryData(['tasks', tenantId], (old: Task[] = []) => old.filter((t) => t.id !== deletedId));
    },
    onError: (error: unknown) => {
  ...
  const addInterventionMutation = useMutation({
    mutationFn: (intervention: Intervention) =>
      api.interventions.create({ ...intervention, tenantId: tenantId || intervention.tenantId }),
  ...
  const updateInterventionMutation = useMutation({
    ...
    onSuccess: async (updatedIntervention) => {
      queryClient.setQueryData(['interventions', tenantId], (old: Intervention[] = []) =>
        old.map((i) => (i.id === updatedIntervention.id ? updatedIntervention : i))
      );
  ...
  const addInvoiceMutation = useMutation({
    mutationFn: (invoice: Invoice) =>
      api.invoices.create({ ...invoice, tenantId: tenantId || invoice.tenantId || 'tenant_default' }),
  ...
  const updateInvoiceMutation = useMutation({
    ...
    onSuccess: (updatedInvoice) => {
      queryClient.setQueryData(['invoices', tenantId], (old: Invoice[] = []) =>
        old.map((i) => (i.id === updatedInvoice.id ? { ...i, ...updatedInvoice } : i))
      );
  ...
  const updateQuoteMutation = useMutation({
    ...
    onSuccess: (updatedQuote) => {
      queryClient.setQueryData(['quotes', tenantId], (old: Quote[] = []) =>
        old.map((q) => (q.id === updatedQuote.id ? updatedQuote : q))
      );
    },
  });
  ...
  const addTicketMutation = useMutation({
    mutationFn: (ticket: Ticket) =>
      api.tickets.create({ ...ticket, tenantId: tenantId || ticket.tenantId || 'tenant_default' }),
    onSuccess: (newTicket) => {
  ...
  const updateUserMutation = useMutation({
    ...
    onSuccess: (updatedUser) => {
      queryClient.setQueryData(['users', tenantId], (old: SystemUser[] = []) =>
  ...
  const addDriverMutation = useMutation({
    mutationFn: (driver: Driver) =>
      api.drivers.create({ ...driver, tenantId: tenantId || driver.tenantId || 'tenant_default' }),
    onSuccess: (newDriver) => {
  ...
  const updateDriverMutation = useMutation({
    ...
    onSuccess: (updatedDriver) => {
      queryClient.setQueryData(['drivers', tenantId], (old: Driver[] = []) =>
        old.map((d) => (d.id === updatedDriver.id ? updatedDriver : d))
      );
    },
  });
  ...
  const updateTechMutation = useMutation({
    ...
    onSuccess: (updatedTech) => {
      queryClient.setQueryData(['techs', tenantId], (old: Tech[] = []) =>
        old.map((t) => (t.id === updatedTech.id ? updatedTech : t))
      );
    },
  });
  ...
  const addGroupMutation = useMutation({
    mutationFn: (group: Group) =>
      api.groups.create({ ...group, tenantId: tenantId || group.tenantId || 'tenant_default' }),
    onSuccess: (newGroup) => {
  ...
  const updateGroupMutation = useMutation({
    ...
    onSuccess: (updatedGroup) => {
      queryClient.setQueryData(['groups', tenantId], (old: Group[] = []) =>
    ...
    onSuccess: (newPayment) => {
      ...
      if (newPayment.type === 'INCOMING') {
        const allocations =
          ...
          (newPayment.invoiceId ? [{ invoiceId: newPayment.invoiceId, amount: newPayment.amount }] : []);

        // 1. Update invoice statuses for each allocation
        allocations.forEach((alloc) => {
          const invoice = invoices.find((i) => i.id === alloc.invoiceId);
          if (invoice) {
            const newPaidAmount = Math.min((invoice.paidAmount || 0) + alloc.amount, invoice.amount);
  ...
  const addSupplierInvoiceMutation = useMutation({
    mutationFn: (invoice: SupplierInvoice) =>
      api.supplierInvoices.create({ ...invoice, tenantId: tenantId || invoice.tenantId || 'tenant_default' }),
    onSuccess: (newInvoice) => {
  ...
  const updateSupplierInvoiceMutation = useMutation({
    ...
    onSuccess: (updatedInvoice) => {
      queryClient.setQueryData(['supplierInvoices', tenantId], (old: SupplierInvoice[] = []) =>
        old.map((i) => (i.id === updatedInvoice.id ? updatedInvoice : i))
      );
  ...
  const addBankTransactionMutation = useMutation({
    mutationFn: (tx: BankTransaction) =>
      api.bankTransactions.create({ ...tx, tenantId: tenantId || tx.tenantId || 'tenant_default' }),
    onSuccess: (newTx) => {
  ...
  const updateBankTransactionMutation = useMutation({
    ...
    onSuccess: (updatedTx) => {
      queryClient.setQueryData(['bankTransactions', tenantId], (old: BankTransaction[] = []) =>
  ...
  const updateBudgetMutation = useMutation({
    ...
    onSuccess: (updatedBudget) => {
      queryClient.setQueryData(['budgets', tenantId], (old: Budget[] = []) =>
  ...
  const addSupplierMutation = useMutation({
    mutationFn: (supplier: Supplier) =>
      api.suppliers.create({ ...supplier, tenantId: tenantId || supplier.tenantId || 'tenant_default' }),
  ...
  const updateSupplierMutation = useMutation({
    ...
    onSuccess: (updatedSupplier) => {
      queryClient.setQueryData(['suppliers', tenantId], (old: Supplier[] = []) =>
  ...
  const addBranchMutation = useMutation({
    mutationFn: (branch: Branch) =>
      api.branches.create({ ...branch, tenantId: tenantId || branch.tenantId || 'tenant_default' }),
    onSuccess: (newBranch) => {
  ...
  const updateBranchMutation = useMutation({
    ...
    onSuccess: (updatedBranch) => {
      queryClient.setQueryData(['branches', tenantId], (old: Branch[] = []) =>
  ...
  const updateCommandMutation = useMutation({
    ...
    onSuccess: (updatedCmd) => {
      queryClient.setQueryData(['commands', tenantId], (old: Command[] = []) =>
        old.map((c) => (c.id === updatedCmd.id ? updatedCmd : c))
      );
    },
  });
  ...
  const updatePOIMutation = useMutation({
    ...
    onSuccess: (updatedPOI) => {
      queryClient.setQueryData(['pois', tenantId], (old: POI[] = []) =>
        old.map((p) => (p.id === updatedPOI.id ? updatedPOI : p))
      );
    },
  });
  ...
  const updateAlertConfigMutation = useMutation({
    ...
    onSuccess: (updatedCfg) => {
      queryClient.setQueryData(['alertConfigs', tenantId], (old: AlertConfig[] = []) =>
        old.map((c) => (c.id === updatedCfg.id ? updatedCfg : c))
      );
    },
  });
  ...
  const addCatalogItemMutation = useMutation({
    mutationFn: (item: CatalogItem) =>
      api.catalog.create({ ...item, tenantId: tenantId || item.tenantId || 'tenant_default' }),
    onSuccess: (newItem) => {
  ...
  const updateCatalogItemMutation = useMutation({
    ...
    onSuccess: (updatedItem) => {
      queryClient.setQueryData(['catalog', tenantId], (old: CatalogItem[] = []) =>
  ...
  const addMaintenanceRuleMutation = useMutation({
    mutationFn: (rule: MaintenanceRule) =>
      api.maintenanceRules.create({ ...rule, tenantId: tenantId || 'tenant_default' }),
    onSuccess: (newRule) => {
  ...
  const updateMaintenanceRuleMutation = useMutation({
    ...
    onSuccess: (updatedRule) => {
      queryClient.setQueryData(['maintenanceRules', tenantId], (old: MaintenanceRule[] = []) =>
        old.map((r) => (r.id === updatedRule.id ? updatedRule : r))
      );
    },
  });
  ...
  const updateScheduleRuleMutation = useMutation({
    ...
    onSuccess: (updatedRule) => {
      queryClient.setQueryData(['scheduleRules', tenantId], (old: ScheduleRule[] = []) =>
        old.map((r) => (r.id === updatedRule.id ? updatedRule : r))
      );
    },
  });
  ...
  const addEcoDrivingProfileMutation = useMutation({
    mutationFn: (profile: EcoDrivingProfile) =>
      api.ecoDrivingProfiles.create({ ...profile, tenantId: tenantId || 'tenant_default' }),
    onSuccess: (newProfile) => {
      queryClient.setQueryData(['ecoDrivingProfiles', tenantId], (old: EcoDrivingProfile[] = []) => [
        ...old,
        newProfile,
      ]);
    },
  ...
  const toggleImmobilizationMutation = useMutation({
    mutationFn: ({ vehicleId, immobilize }: { vehicleId: string; immobilize: boolean }) =>
      api.vehicles.toggleImmobilization(vehicleId, immobilize),
    onSuccess: (_, { vehicleId, immobilize }) => {
      queryClient.setQueryData(['vehicles', tenantId], (old: Vehicle[] = []) =>
        old.map((v) => (v.id === vehicleId ? { ...v, status: immobilize ? 'STOPPED' : 'MOVING' } : v))
      );
    },
  });

  // --- DEFINE FUNCTIONS IN PROVIDER SCOPE ---
  const normalizedAlerts: Alert[] = alerts.map(
    (a) =>
      ({
        ...a,
  ...
  const contextValue = useMemo(
    (): DataContextType => ({
      ...
      addClient: (client: Client, options?: { onSuccess?: () => void; onError?: (error: unknown) => void }) =>
        addClientMutation.mutate(client, options),
    ...
    [
      ...
      bulkUpdateClientStatusMutation,
      addTierMutation,
      updateTierMutation,
      deleteTierMutation,
      ...
      createGroupedJournalEntryMutation,
      addDriverMutation,
      ...
      addPaymentMutation,
      addSupplierInvoiceMutation,
      updateSupplierInvoiceMutation,
      deleteSupplierInvoiceMutation,
      addBankTransactionMutation,
      updateBankTransactionMutation,
      deleteBankTransactionMutation,
      addBudgetMutation,
      ...
      updatePOIMutation,
      deletePOIMutation,
      addAlertConfigMutation,
      updateAlertConfigMutation,
      deleteAlertConfigMutation,
      addMaintenanceRuleMutation,
      updateMaintenanceRuleMutation,
      deleteMaintenanceRuleMutation,
      addScheduleRuleMutation,
      updateScheduleRuleMutation,
      deleteScheduleRuleMutation,
      addEcoDrivingProfileMutation,
      updateEcoDrivingProfileMutation,
      deleteEcoDrivingProfileMutation,
      addMaintenanceRecordMutation,
      toggleImmobilizationMutation,
      queryClient,
      getVehicleHistory,
      getVehicleHistorySnapped,
      getMaintenanceRecords,
