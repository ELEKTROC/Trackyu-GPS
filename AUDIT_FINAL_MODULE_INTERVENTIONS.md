...
describe('calculateResolutionTime', () => {
    it('returns null if not COMPLETED', () => {
        expect(calculateResolutionTime({ status: 'IN_PROGRESS', startTime: '2026-01-01T08:00:00Z', endTime: '2026-01-01T09:00:00Z' } as any)).toBeNull();
    });
    ...
    it('returns null for negative duration', () => {
        expect(calculateResolutionTime({
            status: 'COMPLETED',
...
describe('isOverSLA', () => {
    it('uses 4h SLA for DEPANNAGE', () => {
        expect(isOverSLA({
            type: 'DEPANNAGE', status: 'COMPLETED',
            startTime: '2026-01-01T08:00:00Z',
    ...
    it('uses 24h SLA for INSTALLATION', () => {
        expect(isOverSLA({
            type: 'INSTALLATION', status: 'COMPLETED',
            startTime: '2026-01-01T08:00:00Z',
            endTime: '2026-01-01T13:00:00Z' // 5h
        } as any)).toBe(false);
    });
...
*Rapport généré après audit complet : 22 fichiers frontend, 4 fichiers backend, 1 schéma Zod, build vérifié.*
