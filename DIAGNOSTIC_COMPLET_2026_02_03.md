...
const vehicleResult = await pool.query(
  'SELECT id, name FROM vehicles WHERE imei = $1 AND tenant_id = $2',
  [imei, req.user.tenantId]
);
```
...
router.post('/payments/:id/approve', 
  authenticateToken, 
  requirePermission('APPROVE_PAYMENTS'), 
  approvePayment
);
...
router.put('/:id', 
  authenticateToken, 
  validateRequest(TierUpdateSchema), 
  updateTier
);
```
...
if (companyName) {
  const companyCheck = await pool.query(
    'SELECT id FROM leads WHERE company_name = $1 AND tenant_id = $2',
...
router.post('/:id/convert-to-client', 
  ...
  async (req, res) => {
    ...
    const existingClient = await pool.query(
      'SELECT id FROM tiers WHERE email = $1 AND type = $2 AND tenant_id = $3',
      [lead.rows[0].email, 'CLIENT', tenantId]
    );
...
*Validité : 30 jours (revoir après corrections)*
