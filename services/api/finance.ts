...
import { USE_MOCK, NETWORK_DELAY, API_URL, DB_KEYS, db, sleep, filterByTenant, getHeaders, handleAuthError } from './client';
import { logger } from '../../utils/logger';
import type { Contract, Invoice, Quote, CatalogItem, StockMovement, JournalEntry, Payment, SupplierInvoice, BankTransaction, Budget, Subscription } from '../../types';
...
export function createFinanceApi() {
  ...
  contracts: {
    list: async (tenantId?: string): Promise<Contract[]> => {
      ...
      try {
        ...
        return rawData.map((c: any) => ({
          ...
          items: c.items ? (typeof c.items === 'string' ? JSON.parse(c.items) : c.items).map((item: any) => ({
    ...
    create: async (contract: Contract): Promise<Contract> => {
      ...
      try {
        ...
        const payload = {
          ...
          items: contract.items?.map(item => ({ description: item.description, quantity: Number(item.quantity) || 1, unit_price: Number(item.price) || 0 })),
        ...
        if (!response.ok) {
          ...
          const err = new Error(errData.error || 'Failed to create contract') as Error & { conflicts?: { vehicleId: string; contractNumber: string }[] };
        ...
        return {
          ...
          billingCycle: c.billing_cycle || c.billingCycle || 'MONTHLY',
          autoRenew: c.auto_renew ?? c.autoRenew ?? true,
          items: c.items ? (typeof c.items === 'string' ? JSON.parse(c.items) : c.items).map((i: any) => ({ description: i.description, quantity: i.quantity, price: i.unit_price ?? i.price ?? 0 })) : [],
          pdfUrl: c.pdf_url || c.pdfUrl,
          notes: c.notes,
          subject: c.subject,
          vehicleIds: c.vehicle_ids || c.vehicleIds || []
    ...
    update: async (contract: Contract): Promise<Contract> => {
      ...
      try {
        ...
        if (!response.ok) {
          const errData = await response.json().catch(() => ({}));
          const err = new Error(errData.error || 'Failed to update contract') as Error & { conflicts?: { vehicleId: string; contractNumber: string }[] };
          if (errData.conflicts) err.conflicts = errData.conflicts;
          throw err;
        }
        const c = await response.json();
  ...
  invoices: {
    list: async (tenantId?: string): Promise<Invoice[]> => {
      ...
      try {
        ...
        while (hasMore) {
          const response = await fetch(`${API_URL}/finance/invoices?page=${page}&limit=${limit}`, { headers: getHeaders() });
        ...
        const mappedData = rawArray.map((i: any) => {
          ...
          return {
            ...
            status: ((s) => s === 'PENDING' ? 'DRAFT' : s === 'PARTIAL' ? 'PARTIALLY_PAID' : s)((i.status || 'DRAFT').toUpperCase()), // pending → DRAFT, partial → PARTIALLY_PAID
            items: parsedItems,
            vatRate: vatRate,
    ...
    create: async (invoice: Invoice): Promise<Invoice> => {
      ...
      try {
        ...
        return {
          ...
          items: (() => {
            const raw = rawData.items ? (typeof rawData.items === 'string' ? JSON.parse(rawData.items) : rawData.items) : [];
            return raw.map((item: any) => ({ description: item.description || '', quantity: Number(item.quantity) || 1, price: Number(item.price ?? item.unit_price ?? 0) }));
          })(),
          notes: rawData.notes,
    ...
    list: async (tenantId?: string): Promise<Quote[]> => {
      ...
      try {
        ...
        return rawArray.map((q: any) => ({
          ...
          items: q.items ? (Array.isArray(q.items) ? q.items : JSON.parse(q.items || '[]')).map((item: any) => ({
            description: item.description,
            quantity: parseFloat(item.quantity || '1'),
            price: parseFloat(item.unit_price || item.price || '0')
          })) : [],
  ...
  catalog: {
    ...
    delete: async (id: string): Promise<void> => {
      ...
      if (USE_MOCK) {
        const items = db.get(DB_KEYS.CATALOG, [] as CatalogItem[]);
        db.set(DB_KEYS.CATALOG, items.filter(i => i.id !== id));
        return;
      }
  ...
  accounting: {
    list: async (tenantId?: string): Promise<JournalEntry[]> => {
      ...
      try {
        const response = await fetch(`${API_URL}/finance/journal-entries?tenantId=${tenantId}`, { headers: getHeaders() });
    ...
    delete: async (id: string): Promise<void> => {
      ...
      if (USE_MOCK) {
        const entries = db.get(DB_KEYS.JOURNAL, []);
        db.set(DB_KEYS.JOURNAL, entries.filter((e: JournalEntry) => e.id !== id));
        return;
      }
      try {
        const response = await fetch(`${API_URL}/finance/journal-entries/${id}`, {
    ...
    periods: {
      ...
      manage: async (payload: { periodId: string; action: 'close' | 'lock' | 'reopen'; reason?: string }): Promise<AccountingPeriod> => {
        await sleep(NETWORK_DELAY);
      ...
      validate: async (date: string): Promise<{ status: string; canCreate: boolean; canEdit: boolean; canDelete: boolean; reason?: string }> => {
        await sleep(NETWORK_DELAY);
        if (USE_MOCK) return { status: 'OPEN', canCreate: true, canEdit: true, canDelete: true };
        const response = await fetch(`${API_URL}/finance/accounting-periods/validate?date=${date}`, { headers: getHeaders() });
  ...
  payments: {
    list: async (tenantId?: string): Promise<Payment[]> => {
      ...
      try {
        ...
        return rows.map((p: any) => ({
          ...
          allocations: p.allocations || (p.invoice_id ? [{ invoiceId: p.invoice_id, amount: parseFloat(p.amount || '0') }] : []),
    ...
    create: async (payment: Payment): Promise<Payment> => {
      ...
      try {
        ...
        // Single invoice
        const invoiceId = allocations[0]?.invoiceId || payment.invoiceId || (payment as Payment & { invoice_id?: string }).invoice_id;
    ...
    delete: async (id: string): Promise<void> => {
      ...
      if (USE_MOCK) {
        const payments = db.get(DB_KEYS.PAYMENTS, []);
        db.set(DB_KEYS.PAYMENTS, payments.filter((p: Payment) => p.id !== id));
        return;
      }
      try {
        const response = await fetch(`${API_URL}/finance/payments/${id}`, {
    ...
    getPaymentHistory: async (invoiceId: string): Promise<{ invoice: any; history: any[]; totalPaid: number; balance: number }> => {
      await sleep(NETWORK_DELAY);
      if (USE_MOCK) {
      ...
      try {
        const response = await fetch(`${API_URL}/finance/invoices/${invoiceId}/payment-history`, { headers: getHeaders() });
  ...
  supplierInvoices: {
    ...
    delete: async (id: string): Promise<string> => {
      ...
      if (USE_MOCK) {
        const invoices = db.get(DB_KEYS.SUPPLIER_INVOICES, []);
        db.set(DB_KEYS.SUPPLIER_INVOICES, invoices.filter((i: SupplierInvoice) => i.id !== id));
        return id;
      }
      try {
  ...
  bankTransactions: {
    ...
    delete: async (id: string): Promise<string> => {
      ...
      if (USE_MOCK) {
        const txs = db.get(DB_KEYS.BANK_TRANSACTIONS, []);
        db.set(DB_KEYS.BANK_TRANSACTIONS, txs.filter((t: BankTransaction) => t.id !== id));
        return id;
      }
      try {
  ...
  budgets: {
    ...
    delete: async (id: string): Promise<string> => {
      ...
      if (USE_MOCK) {
        const budgets = db.get(DB_KEYS.BUDGETS, []);
        db.set(DB_KEYS.BUDGETS, budgets.filter((b: Budget) => b.id !== id));
        return id;
      }
      try {
  ...
  subscriptions: {
    ...
    delete: async (id: string): Promise<string> => {
      ...
      if (USE_MOCK) {
        const subs = db.get(DB_KEYS.SUBSCRIPTIONS, []);
        db.set(DB_KEYS.SUBSCRIPTIONS, subs.filter((s: Subscription) => s.id !== id));
        return id;
      }
      try {
  ...
  finance: {
    ...
    createInvoice: async (invoice: any) => {
      ...
      if (!response.ok) { const err = await response.json().catch(() => ({})); throw new Error((err as { error?: string }).error || 'Erreur lors de la création de la facture'); }
      return response.json();
    },
    updateInvoice: async (id: string, updates: any) => {
      ...
      if (!response.ok) { const err = await response.json().catch(() => ({})); throw new Error((err as { error?: string }).error || 'Erreur lors de la mise à jour de la facture'); }
      return response.json();
    },
    deleteInvoice: async (id: string) => {
      ...
      if (!response.ok) { const err = await response.json().catch(() => ({})); throw new Error((err as { error?: string }).error || 'Erreur lors de la suppression de la facture'); }
      return response.json();
    },
    ...
    createExpense: async (expense: any) => {
      ...
      if (!response.ok) { const err = await response.json().catch(() => ({})); throw new Error((err as { error?: string }).error || 'Erreur lors de la création de la dépense'); }
      return response.json();
    },
    updateExpense: async (id: string, updates: any) => {
      ...
      if (!response.ok) { const err = await response.json().catch(() => ({})); throw new Error((err as { error?: string }).error || 'Erreur lors de la mise à jour de la dépense'); }
      return response.json();
    },
    deleteExpense: async (id: string) => {
      ...
      if (!response.ok) { const err = await response.json().catch(() => ({})); throw new Error((err as { error?: string }).error || 'Erreur lors de la suppression de la dépense'); }
      return response.json();
    },
    ...
    createPayment: async (payment: any) => {
      ...
      if (!response.ok) { const err = await response.json().catch(() => ({})); throw new Error((err as { error?: string }).error || 'Erreur lors de la création du paiement'); }
      return response.json();
    },
    deletePayment: async (id: string) => {
      ...
      if (!response.ok) { const err = await response.json().catch(() => ({})); throw new Error((err as { error?: string }).error || 'Erreur lors de la suppression du paiement'); }
      return response.json();
    },
    ...
    createQuote: async (quote: any) => {
      ...
      if (!response.ok) { const err = await response.json().catch(() => ({})); throw new Error((err as { error?: string }).error || 'Erreur lors de la création du devis'); }
      return response.json();
    },
    updateQuote: async (id: string, updates: any) => {
      ...
      if (!response.ok) { const err = await response.json().catch(() => ({})); throw new Error((err as { error?: string }).error || 'Erreur lors de la mise à jour du devis'); }
      return response.json();
    },
    deleteQuote: async (id: string) => {
      ...
      if (!response.ok) { const err = await response.json().catch(() => ({})); throw new Error((err as { error?: string }).error || 'Erreur lors de la suppression du devis'); }
      return response.json();
    },
    convertQuoteToInvoice: async (quoteId: string) => {
      ...
      if (!response.ok) { const err = await response.json().catch(() => ({})); throw new Error((err as { error?: string }).error || 'Erreur lors de la conversion du devis'); }
      return response.json();
    },
    ...
    createJournalEntry: async (entry: any) => {
      ...
      if (!response.ok) { const err = await response.json().catch(() => ({})); throw new Error((err as { error?: string }).error || 'Erreur lors de la création de l\'écriture'); }
      return response.json();
    },
    deleteJournalEntry: async (id: string) => {
      ...
      if (!response.ok) { const err = await response.json().catch(() => ({})); throw new Error((err as { error?: string }).error || 'Erreur lors de la suppression de l\'écriture'); }
      return response.json();
    },
    validateJournalEntry: async (id: string) => {
      ...
      if (!response.ok) { const err = await response.json().catch(() => ({})); throw new Error((err as { error?: string }).error || 'Erreur lors de la validation de l\'écriture'); }
      return response.json();
    },
    ...
    // Email
    sendInvoiceEmail: async (invoiceId: string, data: { to: string; cc?: string; subject: string; message: string }) => {
