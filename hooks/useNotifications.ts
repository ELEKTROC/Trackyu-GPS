...
const STORAGE_KEY = 'trackyu_notification_prefs';
...
export const useNotifications = () => {
  ...
  // Jouer un son
  const playSound = useCallback((severity: string = 'DEFAULT') => {
    ...
    try {
      ...
      audioRef.current.play().catch(err => {
        logger.warn('[Notifications] Sound playback failed:', err);
      });
    } catch (err) {
  ...
  const showNotification = useCallback(async (payload: NotificationPayload) => {
    ...
    if (type && preferences.alertTypes[type as keyof typeof preferences.alertTypes] === false) {
      logger.debug(`[Notifications] Type ${type} is disabled`);
      return;
    }
    ...
    if (permission === 'granted' && preferences.pushEnabled) {
      if (swRegistration) {
        ...
        swRegistration.showNotification(title, {
          body: vehicleName ? `${vehicleName}: ${body}` : body,
          icon: '/icon-192x192.png',
          badge: '/badge-72x72.png',
          tag: type || 'default',
          data: { ...data, type, severity, vehicleName },
  ...
  }, [permission, preferences, swRegistration, playSound, vibrate]);
  ...
  const updatePreferences = useCallback(async (updates: Partial<NotificationPreferences>) => {
    setPreferences(prev => {
      ...
      api.notifications.updatePreferences(next).catch(err => {
  ...
  const toggleAlertType = useCallback((type: keyof NotificationPreferences['alertTypes']) => {
    setPreferences(prev => {
      ...
      // Sync with backend
      api.notifications.updatePreferences(next).catch(err => {
        logger.error('[Notifications] Failed to sync alert type preference:', err);
      });
      return next;
    });
  }, []);
