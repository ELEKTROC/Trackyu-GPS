...
io.on('connection', (socket) => {
  socket.on('join:tenant', (tenantId) => {
    ...
    if (socket.data.user.tenantId !== tenantId && socket.data.user.role !== 'SUPER_ADMIN') {
...
export const updatePosition = async (req: Request, res: Response) => {
  ...
  const vehicle = await pool.query('SELECT id FROM vehicles WHERE id = $1 AND tenant_id = $2', [vehicleId, tenantId]);
...
export const createMovement = async (req: Request, res: Response) => {
  ...
  if (type === 'OUT') {
    ...
    const stock = await pool.query('SELECT current_stock FROM catalog WHERE id = $1 AND tenant_id = $2', [
      itemId,
      req.user.tenantId,
    ]);
...
router.delete('/payments/:id', authenticateToken, requirePermission('DELETE_PAYMENT'), deletePayment);

router.post('/payments/:id/approve', authenticateToken, requirePermission('APPROVE_PAYMENT'), approvePayment);
```
...
1. **backend/src/**tests**/security/tenant-isolation.test.ts**
...
2. **backend/src/**tests**/security/rbac.test.ts**
...
3. **backend/src/**tests**/gps/spoofing.test.ts**
...
_Document généré le 2026-02-03 par audit complet modules + analyse pré-corrections approfondie_
