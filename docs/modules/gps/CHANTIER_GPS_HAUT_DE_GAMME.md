...
| B2  | 🔴 HIGH   | `fuel_liters` NULL en DB malgré `raw_data.fuel` présent — `flushPositions()` dans `positionWorker.ts` : colonnes de l'INSERT bulk et du fallback individuel omettaient `fuel_liters`. Fix : colonne ajoutée aux deux branches INSERT (offset 10). | `src/workers/positionWorker.ts`   | ✅ Corrigé 2026-04-21 |
